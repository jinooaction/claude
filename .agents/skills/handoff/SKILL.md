...
Respond to the operator in **Korean** (AGENTS.md 규칙 1). Plain Korean, no
...
1. **Gather ground-truth first** (do not write from memory):
   ...
   ```
   - Open PRs: `mcp__github__list_pull_requests owner=jinooaction repo=Codex state=open`.
...
4. **Constitution / workflow version drift.** If `AGENTS.md` or the constitution
...
5. **Commit and push** (descriptive message; never `--no-verify`):
   ...
   through the normal PR + autonomous-merge channel (AGENTS.md 규칙 3).
