...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Agent harness regression liveness probe"
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
    ...
    parser.add_argument(
        "--strict-output",
        type=Path,
        help="agent_harness_probe.py --strict output file.",
    )
    parser.add_argument("--released-work", type=Path, help="released_work.json file.")
    parser.add_argument(
        "--evidence-dir",
        type=Path,
        help="Directory containing strict-output.txt and released-work.json.",
    )
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    evidence = collect_repo_evidence(
        args.repo_root,
        strict_output_path=args.strict_output,
