"""스펙 136 — 꼬리위험 방어·볼록성 no-live probe.

워크플로 또는 로컬 검증이 sidecar 브랜치들을 `<key>.md` 파일로 모아두면,
순수 코어(`analytics.broad_no_edge_tail_risk_convexity`)로 계약을 발행한다.
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Broad NO_EDGE tail-risk convexity no-live contract probe"
    )
    parser.add_argument(
        "--manifest",
        action="store_true",
