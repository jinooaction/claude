"""스펙 078 — 돈 경로 게이트 정렬 루프 probe.

워크플로가 automation sidecar 브랜치들을 읽어 `<key>.md` 파일로 모아두면, 이 probe가
순수 코어(`analytics.money_gate_alignment`)로 정렬 보고를 발행한다.

읽기 전용 — 주문 0건, 돈 0 이동. 사용:
  uv run python scripts/money_gate_alignment_probe.py --manifest
  uv run python scripts/money_gate_alignment_probe.py --evidence-dir /tmp/sidecars --json
"""
...
from auto_invest.analytics.money_gate_alignment import build_money_gate_alignment
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Money gate alignment sidecar probe")
    parser.add_argument(
        "--manifest",
        action="store_true",
