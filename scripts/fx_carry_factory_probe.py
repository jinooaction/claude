#!/usr/bin/env python3
"""Run the no-order spec-155 foreign-exchange carry factory."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        rows = [
            ...
            if "1990-01-01" <= row.date <= current_date.isoformat()
        ]
        gold = align_gold_levels(rows, parse_gold(_read(args.gold_file, GOLD_URL)))
        target_dates = [row.date for row in rows]
        snapshots, quality = load_fx_bundle(
            args.macro_data_dir, [*target_dates, current_date.isoformat()]
        )
        ...
        payload = run_fx_carry_factory(
            rows,
            gold,
            snapshots[:-1],
            live_snapshot=snapshots[-1],
            fx_data_quality=quality,
        ...
        print(f"FX carry factory input error: {exc}", file=sys.stderr)
        return 2

    summary = render_fx_factory_markdown(payload)
