#!/usr/bin/env python3
"""Run or verify frozen shock-recovery development research without reading holdout prices."""
...
CONTRACT = ROOT / "specs/191-shock-recovery-research/contracts/preregistration.json"
...
def read_bundle(directory: Path) -> tuple[dict, bytes]:
    ...
    subprocess.run(["git", "cat-file", "-e",
                    f"{commit}:src/auto_invest/analytics/shock_recovery_intraday.py"],
                   cwd=ROOT, check=True, capture_output=True)
