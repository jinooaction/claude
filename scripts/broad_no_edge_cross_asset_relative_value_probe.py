"""스펙 135 — 자산 간 상대가치 no-live probe.

워크플로 또는 로컬 검증이 sidecar 브랜치들을 `<key>.md` 파일로 모아두면,
순수 코어(`analytics.broad_no_edge_cross_asset_relative_value`)로 계약을 발행한다.
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Broad NO_EDGE cross-asset relative-value no-live contract probe"
    )
    parser.add_argument(
        "--manifest",
        action="store_true",
