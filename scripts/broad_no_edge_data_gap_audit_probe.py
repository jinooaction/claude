"""스펙 133 — 데이터 결측 원인 감사 no-live probe.

워크플로 또는 로컬 검증이 sidecar 브랜치들을 `<key>.md` 파일로 모아두면,
순수 코어(`analytics.broad_no_edge_data_gap_audit`)로 감사 계약을 발행한다.
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Broad NO_EDGE data gap no-live audit contract probe"
    )
    parser.add_argument(
        "--manifest",
        action="store_true",
