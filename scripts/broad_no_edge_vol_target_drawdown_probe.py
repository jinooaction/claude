"""스펙 137 — 변동성 목표·낙폭 제어 no-live probe.

워크플로 또는 로컬 검증이 sidecar 브랜치들을 `<key>.md` 파일로 모아두면,
순수 코어(`analytics.broad_no_edge_vol_target_drawdown`)로 계약을 발행한다.
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Broad NO_EDGE vol-target drawdown no-live contract probe"
    )
    parser.add_argument(
        "--manifest",
        action="store_true",
