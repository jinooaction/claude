"""스펙 095 — forward 레짐 엣지 no-live 실험 계약 probe.

워크플로 또는 로컬 검증이 sidecar 브랜치들을 `<key>.md` 파일로 모아두면,
순수 코어(`analytics.forward_regime_edge_experiment`)로 실험 계약을 발행한다.
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Forward regime edge no-live experiment contract probe"
    )
    parser.add_argument(
        "--manifest",
        action="store_true",
