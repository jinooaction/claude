# Contract: Broad NO_EDGE Multi-Horizon Signal
...
`scripts/broad_no_edge_multi_horizon_signal_probe.py --manifest` emits:
...
When released-work scans this spec, it must record `candidate-broad-no-edge-multi-horizon-signal-experiment` as released. Autonomous-work should then advance the broad no-edge frontier to `candidate-broad-no-edge-regime-cost-robustness-experiment`.
