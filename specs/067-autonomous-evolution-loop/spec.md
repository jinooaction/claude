...
**Created**: 2026-06-28
**Status**: Draft
**Input**: User description: "데이터 수집, 데이터 분석, 전략 설계, 포트폴리오 설계, 실시간 매매, 회고 등 모든 분야에서 세계 최고 수준이 될 수 있게 계속 자동 자율 고도화하고 싶다. 기다리는 시간이 아까우니 자동 루프로 설계해서 스스로 진화하게 하고 싶다."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - 전 영역 개선 후보를 자동으로 발굴한다 (Priority: P1)

운영자는 기다리는 시간 동안 시스템이 멈춰 있지 않고, 데이터 수집·데이터 품질·분석·전략 연구·포트폴리오 구성·실행 품질·실시간 매매 상태·회고·에이전트 운영 품질 전 영역에서 다음 개선 후보를 스스로 찾아 우선순위화하기를 원한다.

**Why this priority**: 자동 고도화의 첫 단계는 "무엇을 개선할지"를 매번 사람이 다시 찾지 않게 만드는 것이다. 후보 발굴이 없으면 실험·승격 루프가 시작되지 않는다.

**Independent Test**: 최신 자동화 사이드카, `HANDOFF.md`, 출시된 스펙, 최근 실행 결과를 입력으로 주면 시스템이 영역별 개선 후보와 근거, 위험 등급, 기대 가치, 다음 행동을 포함한 후보 목록을 산출한다.

**Acceptance Scenarios**:

1. **Given** 최신 money-path가 `PREVIEW_ONLY`이고 micro GTAA가 `latest_intent_loss`로 차단됨, **When** 자율 고도화 스캔이 실행됨, **Then** 후보 목록은 "실주문 재개"가 아니라 "전략 증거 재검토 또는 대체 후보 연구"를 상위 후보로 표시한다.
2. **Given** 특정 사이드카가 오래됐거나 누락됨, **When** 스캔이 실행됨, **Then** 시스템은 그 영역을 개선 후보가 아니라 관측 불능 또는 생존 감시 이슈로 분리한다.
3. **Given** 후보가 자본 증액, 허용 종목 확대, 주문 제한 완화, live 전략 교체를 요구함, **When** 후보를 분류함, **Then** 자동 실행 후보가 아니라 안전 경계 또는 돈 경로 검토 후보로 표시한다.

---

### User Story 2 - 개선 후보를 안전한 실험으로 바꾼다 (Priority: P1)

운영자는 좋은 아이디어가 대화로 사라지지 않고, 자동 루프가 각 후보를 작은 실험으로 쪼개고 재현 가능한 성공 기준을 붙이기를 원한다.
...
### User Story 3 - 검증된 개선만 기존 안전 게이트로 승격한다 (Priority: P1)
...
- **FR-003**: System MUST create improvement candidates with domain, evidence, expected benefit, risk grade, safety boundary impact, confidence, required next action, and expiry or recheck condition.
- **FR-004**: System MUST distinguish evidence gaps from improvement opportunities. Missing or stale evidence cannot be treated as proof that a strategy or execution path is bad.
- **FR-005**: System MUST convert selected candidates into bounded experiments with declared goal, non-goal, required data, success metrics, failure criteria, rollback or discard path, and affected safety surfaces.
- **FR-006**: System MUST require every experiment that can affect trading behavior to begin read-only, backtest, paper, or canary before any live-money effect.
- **FR-007**: System MUST prohibit the evolution loop from directly placing, retrying, canceling, or modifying broker orders.
- **FR-008**: System MUST prohibit the evolution loop from directly increasing capital, widening position caps, widening whitelists, changing account allowlists, or enabling real-order mode outside existing safety gates.
- **FR-009**: System MUST route strategy replacement only through the existing autonomous reassignment gate and MUST NOT create a parallel live strategy swap path.
- **FR-010**: System MUST route capital scaling only through the existing capital ladder and MUST NOT create a parallel capital allocation path.
- **FR-011**: System MUST classify candidates that touch constitution, kernel, order limits, audit, secrets, deployment restrictions, external paid services, or live-money authority as safety-boundary review items.
- **FR-012**: System MUST produce an evidence package before any candidate is promoted. The package must include source evidence, experiment results, comparison baseline, known limitations, safety review, and next gate.
- **FR-013**: System MUST maintain a learning ledger of accepted, rejected, expired, and waiting candidates so future sessions do not rediscover the same conclusion without new evidence.
- **FR-014**: System MUST publish a latest-run summary for the evolution loop that identifies highest-priority candidates, blocked candidates, stale evidence, and safety-boundary items.
- **FR-015**: System MUST expose loop health to pipeline liveness monitoring so a silent stop is visible without moving money.
- **FR-016**: System MUST mask secrets and account-sensitive values in all reports, prompts, sidecars, and handoff-ready summaries.
- **FR-017**: System MUST keep operator-facing summaries in Korean while preserving code identifiers, commit hashes, workflow names, and spec IDs.
- **FR-018**: System MUST record when a candidate is not actionable because the required waiting time is real market time, and it must propose the best safe offline work that can proceed meanwhile.
...
- **Improvement Candidate**: A proposed improvement with domain, evidence, expected benefit, confidence, risk grade, safety impact, next action, and expiry condition.
...
- **SC-006**: Latest-run summaries allow a new session to identify within five minutes the top safe offline work, the top waiting-on-market-data item, and any safety-boundary item.
...
- Real market waiting time cannot be eliminated. The loop should use waiting periods for safe offline research, backtests, data-quality checks, and experiment design.
