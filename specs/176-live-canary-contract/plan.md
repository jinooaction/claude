...
17. 시간별 한 번씩 네 번의 예약마저 첫 기회가 생성되지 않은 생산 사실을 반영해, 10:17부터
    13:53까지 최대 12분 간격의 비정각 기회로 촘촘하게 한다. 새 이벤트 종류나 직접 서버 주문
    경로는 추가하지 않고, 기존 서버 거래일 1회 선점이 모든 추가 기회를 주문 한 번으로 축약한다.
...
| 10:17~13:53의 촘촘한 예약 | 예약 run 미생성 뒤 다음 회복 기회를 최대 12분 간격으로 제공 | 시간당 한 번은 첫 실행 자체가 없으면 거의 한 시간을 잃고, 별도 승인 이벤트나 서버 직접 주문은 기존 안전 경계를 넓힌다 |
