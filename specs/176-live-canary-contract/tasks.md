...
- [ ] T098 [US3] 진단 1.1을 검증·merge·exact-main 배포한 뒤 생산 run의 KIS 거부 코드를 확정하고,
  주문 어댑터의 원인을 고친 다음 명시적 미접수·열린 주문 0건·대사 정상일 때만 가능한 추가 전용
  단회 자동 재시도 계약을 별도 헌법·돈 경로 변경으로 설계한다.
...
- T097의 닫힌 진단을 production에서 확인하기 전에는 주문 원인을 추측해 고치거나 재시도하지 않는다.
  T098의 단회 재시도는 첫 선점을 삭제하지 않고 별도 추가 전용 권한·중복 차단을 가져야 한다.
