...
- [x] T113 [US5] 운영자가 KIS 앱의 본인 인증·약관 동의 경계에서 production 진단이 정확히
  지목한 `해외ETP 거래신청`을 완료 또는 활성 상태로 확인한다. 시스템은 금융 약관에 대신
  동의하거나 계좌 서비스를 임의 활성화하지 않는다. 2026-09-05 KST 오너가 대화에서
  해외 ETP 신청 완료를 명시했다. 이는 오너 완료 확인이며 T114의 중개사 수락·체결 증거가 아니다.
...
- [x] T116 [US5] 관련 41개·전체 `3373 passed, 7 skipped`, ruff, strict harness 14/14,
  HANDOFF 사실 검사와 PR 품질 관문을 통과시켜 PR #754 merge `7a69a5cb`와 exact 긴급 복구 배포
  run `33898309128`을 완료했다. 읽기 전용 observer run `33898481820`은 IAUM·SCHX 모두
  `service_registration_scopes=[overseas_etp]`를 원문 노출·주문·재시도·service/timer 시작 없이
  발행했다.
...
- T112는 코드·주문 형식이 아니라 KIS 계좌의 서비스 신청 영역을 지목한다. T113은 운영자 본인
  ...
  목표를 완료로 표시하지 않는다.
- T116 production 범위 관측은 운영자 확인 항목을 `해외ETP 거래신청`으로 좁혔지만 T113을
  자동화하거나 T114의 실제 체결 증거를 대신하지 않는다.
- GitHub 정기 실행 `33901493966`은 기존 서버 최초·복구 실행 쌍과 `first_source=server_timer`를
  반환하고 broker·fills·measure·reconciliation을 모두 건너뛰어 후발 출처 중복 쓰기 0건을
  production에서 확인했다. 이는 T053·T057·T071·T078·T084·T087·T109·T114의 중복 차단
  부분 증거이며, 실제 신규 접수·체결·전략 감사·계좌 대사가 없는 동안 각 과제는 미완료다.
