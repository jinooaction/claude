...
FR042: 전략 시세는 정확히 기존5종목이다. 기존 보유 평가 시세는 별도 구독/스냅샷이며
관측 변환기에서만 함께 사용한다. 조회 시각 가격은 구독 거래소 확인용이고 평가값이 아니다.
ORANY 등 미지원OTC는 미검증으로 남기며 임의0평가/매도를 하지 않는다. 연결은 잠금 후
시작하고 중복 실행에서 추가 구독하지 않으며 중지/취소 시 모두 닫는다.
