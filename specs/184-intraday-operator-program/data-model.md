...
- ExecutionQualification.execution_model_verified(): 같은 계좌/전략/비용 지문으로
  ...
  원시 비용 보고서의 실행 동등성 False는 그 보고서만으로는 모른다는 의미로 유지한다.
  종합 결과는 주문 이후 실행 모델 적합성이며 정확한 체결 시각/계좌 NAV/전략 승격은
  인증하지 않는다. 최종 호출은 별도 서버 운영 승인도 매번 확인한다.
- orders_json.reported_order_date: 미체결을 포함한 모든 원문 주문 날짜. 수량 모델은
  신호 시각의 뉴욕 거래일과 대조한다. 원본 주문량 미달 체결은 명시적 종료가 있어야
  닫힌 실행 집합에 들어가며 잔량0만으로 종료를 추정하지 않는다.
