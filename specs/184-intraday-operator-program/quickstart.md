...
`execution-start`는 FR070의 고정 예산 방식을 사용합니다. 준비 기준은600달러,
주문/종목120달러, 기존 보유까지 포함한 총 노출480달러, 단타 손익 기준 하루 정지
발동12달러입니다. 기존 보유가 한도를 넘으면 매수를 거절합니다. 예산은 실제 현금이나
계좌 순자산 표시가 아니며, 계좌 전체 NAV 하락률 감시를 대신하지 않습니다.

매수 전 종목·지정가별 여력과 예산을 검사하고, 부분 체결·미체결은 장부에서 예약을
유지합니다. 보고 결제일이 지난 거래 중 주문·수량·대금·비용이 대조된 순정산만
예산 복구에 반영합니다. 같은 보고를 다시 읽어도 중복 복구하지 않고 이익으로
600달러 예산을 자동 확대하지 않습니다. 비용 조회 실패는 취소·정리를 지연시키지
않습니다. 기존 시각 없는 체결의 관측 구간 검증과 모든 실제 운영 자격 검사는 유지합니다.

새 방식은 운영 승인 식별자가 달라집니다. 이전 승인이나 준비 기준 파일만으로
실제 주문을 허용하지 않습니다. 이 구현에서 실제 주문/승인 발급/자금 배정은 하지 않았습니다.
...
KIS 모의 운용·원본 시세·매수가능금액 조회와 고정 예산 실행 시작 경로가 있습니다.
실제 가동에는 아래의 연구·전진·체결 모델·서버 운영 자격이 모두 필요합니다.
코드 연결과 모의 검사 통과를 해당 계좌의 현재 실거래 자격 통과로 해석하지 않습니다.
...
시작에는 기존 세 가지 KIS 환경값이 모두 필요합니다. 자격 확인 과정에서 읽기 요청을
수행할 수 있지만 자격 실패는 주문 전송 전에 종료 코드2로 거절합니다. 실제 자료가
체결 동등성 검사를 통과해야 하며 `QUALIFICATION_PARITY_NOT_VERIFIED`를 승인 파일로
덮을 수 없습니다.
...
이 제어 기능은 한국투자 형식의 모의 HTTP와 기존 주문 엔진·영속 장부를 사용하는
통합 시험에서 검증했습니다. 실제 계좌 읽기와 권한 검사도 시작 경로에 연결돼 있습니다.
위의 `run`은 모의 운용이며 `execution-start`는 별도 운영 자격을 요구하는 주문 경로입니다.
