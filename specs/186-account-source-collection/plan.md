...
## 근거
FR010: 신규 네트워크 호출 없이 기존 원본 기록 완료 후 국내 현금 대조기를 연결한다.
국내 반복 요약의 숫자 벡터가 모두 같은 경우에만 기준을 선택하고, 각 현재/결제 응답의
관측 결과를 별개로 반환한다.20행 일반 계좌 자산표만 예수금 분류17을 사용하며17행
별도 상품표에는 이 위치를 적용하지 않는다. Decimal80으로 합산 후보를 비교하되
현금/전체 범위/실행 인증은 false로 유지한다. 되돌림은 진단 연결 제거이며 원본은 보존한다.
국내 필드 근거: https://raw.githubusercontent.com/koreainvestment/open-trading-api/main/examples_llm/domestic_stock/inquire_balance/chk_inquire_balance.py
...
# FR011 전후 조회 안의 실제 원본 수집

기존 잔고 비교기에 선택적 내부 비동기 수집 함수를 연결한다. CURRENT→결제→
미국4개→국내→자산표2개→CURRENT 순서로 같은 인증/원본 장부에 남긴다. 함수가
없는 기존3조회 사용자는 유지한다. 남은 관측 시간만큼 중간 수집을 제한하고 실패/
취소는 전파한다. 실제 SQLite 원본 순서와 실패 기록·모의 HTTP·전체 검사로 확인한다.
등급3 원본 관측 안전 변경. 순현금/전체 계좌 인증이나 실주문은 하지 않는다.
되돌림은 이전 순차 배치 호출로 돌아가는 커밋이며 기존 원본 기록은 삭제하지 않는다.
# FR012 실제 원본에 동일 계산기 적용

184에서 검증한 세 계산 모듈과 단위 시험을 그대로 가져온다. 기존 개인 원본 장부의
CURRENT/MARGIN/CURRENT 응답만 선택하고 국내/미국 파서의 원래 결과를 사용해
정규화한다. 결과는 내부 숫자와 공개 판정을 명시적으로 분리한다. 모의 실제 GET→
SQLite→공개 결과에서 추가 조회0, 금액 비노출, 불일치/누락 판정을 검증한다.
등급3 원본 처리이며 주문/현금 검증 표시는 변경하지 않는다. 되돌림은 새 공개 판정
연결을 제거하는 커밋이며 원본 장부는 보존한다. 실제 서버 검사는 main 반영 후 수행한다.
# FR013 보고 자산 분류 대조

184 b7e2935의 자산 분류 계산기와 단위 검사를 동일하게 가져온다. 기존 원본 장부의
자산 응답 두 개를 선택해 현금/보유 결과와 대조한다. 계좌 상품코드는 수집기 소유
인증 계좌에서 넘긴다. 공개 출력은 고정 필드만 투영한다. 정상 모의 GET10개와
다른 자산/채무/변경/누락을 검증하고 main 배포 후 실제 판정만 읽는다.
위험 등급3이며 기존 가동 관문·조회 횟수·장부·원문 비공개는 유지한다. 되돌림은
새 평가 호출/출력과 계산기 추가만 되돌리고 기존 수집 기록은 보존한다.
