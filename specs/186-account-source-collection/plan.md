...
FR014 (위험3):184 03f8500의 CTOS4001R 파서·공백 거래소 필터·시험을 가져온다.
기존 --history-db 명령이 최근 거래내역을 함께 수집하고 날짜를 UTC 기준
오늘부터 과거2일까지 고정한다. 모든 날짜/수수료 원본은 장부에만 남긴다. 추가 조회는
기존 CURRENT 전후 묶음보다 먼저 실행한다. 실패 시 장부 FAILED를 남기고 부분
성공을 반환하지 않는다. 설치된 root 소유 서버 도우미는 소스 변경만으로 갱신되지
않으므로 수정하지 않는다. 이미 실행하는 --history-db 진입점에서 연결해 별도 서버
권한/설치를 요구하지 않는다. 실제 CLI·11GET/원본/비노출·실패와 기존 고정 명령을
시험한다. 전체 검사 후 배포하고 실제 판정을 확인한다. 복구는 추가 읽기 연결만
되돌리며 기존 원본과 계좌/현금/가동 관문은 유지한다.
3일은 제한된 원본 수집 정책이며 모든 미결제/현금 흐름을 포괄한다는 주장이 아니다.
