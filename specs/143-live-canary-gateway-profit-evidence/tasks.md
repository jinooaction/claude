...
- [x] T017 2026-06-23 실제 주문·체결과 양의 손익을 KIS 권위 증거로 복구·확정한다.
- [x] T018 수동 production 실행을 주문 없는 서명 preflight로 분리하고 예약 주문과의 동시 실행을 막는다.
- [x] T019 주문 당일만 조회하던 체결 동기화를 과거 날짜 범위·브로커 체결시각 보존으로 확장한다.
- [x] T020 검증된 외부 보유 수량·평단을 합성 체결 없이 성과 시작 상태로 연결하고 최신 보유 기준선을 맞춘다.
- [x] T021 2026-06-23 KIS 체결을 복구하고 첫 양의 손익 증거와 멱등 재실행을 실서버에서 확인한다.
- [x] T022 독립 시세가 빠진 현재 보유 종목에 KIS 잔고 평가금액 기반 mark를 사용한다.
