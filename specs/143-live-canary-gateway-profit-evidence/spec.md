...
- 주문 당일 동기화를 놓친 체결은 명시적 과거 날짜 범위로 다시 조회하며, KIS 체결 행이 없으면
  로컬 상태나 손익을 추정해 바꾸지 않는다.
- 시스템 가동 전 보유의 수량·평단은 출처 run이 있는 별도 시작 상태로만 성과에 반영하고 합성
  `FILL`을 만들지 않는다.
...
- **FR-018**: 체결 동기화는 오늘뿐 아니라 유효한 시작·종료일 범위를 KIS에 전달할 수 있어야 한다.
- **FR-019**: 과거 복구 체결은 KIS 주문번호·누적수량·평균 체결가·브로커 시각을 보존하고 기존
  `kis_fill_id` 멱등성과 추가 전용 감사를 유지해야 한다.
...
- **SC-007**: 2026-06-23 범위 복구가 KIS에서 확인한 세 매도 체결만 추가하고, 정확한 손익을 다시
  계산하며, 같은 복구를 반복해도 추가 체결이나 손익이 중복되지 않는다.
