...
- **VIII.A 장중 배포 규율**: 통과. 헌법 15.0.1의 정확한 단회 등록 오너 예외를 구현하며 일반 경로는 계속 차단한다.
...
헌법 15.0.1이 목적 제한 예외과 등록 오너 신원을 명시하므로 정당화가 필요한 위반은 없다. 일반 장중 차단보다 복잡하지만, 주문 잠금·미체결 0건·단회 요청·90초 건강·롤백을 서로 다른 경계에서 검증해야 단일 실패가 실거래와 겹치지 않는다.
