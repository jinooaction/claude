...
- **IV 추가 전용 감사**: 통과. `DEPLOY_EMERGENCY_AUTHORIZED`를 기존 장부에 추가하고 기존 배포 사건과 같은 상관관계 식별자로 연결한다.
...
- **VIII.A 장중 배포 규율**: 통과. 헌법 15.0.0에 추가한 정확한 단회 오너 예외를 구현하며 일반 경로는 계속 차단한다.
- **VIII.B 배포 자동화**: 통과. 승인·시작·성공/실패·롤백 감사, 90초 건강 검사, 복구 실패 시 주문 정지를 보존한다.
- **IX 안전 경계**: 통과. K6/K-meta 변경은 전용 헌법 커밋 `4ce579ae`에 `this changes the safety perimeter`를 기록했다. 구현 커밋도 K6 변경 표식을 남긴다.
- **X 측정 기반 성장**: 통과. 전략·알파·자본 사다리는 바꾸지 않는다.
- **운용 진입 계약**: 10% 단 1, 정확한 전략 지문, 기존 첫 진입·정수주·KIS·정합 증거를 그대로 소비한다. 누락 시 주문은 실패 폐쇄한다.

**Post-design re-check**: 통과. 외부로 노출되는 새 기능은 저장소 오너만 사용할 수 있는 정확한 `workflow_dispatch` 입력과 고정 SSH 명령 하나뿐이다. 서버 요청 파일은 루트 소유 0600, 짧은 수명, 정확한 SHA, 단회 감사 조회로 제한된다. 유지보수 잠금은 세 주문 경계가 독립 확인한다.
...
헌법 15.0.0이 목적 제한 예외를 명시하므로 정당화가 필요한 위반은 없다. 일반 장중 차단보다 복잡하지만, 주문 잠금·미체결 0건·단회 요청·90초 건강·롤백을 서로 다른 경계에서 검증해야 단일 실패가 실거래와 겹치지 않는다.
