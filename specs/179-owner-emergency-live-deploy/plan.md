...
- **VIII.A 장중 배포 규율**: 통과. 헌법 15.2.0의 정확한 단회 등록 오너 예외, 최초 도입 bootstrap, 시작 전 HALTED 인계, terminal rollback orphan의 엄격한 복구를 구현하며 일반 경로는 계속 차단한다.
- **VIII.B 배포 자동화**: 통과. 승인·시작·성공/실패·롤백 감사, 90초 건강 검사, 복구 실패 시 주문 정지를 보존한다.
- **IX 안전 경계**: 통과. K6/K-meta 변경은 전용 헌법 커밋 `b89f01ab`에 `this changes the safety perimeter`를 기록했다. 구현 커밋도 K6 변경 표식을 남긴다.
...
헌법 15.2.0이 목적 제한 예외, 등록 오너 신원, 정확한 target checkout bootstrap, 시작 전 HALTED 인계와 terminal rollback orphan의 엄격한 복구를 명시하므로 정당화가 필요한 위반은 없다. 일반 장중 차단보다 복잡하지만, 주문 잠금·미체결 0건·단회 요청·90초 건강·롤백을 서로 다른 경계에서 검증해야 단일 실패가 실거래와 겹치지 않는다.
