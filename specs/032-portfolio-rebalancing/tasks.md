...
운영자 "②(실제 라이브 실행) 먼저" 지시. 이 컨테이너엔 KIS 시크릿·네트워크가 없어
실주문은 운영자 환경에서만 가능 → 그 실행을 **안전·미리보기 가능**하게 만든다(돈 무이동).

- [x] **T015** `execute_rebalance(dry_run=True)` — 계획(점수→비중→차이→클램프)만 산출하고
  라우터를 절대 호출 안 함(state `DRY_RUN`). 라우터가 호출되면 예외 던지는 테스트로 증명.
- [x] **T016** `cli.py rebalance-once` 안전장치 — ① `--dry-run`(오프라인 미리보기, KIS
  미접촉, 저장 바 기준가), ② `--mode live` 는 `--confirm-live` 없으면 거부(실주문 사고
  방지 인터록), ③ 유니버스 종목이 화이트리스트에 없으면 거부. 오프라인 시연 검증 완료.
