...
- **FR-B07**: Fill model: [NEEDS CLARIFICATION: candidates are (a) Idealized — fill at the trigger bar's close at the rule's declared limit price, no slippage; (b) Conservative — fill at next bar's open ± a configurable slippage in basis points; (c) Range-aware — fill iff the bar's [low, high] straddles the limit price; if so, fill at limit; choice materially changes whether backtest overstates live PnL, which is what constitution principle VI exists to address].
...
- **FR-B17**: New audit-event types (`BACKTEST_STARTED`, `BACKTEST_COMPLETED`, `BACKTEST_FAILED`) MUST be added under append-only invariants. Their schema additions MUST go through a new SQLite migration (`0003_backtest_events.sql` or similar). The migration is NOT a Kernel touch (the migrations themselves at `0001_initial.sql` and `0002_token_usage.sql` are listed in `kernel.toml` group K4; new migration files are not — adding `0003` is outside the Kernel by construction). [NEEDS CLARIFICATION: confirm migration-file naming policy and whether a new migration counts as touching K4 since K4 lists specific filenames; safest reading is that any addition to the migration directory should be added to `kernel.toml` for protection but the *act of adding* is itself a one-time human-merge event].

#### Synthetic-shock dataset (consumer: spec 007)

- **FR-B18**: System MUST support a named-dataset mode in which the operator (or the canary harness) names a curated subset of historical days rather than a contiguous window. The initial named dataset is `synthetic_shock_v1` containing: 2020-03-12, 2020-04-20, 2024-08-05, and the most recent quarterly OPEX day at the time the dataset was frozen.
...
- **FR-B20**: Adding or removing a day from `synthetic_shock_v1` is operator-only (L4 in spec 005 terms — affects the safety surface). The engine MUST refuse to silently mutate a named dataset.

#### Promotion gate output (consumer: constitution principle VI canary stage)

- **FR-B21**: System MUST produce a machine-readable promotion verdict (`promote_eligible: bool`, `reason: str`) computed from a configurable acceptance threshold (default thresholds for v1: `total_return_pct ≥ 0`, `max_drawdown_pct ≤ 10`, `sharpe ≥ 0.5`). The verdict is advisory for the human operator in v1; it becomes one input among many to spec 007 once 007 ships.
