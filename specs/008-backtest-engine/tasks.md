...
- [ ] T006 Update `.specify/memory/kernel.toml` group `[K4_append_only_audit]` `files` array to include `"src/auto_invest/persistence/migrations/0003_backtest_events.sql"`. This edit is itself a K-meta touch and is the explicit gate that makes this whole change set human-merge.
...
- [ ] T017 [P] [US1] Test `tests/backtest/test_kernel_safety.py`: invoke the engine's pre-flight `kernel_touch_check` against a synthetic diff that includes `kernel.toml` and assert it raises `BacktestKernelTouchError`; against an empty diff assert it returns cleanly
