# 자율 작업 실행 루프 (as of 2026-09-19T13:06:26Z)
...
|------|------|------|----------|-----:|-----:|------|
| candidate-parallel-edge-challenger-d59c4161bcba | strategy_design | EXECUTION_READY | CODEX_AUTONOMOUS_START | 2 | 9750 | 현재 전략은 forward 관측 19, PSR unknown < 0.95로 관찰 중이다. 관찰 시계와 연구 후보 식별자를 분리하고, 기다림과 별도로 신규 no-live 후보를 탐색한다. |

## 승인 필요 또는 억제 후보

| 후보 | 영역 | 상태 | 위험 | 안전 표면 | 이유 |
|------|------|------|-----:|-----------|------|
| wait-for-globalfixed-forward-edge | strategy_design | OBSERVATION_WAIT | 0 | - | three_asset_fixed-w10은 역사 검증을 통과했지만 globalfixed 전진 기준은 미달이다: 관측 19, PSR unknown < 0.95. |
...
|------|------|-------:|-----:|-----------|---------|------|
| 검증 명령 재현 계약 | released | 2 | 2480 | candidate-broad-validation-failure-command-replay-contract | validation_command, exit_code, safe_replay, output_digest | 현재 막힌 패키지는 모두 `execution_failed` 진단이며, 실패 명령·종료 코드·제한 출력이 다음 후보 발굴의 병목이다. 같은 패키지를 막연히 재시도하지 말고 재현 가능한 실패 지문 계약으로 먼저 닫아야 한다. |
| 검증 데이터 준비도 | released | 2 | 2380 | candidate-broad-validation-failure-data-readiness-contract | history_root, portfolio_toml, market_history_coverage | 두 패키지 모두 후보 history root와 포트폴리오 입력을 요구한다. 실행 실패가 전략 아이디어의 실패인지 데이터 준비 결함인지 분리하지 못하면 엣지 탐색 폭이 실제보다 좁아진다. |
| 전략·포트폴리오 패키지 분리 | released | 2 | 2280 | candidate-broad-validation-failure-package-kind-expansion-contract | strategy_family, portfolio_design, holding_period | 막힌 패키지는 전략 백테스트와 포트폴리오 백테스트가 섞여 있다. 둘을 같은 실패로 취급하면 전략군, 포트폴리오 구성, 보유 기간, 산출 증거를 넓히는 순서가 흐려진다. |
| 승격 재검토 조건 | released | 2 | 2180 | candidate-broad-validation-failure-promotion-recheck-contract | learning_ledger, promotion_evidence, recheck_condition | learning ledger가 실패 후보를 억제한 뒤 재검토 조건이 없으면, 실패가 고쳐져도 후보가 닫힌 채 남을 수 있다. 억제 기억은 필요하지만 새 증거가 생겼을 때 되살리는 조건도 필요하다. |
...
| public-data | yes | ok | overall_ok=True, published=27 |
| regime-stratify | yes | ok | total_return_days=752 |
...
| trigger | schedule |
| timestamp_utc | 2026-09-19T13:06:26Z |
...
```json
{
  ...
  "evidence_surfaces": [
    ...
    },
    {
      ...
      "summary_ko": "overall_ok=True, published=27"
    },
    {
      "key": "regime-stratify",
      "parse_status": "ok",
      ...
      "summary_ko": "total_return_days=752"
    },
    {
      "key": "released-work",
      "parse_status": "ok",
  ...
  "ranked_work": [
    {
      ...
      "priority_score": 9750,
      "reason_ko": "현재 전략은 forward 관측 19, PSR unknown < 0.95로 관찰 중이다. 관찰 시계와 연구 후보 식별자를 분리하고, 기다림과 별도로 신규 no-live 후보를 탐색한다.",
      "required_inputs": [
  ...
  "suppressed_work": [
    {
      ...
      "priority_score": 1,
      "reason_ko": "three_asset_fixed-w10은 역사 검증을 통과했지만 globalfixed 전진 기준은 미달이다: 관측 19, PSR unknown < 0.95.",
      "required_inputs": [
        "automation/profit-evidence-engine-last-run:profit_evidence.json",
        "automation/rebalance-paper-forward-last-run:LAST_RUN.md",
        "automation/capital-path-readiness-last-run:capital_path_readiness.json",
