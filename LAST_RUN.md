# 자율 작업 실행 루프 (as of 2026-09-15T14:18:20Z)
...
| selected_work | candidate-parallel-edge-challenger-d59c4161bcba |
| title_ko | 실거래 관찰과 병렬인 신규 엣지 challenger |
...
| completion_gates | 관련 focused pytest 통과; uv run pytest 통과; uv run ruff check src tests 통과; uv run python scripts/check_handoff_facts.py 통과; uv run python scripts/agent_harness_probe.py --strict 통과; PR 품질 관문 통과; 필요한 HANDOFF 갱신 |

## 실행 가능 후보

| 후보 | 영역 | 상태 | 자율 수준 | 위험 | 점수 | 이유 |
|------|------|------|----------|-----:|-----:|------|
| candidate-parallel-edge-challenger-d59c4161bcba | strategy_design | EXECUTION_READY | CODEX_AUTONOMOUS_START | 2 | 9750 | 현재 전략은 forward 관측 14, PSR unknown < 0.95로 관찰 중이다. 관찰 시계와 연구 후보 식별자를 분리하고, 기다림과 별도로 신규 no-live 후보를 탐색한다. |

## 승인 필요 또는 억제 후보

| 후보 | 영역 | 상태 | 위험 | 안전 표면 | 이유 |
|------|------|------|-----:|-----------|------|
| wait-for-globalfixed-forward-edge | strategy_design | OBSERVATION_WAIT | 0 | - | three_asset_fixed-w10은 역사 검증을 통과했지만 globalfixed 전진 기준은 미달이다: 관측 14, PSR unknown < 0.95. |
...
| selected_candidate_id | candidate-parallel-edge-challenger-d59c4161bcba |
...
|------|------|-----:|-----:|-----:|-----:|-----:|-----:|-----:|------|
| candidate-parallel-edge-challenger-d59c4161bcba | EXECUTION_READY | 2 | 92 | 100 | 100 | 70 | 100 | 60 | 기존 안전 경계 안에서 검증 가능한 다음 작업 후보입니다. |
| wait-for-globalfixed-forward-edge | OBSERVATION_WAIT | 0 | 64 | 0 | 100 | 85 | 100 | 60 | 기존 안전 경계 안에서 검증 가능한 다음 작업 후보입니다. |
...
| candidate-1ed634d8bf6d | SUPPRESSED | 2 | 90 | 91 | 100 | 90 | 100 | 45 | 기존 안전 경계 안에서 검증 가능한 다음 작업 후보입니다. |

## 거시 후보 지도

| 영역 | 상태 | 실행 | 닫힘 | 완료 | 억제 | 점수 | 추천 후보 | 이유 |
|------|------|-----:|-----:|-----:|-----:|-----:|-----------|------|
| 투자 엣지 | active | 1 | 10 | 8 | 2 | 2400 | candidate-investment-edge-frontier-map | 최근 후보는 운영 체계 개선에 치우쳤고, 장기 목표는 측정 가능한 투자 성과 성장이다. |
...
|------|------|-------:|-----:|-----------|---------|------|
| 검증 명령 재현 계약 | released | 0 | 2480 | candidate-broad-validation-failure-command-replay-contract | validation_command, exit_code, safe_replay, output_digest | 현재 막힌 패키지는 모두 `execution_failed` 진단이며, 실패 명령·종료 코드·제한 출력이 다음 후보 발굴의 병목이다. 같은 패키지를 막연히 재시도하지 말고 재현 가능한 실패 지문 계약으로 먼저 닫아야 한다. |
| 검증 데이터 준비도 | released | 0 | 2380 | candidate-broad-validation-failure-data-readiness-contract | history_root, portfolio_toml, market_history_coverage | 두 패키지 모두 후보 history root와 포트폴리오 입력을 요구한다. 실행 실패가 전략 아이디어의 실패인지 데이터 준비 결함인지 분리하지 못하면 엣지 탐색 폭이 실제보다 좁아진다. |
| 전략·포트폴리오 패키지 분리 | released | 0 | 2280 | candidate-broad-validation-failure-package-kind-expansion-contract | strategy_family, portfolio_design, holding_period | 막힌 패키지는 전략 백테스트와 포트폴리오 백테스트가 섞여 있다. 둘을 같은 실패로 취급하면 전략군, 포트폴리오 구성, 보유 기간, 산출 증거를 넓히는 순서가 흐려진다. |
| 승격 재검토 조건 | released | 0 | 2180 | candidate-broad-validation-failure-promotion-recheck-contract | learning_ledger, promotion_evidence, recheck_condition | learning ledger가 실패 후보를 억제한 뒤 재검토 조건이 없으면, 실패가 고쳐져도 후보가 닫힌 채 남을 수 있다. 억제 기억은 필요하지만 새 증거가 생겼을 때 되살리는 조건도 필요하다. |

## 광역 no-edge frontier 지도

| 영역 | 상태 | 점수 | 추천 후보 | 검토 축 | 이유 |
|------|------|-----:|-----------|---------|------|
| 자산군 확장과 방어 회전 | released | 2490 | candidate-broad-no-edge-asset-universe-rotation-experiment | strategy_family, asset_universe | 기존 비교 트랙이 모두 NO_EDGE라면 같은 자산군 안에서 신호만 바꾸는 탐색은 충분히 넓지 않다. |
| 다중 보유 기간과 신호군 | released | 2390 | candidate-broad-no-edge-multi-horizon-signal-experiment | signal_family, holding_period | 단일 보유 기간이나 유사한 momentum 신호만 반복하면 엣지가 없는 구간을 계속 같은 방식으로 재측정할 수 있다. |
| 레짐과 비용 견고성 | released | 2290 | candidate-broad-no-edge-regime-cost-robustness-experiment | regime_window, cost_sensitivity | paper 성과가 비용, 슬리피지, 레짐 전환에 약하면 실제 돈 경로로 올라갈 수 없다. |
| 데이터 결측 원인 감사 | released | 2190 | candidate-broad-no-edge-data-gap-audit | data_missing_cause, regime_window | 엣지가 없다는 결론이 진짜 전략 문제인지, 공개 데이터 범위·조인·레짐 라벨 결측 때문인지 분리해야 한다. |
| 자산 간 상대가치와 현금 대체 | released | 2090 | candidate-broad-no-edge-cross-asset-relative-value-experiment | relative_value_spread, cash_proxy_yield, asset_pair | 자산군·신호·레짐·데이터 결측 축을 모두 닫았는데도 NO_EDGE_YET가 계속되면, 절대 모멘텀만 보지 말고 자산 간 스프레드와 현금 대체 수익률을 함께 비교해야 한다. |
| 꼬리위험 방어와 볼록성 | released | 1990 | candidate-broad-no-edge-tail-risk-convexity-experiment | tail_regime, convexity_proxy, cost_drag | 평균 수익률 엣지가 약한 구간에서는 큰 하락장에서 손실을 줄이는 볼록성 후보가 자본 사다리의 다음 입력이 될 수 있다. |
| 변동성 목표와 낙폭 제어 | released | 1890 | candidate-broad-no-edge-vol-target-drawdown-experiment | volatility_target, drawdown_control, psr_sensitivity | 현재 전략은 칼마가 벤치마크보다 낫지만 PSR이 기준 미달이다. 포지션 진입 신호가 아니라 변동성 목표와 낙폭 제어가 확률 신뢰도를 올릴 수 있는지 분리해야 한다. |

## 입력 증거

| 증거 | 존재 | 파싱 | 요약 |
|------|:----:|------|------|
| capital-path-readiness | yes | ok | readiness=CAPITAL_ARMABLE, live=REAL_ORDER_PATH_ARMED |
...
| profit-evidence-engine | yes | ok | 구조화 JSON 존재 |
| strategy-factory | yes | ok | 구조화 JSON 존재 |
| ml-edge-ensemble | yes | ok | 구조화 JSON 존재 |
| daily-cross-asset-ml | yes | ok | 구조화 JSON 존재 |
| rebalance-paper-forward | yes | ok | 구조화 JSON 존재 |
| edge-autoarm | yes | malformed | 원문 존재, 구조화 JSON 파싱 실패 |
...
| public-data | yes | ok | overall_ok=False, published=22 |
| regime-stratify | yes | ok | total_return_days=750 |
...
| trigger | schedule |
| timestamp_utc | 2026-09-15T14:18:21Z |
...
```json
{
  ...
  "broad_no_edge_frontier_map": [
    {
      ...
      "next_action_ko": "public-data, rebalance-paper-forward, money-path를 함께 읽어 주식·현금성·채권성·방어 자산군 대체 후보의 no-live 후보군과 제외 기준을 SDD로 정의한다.",
      "priority_score": 2490,
      "reason_ko": "기존 비교 트랙이 모두 NO_EDGE라면 같은 자산군 안에서 신호만 바꾸는 탐색은 충분히 넓지 않다.",
    ...
    },
    {
      ...
      "next_action_ko": "rebalance-paper-forward와 learning ledger를 읽어 단기·중기·장기 보유 기간과 trend·carry·quality·volatility 신호군을 분리한 no-live 실험 후보를 만든다.",
      "priority_score": 2390,
      "reason_ko": "단일 보유 기간이나 유사한 momentum 신호만 반복하면 엣지가 없는 구간을 계속 같은 방식으로 재측정할 수 있다.",
      "recommended_candidate_id": "candidate-broad-no-edge-multi-horizon-signal-experiment",
      "required_inputs": [
        "automation/released-work-last-run:released_work.json",
        "automation/autonomous-evolution-last-run:candidate_backlog.json",
        "automation/autonomous-evolution-last-run:learning_ledger.json",
    ...
    },
    {
      ...
      "label_ko": "레짐과 비용 견고성",
      "next_action_ko": "regime-stratify, execution-quality, money-path 증거를 함께 읽어 레짐 구간별 통과 기준과 비용 민감도 stress test를 no-live 계약으로 정의한다.",
      "priority_score": 2290,
      "reason_ko": "paper 성과가 비용, 슬리피지, 레짐 전환에 약하면 실제 돈 경로로 올라갈 수 없다.",
      "recommended_candidate_id": "candidate-broad-no-edge-regime-cost-robustness-experiment",
      "required_inputs": [
        "automation/released-work-last-run:released_work.json",
        "automation/autonomous-evolution-last-run:candidate_backlog.json",
        "automation/autonomous-evolution-last-run:learning_ledger.json",
    ...
    },
    {
      ...
      "next_action_ko": "public-data summary, regime.json, regime_timeline.csv를 읽어 데이터 결측 원인이 no-edge 판정에 끼친 영향을 분리하는 읽기 전용 감사를 정의한다.",
      "priority_score": 2190,
      "reason_ko": "엣지가 없다는 결론이 진짜 전략 문제인지, 공개 데이터 범위·조인·레짐 라벨 결측 때문인지 분리해야 한다.",
      "recommended_candidate_id": "candidate-broad-no-edge-data-gap-audit",
      "required_inputs": [
        "automation/released-work-last-run:released_work.json",
        "automation/autonomous-evolution-last-run:candidate_backlog.json",
        "automation/autonomous-evolution-last-run:learning_ledger.json",
    ...
    },
    {
      ...
      "next_action_ko": "rebalance-paper-forward, public-data, regime-stratify, money-path를 함께 읽어 주식·채권·원자재·현금성 자산 간 상대가치 no-live 후보와 제외 기준을 SDD로 정의한다.",
      "priority_score": 2090,
      "reason_ko": "자산군·신호·레짐·데이터 결측 축을 모두 닫았는데도 NO_EDGE_YET가 계속되면, 절대 모멘텀만 보지 말고 자산 간 스프레드와 현금 대체 수익률을 함께 비교해야 한다.",
      "recommended_candidate_id": "candidate-broad-no-edge-cross-asset-relative-value-experiment",
      "required_inputs": [
        "automation/released-work-last-run:released_work.json",
        "automation/autonomous-evolution-last-run:candidate_backlog.json",
        "automation/autonomous-evolution-last-run:learning_ledger.json",
    ...
    },
    {
      ...
      "next_action_ko": "regime-stratify, execution-quality, rebalance-paper-forward 증거를 함께 읽어 tail-risk 방어 후보, 비용 부담, 레짐별 대기 조건을 no-live 계약으로 만든다.",
      "priority_score": 1990,
      "reason_ko": "평균 수익률 엣지가 약한 구간에서는 큰 하락장에서 손실을 줄이는 볼록성 후보가 자본 사다리의 다음 입력이 될 수 있다.",
      "recommended_candidate_id": "candidate-broad-no-edge-tail-risk-convexity-experiment",
      "required_inputs": [
        "automation/released-work-last-run:released_work.json",
        "automation/autonomous-evolution-last-run:candidate_backlog.json",
        "automation/autonomous-evolution-last-run:learning_ledger.json",
    ...
    },
    {
      ...
      "next_action_ko": "money-path, edge-autoarm, forward verdict, live drawdown evidence를 함께 읽어 변동성 목표·낙폭 제어 no-live 후보와 자본 사다리로 올릴 수 없는 제외 조건을 정의한다.",
      "priority_score": 1890,
      "reason_ko": "현재 전략은 칼마가 벤치마크보다 낫지만 PSR이 기준 미달이다. 포지션 진입 신호가 아니라 변동성 목표와 낙폭 제어가 확률 신뢰도를 올릴 수 있는지 분리해야 한다.",
      "recommended_candidate_id": "candidate-broad-no-edge-vol-target-drawdown-experiment",
      "required_inputs": [
        "automation/released-work-last-run:released_work.json",
        "automation/autonomous-evolution-last-run:candidate_backlog.json",
        "automation/autonomous-evolution-last-run:learning_ledger.json",
  ...
  "broad_validation_failure_frontier_map": [
    {
      ...
      "next_action_ko": "candidate-result-executor와 candidate-packages 증거를 읽어 실패 명령, 허용된 읽기 전용 재현 범위, 종료 코드, stdout/stderr 요약, 다음 진단 action을 기계 판독 계약으로 만든다.",
      "package_count": 0,
      "package_kinds": [
        "strategy_backtest",
        "portfolio_backtest"
      ...
      "priority_score": 2480,
      "reason_ko": "현재 막힌 패키지는 모두 `execution_failed` 진단이며, 실패 명령·종료 코드·제한 출력이 다음 후보 발굴의 병목이다. 같은 패키지를 막연히 재시도하지 말고 재현 가능한 실패 지문 계약으로 먼저 닫아야 한다.",
    ...
    },
    {
      ...
      "label_ko": "검증 데이터 준비도",
      "next_action_ko": "candidate history support, portfolio TOML, public-data, regime-stratify 증거를 함께 읽어 history root, 관측 기간, 데이터 결측 원인을 검증 패키지별 PASS/WAIT/FAIL로 분리한다.",
      "package_count": 0,
      "package_kinds": [
        "strategy_backtest",
        "portfolio_backtest"
      ...
      "priority_score": 2380,
      "reason_ko": "두 패키지 모두 후보 history root와 포트폴리오 입력을 요구한다. 실행 실패가 전략 아이디어의 실패인지 데이터 준비 결함인지 분리하지 못하면 엣지 탐색 폭이 실제보다 좁아진다.",
      "recommended_candidate_id": "candidate-broad-validation-failure-data-readiness-contract",
      "required_inputs": [
        "automation/released-work-last-run:released_work.json",
        "automation/autonomous-evolution-last-run:candidate_backlog.json",
        "automation/autonomous-evolution-last-run:learning_ledger.json",
    ...
    },
    {
      ...
      "next_action_ko": "strategy_backtest와 portfolio_backtest 실패를 나눠 전략군, 포트폴리오 구성, 보유 기간, 산출 증거별 no-live 후보 축을 재정렬한다.",
      "package_count": 0,
      "package_kinds": [
        "strategy_backtest",
        "portfolio_backtest"
      ...
      "priority_score": 2280,
      "reason_ko": "막힌 패키지는 전략 백테스트와 포트폴리오 백테스트가 섞여 있다. 둘을 같은 실패로 취급하면 전략군, 포트폴리오 구성, 보유 기간, 산출 증거를 넓히는 순서가 흐려진다.",
      "recommended_candidate_id": "candidate-broad-validation-failure-package-kind-expansion-contract",
      "required_inputs": [
        "automation/released-work-last-run:released_work.json",
        "automation/autonomous-evolution-last-run:candidate_backlog.json",
        "automation/autonomous-evolution-last-run:learning_ledger.json",
    ...
    },
    {
      ...
      "label_ko": "승격 재검토 조건",
      "next_action_ko": "learning ledger, autonomous-promotion, candidate-result evidence를 함께 읽어 어떤 실패 지문 변화가 후보 재검토를 허용하는지 결정론적 계약으로 만든다.",
      "package_count": 0,
      "package_kinds": [
        "strategy_backtest",
        "portfolio_backtest"
      ...
      "priority_score": 2180,
      "reason_ko": "learning ledger가 실패 후보를 억제한 뒤 재검토 조건이 없으면, 실패가 고쳐져도 후보가 닫힌 채 남을 수 있다. 억제 기억은 필요하지만 새 증거가 생겼을 때 되살리는 조건도 필요하다.",
      "recommended_candidate_id": "candidate-broad-validation-failure-promotion-recheck-contract",
      "required_inputs": [
        "automation/released-work-last-run:released_work.json",
        "automation/autonomous-evolution-last-run:candidate_backlog.json",
        "automation/autonomous-evolution-last-run:learning_ledger.json",
  ...
  "evidence_surfaces": [
    {
      ...
      "summary_ko": "readiness=CAPITAL_ARMABLE, live=REAL_ORDER_PATH_ARMED"
    },
    {
    ...
    },
    {
      ...
      "parse_status": "ok",
      "present": true,
      "source_ref": "automation/profit-evidence-engine-last-run:profit_evidence.json",
      "summary_ko": "구조화 JSON 존재"
    },
    {
      "key": "strategy-factory",
      "parse_status": "ok",
      "present": true,
      "source_ref": "automation/autonomous-strategy-factory-last-run:strategy_factory.json",
      "summary_ko": "구조화 JSON 존재"
    },
    {
      "key": "ml-edge-ensemble",
      "parse_status": "ok",
      "present": true,
      "source_ref": "automation/ml-edge-ensemble-last-run:report.json",
      "summary_ko": "구조화 JSON 존재"
    },
    {
      "key": "daily-cross-asset-ml",
      "parse_status": "ok",
      "present": true,
      "source_ref": "automation/daily-cross-asset-ml-last-run:report.json",
      "summary_ko": "구조화 JSON 존재"
    },
    {
    ...
    },
    {
      ...
      "summary_ko": "overall_ok=False, published=22"
    },
    {
      "key": "regime-stratify",
      "parse_status": "ok",
      ...
      "summary_ko": "total_return_days=750"
    },
    {
      "key": "released-work",
      "parse_status": "ok",
  ...
  "objective_calibration": {
    "candidate_scores": [
      {
        "candidate_id": "candidate-parallel-edge-challenger-d59c4161bcba",
        "component_scores": {
          "evidence_readiness": 100,
          "growth_leverage": 100,
          "learning_value": 60,
          "safety_margin": 100,
          "validation_cost_fit": 70
        },
      ...
      {
        "candidate_id": "wait-for-globalfixed-forward-edge",
        "component_scores": {
          "evidence_readiness": 100,
          "growth_leverage": 0,
          "learning_value": 60,
  ...
  "ranked_work": [
    {
      ...
      "next_action_ko": "공개 데이터·레짐·비용·paper forward를 사용해 기존 후보와 다른 신호군을 시간 분리 walk-forward로 검증하고, 통과하지 못하면 live 승격 없이 기각한다.",
      "packet_id": "work-d608a96f47ab",
      "priority_score": 9750,
      "reason_ko": "현재 전략은 forward 관측 14, PSR unknown < 0.95로 관찰 중이다. 관찰 시계와 연구 후보 식별자를 분리하고, 기다림과 별도로 신규 no-live 후보를 탐색한다.",
      "required_inputs": [
  ...
  "suppressed_work": [
    {
      ...
      "domain_key": "strategy_design",
      "next_action_ko": "같은 전략 지문으로 새 독립 forward 관측을 누적하고 profit-evidence와 autonomous-work를 다시 평가한다.",
      "packet_id": "work-092e37cf7e80",
      "priority_score": 1,
      "reason_ko": "three_asset_fixed-w10은 역사 검증을 통과했지만 globalfixed 전진 기준은 미달이다: 관측 14, PSR unknown < 0.95.",
      "required_inputs": [
        "automation/profit-evidence-engine-last-run:profit_evidence.json",
        "automation/rebalance-paper-forward-last-run:LAST_RUN.md",
        "automation/capital-path-readiness-last-run:capital_path_readiness.json",
      ...
      "status": "OBSERVATION_WAIT",
      "title_ko": "globalfixed 전진 엣지 관찰",
      "validation_failure_groups": [],
      "work_type": "forward_edge_watch"
    },
    {
  ...
  "timestamp_utc": "2026-09-15T14:18:20Z"
}
```
