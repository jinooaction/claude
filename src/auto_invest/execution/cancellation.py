...
async def request_cancellation(
    ...
    *,
    ...
    market: str,
    reason: str,
    before_write_guard: Callable[[], str | None] | None = None,
) -> str:
    conn = authority.conn
    row = conn.execute("SELECT * FROM orders WHERE correlation_id=?", (correlation_id,)).fetchone()
    ...
    def claim():
        nonlocal claimed
        if before_write_guard is not None and before_write_guard():
            raise ValueError("CANCEL_WRITE_GUARD")
        ...
        try:
            ...
            audit.append(
                conn,
                audit.OrderCancelRequestPayload(
