...
def test_released_package_kind_entry_advances_to_promotion_recheck():
    base_evidence = {
        "capital-path-readiness": _json(
            {
                "priority_candidates": [
    ...
    assert validation_map["package_kind_expansion"].coverage_status == "released"
    assert validation_map["promotion_recheck_contract"].coverage_status == "open"


def test_released_promotion_recheck_entry_does_not_repeat_same_child():
    base_evidence = {
        "capital-path-readiness": _json(
            {
                "priority_candidates": [
    ...
    assert validation_map["promotion_recheck_contract"].coverage_status == "released"
    assert all(entry.coverage_status == "released" for entry in validation_map.values())
...
def test_released_multi_horizon_signal_advances_to_regime_cost_entry():
    parent_report = build_autonomous_work_execution(
        _all_known_released_no_edge_evidence(),
        now=NOW,
    )
    ...
    assert broad_map["regime_cost_robustness"].coverage_status == "open"
    assert report.selected_work is not None
    assert (
        report.selected_work.candidate_id
        == BROAD_NO_EDGE_REGIME_COST_ROBUSTNESS_CANDIDATE_ID
