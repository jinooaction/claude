"""스펙 136 — 꼬리위험 방어·볼록성 no-live 계약 코어 테스트."""
...
from auto_invest.analytics.broad_no_edge_tail_risk_convexity import (
    BLOCKED,
...
def _forward(*, include_tail_drawdown: bool = True) -> str:
    drawdown = "19.890290" if include_tail_drawdown else "4.000000"
    rows = [
        {
            "key": "multiasset",
            "label": "멀티에셋 분산 추세",
        ...
        },
        {
            ...
            "calmar": "10.898148",
            "max_drawdown_pct": "11.595110" if include_tail_drawdown else "3.000000",
            "universe_size": 3,
        ...
        },
        {
            ...
            "calmar": "1.389891",
            "max_drawdown_pct": "13.600883" if include_tail_drawdown else "2.000000",
            "universe_size": 11,
...
def _regime(*, include_tail: bool = True) -> str:
    worst = "-4.09" if include_tail else "-0.50"
    caution_dd = "6.47" if include_tail else "1.00"
    payload = {
        "schema_version": "1.0",
        ...
        "by_label": {
            "CAUTION": {
                ...
                "worst_day_pct": "-2.18" if include_tail else "-0.30",
                "max_drawdown_pct": caution_dd,
                "sharpe": "0.86",
            },
            "RISK_OFF": {
                "n_days": 7 if include_tail else 0,
            ...
            "RISK_ON": {
                ...
                "worst_day_pct": worst,
                "max_drawdown_pct": "6.97" if include_tail else "1.00",
                "sharpe": "1.85",
            },
        },
    }
...
def test_current_evidence_emits_tail_risk_convexity_contract():
    report = build_broad_no_edge_tail_risk_convexity(
        _evidence(),
        now=NOW,
        run_id="123",
        commit="abc123",
    ...
    assert payload["contract_id"] == "broad-no-edge-tail-risk-convexity"
    ...
    assert payload["regime_tail_profile"]["tail_labels"]
    assert len(payload["convexity_lanes"]) == 5
    assert "no orders" in payload["safety_boundary"]
    assert "꼬리위험 방어·볼록성 no-live 실험 계약" in report.as_markdown()
    assert _gates(report)["convexity-lane-coverage"] == "PASS"
...
def test_missing_tail_regime_keeps_contract_waiting():
    report = build_broad_no_edge_tail_risk_convexity(
        _evidence(
            **{
                "rebalance-paper-forward": _forward(include_tail_drawdown=False),
                "regime-stratify": _regime(include_tail=False),
            }
        ),
        now=NOW,
    )
    ...
    assert _gates(report)["tail-regime-coverage"] == "WAIT"
