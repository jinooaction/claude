"""스펙 131 — 다중 보유 기간 신호군 no-live probe 통합 테스트."""
...
_HELPERS_PATH = _ROOT / "tests" / "unit" / "test_broad_no_edge_multi_horizon_signal.py"
_helpers_spec = importlib.util.spec_from_file_location(
    "test_broad_no_edge_multi_horizon_signal_helpers",
    _HELPERS_PATH,
)
...
_PROBE_PATH = _ROOT / "scripts" / "broad_no_edge_multi_horizon_signal_probe.py"
_spec = importlib.util.spec_from_file_location(
    "broad_no_edge_multi_horizon_signal_probe",
    _PROBE_PATH,
)
...
def _write_sidecars(path: Path) -> None:
    values = {
        ...
        "public-data": _helpers._public_data(),
...
def test_probe_writes_json_and_markdown(tmp_path, capsys):
    ...
    assert "다중 보유 기간 신호군 no-live 실험 계약" in summary_out.read_text(
        encoding="utf-8"
    )
