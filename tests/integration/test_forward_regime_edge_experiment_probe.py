"""스펙 095 — forward 레짐 엣지 실험 probe 통합 테스트."""
...
_PROBE_PATH = _ROOT / "scripts" / "forward_regime_edge_experiment_probe.py"
_spec = importlib.util.spec_from_file_location(
    "forward_regime_edge_experiment_probe",
    _PROBE_PATH,
)
...
def test_probe_writes_json_and_markdown(tmp_path, capsys):
    ...
    json_out = tmp_path / "forward_regime_edge_experiment.json"
    ...
    assert written["completed_candidate_id"] == "candidate-forward-regime-edge-experiment"
    assert "forward 레짐 엣지 no-live 실험 계약" in summary_out.read_text(
        encoding="utf-8"
    )
...
def test_probe_repo_root_released_work_overrides_sidecar_lag(tmp_path, capsys):
    ...
    spec_dir = repo_root / "specs" / "095-forward-regime-edge-experiment"
    contracts_dir = spec_dir / "contracts"
    contracts_dir.mkdir(parents=True)
    (spec_dir / "tasks.md").write_text("- [x] 구현 완료\n", encoding="utf-8")
    (contracts_dir / "forward-regime-edge-experiment.md").write_text(
