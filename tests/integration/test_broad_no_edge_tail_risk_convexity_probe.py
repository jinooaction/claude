"""스펙 136 — 꼬리위험 방어·볼록성 no-live probe 통합 테스트."""
...
_HELPERS_PATH = _ROOT / "tests" / "unit" / "test_broad_no_edge_tail_risk_convexity.py"
_helpers_spec = importlib.util.spec_from_file_location(
    "test_broad_no_edge_tail_risk_convexity_helpers",
    _HELPERS_PATH,
)
...
_PROBE_PATH = _ROOT / "scripts" / "broad_no_edge_tail_risk_convexity_probe.py"
_spec = importlib.util.spec_from_file_location(
    "broad_no_edge_tail_risk_convexity_probe",
    _PROBE_PATH,
)
...
def test_probe_writes_json_and_markdown(tmp_path, capsys):
    ...
    assert "꼬리위험 방어·볼록성 no-live 실험 계약" in summary_out.read_text(
        encoding="utf-8"
    )
