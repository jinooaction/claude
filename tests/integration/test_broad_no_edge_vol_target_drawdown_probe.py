"""스펙 137 — 변동성 목표·낙폭 제어 no-live probe 통합 테스트."""
...
_HELPERS_PATH = _ROOT / "tests" / "unit" / "test_broad_no_edge_vol_target_drawdown.py"
_helpers_spec = importlib.util.spec_from_file_location(
    "test_broad_no_edge_vol_target_drawdown_helpers",
    _HELPERS_PATH,
)
...
_PROBE_PATH = _ROOT / "scripts" / "broad_no_edge_vol_target_drawdown_probe.py"
_spec = importlib.util.spec_from_file_location(
    "broad_no_edge_vol_target_drawdown_probe",
    _PROBE_PATH,
)
...
def _write_sidecars(path: Path) -> None:
    values = {
        ...
        "execution-quality": _helpers._execution(),
...
def test_probe_writes_json_and_markdown(tmp_path, capsys):
    ...
    assert "변동성 목표·낙폭 제어 no-live 실험 계약" in summary_out.read_text(
        encoding="utf-8"
    )
