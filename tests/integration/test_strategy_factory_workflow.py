...
def test_workflow_runs_complete_batch_without_broker_or_order_command() -> None:
    ...
    assert "data_fingerprint: $root.energy_cross_market_data_fingerprint" in text
    assert "prior_data_fingerprint: $root.usda_crop_data_fingerprint" in text
    ...
    assert 'global_audit_trial_count\' /tmp/strategy_factory.json)" = "736' in text
