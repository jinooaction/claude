"""스펙 135 — 자산 간 상대가치 no-live probe 통합 테스트."""
...
_HELPERS_PATH = _ROOT / "tests" / "unit" / "test_broad_no_edge_cross_asset_relative_value.py"
_helpers_spec = importlib.util.spec_from_file_location(
    "test_broad_no_edge_cross_asset_relative_value_helpers",
    _HELPERS_PATH,
)
...
_PROBE_PATH = _ROOT / "scripts" / "broad_no_edge_cross_asset_relative_value_probe.py"
_spec = importlib.util.spec_from_file_location(
    "broad_no_edge_cross_asset_relative_value_probe",
    _PROBE_PATH,
)
...
def _write_sidecars(path: Path) -> None:
    values = {
        "rebalance-paper-forward": _helpers._forward(),
        "public-data-summary": _helpers._public_data(),
        "regime-stratify": _helpers._regime(),
...
def test_probe_writes_json_and_markdown(tmp_path, capsys):
    ...
    assert "자산 간 상대가치 no-live 실험 계약" in summary_out.read_text(
        encoding="utf-8"
    )
