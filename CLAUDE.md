...
현재 계획 포인터: `specs/104-execution-cost-basis-contract/plan.md` — execution-quality와 money-path 증거를 읽어 accepted/fill 체결 비용 기준이 충분한지와 관측 대기 상태를 분리하고, 완료 뒤 브로커 진단 생존성 후보로 전진시키는 읽기 전용 계약 설계.
