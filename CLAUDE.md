...
현재 계획 포인터: `specs/132-broad-no-edge-regime-cost-robustness/plan.md` — broad no-edge 후보 `candidate-broad-no-edge-regime-cost-robustness-experiment`를 닫기 위해 regime-stratify, execution-quality, money-path 증거를 함께 읽어 레짐 구간별 통과 기준과 10/25/50bp 비용 민감도 stress test를 분리하는 no-live 운영 계약.
