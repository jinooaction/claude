...
현재 계획 포인터: `specs/131-broad-no-edge-multi-horizon-signal/plan.md` — broad no-edge 후보 `candidate-broad-no-edge-multi-horizon-signal-experiment`를 닫기 위해 rebalance-paper-forward, learning ledger, public-data, regime-stratify 증거를 함께 읽어 단기·중기·장기 보유 기간과 trend·carry·quality·volatility 신호군을 분리하는 no-live 운영 계약.
...
후속 후보(스펙 012 이후 선택지):
- L1 적용 표면 확장 — 모델 라우팅·max_tokens 를 즉시 자동 적용 L1 노브로(스펙 012 가 캐너리 검증 경로를 깔면 후속으로 안전 적용 가능).
- L2/L3 후보 → 캐너리 합격 시 운영자 승격 큐(자동 승격은 여전히 운영자 게이트).
