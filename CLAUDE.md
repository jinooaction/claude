...
<!-- SPECKIT START -->
Active feature: `specs/009-paper-trading-daemon/` (specified + planned 2026-05-19)
...
2. `.specify/memory/kernel.toml` — machine-readable Kernel manifest (this feature touches K4 once, additively — same pattern as spec 008).
3. `specs/009-paper-trading-daemon/spec.md` — feature spec (3 user stories, 16 FRs, 8 SCs, 6 edge cases, clarifications 2026-05-19).
4. `specs/009-paper-trading-daemon/plan.md` — implementation plan (technical context, constitution check, source-code layout).
5. `specs/009-paper-trading-daemon/research.md` — Phase 0 decisions (R-P1 … R-P11).
6. `specs/009-paper-trading-daemon/data-model.md` — entities, 4 new K4 payloads, derived virtual positions, state transitions.
7. `specs/009-paper-trading-daemon/contracts/` — `paper-run-cli.md`, `paper-report-cli.md`, `paper-audit-events.md`.
8. `specs/009-paper-trading-daemon/quickstart.md` — operator onboarding for paper-trading.

Tasks generated by `/speckit-tasks` will live at `specs/009-paper-trading-daemon/tasks.md`.

Branch convention: SDD work for this spec lives on `claude/continue-previous-session-20p3O`; spec dir name is `009-paper-trading-daemon`. Use `SPECIFY_FEATURE=009-paper-trading-daemon` when the spec-kit scripts ask for a feature branch name.
...
- Specs 002 (telemetry, shipped), 003 (session-cache, shipped), 004 (judgment, stub), 005 (autonomous-tuner, stub), 006 (deploy automation, kernel guard shipped + runner pending), 007 (hardened canary, stub — paper-trading is the pre-canary observation step), 008 (backtest-engine, in-flight on `claude/continue-work-ID7Ec` — complementary to paper-trading: backtest = past CSV validation, paper = live-market sandbox).
<!-- SPECKIT END -->
