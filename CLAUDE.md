...
현재 계획 포인터: `specs/130-validation-failure-promotion-recheck/plan.md` — 검증 실패 child 후보 `candidate-broad-validation-failure-promotion-recheck-contract`를 닫기 위해 learning ledger, autonomous-promotion, candidate-result evidence를 함께 읽어 같은 실패 지문이면 억제를 유지하고 새 증거가 생기면 재검토를 허용하는 no-live 운영 계약.
