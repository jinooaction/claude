...
main 베이스라인: `8f9a99f`(PR #400). 운영자가 기다리는 시장 관측 시간 동안 데이터 수집,
데이터 분석, 전략 설계, 포트폴리오 설계, 실시간 매매, 회고, 에이전트 운영 전 영역을 계속
자동 자율 고도화하고 싶다고 지시했다. 이번 작업은 실주문·자본·전략 설정을 바꾸지 않고,
그 목표를 안전하게 구현할 수 있는 스펙 067 SDD 산출물을 만든 등급 2 운영 설계 변경이다.

## 무엇이 바뀌었나

- `specs/067-autonomous-evolution-loop/spec.md`: 전 영역 개선 후보 발굴, 안전한 실험 설계,
  기존 게이트를 통한 승격, 학습 기록·생존 감시를 사용자 시나리오와 요구사항으로 고정했다.
- `specs/067-autonomous-evolution-loop/plan.md`: 첫 구현 슬라이스를 read-only 스캔, 후보 점수화,
...
- 구현은 아직 시작하지 않았다. 다음 구현 세션은 `specs/067-autonomous-evolution-loop/tasks.md`
  T001부터 진행하면 된다.
- 스펙 067의 첫 구현 범위는 읽기 전용이다. evidence 수집, 후보 발굴, 실험 계획, 학습 장부,
  latest-run sidecar, liveness 편입을 만든다.
- 자동 고도화 루프는 기존 스펙 005 자율 튜너, 스펙 055 자율 재지정, 스펙 050 자본 사다리,
  ...
  위에 얹혀 "무엇을 다음에 안전하게 개선할지"를 정하는 상위 루프다.
...
스펙 067은 "기다리는 동안 스스로 다음 개선 후보를 찾고 안전한 실험으로 승격하는 read-only
상위 루프"의 설계다. 구현은 아직 시작하지 않았으므로, 다음 세션은
