...
main 코드 베이스라인: `e77a42c`(PR #451). 이 작업은 자율 작업 실행 루프가 고른 `candidate-6ee3370e933d`를 처리한 등급 2 운영 보정이다. 오래된 증거, 완료 후보 잔향, sidecar 신선도 문제를 전략 성과 실패나 새 작업 후보처럼 보이지 않게 `capital-path-readiness` 보고서의 `observability_issues`로 분리했다.
...
- 같은 main push 안에서는 `capital-path-readiness`가 `released-work`보다 먼저 실행될 수 있다. 그래서 최신 capital-path-readiness sidecar는 `candidate-6ee3370e933d`를 아직 priority에 남겼지만, 뒤이어 발행된 released-work와 autonomous-work sidecar가 이를 닫았다.
...
- `priority_candidates=["candidate-6ee3370e933d", "candidate-facf2fa31834"]`
- `suppressed_candidates`에 `candidate-fd04772a23c5`, `candidate-e481b0309206`, `candidate-dff4f9344b02` 포함
- `observability_issues`에 `released-candidate-echo` 3건 포함
...
- 이 HANDOFF 갱신 뒤 `check_handoff_facts.py`와 strict 하네스를 다시 통과해야 한다.

## 다음 세션 한 줄

스펙 084는 완료됐다. 오래된 증거와 완료 후보 잔향은 이제 `capital-path-readiness.observability_issues`로 분리되고, `candidate-6ee3370e933d`는 `released-work`에 소비되어 다음 실제 착수 후보는 `candidate-facf2fa31834`다.
