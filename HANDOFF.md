...
* **스펙 003 (세션 캐시)** — 출시 완료.
* **스펙 004 (LLM 판단 지점)** — 골격만. **즉시 착수 가능** (운영자 지시 2026-05-24: 텔레메트리 30일 착수 게이트 제거). 판단 지점은 여전히 헌법 VI 캐너리 ≥10 거래일을 탄다.
* **스펙 005 (자율 튜너)** — 골격만. **즉시 착수 가능** (운영자 지시 2026-05-24: 텔레메트리 30일 게이트 제거; 스펙 006·007·011 선행 조건 충족). `/speckit-specify`→`/speckit-plan`→`/speckit-tasks`로 승격. 런타임 자율 튜닝은 원칙 X, 자율 머지는 스펙 007 캐너리에 계속 종속(안전 경계 불변).
...
**다음 후보 (둘 다 즉시 착수 가능 — 운영자 지시 2026-05-24로 텔레메트리 30일 게이트 제거)**:

* **스펙 004 (LLM 판단 지점)** — Claude를 거래 결정 루프에 처음 끌어들이는 작업. 결정성을 일부 양보하고 추론 능력을 얻는 트레이드오프. 골격은 `specs/004-llm-judgment-points/spec.md`. 텔레메트리 30일 착수 게이트는 제거됨 — 즉시 `/speckit-specify`부터 시작 가능. 판단 지점은 여전히 헌법 VI 캐너리 ≥10 거래일.
* **스펙 005 (자율 튜너)** — KPI·룰을 자동 조정해 측정→분석→행동 루프를 닫는 작업. 선행 조건(스펙 006·007·011) 충족 + 텔레메트리 30일 게이트 제거 → 즉시 착수 가능. 단 런타임 자율 튜닝 행동은 헌법 원칙 X(측정 기반), 자율 머지는 스펙 007 하드닝 캐너리(IX.B-2)에 계속 종속 — "코드 착수 시점"만 앞당겼지 안전 경계는 불변.

위 운영 절차 + 스펙 010 `design` 명령 + 스펙 011 `performance` 측정만으로도 v1 자동 거래 서비스는 굴러가며, 이제 004·005로 자율 성장 루프를 채울 수 있습니다.
...
| 활성 작업 | **없음 — 빌드 가능한 스펙 전부 완료.** 라이브 worker dry-run 가동 중 (2026-05-23 시작, 1주일 안전 관찰). spec 004·005는 운영자 지시(2026-05-24)로 즉시 착수 가능(텔레메트리 30일 게이트 제거). 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001, 002, 003, 006, 007, 008, 009, 010, 011 |
| 진행 중 스펙 | 없음 |
| 골격 스펙 (즉시 착수 가능) | 004 (LLM 판단 지점), 005 (자율 튜너) — 운영자 지시(2026-05-24)로 텔레메트리 30일 착수 게이트 제거. `/speckit-specify`부터 바로 시작. 안전 경계(원칙 X·스펙 007 캐너리·캐너리 ≥10일)는 불변 |
