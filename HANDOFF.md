...
**다음 단계(이 미해소 리스크를 푸는 길) = 고정가중을 forward-paper 트랙으로 추가**해 일별
강등 빈도를 실측(스펙 049/050 경로, 페이퍼·돈 0). 그 결과로 운영자가 고정가중 재지정 확정.
또는 운영자가 ① 레버리지 위해 안전 경계 변경을 승인하면 그 경로로(K-meta).
...
| 마지막 main 커밋 | `9673954 Merge pull request #313 — feat(050) 자본 사다리 순 복리 시뮬레이션` |
...
| main 테스트 | 2061 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
