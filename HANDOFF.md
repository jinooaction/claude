...
2026-09-21 후속195 진행: 예정/실제 실적 사건과 로컬 관측 시각을 분리하는 입력 도구 구현·검증 완료.
코드522ac3d의 관련42개·전체4927 passed/13 skipped(817.90초), ruff·하네스14/14·인계 검사 통과.
전체 검사71555는 종료 코드0이며 /tmp/195-full-pytest.log에 보존했다. 병합·배포 확인은 남아 있다.
...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `748b14e` — Merge pull request #850 from jinooaction/codex/194-next-input-audit |
| main 테스트 | 코드4d22e7b 전체4901 passed/13 skipped(830.94초), 관련194검사37개. 이후 문서만 변경. 마지막 실제KIS는 이전main6580c05의35541770144 12개 통과;194는 브로커 코드 미변경. |
| main 린트 | 코드4d22e7b의 `uv run ruff check src tests`와 신규 연구CLI, 하네스14/14·인계 사실 검사 통과. |
| 열린 PR | #827 유료 공급자 초안 보류.194 연구 도구 구현 PR848은 병합 완료. |
| 출시 완료 스펙 | 194 누락 보존 신호·공유현금·익일잔량·독립장부 연구 도구 출시, 실제 후보는 탈락.184 고정 예산 시작·중지·재시작 프로그램과 관측 구간 체결·정산 연결 유지.182 계좌 조회와183 모바일 운영판 유지. 전체 단타 실사용 목표는 별도다. |
...
| 최근 출시 작업 |194 연구 도구 PR848, 배포35559689034·감사35559747577 성공. target9df3a25 완료 확인. 기존 수집기·주문 권한 유지. |
| 활성 작업 |195 실적 사건 입력 구현·검증 진행. 194 실제30종목1515일 후보는 양비용 탈락, 최소31가설을 보존하며 새 성과 가설은 미선정. 최종확인495일 미개봉. 통과 전략·전진 관찰·실행 동등성·실제 운영 검증이 남음. |
