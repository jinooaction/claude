...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `9d22a2b` — Merge pull request #706 from jinooaction/Codex/175-pead-gate-recalibration |
...
| 열린 PR | 2026-08-31 #706까지 병합됐고 열린 기능 PR은 없다. 이 인계 갱신만 별도 브랜치에서 진행 중이다. |
...
| 활성 작업 | 배포 run `33337092305`는 성공했다. 전략 공장 run `33337092281`은 두 시도 모두 PEAD `PUBLISHED_EDGE` 계산 뒤 무출력 셸 문자열 검증에서 실패해 sidecar를 게시하지 못했다. 현재 인계 브랜치는 같은 계약을 JSON 자료형으로 검사하고 불일치 필드를 출력하도록 보정한다. 기존 돈 경로는 진단용 `3.2`를 거부하며 자본·실주문은 0건이다. |
