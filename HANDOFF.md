...
기존 서버 main 한정 검사에서 현재 수집기의 보관 자료를 읽는 시험을 추가한다.
현재 실제 서버 자료로 분석한 결과는 없으며, 실주문·정식 전진·NAV 승격이 아니다.
신규17개·전체3804 passed/12 skipped(781.43초), 린트·하네스14/14·인계 통과.
184 통합195c23d는 별도 전체3906 passed/11 skipped(796.72초)를 통과했다.

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `8e2e5ce` — Merge pull request #801 from jinooaction/codex/185-intraday-archive-review |
| main 테스트 | 코드30d27ed/main6e66d04의 전체3787 passed/11 skipped, 실패0(819.92초). 제외는 실제 KIS10개와 가동 전 조건1개다. 실제34307196553은10 passed/21.49초, 자산표6일치·해외 내부15일치/1미확인이다. |
| main 린트 | 코드30d27ed/main6e66d04의 `uv run ruff check src tests` 및 잔고 점검 스크립트 검사 통과. |
| 열린 PR | #793은 스펙184 시세 입력·기존 보유 분리·주문 관리의 초안이다. US11을 f737923으로 통합하고 관련305개를 통과했다. 이 후속 인계는 실행 코드 변경 없는 실제 결과 기록이다. |
...
| 최근 출시 작업 | 미입증 교차 출력 등식을 검증 실패로 다루던 가정을 수정했다. 실제 자산표19분류·합계5개·안정성 일치, 해외 내부15일치/1반올림 미확인. 교차 숫자4개 DIFFERENT는 별도 보존하며 계좌 오류나 실시간 NAV 인증으로 해석하지 않는다. |
| 활성 작업 | US11의 코드·실제 조회·배포·#793 통합을 완료했다. #793의 생산 계좌 입력·적격 전략 권한·실주문 시작 연결(T009/T010)은 미완료다. 현재 서버는 진단 수집기이며 정식 전진 실적을 누적하는 경로가 아니다. 기존 한국투자 계좌를 유지한다. |
