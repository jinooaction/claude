...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `5872ff4` — Merge pull request #660 from jinooaction/Codex/152-public-data-disclosure-fix |
| main 테스트 | #660 머지 코드 기준 `uv run pytest -q` → 2960 passed, 5 skipped. |
| main 린트 | #660 머지 코드 기준 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. |
...
| 골격 스펙 | 없음. 스펙 152 T001~T033은 모두 완료됐다. 다음 `independent_credit_or_fx_carry` 전략군은 아직 새 SDD로 사전등록하지 않았다. |
| 최근 출시 작업 | 스펙 152의 국채 캐리·금리 추세·방어적 곡선·바벨 64개를 생산에서 검증했다. 기존 512회와 합친 576개 고유 지문은 완전했지만 여섯 통계·경제 효용 관문 실패로 `NO_FACTORY_EDGE`다. |
| 활성 작업 | 코드 PR 없음. 다음 작업은 회사채 또는 외환 캐리처럼 국채와 다른 위험 프리미엄을 쓰는 `independent_credit_or_fx_carry`의 사전등록이다. 현재 `PREVIEW_ONLY`, `armed:false`, 단 0/5, 자본 0, 주문·전략 체결 0건, 전략 손익 0달러다. |
