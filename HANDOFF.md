...
## 최근 마일스톤 — 2026-05-27 (스펙 016 슬라이스 2: 단일 잣대 통일)
...
* **스펙 016 (백테스트 거래비용 + 단일 잣대)** — **슬라이스 1·2 출시 완료** (2026-05-27). 슬라이스 1(PR #77 `f8552c6`): 무비용·무슬리피지였던 백테스트에 거래비용 오버레이(슬리피지·수수료, KIS 현실값 기본). 새 모듈 `backtest/costs.py`. 슬라이스 2(PR #79 `83abbbb`): 거래 단위 지표 정의(승률·손익비·실현거래 재구성·Sortino)를 `backtest/metrics.py` 한 곳에 모아 라이브 성과 엔진과 백테스트가 같은 함수를 호출(헌법 X.2 완성). 리포트 schema 1.1→1.2. **안전 경계: 둘 다 오프라인·읽기 전용·Kernel 터치 0건(감사 스키마 K4 무변경).** 후속 = 슬라이스 3(워크포워드 표본 외 검증). 자세히는 `HANDOFF-022`·`HANDOFF-023`.
...
* **main의 테스트**: 1058 통과, 4 스킵 (라이브 KIS 스모크 4건은 `KIS_LIVE_TEST=1` 환경변수로 게이트).
...
- `HANDOFF-023-SPEC-016-SLICE2-SINGLE-YARDSTICK.md` — **최신**. 스펙 016 슬라이스 2 단일 잣대 통일 (2026-05-27, PR #79 `83abbbb`). 거래 단위 지표 정의(승률·손익비·실현거래 재구성·Sortino)를 `backtest/metrics.py` 한 곳에 모아 라이브 성과 엔진과 백테스트가 같은 함수를 호출하게 함(헌법 X.2 완성). 그동안 승률·손익비는 라이브에만 있었고 둘 다 Sortino 없었음. 오프라인·읽기 전용·Kernel 터치 0건(감사 스키마 K4 무변경). 테스트 신규 18건. 후속 = 슬라이스 3(워크포워드 표본 외 검증). **다음 세션 참고.**
...
| 마지막 main 커밋 | `83abbbb Merge PR #79 — feat(016): 백테스트 단일 잣대 통일 (슬라이스 2)` |
| 활성 작업 | **없음 — 스펙 016 슬라이스 2(단일 잣대 통일) 완료·머지(2026-05-27, PR #79 `83abbbb`).** 라이브 worker dry-run 가동 중. 다음 후보(세계 최고 수준 로드맵): **스펙 016 슬라이스 3 — 워크포워드(표본 외) 검증**(롤링 윈도우 과적합 탐지), 그 뒤 신호/알파 과학(다요인·레짐 인식)·포지션 사이징(변동성·상관). 측정 토대 1·2단계(정직·통일)는 완료. 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015, 016(슬라이스 1: 거래비용 + 슬라이스 2: 단일 잣대 통일) |
| 진행 중 스펙 | 없음 (스펙 016은 슬라이스 1·2 출시; 슬라이스 3은 후속 후보) |
| 골격 스펙 (즉시 착수 가능) | **스펙 016 슬라이스 3(워크포워드 검증)** — `specs/016-backtest-cost-model/spec.md`의 "후속 슬라이스" 절 참조. 정직·통일된 잣대 위에서 과적합을 탐지하는 다음 작업 |
...
| main 테스트 | 1058 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
