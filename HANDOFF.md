...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `7faa902` — Merge pull request #550 from jinooaction/codex/handoff-after-userless-server-blocker-audit-isolated (`HANDOFF.md`만 바꾼 인수인계 머지; 최신 코드 변경 머지는 `d3d1117` #547) |
| main 테스트 | #550 handoff refresh 기준 `uv run pytest -q` → 2691 passed, 5 skipped. 5개 skip은 `KIS_LIVE_TEST=1` opt-in live smoke다. |
| main 린트 | #550 handoff refresh 기준 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음(이 문서 편집 시점). |
...
| 최근 출시 작업 | #550은 #549 이후의 사용자 무개입 서버 수리 경로 점검 결과를 `HANDOFF.md`에 반영한 인수인계 갱신이다. #547은 KIS smoke가 SSH exit 255에서 멈춰 상태를 못 남기던 문제를 고쳐 `setup_pending`을 sidecar에 남기게 했다. money-gate-alignment도 이 상태를 `BLOCKED` KIS 이슈로 읽고 "서버에 새 deploy 공개키 설치 후 KIS smoke 재실행"을 다음 행동으로 낸다. #545는 operator-status Telegram 알림 재시도를 보강했고, #543/#542/#540/#538/#536은 돈 경로 차단·KIS secret 누락·전략 의도 게이트 실패를 사람이 읽을 수 있게 드러냈다. |
| 활성 작업 | 열린 PR 없음. GitHub repo secret 목록에는 `VULTR_SSH_HOST`/`PORT`/`KNOWN_HOSTS`와 `VULTR_SSH_USER`(2026-07-29T14:18:16Z), `VULTR_SSH_PRIVATE_KEY`(2026-07-29T14:18:17Z)가 등록돼 있다(값은 출력·커밋 금지). 2026-07-30 KST PC 제어 재점검에서 Orca computer runtime은 실행 후 `ready`가 됐고 Chrome 접근성 트리도 읽혔다. Chrome으로 `https://console.vultr.com/`를 열었지만 Vultr는 로그인 화면이며 이메일/비밀번호 칸은 자동 채움처럼 보였고 `reCAPTCHA` 미확인 때문에 `Log in` 버튼은 비활성화됐다. Codex는 reCAPTCHA를 대신 풀지 않았고 비밀번호 관리자·저장 비밀번호·쿠키를 읽지 않았다. `ssh macbookair`는 `192.168.219.122:22 Operation timed out`으로 닿지 않았다. 로컬 history 대상 검색에도 `VULTR_API_KEY`/Vultr API/서버 수리 힌트는 없었다. KIS smoke 최신 sidecar는 commit `7faa902`, timestamp `2026-07-30T05:41:52Z`, `secrets_present=true`, `key_valid=true`, `smoke_state=setup_pending`, `smoke_exit=255`다. money-path 최신 run은 `2026-07-30T10:22:18Z` 기준 `PREVIEW_ONLY`/자본 사다리 `BLOCKED`, money-gate-alignment는 `2026-07-30T11:22:58Z` 기준 `BLOCKED`, operator-status는 `2026-07-30T11:23:11Z` 기준 `ACTION_REQUIRED`/Telegram `SENT`다. 최신 다음 행동은 서버에서 `deploy/repair-ssh-boundary.sh`에 새 deploy 공개키를 설치한 뒤 KIS smoke를 다시 실행하는 것이다. #550은 문서-only라 deploy-on-merge 트리거가 없고, 직전 코드 머지 #547 deploy-on-merge run `30461091918`은 같은 서버 SSH 거부로 실패했다. micro GTAA 최신 run은 `armed=false`, `LIVE 스텝=skipped`, strategy-intent gate `ok=false`, `reason=latest_intent_loss`, `cumulative_pnl_usd=-1.14`다. |
| 안전 경계 | #547은 등급 2 운영 진단 보정이고 #548/#549/#550 및 이 갱신은 등급 2 인수인계 갱신이다. SSH 실패를 성공으로 꾸미지 않고 `setup_pending`으로 정확히 남긴다. GitHub repo secret 값은 출력·커밋하지 않았다. 이번 재검증은 PC 제어, 로컬 SSH, GitHub read-only 진단, 로컬 credential 존재 여부 확인, 상태판 갱신만 수행했고, reCAPTCHA 우회·비밀번호/쿠키 열람·브로커 호출·주문·자본 배분·live 재무장·전략 교체·whitelist/caps 확대·손실 예산·KIS secret·감사 로그·헌법·kernel manifest 변경은 하지 않았다. 현재 돈 경로는 `PREVIEW_ONLY`라 실주문 불가다. |
...
6. 현재 기준(2026-07-30T11:23Z operator-status, 11:22Z money-gate-alignment, 10:22Z money-path, 05:41Z KIS smoke): micro GTAA는 `armed:false`, money-path `live_money_state.status=PREVIEW_ONLY`, 최신 micro sidecar는 `event=schedule`, `LIVE 스텝=skipped`, strategy-intent gate `ok=false`, `reason=latest_intent_loss`, `cumulative_pnl_usd=-1.14`다. money-gate-alignment 최신 보고는 `BLOCKED`이고 KIS smoke를 `setup_pending`으로 읽는다. GitHub repo secrets는 등록됐지만 서버가 아직 새 deploy 공개키를 받아들이지 않아 `Permission denied (publickey,password)`로 막힌다. PC 제어로 Vultr 로그인 화면까지 열었지만 reCAPTCHA/로그인 경계에서 멈췄고, `macbookair` SSH도 타임아웃이라 사용자를 거치지 않는 수리 경로는 이 세션에서 발견되지 않았다. 다음 행동은 서버에서 `deploy/repair-ssh-boundary.sh`로 forced-command gateway를 설치하고 KIS smoke를 다시 실행한 뒤 자본 사다리와 전략 의도 게이트를 다시 읽는 것이다. 스펙 062의 2026-06-22 `armed:true` 기록은 역사이며 현재 상태 근거로 쓰지 않는다.
