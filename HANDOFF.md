...
| main 테스트 | 코드309144a/main cd17fb9에서 전체3733 passed/10 skipped, 실패0(804.19초). 제외10개는 실제 KIS9개와 가동 전 조건1개다. 실제 KIS34291826106은9/9 통과, 산술15일치·3불일치·1미확인이다. |
| main 린트 | 코드309144a/main cd17fb9에서 `uv run ruff check src tests` 통과. |
| 열린 PR | #793은 스펙184 시세 입력·기존 보유 분리·주문 관리의 초안이다. #794 잔고 산술 검사는 main 반영 완료. 별도 codex/182-balance-audit에서 실제 불일치의 단위·정밀도 진단을 검증 중이다. |
...
| 활성 작업 | 스펙182 US9 실제2개 자산의 개별 산술은 일치했고 원화 합계3개는 불일치했다. 원인 확정 없이 단위·정밀도 가설을 구분하는 진단을 추가 검증 중이다. 전체 NAV 합산식과 장중 발생시각은 미검증이며 #793의 실주문 시작 연결도 미완료다. 기존 한국투자 계좌 사용은 확정됐다. |
