...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `598be96` — Merge pull request #692 from jinooaction/codex/171-parallel-regime-edge-challenger |
| main 테스트 | #692 머지 직전 `uv run pytest` → 3131 passed, 6 skipped. 건너뛴 6개는 KIS 실연결 전용이다. |
| main 린트 | #692 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | #693은 장중 배포 지연 중 최신 관찰 스크립트와 이전 서버 명령줄의 버전 차이를 허용하는 안전 실패 수정이다. 코드 품질 관문은 통과했고 전체 재검증 중이다. |
...
| 골격 스펙 | 없음. 스펙 171 연구는 완료됐고, #693 생산 관찰 호환성 수정과 주문 없는 재검증만 진행 중이다. |
| 최근 출시 작업 | 주식·채권 동반 약세 때 현금 또는 금으로 옮기는 16개 후보를 사전등록 후 검증했다. 선택 후보는 수익·샤프·낙폭·비용·검정력 관문을 통과했지만 최근 세 구간 중 1개만 이기고 실제 차이가 3개월에 집중돼 `NO_RESEARCH_EDGE`다. |
| 활성 작업 | 자동 판정 `33088105605`은 `WAIT_EDGE`, 단 0→0, 자본·주문 0건이다. 계좌와 주문 구현성은 읽었지만 장중 배포 지연으로 관찰 도구가 이전 서버 명령줄에 없는 선택지를 호출해 동등성 결과가 비었다. #693이 이 버전 차이를 고친 뒤 재실행한다. |
...
- **운영 결함 발견**: #692 배포는 미국 장중이라 서버 패키지 교체를 안전하게 미뤘지만
  ...
  같은 `data/` 아래 만들도록 해 두 버전 모두에서 동작하게 한다.
- **돈 경로**: 자동 판정 `33088105605`은 계좌 NAV `$1447.51`, IAUM 1주와 SCHX
  2주의 주문 구현 가능성을 확인했지만 `WAIT_EDGE`, 단 0, 자본·주문 0건이다.
