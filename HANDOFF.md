...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `478a2f6` — Merge pull request #795 from jinooaction/codex/182-balance-audit |
| main 테스트 | 코드14ddfe2/main478a2f6에서 전체3741 passed/10 skipped, 실패0(755.31초). 제외10개는 실제 KIS9개와 가동 전 조건1개다. 실제 KIS34293409761은9/9 통과, 산술15일치·3불일치·1미확인이다. |
| main 린트 | 코드14ddfe2/main478a2f6에서 `uv run ruff check src tests` 통과. |
| 열린 PR | #793은 스펙184 시세 입력·기존 보유 분리·주문 관리의 초안이다. #794/#795 잔고 산술/진단은 main 반영 완료. codex/182-account-assets에서 별도 공식 전체 자산 분류 조회를 검증 중이다. |
...
| 활성 작업 | US9 실제2개 자산의 개별 산술은 일치, 원화 합계3개는 단순 외화합·행별1센트 환산 범위로 설명되지 않는 불일치다. US10은 CTRP6548R의 선택 계좌 결제기준 전체 자산 분류와 합계 대조를 구현하고 검증 중이다. 전체 장중 NAV·#793 실주문 시작 연결은 미완료다. 기존 한국투자 계좌를 유지한다. |
...
추가했다. 새43개 포함 관련116개 통과, 전체 회귀 진행 중이다. 동일 선택 계좌의
