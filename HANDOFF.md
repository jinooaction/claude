...
## 최근 마일스톤 — 2026-05-29 (스펙 017 슬라이스 2b: 역변동성 그룹 리스크 패리티)
...
* **스펙 017 (변동성 기반 포지션 사이징)** — **슬라이스 1·2·2b 출시 완료** (2026-05-28~29). 측정 토대 위에 리스크 사이징. 슬라이스 1(PR #83 `c291d75`): 실현 변동성이 타깃 초과 시 기준 수량을 줄이는 **변동성 throttle**(하향 전용). 슬라이스 2(PR #85 `ab4a140`): 잔잔한 구간에서 기준 수량 위로 **확대**하는 양방향 타깃팅(룰의 `max_scale`, K1이 진짜 천장). 슬라이스 2b(PR #87 `b8fb7e9`): 여러 종목을 한 바구니(`sizing_group`)로 묶어 변동성 높은 종목을 줄여 리스크 기여도를 균형화하는 **역변동성 그룹 리스크 패리티**(`mode="inverse_vol"`, 가중치=`min(그룹 변동성)/자기 변동성`, 하향 전용). 새 비커널 모듈 `strategy/sizing.py`(`build_sizing_groups`·`inverse_vol_group_scale`) + worker가 그룹을 만들어 `OrderRouter.sizing_groups`로 넘김. 백테스트 `replay`와 라이브 양쪽이 K1 게이트 **전에** 같은 함수로 가중치 계산 → 단일 잣대(헌법 X.2). **안전 경계: 전부 하향 전용 또는 K1이 천장이라 노출 증가 불가. 그룹/사이징 옵트인이라 미설정 룰은 byte 동일(회귀 무손상). Kernel 터치 0건(전부 `strategy/sizing.py`·`config/rules.py`·`backtest/replay.py`·`execution/order_router.py`·`worker/loop.py` 비커널 — 커널인 `worker/schedule.py` 아님). 감사 K4 무변경. 결정론적·LLM 미사용. dry-run 그대로.** 테스트 슬라이스 1 신규 18 + 2 신규 9 + 2b 신규 8건. 자세히는 `HANDOFF-025`·`HANDOFF-026`·`HANDOFF-027`.
...
| 마지막 main 커밋 | `b8fb7e9 Merge PR #87 — feat(017): 역변동성 그룹 리스크 패리티 (슬라이스 2b)` |
| 활성 작업 | **없음 — 스펙 017 슬라이스 2b(역변동성 그룹 리스크 패리티) 완료·머지(2026-05-29, PR #87 `b8fb7e9`).** 라이브 worker dry-run 가동 중. 한 포지션 변동성 타깃팅(슬라이스 1·2)에 이어, 여러 종목을 한 바구니(sizing group)로 묶어 변동성 높은 종목을 줄여 리스크 기여도를 균형화하는 역변동성 배분(하향 전용)을 추가했다. 다음 후보: 스펙 017 슬라이스 3(상관 인식 합산 한도, 공분산 추정), 양방향 그룹 budget-split(K1 봉투 안 확대), 또는 신호/알파 과학(다요인·레짐 인식) — 전부 워크포워드로 표본 외 검증 받으며 진행. 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015, 016(슬라이스 1·2·3 전부), 017(슬라이스 1 변동성 throttle 하향 + 2 양방향 타깃팅 상향 + 2b 역변동성 그룹 리스크 패리티) |
| 진행 중 스펙 | 스펙 017 — 슬라이스 1·2·2b 출시, 슬라이스 3(상관 인식)·양방향 그룹 budget-split 미착수 |
| 골격 스펙 (즉시 착수 가능) | **스펙 017 슬라이스 3 또는 신호/알파 과학** — 포지션·그룹 리스크 사이징(타깃팅·역변동성 패리티)이 깔렸으니, 다음은 상관 인식(슬라이스 3)이나 신호 자체 개선. 전부 워크포워드 검증 필수 |
...
| main 테스트 | 1103 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
