...
⚠️ **운영자 결정 지점 — 운영자가 "레버리지 후 복리 극대화" 선택(2026-06-16). 그러나
레버리지가 안전 캡에 막힘**: `caps.py` 의 `global_exposure_pct ≤ 100`(헌법 원칙 I, 비협상
포지션 캡)이 **레버리지(노출 >100%)를 원천 차단**한다. 레버리지 적용 = 안전 경계 변경
(K-meta) = **운영자 명시 승인 필수, 자율 불가.** 상세: `specs/044-growth-optimal-leverage/
LEVERAGE-CAP-BOUNDARY.md`. 두 갈래(독립): ① 레버리지 = 안전 경계 변경 승인 필요(노출 캡↑),
② **고정가중 무레버 재지정 = 안전 경계 변경 0**(캡·20% 예산 안에서 복리 +1.3~2.3%p, 거래비용
10bp 반영). ②가 캡 안에서 "더 버는" 안전한 길(재지정은 X.4 게이트지 안전 경계 변경 아님).

### 거래비용 반영 완료 (PR #311, main `2835a52`)

레버리지 후 복리 고정가중 우위가 비용에 견고(0bp +2.4%p → 10bp +2.6%p, 역변동성 회전율↑).
전략 빌더에 `cost_bps`(기본 0 역호환). **남은 후속(읽기 전용·비게이트)**:

1. (선택) 깊은 검사(OOS·레버리지)를 사이드카로 상시화 — `regime-stratify.yml` 패턴, 인스턴스
   비용 0(공개 데이터).
2. (선택) 깊은 검사(OOS·레버리지)를 사이드카로 상시화 — `regime-stratify.yml` 패턴, 인스턴스
   비용 0(공개 데이터).
3. 비상관 차원 추가 연구(추세추종 외)는 여전히 유효하나 긴급하지 않다(현 전략 정당화됨).
...
| 마지막 main 커밋 | `2835a52 Merge pull request #311 — feat(044) 거래비용 반영 레버리지 재측정` |
...
| main 테스트 | 2051 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
