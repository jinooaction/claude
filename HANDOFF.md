...
2026-09-25 후속196 진행: PR853의 코드1b02509에 과거 보관 원문·수집 기록 검사기를 구현했다.
...
현재 명세는 specs/196-archived-event-evidence/tasks.md이며 T011 병합·배포 확인이 남았다.
...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `9c3acd0` — Merge pull request #852 from jinooaction/codex/195-release-evidence |
| main 테스트 | 코드522ac3d 전체4927 passed/13 skipped(817.90초), 관련42개. 이후 문서만 변경. 마지막 실제KIS는 이전main6580c05의35541770144 12개 통과;195는 브로커 코드 미변경. |
| main 린트 | 코드522ac3d의 `uv run ruff check src tests`와 신규 공시CLI 통과. 하네스14/14·인계 사실 검사 통과. |
| 열린 PR | #853 과거 보관 원문·수집 근거 검사, 전체4989 통과 후 출시 진행. #827 유료 공급자 초안 보류. |
| 출시 완료 스펙 | 195 예정/실제 공시 원문·로컬 관측·시점별 조회 도구를 main에 병합.194 누락 보존 연구 도구의 실제 후보는 탈락.184 고정 예산 운영·관측 구간 체결·정산,182 계좌 조회,183 모바일 운영판 유지. 전체 단타 실사용 목표는 별도다. |
...
| 최근 출시 작업 |195 공시 입력 PR851, 배포35562612202·감사35562644448 성공. target da11ad893573 완료 확인. 기존 수집기·주문 권한 유지. |
| 활성 작업 |196 보관 근거 검사 관련62개·전체4989개·실제WMT 정상/변조 검사 통과. 출시 확인 진행. 최소31가설 실패를 보존하며 새 성과 가설은 미선정. 최종확인495일 미개봉. 전체 자료 범위·통과 전략·전진 관찰·실행 동등성·실제 운영 검증이 남음. |
