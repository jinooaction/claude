...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `8a1bafc4` — Merge pull request #733 from jinooaction/codex/179-operator-emergency-live-deploy |
| main 테스트 | #733 exact 기능 트리에서 `uv run pytest` → 3320 passed, 7 skipped, 실패 0. |
| main 린트 | #733 exact 기능 트리에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 등록 시스템 오너 신원 보정 브랜치 `codex/179-system-owner-identity` — 생산 긴급 요청에서 확인된 namespace 소유자·실제 운영자 불일치를 좁게 고침. PR 생성·검증 진행 중. |
...
| 활성 작업 | #733은 main에 머지됐고 일반 push 배포는 장중 관문에서 안전하게 연기됐다. 첫 긴급 run `33667656920`은 실제 시스템 오너 `masonoh-kidsnote`와 namespace 소유자 `jinooaction`이 다른 구현 결함으로 SSH 전에 거부됐다. 현재 정확한 등록 오너만 추가 허용하는 헌법 15.0.1 보정과 생산 재배포가 진행 중이다. |
| 안전 경계 | main은 헌법 15.0.0이고 작업 브랜치는 15.0.1이다. namespace 소유자 또는 소스에 고정된 정확한 등록 시스템 오너만 승인할 수 있고 actor를 입력·변수·secret·일반 협업자 권한으로 늘릴 수 없다. 수동 주문, 자본 증액, 전략 승격, 허용목록·위험 관문 우회는 계속 금지된다. |
...
- **검증**: `uv run pytest` 3320 passed/7 skipped, `uv run ruff check src tests` 통과,
  관련 배포·감사·예약·주문 시험 104개 통과, 셸 구문과 `git diff --check` 통과,
  엄격 하네스 14/14와 HANDOFF 사실 검사 통과다.
- **현재 관문**: PR #733의 원격 품질 관문과 머지 가능 상태를 확인해 merge commit으로 병합한다.
  이어 exact-main 긴급 배포에서 KIS 6/6·미체결 0·승인/시작/완료 감사·90초 건강·잠금 해제를
  확인한다. 완료 판정은 그 뒤 정상 자동 예약의 실제 주문·체결·전략 감사·같은 실행 계좌 대사와
  다른 예약 출처의 중복 중개사 쓰기 0건까지 확인한 후에만 한다.
