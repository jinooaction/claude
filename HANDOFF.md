...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `2d6054ee` — Merge pull request #728 from jinooaction/codex/178-flutter-operator-mobile |
...
| 열린 PR | #728은 merge됐다. 이 문서 갱신 시점에는 HANDOFF 전용 후속 PR만 진행한다. |
...
| 골격 스펙 | 스펙 178 코드·계약·시뮬레이터 검증은 완료됐고 실제 iPhone 설치만 무료 개발자 프로필의 기존 앱 3개 제한에 막혀 있다. 스펙 177은 756세션 실제 5분봉 자료가 없어 `INSUFFICIENT_EVIDENCE`이며, 스펙 176은 실제 KIS 자동 주문·체결·감사·정합과 중복 주문 0건 확인이 남았다. |
...
| 활성 작업 | 앱 코드는 비공개 저장소 main에 merge됐고 실제 Pages 자료 25개를 iOS 시뮬레이터에서 표시했다. 실제 iPhone 검증을 닫으려면 기존 `Price Radar`, `CEO Office`, `SayMemo` 중 사용자가 고른 하나를 제거하거나 유료 프로비저닝을 써야 한다. 라이브 경로는 기존 `globalfixed-ensemble-3-6-9-12`, NAV 10%, 단 1을 그대로 유지한다. |
| 안전 경계 | main은 헌법 X.4 v13.0.0이다. 운영 경로는 NAV 10%, 최대 단 1, `alpha_confirmed=false`, 단 1 초과 승격 금지다. 손실 예산 20%, 정규장 지정가, 서명·nonce, 킬스위치, 주문·노출 한도와 추가 전용 감사는 유지한다. 수동 서비스 시작과 수동 실주문은 계속 금지된다. |

## 최근 완료 — 2026-09-02 KST (#728 읽기 전용 Flutter 운영자 앱과 공개 상태 계약)
...
- **실제 iPhone 관문**: 연결된 iPhone 14 Pro는 페어링·개발자 서비스 진입까지 성공했지만
  무료 개발자 프로필에 `Price Radar`, `CEO Office`, `SayMemo` 세 앱이 이미 있어 Apple의
  설치 한도를 초과했다. 기존 앱 삭제는 사용자 선택이 필요한 파괴적 동작이라 수행하지 않았다.
  하나를 제거하거나 유료 프로비저닝을 쓰면 `com.jinooaction.autoinvest` 서명 빌드를 재설치한다.
