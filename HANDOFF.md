...
## 한눈 요약표 — 2026-08-30 KST 최신 코드 main 기준

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `fc5af95` — Merge pull request #699 from jinooaction/codex/173-parallel-independent-edge |
| main 테스트 | #699 기능 브랜치에서 `uv run pytest -q` → 3155 passed, 6 skipped. 건너뛴 6개는 KIS 실연결 전용이며, 같은 main 커밋의 KIS 스모크에서 별도 6/6 통과했다. |
| main 린트 | 이 인계 브랜치에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 2026-08-30 사실 확인 시 열린 기능 PR은 없다. #699 코드·배포·KIS 스모크는 완료됐고 이 문서 전용 마감만 진행한다. |
| 출시 완료 스펙 | 최신 기능: #699(스펙 173, 표준 자본 경로 교정·판정 거짓 음성 수정·레짐 장부 복원·독립 월말월초 16개 검증), #697(스펙 172, 실제 8관문 합격 경로 감사·절대 검출력 하한), #692~#695(스펙 171, 독립 저회전 레짐 후보), #689·#690(스펙 170, 소액 계좌 실행 동등성). |
| 골격 스펙 | 없음. 스펙 173은 완료됐다. 다음 독립 연구 우선순위는 가격 변형이 아닌 회계 기반 횡단면 다요인이다. |
| 최근 출시 작업 | 빈 앵커드 결합이 정상 표준 확정의 유의성 방법을 잃던 오류, 교정 결과를 자본 소비자가 읽지 않던 오류, 레짐 16개 장부 누락, 표준 첫 자본 20% 설명 불일치를 고쳤다. 월말·월초 후보는 PBO 0.126984를 통과했지만 PSR 0.619345·연 현금 초과 0.214%·비용 스트레스로 탈락했다. |
| 활성 작업 | `globalfixed-ensemble-3-6-9-12`는 전진 관측 4/20으로 `ACCUMULATING_EDGE`; 고정 레짐 후보 관찰도 자동 지속한다. 라이브 상태는 `PREVIEW_ONLY`, 단0, 자본·실주문 0건이다. 새 공장 장부는 784개·19가족이며 다음 독립 가족은 회계 기반 횡단면 다요인이다. |
