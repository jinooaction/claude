...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `eed92c6` — Merge pull request #686 from jinooaction/Codex/166-family-complete-v3-fundability |
| main 테스트 | #686 머지 직전 `uv run pytest` → 3100 passed, 5 skipped. 건너뛴 5개는 KIS 실연결 전용이며 머지 후 KIS 읽기 전용 점검은 5/5 통과했다. |
| main 린트 | #686 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | #686 기능·생산·배포는 완료됐다. 로컬 `codex/167-calibrated-research-entry`에서 통계 검정력 교정을 구현·검증 중이며 아직 PR은 없다. |
| 출시 완료 스펙 | 최신 기능: #686(스펙 166, 소비자 독립 v3 증거와 정확한 소액 자금 구현성), #684(스펙 165, 옵션 선택·목적 기준 교정과 WPUT 독립 재생), #682(스펙 164, 옵션 변동성 위험 프리미엄 16개·752개 고유 감사), #680(스펙 163, 에너지 교차시장 16개·736개 고유 감사), #678(스펙 162, USDA 작물 수급 16개·720개 고유 감사). |
| 골격 스펙 | `specs/167-calibrated-research-entry/`: 752개 후보를 17개 실제 전략군으로 재구성하고, 중복 통계 관문을 검정력 보정된 PSR+PBO 계약으로 교체한다. 현재 옵션 후보는 새 기준에서도 PBO 0.371429로 탈락한다. |
| 최근 출시 작업 | 전략 공장 증거를 생산자 문구가 아니라 소비자가 752개 원시 행, 지문, 현재 16개 꼬리, DSR/PBO, 실행 정합으로 재계산한다. 현재 NAV 10% 주문 미리보기까지 정수 주·현금·캡으로 재현해 자금 구현이 안 되면 진입을 막는다. |
| 활성 작업 | 최신 돈 경로는 `PREVIEW_ONLY`, 단 0, 자본 0, 실주문 불가다. 자동 승인 점검 `32973803207`은 `WAIT_EDGE`, 0→0, 주문 0건이다. 현재 약 145달러 연구 자본은 SPYM/GLDM 정수 주와 50% 주문 캡 때문에 구현 불가다. |
| 안전 경계 | main은 헌법 X.4 v10.0.0, 현재 기능 브랜치는 v10.1.0이다. 연구 단은 NAV 10%로 유지하며, 기존 20% 이상 승격, K1/K2, 손실 예산 20%, 정규장, 현금 1%, 서명·nonce, 추가 전용 감사 로그는 변경하지 않는다. ORANY 28주는 시작 전 외부 보유로 전략 손익에서 제외한다. |

## 최근 완료 — 2026-08-26 KST (#686 소비자 독립 증거와 소액 자금 구현성)
