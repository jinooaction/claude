...
## 최근 마일스톤 — 2026-05-24 (spec 011 완결 + stale 추적 진실화)
...
**중요한 결론**: 빌드 가능한 스펙(006·007·008·009·010·011)은 **전부 완료**. 남은 spec 004·005는 코드가 막힌 게 아니라 **달력에 막혀** 있습니다 — 자기 promotion 기준이 "토큰 텔레메트리 30일 누적"을 요구하는데 라이브 dry-run이 2026-05-23 시작이라 현재 약 1일치뿐입니다. 데이터가 쌓일 때까지 004·005는 시작하면 안 됩니다(자기 기준 위반).
...
* **스펙 003 (세션 캐시)** — 출시 완료.
* **스펙 004 (LLM 판단 지점)** — 골격만. **달력 대기** — 30일치 spec 002 텔레메트리 누적 후 구현 (현재 라이브 dry-run 1일치).
* **스펙 005 (자율 튜너)** — 골격만. promotion 조건 3개 중 스펙 006·007 출시 + 스펙 011 측정 토대는 **충족**, 남은 1개(텔레메트리 30일 누적)는 **달력 대기**. 데이터 누적 후 `/speckit-specify`→`/speckit-plan`→`/speckit-tasks`로 골격을 본 스펙으로 승격하고 착수.
...
* **main의 테스트**: 792 통과, 4 스킵 (라이브 KIS 스모크 4건은 `KIS_LIVE_TEST=1` 환경변수로 게이트).
...
**다음 후보 (둘 다 달력 대기 — 지금 시작 금지)**:

* **스펙 004 (LLM 판단 지점)** — Claude를 거래 결정 루프에 처음 끌어들이는 작업. 결정성을 일부 양보하고 추론 능력을 얻는 트레이드오프. 골격은 `specs/004-llm-judgment-points/spec.md`. **30일치 토큰 텔레메트리(스펙 002) 누적이 선행 조건** — 현재 라이브 dry-run 1일치라 아직 대기.
* **스펙 005 (자율 튜너)** — KPI·룰을 자동 조정해 측정→분석→행동 루프를 닫는 작업. promotion 조건 3개 중 스펙 006(배포)·007(캐너리) 출시 + 스펙 011(측정 신호 면)은 **충족**됐고, 남은 1개(텔레메트리 30일 누적)만 **달력 대기**. 데이터가 쌓이면 골격을 본 스펙으로 승격(`/speckit-specify`→`/speckit-plan`→`/speckit-tasks`) 후 착수. 자율 튜너 자체의 머지는 헌법 IX.B-2상 스펙 007 하드닝 캐너리 통과가 유일한 경로.

이 두 스펙은 "운영자 손이 더 줄지만 필수는 아니다" 영역이며, **코드가 막힌 게 아니라 측정 데이터 누적을 기다리는 중**입니다. 위 운영 절차 + 스펙 010 `design` 명령 + 스펙 011 `performance` 측정만으로도 v1 자동 거래 서비스는 굴러갑니다.
...
- `HANDOFF-014-LIVE-DRYRUN-STARTED.md` — **활성**. 라이브 worker dry-run 시작 + HANDOFF-010/011 오해 정정 (2026-05-23). 다음 세션 필독.

## 다음 세션이 하지 말아야 할 것

- 진행 중인 브랜치가 있는데 main에서 새 브랜치를 만들지 **마세요** (위 발견 순서가 이를 막아줍니다).
- 열린 PR + 활성 인수인계 파일이 다음 작업을 알려주고 있는데 운영자에게 "어떤 작업을 원하세요?"라고 묻지 **마세요**.
- 출시 완료된 스펙(001 / 002 / 003 / 006 / 007 / 008 / 009 / 010 / 011)의 소스를 운영자의 명시적 수정 지시 없이 건드리지 **마세요**.
...
| 마지막 main 커밋 | `625165c Merge PR #55 — spec 011 완결(P3·P4) + spec 006·007 stale 추적 진실화` |
| 활성 작업 | **없음 — 빌드 가능한 스펙 전부 완료.** 라이브 worker dry-run 가동 중 (2026-05-23 시작, 1주일 안전 관찰). 남은 spec 004·005는 달력 대기(텔레메트리 30일, 현재 ~1일). 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001, 002, 003, 006, 007, 008, 009, 010, 011 |
| 진행 중 스펙 | 없음 |
| 골격 스펙 (달력 대기) | 004 (LLM 판단 지점), 005 (자율 튜너) — 둘 다 텔레메트리 30일 누적 후 착수. 코드가 막힌 게 아니라 데이터 대기 |
...
| main 테스트 | 792 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
