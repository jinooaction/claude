...
| 운영자 상황 | 진입점 |
|------------|--------|
| **개발 지식 없음, Vultr 계정 보유, 자율 수행 최우선** | `docs/OPERATOR_VULTR_ONE_STEP_KR.md` — cloud-init User-Data 한 번 붙여넣기만으로 부팅과 동시에 모든 셋업 + dry-run 가동 자동 완료. **이 길이 헌법 IX.D 자율 수행 정신에 가장 가까움.** |
| **개발 지식 없음, 명령어 하나씩 학습하고 싶음** | `docs/OPERATOR_START_NONDEV_KR.md` — Vultr 서버 만들기부터 8단계 손으로 학습. |
| **개발자, Linux/systemd 호스트 보유** | `docs/OPERATOR_START.md` — `git clone` → `.env` → `bash scripts/operator_install.sh` 5분 경로. |

### 운영자가 "굴리고 싶다 + 개발 지식 없음 + 자율 수행 최우선"이라고 답한 경우 (2026-05-15 세션)

운영자 환경: Vultr 계정, 자본금 100달러로 시작. 운영자가 "내가 직접 가이드 따라하는 게 아니라 자율 수행이 목표 아니냐"고 정확히 짚어줘서, **cloud-init 기반 한 번에 가동 모델**(`docs/OPERATOR_VULTR_ONE_STEP_KR.md`)이 진짜 최단 경로로 자리잡음. 다음 세션이 도와드릴 때는:

1. **기본 가정**: 운영자는 `docs/OPERATOR_VULTR_ONE_STEP_KR.md` 경로를 따른다. 이 가이드는 운영자가 콘솔에 들어가지 않고 Vultr 콘솔에서 User-Data 한 번 붙여넣기만 한다.
2. **운영자 비밀(KIS 키)을 채팅으로 받지 마세요.** 헌법 V 비밀 격리 위반이며, KIS 키는 운영자가 Vultr User-Data에 직접 박는 것이 유일하게 안전한 방법입니다. "키 알려주시면 제가..."라고 절대 말하지 마세요.
3. **자본금 100달러 + 1주일 dry-run 안전 약속**을 운영자에게 매번 상기. 헌법 원칙 I-VII + VIII.A는 시스템이 지키지만, "자본금을 너무 빨리 늘리지 않는다" 같은 운영 수칙은 운영자만 지킬 수 있습니다.
4. 운영자가 막혔다고 가져오는 정보는 보통 `cat /var/log/auto-invest-cloud-init.log` 또는 `journalctl -u auto-invest.service`입니다. 가이드의 "막혔을 때 다음 세션에 가져올 것" 절 참조.
