...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `4a305ba` — Merge pull request #722 from jinooaction/Codex/176-quote-market-5xx-fallback |
...
| 활성 작업 | #722 merge `4a305ba`와 배포 `33445034323`이 성공했다. exact-main KIS 스모크 `33445033903`은 IAUM을 실제 `AMS`에서 `$44.3938`로 찾아 6/6 통과했고 현금 `$934.27`, NAV `$1427.63`, ORANY 28주, 미체결 0건을 확인했다. 무주문 사전점검 `33445149085`도 `ENTRY_READY`, `orders_submitted: 0`, 사후 측정 `CLEAR`, 계좌 정합 `OK`, 증거 `VALID`로 성공했다. 다음 관찰은 2026-09-01 14:17 UTC부터의 최초 유효 schedule 실제 주문·체결·감사·정합과 같은 날 후속 schedule 중복 차단이다. 실제 전략 주문·체결은 아직 0건이다. |
