...
## 최근 마일스톤 — 2026-05-24 (스펙 004 LLM 판단 지점 출시)
...
* **스펙 005 (자율 튜너)** — 골격만. **즉시 착수 가능** (선행 조건 스펙 006·007·011 충족 + 텔레메트리 30일 게이트 제거). 스펙 004가 만든 판단 지점(프롬프트·파라미터)을 L2/L3 튜닝 대상으로 삼는다. `/speckit-specify`→`/speckit-plan`→`/speckit-tasks`로 승격. 런타임 자율 튜닝은 원칙 X, 자율 머지는 스펙 007 캐너리에 계속 종속(안전 경계 불변).
...
- `HANDOFF-015-SPEC-004-JUDGMENT-POINTS.md` — **활성**. 스펙 004 LLM 판단 지점 출시 (2026-05-24). 다음 세션 필독.
...
| 마지막 main 커밋 | `78286eb Merge PR #58 — spec 004 LLM 판단 지점 (Claude 거래 루프 첫 진입)` |
| 활성 작업 | **없음 — 스펙 004(LLM 판단 지점) 완료·머지(2026-05-24).** 라이브 worker dry-run 가동 중 (2026-05-23 시작). 다음 후보: 스펙 005(자율 튜너) 즉시 착수 가능(스펙 004가 만든 판단 지점 프롬프트·파라미터를 L2/L3 튜닝 대상으로). 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001, 002, 003, 004, 006, 007, 008, 009, 010, 011 |
| 진행 중 스펙 | 없음 |
| 골격 스펙 (즉시 착수 가능) | 005 (자율 튜너) — 선행 조건(스펙 006·007·011) 충족 + 텔레메트리 30일 게이트 제거(운영자 지시 2026-05-24). 스펙 004가 만든 판단 지점이 L2/L3 튜닝 표면. `/speckit-specify`부터 시작. 안전 경계(원칙 X·스펙 007 캐너리)는 불변 |
...
| main 테스트 | 847 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
