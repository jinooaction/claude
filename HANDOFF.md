...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `ee1978d` — Merge pull request #843 from jinooaction/codex/192-exit-ceiling-note |
| main 테스트 | 실행dfced27 전체4841 passed/13 skipped(1093.55초), 관련79개. 이후 코드 변경 없음. 마지막 실제KIS는 이전main6580c05의35541770144 12개 통과; 이번192는 브로커 코드 미변경. |
| main 린트 | 실행dfced27의 `uv run ruff check src tests`와 신규 연구CLI, 하네스14/14·인계 사실 검사 통과. |
| 열린 PR | #827 유료 공급자 초안 보류. #845 누락 보존 입력 구현 초안, 전체 회귀 진행 중. |
...
| 최근 출시 작업 | 스펙192 과거 시각 변동 폭 연구 PR841, 배포35551371254·감사35551419380 성공. target230383a 완료 확인. 단일후보 탈락·10160장부행 재계산 일치 보존. 기존 수집기·주문 권한 유지. |
| 활성 작업 |193 누락 보존 입력 구현·실제2종목 대조 완료, 전체pytest13601진행 중. 기존29전략 미통과와 추가 거래량 결합 가설 실패를 보존해 향후 최소30가설을 계상한다. 최종확인495일 성과 미개봉. 통과전략·전진관찰·실행 동등성·실제 운영 검증이 남음. |
