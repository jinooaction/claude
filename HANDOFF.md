...
| 활성 작업 | #715 merge `c413742`, 배포 `33361511135`, KIS 스모크 `33361511151`이 성공했고, 정확한 main 무주문 사전점검 `33361615036`은 `ENTRY_READY`, 자본 `$142`, IAUM 1주·SCHX 2주 계획, 계좌 정합 `OK`를 확인했다. 다만 예약 실행 지연 시 정규장 밖 주문으로 이어질 수 있는 결함을 발견해 `Codex/176-live-session-gate`에서 주문 직전 XNYS 정규장 실패 폐쇄를 추가 검증 중이다. 실제 전략 주문·체결은 아직 0건이다. |
...
- **현재 진행 중인 정규장 방어**: GitHub 예약은 과거 최대 약 9시간 지연됐지만
  `rebalance-once` 명령 자체에는 XNYS 정규장 확인이 없었다. 아직 출시되지 않은
  `Codex/176-live-session-gate`에서 실제 live·비미리보기 주문만 XNYS 정규장 안에서 허용하고,
  휴장·장외 시간에는 데이터베이스 변경과 중개사 호출 전에 종료코드 75로 실패 폐쇄한다.
