...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `6e66d04` — Merge pull request #799 from jinooaction/codex/182-account-assets |
| main 테스트 | 코드2723589/main d2528ad에서 전체3784 passed/11 skipped, 실패0(776.90초). 제외11개는 실제 KIS10개와 가동 전 조건1개다. 실제 KIS34298665338은10/10 조회 시험 통과, 전체 자산 분류표6일치·요약 대조1불일치다. |
| main 린트 | 코드2723589/main d2528ad에서 `uv run ruff check src tests` 및 잔고 점검 스크립트 검사 통과. |
| 열린 PR | #793은 스펙184 시세 입력·기존 보유 분리·주문 관리의 초안이다. #794~#798의 산술/진단/전체 자산 조회와 실제 인계를 이 개발 브랜치에도 반영했다. 실주문 시작 연결은 미완료다. |
| 출시 완료 스펙 | 스펙182 US8~US10 직접 조회·산술/진단·전체 자산 분류 대조가 실제10개 읽기 시험까지 완료됐다(#790~#797). 스펙183 모바일 운영판과 기존182 확정600/12·모의 주문은 유지한다. 장중 현금/NAV(T027)와 단타 실거래(T010~T012)는 미완료다. |
...
| 최근 출시 작업 | 기존 계좌의 전체 자산19분류와 합계5개가 일치했다. 별도 순자산 요약은 불일치이며 해외 현재잔고2개 자산의 개별 산술은 일치·원화 합계3개는 불일치다. 실제34298665338 조회10개 통과와 실시간 NAV 합격을 구분한다. |
| 활성 작업 | US11은 미입증 교차 출력 등식을 검증 실패로 분류한 가정을 보정했다. 관련119개·전체3787 passed/11 skipped(819.92초) 통과, 정상 PR 반영과 새 실제 관측 대기다. #793 실제 시작 연결·장중 현금/NAV는 별도 미완료다. 기존 계좌를 유지한다. |
