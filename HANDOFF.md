...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `ca8e1c1` — Merge pull request #839 from jinooaction/codex/191-shock-recovery-research |
| main 테스트 | 실행91a2b55 전체4820 passed/13 skipped(947.25초). 이후 코드 변경 없음. 마지막 실제KIS는 이전main6580c05의35541770144 12개 통과; 이번191은 브로커 코드 미변경. |
| main 린트 | 실행91a2b55의 `uv run ruff check src tests`와 신규 연구CLI, 하네스14/14·인계 사실 검사 통과. |
| 열린 PR | #827 유료 공급자 초안 보류. #839 병합·배포 확인 완료. 이번 변경은191 출시 인계 문서만 반영한다. |
...
| 최근 출시 작업 | 스펙191 급락 후 회복 연구를 PR839로 병합했다. 실제4후보 탈락과 원본 재계산 일치를 보존했다. 배포35546033949·감사35546215493 성공, targetca8e1c1 완료 확인. 기존 수집기·주문 권한 유지. |
| 활성 작업 | HF Data Library 실제1645세션 확보. 기존18+190의6+191의4후보 모두 미통과. 최종확인495일 성과 미개봉. 후속 가설은 새 사전등록/누적시도 집계와 비용·실행 조건 검토 필요. |
