...
| 활성 작업 | #751 배포 뒤 2026-09-04 자동 복구가 속도 제한 오류를 제거했지만 IAUM·SCHX를 모두 `APBK1672`로 거부했다. 원문을 외부에 내보내지 않고 계좌·서비스·권한 등 닫힌 원인 주제만 보여주는 진단 1.2를 검증·배포한 뒤 다음 수정 지점을 확정한다. |
...
## 현재 진행 — 2026-09-05 KST (KIS `APBK1672` 비밀 노출 없는 원인 분류)
...
설계 커밋은 `290cc31`, 안전 경계 구현 커밋은 `81dc3cb`이다. 관련 회귀 41개와
전체 `3373 passed, 7 skipped`, ruff, shell 문법, 엄격 하네스 14/14, HANDOFF 사실
검사가 통과했다. production에서 읽기 전용 observer가 실제 `message_topics`를
발행하기 전에는 원인 분류 작업을 완료로 표시하지 않는다.
