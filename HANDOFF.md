...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `0f1cc9f` — Merge pull request #694 from jinooaction/codex/171-zero-capital-live-performance-noop |
| main 테스트 | #694 머지 직전 `uv run pytest` → 3132 passed, 6 skipped. 건너뛴 6개는 KIS 실연결 전용이다. |
| main 린트 | #694 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | #694 생산 재실행에서 드러난 건너뛴 증거 파일과 임시 DB 정리 트랩의 수명주기 오류를 후속 보정 중이다. 판정·자본·주문 기준은 바꾸지 않는다. |
| 출시 완료 스펙 | 최신 기능: #692(스펙 171, 독립 저회전 레짐 후보 16개 사전등록·즉시 검증과 KIS 토큰 재사용), #689·#690(스펙 170, 소액 계좌 실행 ETF 동등성·주문 구현성·실패 증거 거짓 승인 차단), #687(스펙 167, 실제 17개 전략군과 검정력 보정 연구 진입), #686(스펙 166, 소비자 독립 v3 증거와 정확한 소액 자금 구현성). |
| 골격 스펙 | 없음. 스펙 171 연구와 생산 동등성 복구는 완료됐고, 생산 자동 판정의 두 파일 수명주기 오류만 후속 보정 중이다. |
| 최근 출시 작업 | 16개 레짐 후보는 개선이 3개월에 집중돼 `NO_RESEARCH_EDGE`다. #693 뒤 생산 동등성 세 쌍은 모두 통과했고 실행 준비와 144달러 주문 구현 가능성도 확인됐다. |
| 활성 작업 | 마지막 완결 판정 `33090550150`은 `WAIT_EDGE`, 단 0→0, 자본·주문 0건이다. #694 확인 실행 `33092282493`은 동등성 3/3과 주문 구현성을 다시 통과했지만 파일 수명주기 오류로 자본 결정 전에 실패했다. 자본·주문 변경은 없었다. |
...
- **운영 결함 발견**: #692 배포는 미국 장중이라 서버 패키지 교체를 안전하게 미뤘지만
  ...
  통과했다.
- **돈 경로**: 자동 판정 `33090550150`은 계좌 NAV `$1447.51`, IAUM 1주와 SCHX
...
- **후속 오류와 보정**: 단 0의 불필요한 성과 조회는 #694에서 제거했다. 재실행은 동등성
  ...
  때문에 자본 결정 전에 실패했다. 빈 증거 파일을 항상 만들고 성공 경로에서 임시 파일을
  지운 뒤 트랩을 해제하는 후속 보정을 진행 중이다.
- **상세 인계**: `HANDOFF-171-PARALLEL-REGIME-EDGE-CHALLENGER.md`와
  `specs/171-parallel-regime-edge-challenger/production-result.md`.
