...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `ecc84b9` — Merge pull request #689 from jinooaction/codex/170-small-account-execution-parity |
| main 테스트 | #689 머지 직전 `uv run pytest` → 3117 passed, 6 skipped. 건너뛴 6개는 KIS 실연결 전용이며 머지 후 KIS 읽기 전용 점검은 6/6 통과했다. |
| main 린트 | #689 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 현재 열린 PR은 없다. 로컬 `codex/170-proxy-evidence-truth`에서 실패 동등성 증거 거짓 승인과 장기 DB 오염 핫픽스를 검증 중이다. |
| 출시 완료 스펙 | 최신 기능: #689(스펙 170, 소액 계좌 실행 ETF 동등성과 주문 구현성), #687(스펙 167, 실제 17개 전략군과 검정력 보정 연구 진입), #686(스펙 166, 소비자 독립 v3 증거와 정확한 소액 자금 구현성), #684(스펙 165, 옵션 선택·목적 기준 교정과 WPUT 독립 재생), #682(스펙 164, 옵션 변동성 위험 프리미엄 16개·752개 고유 감사). |
| 골격 스펙 | 없음. 스펙 170 후속 핫픽스는 실패 문서 자체를 진입 거부로 해석하고, 실행 ETF 동등성을 매번 깨끗한 임시 KIS DB에서 계산한다. |
| 최근 출시 작업 | 신호 `SPY/IEF/GLD`는 유지하고 실행을 `SCHX/SPTI/IAUM`으로 교체했다. KIS 새 데이터에서 세 쌍 모두 동등성 기준을 통과했고, 공통 거래일 캐너리 무결성 구멍은 0건이다. |
| 활성 작업 | 자동 판정 `33024217264`은 `WAIT_EDGE`, 단 0→0, 자본·주문 0건이다. 145달러 주문 구현성은 통과했지만 앵커드 판정은 `NO_EDGE`; 생산 장기 DB의 실패 동등성 문서를 실행 준비로 잘못 읽은 소비자 버그를 핫픽스 중이다. |
...
## 최근 완료 — 2026-08-27 KST (#689 소액 계좌 실행 동등성, 후속 핫픽스 진행)
...
- **생산 확인**: merge `ecc84b9`, 배포 `33024089353`, KIS smoke `33024089368`이
  성공했다. KIS 6/6 통과, 현금 `$934.27`, NAV `$1456.75`, 최근 미체결 0건이다.
...
- **후속 결함**: 장기 first-write-wins DB의 옛 가격으로 만든 실패 동등성 문서를 소비자가
  구조상 진짜라는 이유만으로 통과 처리했다. 별도 전략 `NO_EDGE`가 자본 이동을 막아 손실은
  없었으며, 실패 결과 거부와 임시 KIS DB 재계산 핫픽스를 진행 중이다.
- **상세 기록**: `specs/170-small-account-execution-parity/production-result.md`.
...
* **2026-08-26 최신 베이스라인** — `main` `08970e7`에 스펙 163 에너지 교차시장
  전략 공장이 출시됐다. 생산 공장은 16/16 현재 후보와 736/736 누적 고유 감사를
  발행했고 판정은 `NO_FACTORY_EDGE`다.
* **현재 돈 경로** — `PREVIEW_ONLY`, `ACCUMULATING_EDGE`, 단 0/5, 자본 0,
  실주문 불가, 전략 체결 0건·손익 0달러다. `globalfixed` 전진 관측은 0/20이다.
* **현재 운영 검증** — KIS run `32861858561`은 현재 merge 커밋에서 읽기 전용
  5/5를 통과했고 최근 주문·열린 미체결은 0건이다. 생산 전략 공장 run
  `32861194112`도 성공했다.
* **서버 배포** — deploy run `32861194460`의 GitHub 작업은 성공했지만 서버는 미국
  장중 변경 금지 규칙에 따라 실제 반영을 연기했다. `auto-invest-deploy.timer`가
  2026-08-25 21:00 UTC 이후 재시도하므로 그 전에는 서버 반영 완료로 보지 않는다.
...
- `HANDOFF-169-CALIBRATED-RESEARCH-ENTRY.md` — 752개 원시 후보를 17개 실제 연구
  가족으로 재구성하고 중복 통계 문턱을 검정력 보정 기준으로 교체했다. 현재 옵션 가족은
  새 기준에서도 PBO 0.371429와 소액 구현 불가로 단 0·자본 0·주문 0건을 유지한다.
