...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `2745f99` — Merge pull request #777 from jinooaction/codex/182-account-exchange-code |
| main 테스트 | #777 코드b1c8e0a 전체3591 passed/9 skipped,실패0(762.39초). 관련75개 통과. 실제 KIS34073693815는 기존7개 통과/새 계좌 조회1개 실패(NON_US_MARKET/OTHER); 실제 코드 진단 중이다. |
| main 린트 | #777 코드b1c8e0a에서 `uv run ruff check src tests scripts/intraday_execution.py` 통과. |
| 열린 PR | #777 병합 완료. `codex/182-account-market-diagnostic`에서 허용 범위를 유지하며 실패 endpoint·공개 거래소 식별자를 제한적으로 진단한다. 열린 PR은 GitHub에서 다시 확인한다. |
...
| 활성 작업 | 600/12 준비 기준·모의 사전점검·페이지 보정은 병합·배포됐다. #777 거래소 보정 뒤에도 실제 계좌 읽기는 OTHER 시장에서 거절돼 제한된 코드 진단을 추가했다. 관련77개 통과, 전체 회귀/병합/실제 진단 중이다. 실제 자금 배정·주문0이며 검증된NAV·전략/전진 증거·생산 주문 연결은 남았다. |
