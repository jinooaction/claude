...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `9fbdef8` — Merge pull request #802 from jinooaction/codex/185-intraday-archive-review |
| main 테스트 | 코드f340175/main8e2e5ce의 전체3804 passed/12 skipped, 실패0(781.43초). 제외는 실제 읽기11개와 가동 전 조건1개다. 실제34353848847은11 passed/24.82초, 보관2일이 기존 연구기로 전달됐다. |
| main 린트 | 코드f340175/main8e2e5ce의 `uv run ruff check src tests` 통과. |
| 열린 PR | #793은 스펙184 초안이다. 코드93ee2fa 전체4215 passed/12 skipped(804.63초), 린트·사용자 자체 시험 통과. T009/T010 정상 계좌 입력·최종 정상 운용 경로는 미완료다. |
...
| 활성 작업 | 185 코드·실제 자료·배포·#793 통합 완료. #793의 생산 계좌 입력·적격 전략 권한·실주문 시작 연결(T009/T010)은 미완료다. 현재 진단 수집기는 정식 전진 실적 누적 경로가 아니며 기존 한국투자 계좌를 유지한다. |
