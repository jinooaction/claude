...
## 한눈 요약표 — 2026-08-26 KST 최신 코드 main 기준

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `42f3c54` — Merge pull request #687 from jinooaction/Codex/167-calibrated-research-entry |
| main 테스트 | #687 머지 직전 `uv run pytest` → 3106 passed, 5 skipped. 건너뛴 5개는 KIS 실연결 전용이며 머지 후 KIS 읽기 전용 점검은 5/5 통과했다. |
| main 린트 | #687 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | #687 기능은 병합됐고 생산 전략 공장·자본 판정·KIS 읽기 점검까지 완료됐다. 이 인수인계 갱신 외 새 기능 PR은 없다. |
| 출시 완료 스펙 | 최신 기능: #687(스펙 167, 실제 17개 전략군과 검정력 보정 연구 진입), #686(스펙 166, 소비자 독립 v3 증거와 정확한 소액 자금 구현성), #684(스펙 165, 옵션 선택·목적 기준 교정과 WPUT 독립 재생), #682(스펙 164, 옵션 변동성 위험 프리미엄 16개·752개 고유 감사), #680(스펙 163, 에너지 교차시장 16개·736개 고유 감사). |
| 골격 스펙 | 없음. 다음 기능 우선순위는 약 145달러 연구 자본으로도 한도를 지키며 표현 가능한 저가 상품·단일 다리·소수점 주문 경로를 독립 검증하고, `globalfixed` 강화 점검의 감사 무결성 실패 2건을 원인별로 해소하는 것이다. |
| 최근 출시 작업 | 752개 원시 후보를 17개 독립 연구 가족으로 재구성했다. 홀드아웃 PSR 0.95와 가족 PBO 0.25를 실제 진입 관문으로 쓰고, DSR 0.95와 원시 752개 Bonferroni는 중복 처벌을 막기 위해 진단으로 남겼다. |
| 활성 작업 | 최신 자동 판정 `32980657704`은 `WAIT_EDGE`, 단 0→0, 자본 이동·주문 0건이다. 현재 옵션 가족 PBO는 0.371429로 탈락하고, 약 145달러 연구 자본은 SPYM/GLDM 정수 주와 50% 주문 캡 때문에 구현 불가다. |
| 안전 경계 | main은 헌법 X.4 v10.1.0이다. 연구 단은 NAV 10%로 유지하며, 기존 20% 이상 승격, K1/K2, 손실 예산 20%, 정규장, 현금 1%, 서명·nonce, 추가 전용 감사 로그는 변경하지 않았다. ORANY 28주는 시작 전 외부 보유로 전략 손익에서 제외한다. |
