...
## 최근 머지 — 2026-05-22 (KIS 회귀 자율 검증 도입)

자세한 내용은 `HANDOFF-012-KIS-AUTONOMOUS-VERIFY.md` 참조. 한 줄 요약:

- PR #33 (`9096e21`): KIS 잔고 0원 표시 회귀 + 보유 포트폴리오 stub + 의도 자본 100달러 정책 제거.
- PR #34 (`8cfb7d3`): 라이브 KIS smoke 3건 + `.github/workflows/kis-smoke.yml` 신규 — main push 시 자동 회귀 검증 (운영자 무개입).

다음 세션이 첫 번째로 할 일: GitHub Actions 의 `KIS smoke (autonomous)` 첫 실행 결과 확인. 실패 시 `HANDOFF-012-KIS-AUTONOMOUS-VERIFY.md` 의 "다음 세션이 해야 할 일" 1번 절차로 자동 픽스.

## 현재 main 상태 (2026-05-22 기준)
...
* **스펙 003 (세션 캐시)** — 출시 완료.
* **스펙 004 (LLM 판단 지점)** — 골격만. 구현 보류 (30일치 spec 002 텔레메트리 누적 후 본격 구현 권장).
* **스펙 005 (자율 튜너)** — 골격만. 스펙 007 의존성은 해소됨; 후속 구현 가능.
...
* **KIS smoke 자율 감시** — 활성 상태. main push 시 `KIS smoke (autonomous)` 워크플로우 자동 실행. 매일 03:00 UTC cron. 진단은 `automation/kis-smoke-last-run` 사이드카 브랜치에 force-push (`git show origin/automation/kis-smoke-last-run:LAST_RUN.md` 한 줄로 조회).
* **main의 테스트**: 742 통과, 4 스킵 (라이브 KIS 스모크 4건은 `KIS_LIVE_TEST=1` 환경변수로 게이트).
...
이 두 스펙은 "운영자 손이 더 줄지만 필수는 아니다" 영역. 위 운영 절차 + 스펙 010 `design` 명령만으로도 v1 자동 거래 서비스는 굴러갑니다.
...
- `HANDOFF-010-OPERATOR-RESUME.md` — 스펙 010 운영자 자율 수행 셋업 흐름.
- `HANDOFF-011-AUTONOMOUS-OPS.md` — GitHub Actions 자율 수행 셋업 완료 노트.
- `HANDOFF-012-KIS-AUTONOMOUS-VERIFY.md` — **활성**. KIS 회귀 자율 검증 워크플로우 도입 (2026-05-22).
...
| 마지막 main 커밋 | `8cfb7d3 KIS 회귀 자율 검증 — main push 시 자동 smoke (#34)` |
| 활성 작업 | KIS 회귀 자율 검증 도입 완료 (2026-05-22). 다음: kis-smoke 워크플로우 첫 실행 결과 확인 — `HANDOFF-012-KIS-AUTONOMOUS-VERIFY.md` |
...
| main 테스트 | 741 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
| main 린트 | 깨끗 |
