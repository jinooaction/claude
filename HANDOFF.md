...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `cd17fb9` — Merge pull request #794 from jinooaction/codex/182-balance-audit |
| main 테스트 | main cd17fb9와 같은 실행 코드309144a에서 전체3733 passed/10 skipped, 실패0(804.19초). 기존 서버 실제 읽기 검증 진행 중. |
| main 린트 | 코드309144a에서 `uv run ruff check src tests` 통과. |
| 열린 PR | #793 초안: 실행 명령·시세 입력·기존 보유 분리·주문 관리 수명주기. 생산 관측과 권한을 생성하는 실주문 시작 연결 미완료로 병합하지 않았다. #790~#792 병합 완료. |
...
| 활성 작업 | 스펙184 주문 관리·지속 중지·재시작 복구(T012/T013)까지 구현했다. 전체3812 passed/10 skipped(806.13초)·린트·하네스 통과, PR793 초안이다. 기존 한국투자 계좌 사용은 확정됐다. 생산 관측·적격 권한을 생성하는 실주문 시작 연결(T009/T010)과 새 입력기의 인증 후 KIS 검증이 남았다. 기존 main의 실제 조회9개는 통과했지만 전체 현금/NAV는 미검증이다. 실제 자료·전략 합격·정식 전진60세션·생산 체결 증거를 대체하지 않는다. |
