...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `3d2592b` — Merge pull request #716 from jinooaction/Codex/176-live-session-gate |
| main 테스트 | `Codex/176-live-session-gate`에서 현재 main을 포함해 `uv run pytest` → 3221 passed, 7 skipped, 실패 0. 6개는 `KIS_LIVE_TEST=1` 전용이며 생산 KIS 스모크에서 6/6 통과했다. 나머지 1개는 사다리 가동 전 상태 전용 시험이라 현재 단 1에서 건너뛴다. |
| main 린트 | `Codex/176-live-session-gate`에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. |
| 출시 완료 스펙 | 최신 기능: #716(스펙 176, 시작 시점과 각 중개사 쓰기 직전 XNYS 정규장 실패 폐쇄·부분 실행 증거 보존), #715(스펙 176, 선언한 NAV 10%와 실제 주문 자본 일치), #714(스펙 176, 정확한 main 사전점검 인계), #712(스펙 176, 뉴욕 현지 10:17 비정각 실거래 예약), #709(스펙 176, 역할 분리 운영 증거와 NAV 10% 실거래 검증 캐너리), #706(스펙 175, 21가족 교정·PEAD 진단), #701(스펙 174, 회계 기반 횡단면 팩터), #699(스펙 173, 판정 경로 교정·월말월초 전략). |
| 골격 스펙 | 없음. 스펙 176 코드·단 1 센티넬·정규장 주문 방어는 출시됐고, 남은 완료 관문은 예약 실행의 실제 KIS 주문·체결·감사·정합 확인이다. 단일 예약의 호출 누락을 막는 같은 날 다중 실행 기회와 거래일 1회 주문권은 후속 브랜치에서 검증 중이다. |
...
| 활성 작업 | #716 merge `3d2592b`, 배포 `33379808130`, KIS 스모크 `33379808023`이 성공했고, 정확한 main 무주문 사전점검 `33380005999`은 `ENTRY_READY`, 자본 `$142`, IAUM 1주·SCHX 2주 계획, 계좌 정합 `OK`를 확인했다. 그러나 2026-08-31 뉴욕 10:17 예약은 뉴욕 11:07까지 실행 기록 자체가 생기지 않았다. `Codex/176-schedule-redundancy`에서 뉴욕 10:17·11:17·12:17·13:17 다중 기회와 거래일 1회 주문권을 구현·검증 중이며, 실제 전략 주문·체결은 아직 0건이다. |
...
- **단일 예약 호출 누락과 후속 보정**: #716 merge `3d2592b`, 배포 `33379808130`, KIS 스모크
  ...
  않는다. 이 후속은 전체 검증과 PR·머지·장 마감 후 정확 배포가 남아 있어 아직 생산 활성 상태가 아니다.
