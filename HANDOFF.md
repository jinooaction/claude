...
## 한눈 요약표 — 2026-09-08 KST 직접 잔고 대조 후속

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `1cb1af0` — Merge pull request #790 from jinooaction/codex/182-direct-balance-crosscheck |
| main 테스트 | main `1cb1af0`과 동일 코드08ef16b에서 전체3707 passed/10 skipped, 실패0(1079.30초). 제외10개는 실제 KIS9개와 가동 전 조건1개다. 실제 KIS34193364010은 기존8개 통과·새 잔고1개 실패다. |
| main 린트 | main `1cb1af0`과 같은 코드에서 `uv run ruff check src tests` 통과. |
| 열린 PR | #790 병합 완료. 직접 잔고 종료 판정 보정 후속을 진행하며 GitHub에서 열린 PR을 다시 확인한다. |
| 출시 완료 스펙 | 스펙183 모바일 자금·전략 운영판(#788, 앱 #3)과 스펙182 확정600/12·모의 주문·실제 GET 조회(T020/T028, #775~783) 완료. 스펙182 집계/NAV(T027)와 단타 실거래(T010~T012)는 미완료다. 기존182 실행 라이브러리(#771), 자본 검토(#773), 178 앱, 177/181 서버 진단 모의 수집은 아래 경과를 참조한다. |
...
| 최근 출시 작업 | 기존 HTML·형식 `1.0`·`read_only: true` 계약을 유지하면서 `status.json`에 자금·전략 요약을 추가했다. 비공개 Flutter 앱은 선택한 클레이 질감과 선명한 블록 디자인으로 홈·자금·전략·시스템 4개 탭을 제공하고, 정보 화면은 별도다. 1분 전경 갱신, 복귀 즉시 갱신, 마지막 성공 캐시, 계좌 값 기기 인증, 배경 불투명 보호막을 적용했다. |
| 활성 작업 | 직접 잔고 경로 #790 배포 성공, 실제 검사에서 종료 헤더 오류를 확인해 보정 중이다. 문의는 선택 사항이며 개인정보 단계는 개발·조회 검증의 선행 조건이 아니다. 계좌 총액/NAV와 단타 실거래 T010~T012는 미완료다. 앱은 출시됐으며 실제 iPhone 검증은 기기 연결을 기다린다. |
...
## 현재 진행 — 2026-09-08 KST (스펙182 문의 없는 직접 잔고 대조)
