...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `8b05fb9` — Merge pull request #675 from jinooaction/Codex/161-research-canary-evidence-parity |
| main 테스트 | #675 기능 머지 전 `uv run pytest -q` → 3048 passed, 5 skipped. 건너뛴 5개는 KIS 실연결 환경 전용이며 KIS 생산 검사는 별도로 5/5 통과했다. |
| main 린트 | #675 기능 머지 전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. 운영 재검증에서 발견한 자본 사다리 `10%` 계산과 돈 경로 `25%` 안내문 불일치를 격리 브랜치에서 보정 중이다. |
...
| 최근 출시 작업 | 생산자는 16/16 완결 전략군을 내는데 소비자가 64/64만 받던 계약 오류를 제거했다. 현재 EIA 공급·수요 후보는 16/16·704/704가 완전하지만 홀드아웃 PSR 0.705637로 `NO_FACTORY_EDGE`이며 단 0을 유지한다. |
| 활성 작업 | 자본 사다리 계산은 단1=10%인데 돈 경로·재지정 안내 일부가 옛 25%를 표시하던 보고 오류를 공통 사다리 원천으로 보정 중이다. 최신 돈 경로는 `PREVIEW_ONLY`, `ACCUMULATING_EDGE`, 단 0/5, 자본 0, 실주문 불가다. |
...
- **돈 경로**: 현재 `PREVIEW_ONLY`, 단 0, 자본 0, 전략 체결 0건이다. 자본 사다리 계산은
  단1=10%인데 돈 경로 안내문 일부가 옛 25%를 표시한 후속 보고 오류를 발견해 보정 중이다.
- **검증**: #675 전 전체 3048 passed/5 skipped, ruff, YAML, 엄격 하네스 14/14,
  PR 품질 관문을 통과했다. 현재 후보가 미확정이라 계약 교정 뒤에도 자본·주문은 변하지 않았다.
