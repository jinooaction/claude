...
## 최근 마일스톤 — 2026-05-29 (스펙 018: 다요인 신호 + 사이징 감사 기록)
...
| 마지막 main 커밋 | `aeed831 Merge pull request #91 from jinooaction/claude/intelligent-meitner-DtzD5` |
| 활성 작업 | **없음 — 스펙 018(다요인 신호·사이징 감사 기록) 완료·머지(2026-05-29, PR #91 `aeed831`).** 라이브 worker dry-run 가동 중. 모멘텀(시계열 N기간 수익률)·볼린저밴드 %B(평균회귀) 신호 2종 + 트리거 4종 추가. 사이징 결정(실현 변동성·역변동성 가중치·상관·최종 수량)을 K4 감사 기록으로 남기는 관찰 기반 구축. 다음 후보: 레짐 인식·교차 단면 랭킹·모멘텀 요인 결합, 완전 공분산 ERC, 양방향 그룹 budget-split. 전부 워크포워드로 표본 외 검증 받으며 진행. 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015, 016(슬라이스 1·2·3 전부), 017(슬라이스 1·2·2b·3 전부), 018(슬라이스 1 다요인 신호 + 슬라이스 2 사이징 감사 기록) |
| 진행 중 스펙 | 없음. 스펙 017·018 출시 완료. 후속(미착수): 레짐 인식·교차 단면 랭킹, 완전 공분산 ERC, 양방향 그룹 budget-split |
| 골격 스펙 (즉시 착수 가능) | **레짐 인식 또는 완전 공분산 ERC** — 모멘텀·BB 신호(018)와 사이징 감사 기반 위에서, 다음은 레짐(추세/횡보/하락) 인식으로 신호 강도를 조절하거나 공분산 기반 등기여 최적화(ERC)로 리스크 배분을 고도화. 전부 워크포워드 검증 필수 |
...
| main 테스트 | 1142 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
