...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `96d9c31` — Merge pull request #710 from jinooaction/automation/capital-ladder/promote-33351880615 |
| main 테스트 | 스펙 176 후속 브랜치에서 `uv run pytest` → 3207 passed, 7 skipped, 실패 0. 6개는 `KIS_LIVE_TEST=1` 전용이며 생산 KIS 스모크에서 6/6 통과했다. 나머지 1개는 사다리 가동 전 상태 전용 시험이라 현재 단 1에서 건너뛴다. |
| main 린트 | 스펙 176 기능 브랜치에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. 후속 오류 보정과 실제 정규장 체결 검증은 `Codex/176-autoarm-dispatch-followup`에서 진행 중이다. |
...
| 골격 스펙 | 없음. 스펙 176 코드와 단 1 센티넬은 출시됐고, 남은 완료 관문은 자동화 연쇄 오류 보정과 2026-08-31 15:00 UTC 정규장 주문·체결·감사·정합 확인이다. |
...
| 활성 작업 | 수익 증거 run `33352149256`은 새 main 커밋과 정확히 일치해 통과했다. 무주문 사전점검 `33352375809`은 원격 강화·수익·체결 대리·구현성 검사를 모두 통과했지만 GitHub 실행기의 로컬 재검증에 `pydantic`이 설치되지 않아 실패했고 주문은 0건이었다. 의존성 설치와 자동 머지 뒤 증거 갱신 누락을 보정 중이다. |
...
## 최근 진행 — 2026-08-31 KST (#709·#710 운영 검증 캐너리 무장, 실제 체결 대기)
...
- **생산 상태**: #709 merge `0b955ac`, 배포 `33351793153`, KIS 스모크 `33351793304`
  ...
  `entry_route: operational_canary`를 기록했다. 현재 전략 주문과 체결은 0건이다.
...
- **현재 작업**: 자동 PR 머지 전 전체 테스트·린트·엄격 하네스·본문 관문을 직접 실행하고,
  직접 머지 뒤 exact-main 수익 증거 완료 후 무주문 사전점검을 연쇄 실행한다. 실거래 작업에는
  고정 `uv` 런타임을 설치해 같은 재검증 코드를 실행한다.
- **완료 조건**: 후속 보정 PR과 배포·사전점검을 통과한 뒤 15:00 UTC 예약 실행에서 실제 KIS
