...
## 한눈 요약표 — 2026-09-06 KST 원격 main과 단타 작업 브랜치 구분

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `9f99501` — Merge pull request #767 from jinooaction/codex/181-intraday-runtime |
| main 테스트 | #767 코드592c106에서 `uv run pytest -q` → 3439 passed, 8 skipped, 실패 0 (778.75초). 최종f519d87은 검증 기록 전용이다. |
| main 린트 | #767 최종f519d87에서 `uv run ruff check src tests scripts/intraday_runtime.py` → All checks passed. |
| 열린 PR | #767은 병합됐다. 현재 `codex/181-production-proof`에서 실제 분봉·배포 확인 인계를 작성하며 열린 PR은 GitHub에서 다시 확인한다. |
...
| 활성 작업 | 스펙181 수집기·18후보 영속 진단 모의 실행·CLI는 #767로 출시됐다. 서버 실제 KIS34007993027은7/7, 최근 세션5종목×78봉 검증 통과. 756세션 역사·60세션 전진 관찰·단타 실주문 구현·생산 체결은 미완료다. 기존 저빈도 승인143·rung1·run_seq9와 ORANY28은 변경하지 않았다. 스펙180 실제 신규 접수·체결 T009도 미완료다. |
...
## 현재 진행 — 2026-09-07 KST (스펙181 서버 진단 운용 연결, 검증 중)
