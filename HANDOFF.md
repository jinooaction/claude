...
| 활성 작업 | #712 배포 `33355972625`, exact-main 수익 증거 `33355972593`, money-path `33355972617`이 성공했다. 주문 없는 사전점검 `33353865976`도 첫 진입 재검증·권한·체결 동기화·사후 측정·계좌 대사에 모두 성공하고 주문 0건을 기록했다. 현재 운영 보고서가 이 성공을 `preflight evidence absent`로 잘못 읽는 표시 오류를 고친 뒤, 14:17 UTC 예약 실행의 실제 주문·체결을 추적한다. |
...
## 최근 진행 — 2026-08-31 KST (#709~#712 운영 검증 캐너리 무장·예약 신뢰성 보정, 실제 체결 대기)
...
- **현재 작업**: 생산 사전점검은 실제로 `ENTRY_READY`였지만 money-path가 JSON 앞 메타데이터와
  같은 이름의 표 행 때문에 이를 `preflight evidence absent`로 표시했다. 주문 권한은 바꾸지
  않고 제목 범위의 기계 판독 JSON만 읽도록 보정 중이다.
