...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `fdb9149a` — Merge pull request #763 from jinooaction/automation/capital-ladder/resize-33935656168 |
| main 테스트 | #763 자동 승인33935656168의 `uv run pytest` → 3405 passed, 7 skipped, 실패 0 (1397.10초). |
| main 린트 | #763 자동 승인33935656168에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | #763 승인143 머지 완료. 이전 이벤트의 code_commit 불일치로 생긴 #764 강등 제안은 최신 검증 근거를 남기고 미병합 종료했다. FR-010 보정 진행 중이며 현재 열린 PR은 GitHub에서 확인한다. |
...
| 활성 작업 | 오너의 해외 ETP 신청 완료를 확인했다. #763으로 승인143·rung1·run_seq9가 main에 반영됐고 deploy33937154453·감사33937198586이 exact fdb9149a 배포를 확인했다. KIS33937196360 6/6. 배포 후 no-order33937207081은143 첫진입 success/CLEAR/OK/VALID/orders0이다. T008/T011/T013 완료. 지연 autoarm의 이전 이벤트·최신 증거 혼합을 FR-010으로 보정한다. 실제 신규 접수·체결은0건이며 T009 미완료다. |
