...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `2141a7c` — Merge pull request #682 from jinooaction/Codex/164-independent-options-variance-risk-premium |
| main 테스트 | #682 머지 직전 `uv run pytest` → 3084 passed, 5 skipped. 건너뛴 5개는 KIS 실연결 환경 전용이며 merge 커밋 읽기 전용 스모크에서 5/5 통과했다. |
| main 린트 | #682 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | #682 기능·생산·배포·계좌 재검증은 완료됐다. 이 인계 갱신 외 진행 중 기능은 없다. |
| 출시 완료 스펙 | 최신 기능: #682(스펙 164, 옵션 변동성 위험 프리미엄 16개·752개 고유 감사·목적 관문 오류 진단), #680(스펙 163, 에너지 교차시장 16개·736개 고유 감사), #678(스펙 162, USDA 작물 수급 16개·720개 고유 감사), #675(스펙 161, 연구 캐너리 증거 계약 정합), #674(스펙 160, 무레버리지 전진 장부), #673(스펙 159, 쌍체결 활성수익 PSR). |
| 골격 스펙 | 없음. 다음 우선순위는 현재 홀드아웃 후보를 승격하지 않고, 독립 옵션 기준지수와 중첩형 워크포워드로 목적·선택을 다시 사전등록하는 `options_selection_and_objective_repair`다. |
| 최근 출시 작업 | 공식 Cboe PUT/VIX로 16개 옵션 후보를 검증하고 누적 752개 감사를 생산화했다. 알려진 PUT 현금 프리미엄은 연 +5.177143%, PSR 0.964744로 존재하지만 최근 주식 샤프보다 -0.124261 낮다는 이유만으로 채택 관문이 탈락시켜 `GATE_OR_REFERENCE_SUSPECT`다. |
| 활성 작업 | 기능 구현 없음. 최신 돈 경로는 main `2141a7c`에서 `PREVIEW_ONLY`, `ACCUMULATING_EDGE`, 단 0, 자본 0, 실주문 불가다. 깨끗한 `globalfixed` 전진 관측은 0/20이다. #682 생산 전략 공장·서버 배포·KIS 읽기 전용 점검은 모두 성공했다. |
...
## 최근 완료 — 2026-08-26 KST (#682 옵션 변동성 위험 프리미엄·752회 판정)
