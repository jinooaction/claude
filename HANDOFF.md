...
## 한눈 요약표 — 2026-09-02 KST 최신 코드 main 기준

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `e12b7e90` — Merge pull request #731 from jinooaction/codex/178-iphone-verification-final-handoff |
| main 테스트 | exact 기능 코드 `a6fe2124`에서 `uv run pytest` → 3279 passed, 7 skipped, 실패 0. |
| main 린트 | exact 기능 코드 `a6fe2124`에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | #732 `fix(176): keep server scheduler live across documentation-only main drift`가 `MERGEABLE/CLEAN`이고 원격 PR 품질 관문이 성공했다. 장 마감 뒤에만 병합한다. |
...
| 활성 작업 | 2026-09-02 GitHub 예약은 15:43 UTC까지 생성되지 않았고, 14:35 UTC 독립 서버 예약도 실행 요약을 만들지 못했다. 배포 뒤 `HANDOFF.md`·`specs/**`만 진행한 현재 main을 미배포 실행 코드로 오판하는 독립 예약의 정확 일치 검사를 보정 중이다. 라이브 경로는 기존 `globalfixed-ensemble-3-6-9-12`, NAV 10%, 단 1을 그대로 유지한다. |
| 안전 경계 | 작업 브랜치는 헌법 X.4 v14.0.0이다. 배포 커밋이 현재 main의 조상이고 사이 변경이 고정 비실행 경로 `*.md`·`specs/**`·`.verify/**`·`.trigger/**`뿐일 때만 운영 동등으로 본다. 실행 코드·배포·workflow·설정·자동화·분기·읽기 실패는 계속 차단한다. 운영 경로는 NAV 10%, 최대 단 1, `alpha_confirmed=false`, 단 1 초과 승격 금지이며 수동 서비스 시작과 수동 실주문도 계속 금지된다. |
...
- **현재 검증**: 새 운영 리비전·서버 예약·주문 경계 관련 50개 시험과 쉘 구문 검사가 통과했다.
  ...
  `MERGEABLE/CLEAN`이고 exact head `7b345a24`의 원격 PR 품질 관문도 성공했다.
