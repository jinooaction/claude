...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `cb14c9e` — Merge pull request #697 from jinooaction/codex/172-next-priority-strategy-audit |
| main 테스트 | 이 인계 브랜치에서 최신 `main` 기준 `uv run pytest -q` → 3139 passed, 6 skipped. 첫 재실행의 2개 실패는 낡은 HANDOFF 커밋 행 하나가 원인이었고, 최신화 뒤 전체 통과했다. 건너뛴 6개는 KIS 실연결 전용이다. |
| main 린트 | 이 인계 브랜치에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 2026-08-30 사실 확인 시 열린 기능 PR은 없다. #697 코드와 생산 관찰은 완료됐고 이 문서 전용 마감만 진행한다. |
| 출시 완료 스펙 | 최신 기능: #697(스펙 172, 실제 8관문 합격 경로 감사·절대 검출력 하한·고정 레짐 후보 전진 관찰·후보 ID 교착 해소), #692~#695(스펙 171, 독립 저회전 레짐 후보 검증·생산 관찰 보정), #689·#690(스펙 170, 소액 계좌 실행 동등성·주문 구현성), #687(스펙 167, 실제 17개 전략군과 검정력 보정 연구 진입). |
| 골격 스펙 | 없음. 스펙 172는 완료됐다. 자동 실행 루프가 다음 독립 신호군 후보 `candidate-parallel-edge-challenger-d59c4161bcba`를 안정된 식별자로 발행했다. |
| 최근 출시 작업 | `regime-corr24-thr0p2-weak6-cash`는 역사 관문 7/8이지만 최근 구간 승리 실패로 실자본 부적격이다. 기존 검정력 교정이 8관문 전체가 아니라 핵심 2관문만 덮는다는 점과, 48관측 전진 판정의 절대 검출력이 80%에 크게 못 미친다는 점을 코드가 정직하게 드러내도록 고쳤다. |
| 활성 작업 | `globalfixed-ensemble-3-6-9-12`는 전진 관측 4/20으로 `ACCUMULATING_EDGE`; 고정 레짐 후보는 동결 뒤 0/20으로 `OBSERVATION_WAIT`다. 기다림과 별도로 다음 독립 신호군 후보는 `EXECUTION_READY`다. 라이브 상태는 `PREVIEW_ONLY`, 자본·실주문 0건이다. |
| 안전 경계 | main은 헌법 X.4 v10.1.0이다. 연구 단 NAV 10%, 20% 이상 승격, 손실 예산 20%, 정규장, 킬스위치, 주문·노출 한도는 유지했다. 자본은 0이고 ORANY 28주는 외부 보유로 전략 손익에서 제외한다. |
