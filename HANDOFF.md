...
| 마지막 main 커밋 | `e8b3876 Merge PR #65 — feat(deploy): 머지 시 systemd 유닛 자동 동기화` |
| 활성 작업 | **없음 — 스펙 005 후속(자율 튜너 타이머) + 유닛 자동 설치까지 완료·머지(2026-05-26, PR #63·#65).** 라이브 worker dry-run 가동 중. 다음 후보: L1 적용 표면 확장(모델 라우팅·캐시 TTL 노브 — **K3 인접, 자동화 신중**), L2/L3→캐너리 자동 큐. 실거래 전환은 운영자 명시 지시 필요 |
...
| main 테스트 | 902 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
