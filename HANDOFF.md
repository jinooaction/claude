...
⚠️ **운영자 결정 지점(헌법 X.4 — 전략 재지정·레버리지는 운영자 게이트)**: 라이브 지정을
*무레버 안전성* 기준(현 역변동성 유지)으로 둘지, *레버리지 후 복리 극대화* 기준(고정가중으로
재지정 + 레버리지 켜기)으로 바꿀지. 두 목표는 다른 축. **자율로 바꾸지 말 것** — 측정만 했고
재지정/레버리지 적용은 0. 운영자 방향 확인 후 진행.

### 다음 세션 후속 (읽기 전용·비게이트 — 운영자 결정 전에도 가능)

1. **거래비용 반영 레버리지 재측정** — 역변동성은 매월 가중 재조정이라 회전율·비용이 고정가중
   보다 높다 → 비용 넣으면 고정가중 우위가 더 커질 가능성. 스펙 016 비용 모델 재사용. 운영자
   결정 기반을 더 단단히.
...
| 마지막 main 커밋 | `92b4177 Merge pull request #309 — feat(044) 라이브 전략 성장 최적 레버리지 정량화` |
...
| main 테스트 | 2044 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
