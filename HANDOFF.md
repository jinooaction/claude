...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `7fe7334` — Merge pull request #667 from jinooaction/Codex/156-commodity-term-structure |
| main 테스트 | #667 기능 머지 직전 `uv run pytest -q` → 3002 passed, 5 skipped. 건너뛴 5개는 KIS 실연결 환경 전용이며 머지 뒤 KIS 생산 검사는 별도로 5/5 통과했다. |
| main 린트 | #667 기능 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 기능 PR 없음. 이 인계 갱신은 `Codex/handoff-after-commodity-term-structure`에서 진행한다. |
| 출시 완료 스펙 | 최신 기능: #667(스펙 156, 독립 원자재 기간구조 16개·672개 고유 감사 장부), #665(스펙 155, 독립 외환 금리차 16개·관문 검출력 곡선), #663(스펙 154, 회사채 스프레드 64개), #662(스펙 153, 가족별 다중검정과 개발/홀드아웃 분리), #659/#660(스펙 152, 미국 국채 캐리), #657(스펙 151, 독립 거시 전략), #653(스펙 150, 자동 전략 공장). |
| 골격 스펙 | 없음. 다음 신규 기능은 상품 홀드아웃을 보고 다시 고르지 않고 독립 원자재 재고·포지셔닝 전략군을 새 SDD로 사전등록한다. |
| 최근 출시 작업 | 스펙 156이 GSG 총수익과 세계은행 현물 지수의 차이에서 이전 달 현금수익률을 뺀 원자재 기간구조 대용 신호로 16개를 생산 검증했다. 최고 후보도 홀드아웃 PSR 0.611866, 50bp 비용 후 연 초과수익 -0.641336%, 혼합 샤프 변화 -0.183303이라 실거래·종이 후보 모두 탈락했다. |
| 활성 작업 | 기능 구현은 없음. 스펙 156 생산 결과와 계좌 사실을 인계에 닫는 중이다. 다음 우선순위는 독립 원자재 재고·포지셔닝 전략군이며 상품 기간구조 기준을 낮추거나 홀드아웃에서 후보를 다시 고르지 않는다. |
...
## 최근 완료 — 2026-08-25 KST (#667 독립 원자재 기간구조 전략 공장·672회 판정)
