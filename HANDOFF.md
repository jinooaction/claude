...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `427a0f7f` — Merge pull request #745 from jinooaction/codex/176-zero-order-diagnostics |
| main 테스트 | #745 exact 기능 트리에서 `uv run pytest` → 3347 passed, 7 skipped, 실패 0. |
| main 린트 | #745 exact 기능 트리에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. `codex/176-kis-order-rejection-fix`에서 브로커 거부의 닫힌 KIS 코드 진단 1.1을 검증 중이다. |
| 출시 완료 스펙 | 최신 기능: #745(스펙 176, 자동 실행 주문 0건의 고정 읽기 전용 결과 진단), #744(스펙 176, 서버 timer 조기 실패의 고정 읽기 전용 실행 진단), #728(스펙 178, 읽기 전용 Flutter 운영자 상태 계약과 앱 설계), #726(스펙 177, 자본 0의 비용 현실형 장중 페이퍼 연구 엔진), #724(스펙 176, 독립 서버 예약과 정수주 표현 가능 목표 기준의 자금성 계약), #722(스펙 176, 거래소별 5xx 뒤 제한된 시세 탐색과 전 후보 장애 보존), #721(스펙 176, 고정 읽기 전용 배포 감사 명령), #719(스펙 176, 10:17~13:53 최대 12분 간격 19개 예약 기회), #717(스펙 176, 거래일 1회 주문 선점·최초 실행 증거 보존), #716(스펙 176, 시작 시점과 각 중개사 쓰기 직전 XNYS 정규장 실패 폐쇄·부분 실행 증거 보존), #715(스펙 176, 선언한 NAV 10%와 실제 주문 자본 일치), #714(스펙 176, 정확한 main 사전점검 인계), #712(스펙 176, 뉴욕 현지 비정각 실거래 예약), #709(스펙 176, 역할 분리 운영 증거와 NAV 10% 실거래 검증 캐너리), #706(스펙 175, 21가족 교정·PEAD 진단), #701(스펙 174, 회계 기반 횡단면 팩터), #699(스펙 173, 판정 경로 교정·월말월초 전략). |
...
| 활성 작업 | #745를 긴급 배포하고 production run `20260903161106`을 읽은 결과 IAUM 1주와 SCHX 2주가 모두 내부 gate가 아니라 KIS에서 명시적으로 거부됐다. 현재 브랜치에서 원문 없이 KIS 결과 코드와 주문 분류만 읽어 정확한 어댑터 원인을 확정 중이다. |
...
현재 `codex/176-kis-order-rejection-fix`는 진단을 1.1로 올려 원문·가격·현금·계좌·주문번호를
계속 버리면서 `kis_rt_cd`, `kis_msg_cd`, HTTP 상태, 예외 종류, 고정 TR ID·미국 주문 거래소·
주문 구분만 종목별로 반환한다. 이 생산 코드를 확인한 뒤 정확한 KIS 주문 어댑터 원인을 고치고,
첫 선점을 지우지 않는 추가 전용 단회 자동 재시도 계약을 별도 안전 변경으로 검증한다. 수동 주문·
서비스 수동 시작·선점 삭제는 하지 않았다.
