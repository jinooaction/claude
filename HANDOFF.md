...
## 한눈 요약표 — 2026-09-01 KST 최신 코드 main 기준

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `bbbfa70` — Merge pull request #721 from jinooaction/Codex/176-deploy-audit-gateway |
| main 테스트 | `Codex/176-deploy-audit-gateway`에서 #721의 정확한 코드 트리로 `uv run pytest` → 3233 passed, 7 skipped, 실패 0. 생산 KIS 스모크 `33442299948`은 AAPL·현금 `$934.27`·ORANY 28주·NAV `$1427.63`·미체결 0건은 정상이나 IAUM의 첫 `EXCD=NAS` 시세 조회 500으로 5/6 실패했다. |
| main 린트 | `Codex/176-deploy-audit-gateway`에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. |
| 출시 완료 스펙 | 최신 기능: #721(스펙 176, 고정 읽기 전용 배포 감사 명령), #719(스펙 176, 10:17~13:53 최대 12분 간격 19개 예약 기회), #717(스펙 176, 거래일 1회 주문 선점·최초 실행 증거 보존), #716(스펙 176, 시작 시점과 각 중개사 쓰기 직전 XNYS 정규장 실패 폐쇄·부분 실행 증거 보존), #715(스펙 176, 선언한 NAV 10%와 실제 주문 자본 일치), #714(스펙 176, 정확한 main 사전점검 인계), #712(스펙 176, 뉴욕 현지 비정각 실거래 예약), #709(스펙 176, 역할 분리 운영 증거와 NAV 10% 실거래 검증 캐너리), #706(스펙 175, 21가족 교정·PEAD 진단), #701(스펙 174, 회계 기반 횡단면 팩터), #699(스펙 173, 판정 경로 교정·월말월초 전략). |
| 골격 스펙 | 스펙 176의 거래소별 5xx 후 제한된 시세 탐색 보정이 `Codex/176-quote-market-5xx-fallback`에서 진행 중이며 전체 시험 3236 passed, 7 skipped와 린트를 통과했다. 단 1 센티넬·정규장 주문 방어·거래일 1회 선점·12분 예약 복구·읽기 전용 배포 감사는 main에 출시됐다. 남은 완료 관문은 시세 보정의 PR 품질 관문·merge·exact-main 배포·KIS 스모크 6/6과 다음 거래일 실제 KIS 주문·체결·감사·정합 및 후속 중복 차단 확인이다. |
| 최근 출시 작업 | 정확 배포 전략 `globalfixed-ensemble-3-6-9-12`를 알파 확정이 아닌 운영 검증으로 분류하고, 실계좌 NAV `$1427.63`의 10%를 내림한 `$142`만 단 1에 무장했다. 예상 주문은 IAUM 1주와 SCHX 2주이며 ORANY 28주는 비관리 보유로 자동 매도하지 않는다. #715는 자본 계약을 바로잡았고, #716은 장 밖 주문을 실패 폐쇄하면서 부분 주문 뒤에도 증거를 보존한다. 정확한 main 사전점검은 총 `$105.22`의 주문 계획과 주문 0건을 확인했다. |
| 활성 작업 | exact-main `bbbfa70` 배포 run `33442103343`과 배포 감사 run `33442221977`이 성공해 서버 감사 사슬은 `DEPLOY_COMPLETED`다. 이어 KIS 스모크 `33442299948`이 IAUM의 첫 `NAS` 시세 조회 500으로 5/6 실패해, 5xx 뒤 제한된 다음 거래소를 확인하되 전 후보 실패 시 5xx를 보존하는 보정을 구현했다. 관련 37개와 전체 3236개 시험, 린트가 통과했으며 PR 품질 관문 뒤 merge·배포·KIS 스모크 6/6·무주문 사전점검을 확인한다. 그 뒤 다음 거래일 최초 주문·체결·감사·정합과 후속 중복 차단을 계속 추적한다. 실제 전략 주문·체결은 아직 0건이다. |
| 안전 경계 | main은 헌법 X.4 v11.0.0이다. 운영 경로는 NAV 10%, 최대 단 1, `alpha_confirmed=false`, 20% 승격 금지다. 손실 예산 20%, 정규장 지정가, 서명·nonce, 킬스위치, 주문·노출 한도와 추가 전용 감사는 유지한다. |

## 최근 진행 — 2026-09-01 KST (#709~#721 운영 검증 캐너리 무장·예약·보고·자본·정규장·배포 감사 보정, 시세 조회 보정 진행)
...
- **거래소별 시세 오류 발견과 보정 진행**: exact-main KIS 스모크 `33442299948`은 AAPL·현금
  `$934.27`·ORANY 28주·NAV `$1427.63`·미체결 0건은 정상이나 IAUM을 첫 후보 `NAS`로 조회할 때
  KIS가 500을 반환해 5/6 실패했다. `Codex/176-quote-market-5xx-fallback`은 5xx 뒤 `NYS/AMS`의
  제한된 후보를 계속 읽고 유효 시세의 실제 거래소를 주문 코드에 연결한다. 전 후보가 실패하면 마지막
  5xx를 다시 던지고 4xx는 즉시 실패해 중개사 전체 장애나 인증 오류를 숨기지 않는다. 관련 37개
  시험과 전체 3236개 시험·린트는 통과했으며 PR 품질 관문·merge·exact-main 배포·생산 KIS
  스모크 6/6은 아직 남아 있다.
