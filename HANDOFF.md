...
## 한눈 요약표 — 2026-08-31 KST 최신 코드 main 기준

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `c413742` — Merge pull request #715 from jinooaction/Codex/176-canary-capital-contract |
| main 테스트 | `Codex/176-canary-capital-contract`에서 현재 main을 포함해 `uv run pytest` → 3212 passed, 7 skipped, 실패 0. 6개는 `KIS_LIVE_TEST=1` 전용이며 생산 KIS 스모크에서 6/6 통과했다. 나머지 1개는 사다리 가동 전 상태 전용 시험이라 현재 단 1에서 건너뛴다. |
| main 린트 | `Codex/176-canary-capital-contract`에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. |
| 출시 완료 스펙 | 최신 기능: #715(스펙 176, 선언한 NAV 10%와 실제 주문 자본 일치), #714(스펙 176, 정확한 main 사전점검 인계), #712(스펙 176, 뉴욕 현지 10:17 비정각 실거래 예약), #709(스펙 176, 역할 분리 운영 증거와 NAV 10% 실거래 검증 캐너리), #706(스펙 175, 21가족 교정·PEAD 진단), #701(스펙 174, 회계 기반 횡단면 팩터), #699(스펙 173, 판정 경로 교정·월말월초 전략). |
| 골격 스펙 | 없음. 스펙 176 코드·단 1 센티넬·비정각 예약은 출시됐고, 남은 완료 관문은 2026-08-31 뉴욕 10:17 정규장 주문·체결·감사·정합 확인이다. |
| 최근 출시 작업 | 정확 배포 전략 `globalfixed-ensemble-3-6-9-12`를 알파 확정이 아닌 운영 검증으로 분류하고, 실계좌 NAV `$1427.63`의 10%를 내림한 `$142`만 단 1에 무장했다. 예상 주문은 IAUM 1주와 SCHX 2주이며 ORANY 28주는 비관리 보유로 자동 매도하지 않는다. #715는 설정과 주문 계산이 모두 NAV 10%를 사용하도록 실패 폐쇄했고, 정확한 main 사전점검은 총 `$105.32`의 주문 계획과 주문 0건을 확인했다. |
| 활성 작업 | #715 merge `c413742`, 배포 `33361511135`, KIS 스모크 `33361511151`이 성공했고, 정확한 main 무주문 사전점검 `33361615036`은 `ENTRY_READY`, 자본 `$142`, IAUM 1주·SCHX 2주 계획, 계좌 정합 `OK`를 확인했다. 다만 예약 지연과 장 마감 직전 경쟁 상태가 정규장 밖 주문으로 이어질 수 있는 결함을 발견해 PR #716에서 CLI 시작 전과 각 실제 중개사 쓰기 직전 XNYS 실패 폐쇄 및 종료 코드 전파를 검증 중이다. 실제 전략 주문·체결은 아직 0건이다. |
...
## 최근 진행 — 2026-08-31 KST (#709~#715 운영 검증 캐너리 무장·예약·보고·자본 계약 보정, 실제 체결 대기)
...
- **현재 진행 중인 정규장 방어**: GitHub 예약은 과거 최대 약 9시간 지연됐지만
  `rebalance-once` 명령 자체에는 XNYS 정규장 확인이 없었다. 아직 출시되지 않은 PR #716은
