...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `965ecee` — Merge pull request #815 from jinooaction/codex/account-source-collection |
| main 테스트 | 코드adfb510/main cd1bec9 전체3908 passed/12 skipped(788.78초). 제외는 실제 KIS11개와 가동 전 상태 전용1개. 실제34691717131은11 passed, 원본10응답·순번9·국내/해외 현금 대조 성공. 실제 검사 시간은 마스킹됨. |
| main 린트 | 코드adfb510/main cd1bec9의 `uv run ruff check src tests` 통과. |
| 열린 PR | #793은 스펙184 초안이다. 실행 코드8658792 전체4474 passed/12 skipped(829.41초), 린트·하네스14/14·인계 통과. 현재cbd1334는 검증 기록 문서 후속이다. 전체 계좌 계산과 T009/T010은 미완료. |
