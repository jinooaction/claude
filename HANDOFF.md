...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `d3d028d7` — Merge pull request #723 from jinooaction/Codex/176-quote-market-5xx-handoff |
| main 테스트 | `Codex/176-quote-market-5xx-fallback`에서 #722의 정확한 코드 트리로 `uv run pytest` → 3236 passed, 7 skipped, 실패 0. exact-main 생산 KIS 스모크 `33445033903`도 6/6 통과했다. |
| main 린트 | `Codex/176-quote-market-5xx-fallback`에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | #724 `Codex/176-independent-live-scheduler` — 독립 서버 예약 보정과 소액 정수주 자금성 계약 보정을 함께 검증 중. |
| 출시 완료 스펙 | 최신 기능: #722(스펙 176, 거래소별 5xx 뒤 제한된 시세 탐색과 전 후보 장애 보존), #721(스펙 176, 고정 읽기 전용 배포 감사 명령), #719(스펙 176, 10:17~13:53 최대 12분 간격 19개 예약 기회), #717(스펙 176, 거래일 1회 주문 선점·최초 실행 증거 보존), #716(스펙 176, 시작 시점과 각 중개사 쓰기 직전 XNYS 정규장 실패 폐쇄·부분 실행 증거 보존), #715(스펙 176, 선언한 NAV 10%와 실제 주문 자본 일치), #714(스펙 176, 정확한 main 사전점검 인계), #712(스펙 176, 뉴욕 현지 비정각 실거래 예약), #709(스펙 176, 역할 분리 운영 증거와 NAV 10% 실거래 검증 캐너리), #706(스펙 175, 21가족 교정·PEAD 진단), #701(스펙 174, 회계 기반 횡단면 팩터), #699(스펙 173, 판정 경로 교정·월말월초 전략). |
| 골격 스펙 | 없음. 단 1 센티넬·정규장 주문 방어·거래일 1회 선점·12분 예약 복구·읽기 전용 배포 감사·거래소별 5xx 시세 탐색은 main에 출시됐다. 스펙 176의 남은 완료 관문은 다음 거래일 실제 KIS 주문·체결·감사·정합 및 같은 날 후속 예약의 중복 주문 0건 확인이다. |
| 최근 출시 작업 | 정확 배포 전략 `globalfixed-ensemble-3-6-9-12`를 알파 확정이 아닌 운영 검증으로 분류하고, 실계좌 NAV `$1427.63`의 10%를 내림한 `$142`만 단 1에 무장했다. exact-main 사전점검 `33445149085`의 계획은 IAUM 1주를 지정가 `$44.48`, SCHX 2주를 지정가 `$30.32`에 매수하는 총 `$105.12`이며 ORANY 28주는 비관리 보유로 자동 매도하지 않는다. 실제 주문은 0건이었다. |
| 활성 작업 | schedule run `33540003731`은 2026-09-01 17:49 UTC에 exact main `d3d028d`로 장중 생성됐지만, IAUM 목표 금액이 1주 가격보다 작아 기존 자금성 분모가 전체 진입을 막았다. 주문·체결 0건, 사후 측정 `CLEAR`, 계좌 정합 `OK`, 증거 `VALID`로 안전 차단됐다. PR #724는 GitHub 예약 지연에 독립 서버 예약을 추가하고, 후속 헌법 13.0.0·스펙 176 보정은 66% 기준을 낮추지 않은 채 현재 목표 금액으로 1주를 표현할 수 있는 종목만 그 분모에 넣는다. 모든 목표의 시세·오차·현금·최소 주문액·허용 종목·노출 한도 검사는 유지한다. 실제 전략 주문·체결은 아직 0건이며, 실제 자동 체결·감사·정합·이중 예약 중복 차단이 완료 관문이다. |
