...
## 한눈 요약표 — 2026-08-23 KST 최신 코드 main 기준

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `caae2e5` — Merge pull request #661 from jinooaction/Codex/handoff-after-treasury-carry-factory |
| main 테스트 | #661 머지 코드 기준 `uv run pytest -q` → 2960 passed, 5 skipped. 현재 스펙 153 구현 브랜치는 2968 passed, 5 skipped. |
| main 린트 | #661 머지 코드와 현재 스펙 153 구현 브랜치 모두 `uv run ruff check src tests scripts` → All checks passed. |
| 열린 PR | 없음. |
| 출시 완료 스펙 | 최신 기능: #659/#660(스펙 152, 미국 국채 5개 만기 캐리 전략 64개와 총 576회 검정, 공개자료 보고 정합 보정), #657(스펙 151, 독립 거시 전략 64개와 총 512회 검정), #653(스펙 150, 자동 전략 공장), #646~#651(스펙 149, 첫 주문 직전 최신 엣지 재검증·자동 강등). |
| 골격 스펙 | `specs/153-edge-gate-calibration/` T001~T021 완료, T022~T027 검증·출시 진행 중. 다음 신규 전략군보다 판정 체계 교정이 우선이다. |
| 최근 출시 작업 | 스펙 152 국채 공장은 출시됐지만, 스펙 153 감사에서 이질적 576개를 한 DSR/PBO 가족으로 처리하고 홀드아웃에서 승자를 고른 결함을 확인했다. 교정 구현은 아직 PR·생산 재생 전이다. |
| 활성 작업 | `Codex/153-edge-gate-calibration`. 고정 시드 500회에서 새 계층형 관문은 무엣지 거짓 합격 3.6%, 샤프 0.60 심은 엣지 발견 83.4%; 구버전 관문 발견 8.2%다. 전체 2968 passed/5 skipped, ruff 통과. 실제 공개자료 판정·PR·배포는 아직 남았다. 돈 상태는 `PREVIEW_ONLY`, `armed:false`, 단 0/5, 자본 0, 주문·전략 체결 0건, 전략 손익 0달러다. |
...
## 진행 중 — 2026-08-23 KST (스펙 153 수익 엣지 판정 교정)
