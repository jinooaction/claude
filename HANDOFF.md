...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `e6674c8` — Merge pull request #778 from jinooaction/codex/182-account-market-diagnostic |
| main 테스트 | #778 코드cae624f 전체3593 passed/9 skipped,실패0(801.91초). 관련77개 통과. 실제 KIS34074654444는 기존7개 통과/새 계좌 조회1개 실패(inquire-balance의 OTCB); 원인을 확인했다. |
| main 린트 | #778 코드cae624f에서 `uv run ruff check src tests` 통과. |
| 열린 PR | #778 병합 완료. `codex/182-unverified-account-assets`에서 OTCB를 별도 미검증 자산으로 보존하고 명시적인 경고 결과를 제공한다. 열린 PR은 GitHub에서 다시 확인한다. |
...
| 활성 작업 | 600/12 준비 기준·모의 사전점검·페이지 보정은 병합·배포됐다. 실제 오류는 잔고의 OTCB로 확인했고 별도 미검증 자산 보고를 구현했다. 관련89개 통과, 전체 회귀/병합/실제 조회 검증 중이다. 실제 자금 배정·주문0, NAV·전략/전진 증거·생산 주문 연결은 미완료다. |
