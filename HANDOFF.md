...
- **안전 경계**: 읽기 전용·순수·Kernel 0·주문 0·돈 0. **아직 라이브 게이트 무변경**(엔진+파이프
  라인만). 
- **다음 단계(미완)**: ① 인스턴스에서 도는 CLI `forward-verdict-anchored`(인제스트 깊은
  데이터로 walk-forward OOS + 라이브 forward 스냅샷 읽어 앵커드 판정 JSON 발행, 읽기 전용
  진단) → ② autoarm/사다리 게이트가 그 판정을 소비(실제 돈 게이트 변경 — 신중히). 인스턴스
  데이터(인제스트 데이터셋) 가용성 확인 필요. 이 두 단계 전까지 라이브 무장은 기존 forward
  20일 게이트 유지.
...
| 마지막 main 커밋 | `c9a307b Merge pull request #299 — feat(035) walk-forward OOS 노출 + 앵커드 파이프라인 검증` |
...
| main 테스트 | 2003 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
