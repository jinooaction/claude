...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `08970e7` — Merge pull request #680 from jinooaction/codex/163-independent-energy-cross-market |
| main 테스트 | #680 인계 갱신 기준 `uv run pytest -q` → 3072 passed, 5 skipped. 건너뛴 5개는 KIS 실연결 환경 전용이며 현재 merge 커밋 읽기 전용 스모크에서 5/5 통과했다. |
| main 린트 | #680 인계 갱신 기준 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 기능 구현과 생산 재검증은 완료됐다. 이 인계 갱신 외 진행 중 기능은 없다. |
| 출시 완료 스펙 | 최신 기능: #680(스펙 163, 에너지 교차시장 16개·736개 고유 감사·목적별 판정 경로), #678(스펙 162, USDA 작물 수급 16개·720개 고유 감사·기준 검출력), #675(스펙 161, 16/16 연구 캐너리 증거 계약 정합), #674(스펙 160, 무레버리지 전진 장부 무결성), #673(스펙 159, 쌍체결 활성수익 PSR·전략 사용성), #671(스펙 158, 전체 다섯 관문 현실 감사·독립 EIA 공급/수요), #669(스펙 157, 현실 통계 대조군·독립 원자재 재고/포지셔닝), #667(스펙 156, 독립 원자재 기간구조). |
| 골격 스펙 | 없음. 다음 신규 전략 우선순위는 결과 전에 별도 SDD로 고정할 `independent_options_variance_risk_premium`(독립 옵션 변동성 위험 프리미엄)이다. |
| 최근 출시 작업 | EIA·French Oil 자료로 에너지 추세·정제마진·시장 폭·확장형 릿지 후보 16개를 검증하고 누적 736개 감사를 생산화했다. 사전 선택 후보는 PSR 0.822115, 50bp 후 연 현금 초과 +1.912757%지만 에너지 단순보유 대비 샤프가 -0.002315라 `NO_FACTORY_EDGE`다. |
| 활성 작업 | 기능 구현 없음. 최신 돈 경로는 `PREVIEW_ONLY`, `ACCUMULATING_EDGE`, 단 0, 자본 0, 실주문 불가다. 깨끗한 `globalfixed` 전진 관측은 0/20이다. #680 서버 배포는 미국 장중 금지 규칙으로 연기됐고 서버 타이머가 2026-08-25 21:00 UTC 이후 재시도한다. |
...
## 최근 완료 — 2026-08-26 KST (#680 에너지 교차시장 전략·736회 판정)
