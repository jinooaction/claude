...
## 한눈 요약표 — 2026-09-08 KST 모바일 자금·전략 운영판 출시

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `7a915c19` — Merge pull request #788 from jinooaction/codex/183-mobile-capital-strategy |
| main 테스트 | main `7a915c19` 전체 3667 passed/9 skipped, 실패 0(875.64초). 제외 9개는 실제 KIS 환경 8개와 가동 전 조건 1개다. |
| main 린트 | main `7a915c19`과 같은 코드에서 `uv run ruff check src tests` 통과. |
| 열린 PR | #788과 Flutter 앱 #3은 모두 병합됐다. 이 인계 갱신 전 기준 두 저장소의 열린 PR은 0개이며 GitHub에서 다시 확인한다. |
...
| 활성 작업 | 스펙182 US8에서 문의 없이 체결·결제 잔고를 직접 GET 대조하는 경로를 검증한다. 문의는 선택 사항이며 개인정보 단계는 개발·조회 검증의 선행 조건이 아니다. 계좌 총액/NAV와 단타 실거래 T010~T012는 미완료다. 앱은 출시됐으며 실제 iPhone 검증은 기기 연결을 기다린다. |
