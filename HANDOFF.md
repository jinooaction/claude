...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `73d07485` — Merge pull request #726 from jinooaction/codex/177-intraday-paper-challenger |
| main 테스트 | exact 기능 코드 `f4b4f8ba`에서 `uv run pytest` → 3278 passed, 7 skipped, 실패 0. exact-main KIS 스모크 `33571146732`도 6/6 통과했다. |
| main 린트 | exact 기능 코드 `f4b4f8ba`에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | #726은 merge됐다. 이 문서 갱신 시점에는 HANDOFF 전용 후속 PR만 진행한다. |
| 출시 완료 스펙 | 최신 기능: #726(스펙 177, 자본 0의 비용 현실형 장중 페이퍼 연구 엔진), #724(스펙 176, 독립 서버 예약과 정수주 표현 가능 목표 기준의 자금성 계약), #722(스펙 176, 거래소별 5xx 뒤 제한된 시세 탐색과 전 후보 장애 보존), #721(스펙 176, 고정 읽기 전용 배포 감사 명령), #719(스펙 176, 10:17~13:53 최대 12분 간격 19개 예약 기회), #717(스펙 176, 거래일 1회 주문 선점·최초 실행 증거 보존), #716(스펙 176, 시작 시점과 각 중개사 쓰기 직전 XNYS 정규장 실패 폐쇄·부분 실행 증거 보존), #715(스펙 176, 선언한 NAV 10%와 실제 주문 자본 일치), #714(스펙 176, 정확한 main 사전점검 인계), #712(스펙 176, 뉴욕 현지 비정각 실거래 예약), #709(스펙 176, 역할 분리 운영 증거와 NAV 10% 실거래 검증 캐너리), #706(스펙 175, 21가족 교정·PEAD 진단), #701(스펙 174, 회계 기반 횡단면 팩터), #699(스펙 173, 판정 경로 교정·월말월초 전략). |
| 골격 스펙 | 스펙 177 엔진은 완료됐지만 저장소에 756세션의 실제 5분봉 자료가 없어 시장 판정은 `INSUFFICIENT_EVIDENCE`다. 자료 수집은 비용·자격증명·시점보존을 다루는 별도 스펙과 운영자 승인 전에는 시작하지 않는다. 스펙 176의 남은 완료 관문은 실제 KIS 자동 주문·체결·감사·정합 및 다른 예약 출처의 중복 주문 0건 확인이다. |
| 최근 출시 작업 | 현재 라이브 전략과 분리된 장중 연구 명령을 출시했다. SPY·QQQ·IWM·TLT·GLD의 15·30·60분, 3가족 18후보를 기준·스트레스 비용과 미래정보 차단 뒤 평가하지만, 합격해도 자본 0·실주문 0·라이브 적격 거짓이다. 현재 저장소에는 요구 실제 자료가 없으므로 장중 엣지가 확인됐다고 말할 수 없다. |
| 활성 작업 | 라이브 경로는 기존 `globalfixed-ensemble-3-6-9-12`, NAV 10%, 단 1을 그대로 유지한다. 첫 유효 GitHub 또는 서버 예약에서 실제 주문 접수·체결·전략 추가 전용 감사·같은 실행의 계좌 정합과 다른 출처의 중복 중개사 쓰기 0건을 확인해야 한다. 장중 연구 쪽은 승인된 실제 5분봉 자료가 들어오기 전 자동 실행·자본 승격이 없다. |
