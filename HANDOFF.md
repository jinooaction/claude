...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `aa7b333` — Merge pull request #711 from jinooaction/Codex/176-autoarm-dispatch-followup |
| main 테스트 | 스펙 176 예약 신뢰성 브랜치에서 `uv run pytest` → 3209 passed, 7 skipped, 실패 0. 6개는 `KIS_LIVE_TEST=1` 전용이며 생산 KIS 스모크에서 6/6 통과했다. 나머지 1개는 사다리 가동 전 상태 전용 시험이라 현재 단 1에서 건너뛴다. |
| main 린트 | 스펙 176 예약 신뢰성 브랜치에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. 정각 예약 지연 보정과 실제 정규장 체결 검증은 `Codex/176-schedule-reliability`에서 진행 중이다. |
| 출시 완료 스펙 | 최신 기능: #709(스펙 176, 역할 분리 운영 증거와 NAV 10% 실거래 검증 캐너리), #706(스펙 175, 21가족 교정·PEAD 진단), #701(스펙 174, 회계 기반 횡단면 팩터), #699(스펙 173, 판정 경로 교정·월말월초 전략). |
| 골격 스펙 | 없음. 스펙 176 코드와 단 1 센티넬은 출시됐고, 남은 완료 관문은 정각 예약 지연 보정과 2026-08-31 뉴욕 10:17 정규장 주문·체결·감사·정합 확인이다. |
| 최근 출시 작업 | 정확 배포 전략 `globalfixed-ensemble-3-6-9-12`를 알파 확정이 아닌 운영 검증으로 분류하고, 실계좌 NAV `$1427.63`의 10%를 내림한 `$142`만 단 1에 무장했다. 예상 주문은 IAUM 1주와 SCHX 2주이며 ORANY 28주는 비관리 보유로 자동 매도하지 않는다. |
| 활성 작업 | #711 배포 `33353769899`, KIS 스모크 `33353769849`, exact-main 수익 증거 `33353769909`이 성공했다. 무주문 사전점검 `33353865976`도 첫 진입 재검증·권한·체결 동기화·사후 측정·계좌 대사에 모두 성공하고 주문 0건을 기록했다. 최근 예약이 42~80분, 두 차례 약 9시간 지연된 정각 혼잡 오류를 뉴욕 10:17 비정각 예약으로 보정 중이다. |
...
## 최근 진행 — 2026-08-31 KST (#709~#711 운영 검증 캐너리 무장·실주문 전 오류 폐쇄, 실제 체결 대기)
...
- **현재 작업**: 최근 schedule 실행은 15:00 UTC 정각 예약인데도 42~80분, 두 차례는 약 9시간
  늦었다. 정규장 밖 실행으로 실제 주문이 차단될 수 있으므로 라이브 캐너리만
  `America/New_York` 10:17 단일 예약으로 옮기고 money-path 표시도 같은 시각으로 맞춘다.
- **완료 조건**: 예약 신뢰성 보정 PR과 배포를 통과한 뒤 뉴욕 10:17 예약 실행에서 실제 KIS
