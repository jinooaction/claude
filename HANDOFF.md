...
## 최근 마일스톤 — 2026-05-30 (🟢 실거래 전환: 라이브 캐너리 무장 + 헌법 X.4 개정)
...
| 마지막 main 커밋 | `c286310 Merge pull request #108 from jinooaction/claude/serene-knuth-xDbnN` |
| 활성 작업 | **🟢 라이브 캐너리 무장됨 (AUTO_INVEST_MODE=live, 2026-05-30).** 운영자 지시로 dry-run → 라이브 캐너리 자율 전환 완료(헌법 X.4 v4.0.0 가드형 채널). 워커 live 모드 가동(자본 $100, 캐너리 룰셋 SPY·MSFT·AAPL, K1 캡 작동). per-trade 5% 캡 + 소액이라 블루칩 1주 값이 캡 초과 → 실질 체결 ~0(실거래 검증됨·노출 극소). 첫 주문 기회는 다음 미국 정규장. **다음 후보: (a) 캐너리 자본 상향(체결 나오게 — 운영자 결정·돈 증가), (b) 풀라이브 승격(헌법 VI 3단계·운영자 전용), (c) 베타 인식 노출 조절, (d) 합성 가중치 워크포워드 검증** |
...
| main 테스트 | 1215 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
