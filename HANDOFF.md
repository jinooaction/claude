...
## ⭐ 다음 세션 최우선 (2026-06-15 세션 인계 — 반드시 먼저 읽을 것)

**결정적 발견 (가속기가 실데이터에서 드러냄)**: 백테스트 앵커드 워크플로(#302)가 인스턴스에서
실행돼 라이브 지정 전략 GLOBAL-TREND 를 깊은 표본외(OOS)로 돌린 결과 —
**748 OOS 관측(2022~2026)에서 단순 보유(SPY·IEF·GLD 보유)를 못 이김: 3구간 중 0구간 승,
평균 샤프 단순 보유 이하, "라이브 배포 정당화 안 됨".** forward 관측도 진짜 1개뿐.
→ 사이드카 `automation/forward-anchored-verdict-last-run` 의 LAST_RUN.md 에 전체 JSON 있음.

**의미**: 진짜 "돈 버는" 병목은 운영자가 지적한 "4주 대기"가 아니라 **현재 지정 전략이 최근
데이터에서 강건한 엣지가 없다**는 것. 가속기가 4주 안 기다리고 이걸 지금 드러냈다(돈 안 잃게
막는, 시스템 의도대로). 단, 2022~2026은 강세장 편향 창이라 추세추종(방어적 현금화)이 단순
보유에 불리한 점은 감안 — 그래도 "강건한 엣지 부재"는 진지하게 받아들일 신호.
...
| 마지막 main 커밋 | `638d22f Merge pull request #305 — feat(035) ladder-decide 앵커드 판정 선택 결합` |
...
| main 테스트 | 2013 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
