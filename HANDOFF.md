...
## 최근 마일스톤 — 2026-05-27 (스펙 016 슬라이스 1: 백테스트 거래비용·슬리피지 모델)
...
* **main의 테스트**: 942 통과, 4 스킵 (라이브 KIS 스모크 4건은 `KIS_LIVE_TEST=1` 환경변수로 게이트).
...
- `HANDOFF-022-SPEC-016-BACKTEST-COSTS.md` — **최신**. 스펙 016 슬라이스 1 백테스트 거래비용·슬리피지 모델 (2026-05-27, PR #77 `f8552c6`). 무비용·무슬리피지였던 백테스트(헌법 VI가 경고한 거짓 잣대)에 거래비용 오버레이를 입힘 — 슬리피지(체결가 악화)+수수료(현금 차감), KIS 현실값 기본. 새 모듈 `backtest/costs.py`. 오프라인·읽기 전용·Kernel 터치 0건. **세계 최고 수준 로드맵의 토대**: 정직한 잣대 위에서만 신호·사이징 개선이 의미를 가짐. 후속 = 슬라이스 2(단일 잣대 통일)·3(워크포워드 검증). 다음 세션 참고.
...
| 마지막 main 커밋 | `f8552c6 Merge PR #77 — feat(016): 백테스트 거래비용·슬리피지 모델` |
| 활성 작업 | **없음 — 스펙 016 슬라이스 1(백테스트 거래비용·슬리피지 모델) 완료·머지(2026-05-27, PR #77 `f8552c6`).** 라이브 worker dry-run 가동 중. 다음 후보(세계 최고 수준 로드맵): **스펙 016 슬라이스 2 — 단일 잣대 통일**(백테스트가 라이브 엔진과 같은 승률·손익비·Sortino 계산, 헌법 X.2 완전 충족), **슬라이스 3 — 워크포워드(표본 외) 검증**(과적합 탐지), 그 뒤 신호/알파 과학(다요인·레짐 인식)·포지션 사이징(변동성·상관). 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015, 016(슬라이스 1: 백테스트 거래비용) |
| 진행 중 스펙 | 없음 (스펙 016은 슬라이스 1 출시; 슬라이스 2·3은 후속 후보) |
| 골격 스펙 (즉시 착수 가능) | **스펙 016 슬라이스 2(단일 잣대 통일)** — `specs/016-backtest-cost-model/spec.md`의 "후속 슬라이스" 절 참조. 백테스트 정직성을 마저 채우는 가장 가치 높은 다음 작업 |
...
| main 테스트 | 1040 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
