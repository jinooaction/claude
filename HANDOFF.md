...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `8e0d4ffe` — Merge pull request #732 from jinooaction/codex/176-server-scheduler-deploy-equivalence |
| main 테스트 | exact 기능 코드 `7b345a24`에서 `uv run pytest` → 3291 passed, 7 skipped, 실패 0. |
| main 린트 | exact 기능 코드 `7b345a24`에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | #733 `feat(179): add one-shot owner emergency live deploy` — 헌법 15.0.0과 단회 오너 장중 배포·주문 상호배제 경로. 로컬 전체 검증 완료, 원격 품질 관문·머지·생산 검증 진행 중. |
...
| 활성 작업 | 스펙 176의 실제 KIS 자동 주문·체결 증거 확보가 남아 있다. 오너 지시에 따라 헌법 15.0.0과 스펙 179의 정확한 current-main 단회 장중 긴급 배포·주문 상호배제 경로를 격리 브랜치에서 구현 중이다. 라이브 경로는 기존 `globalfixed-ensemble-3-6-9-12`, NAV 10%, 단 1을 그대로 유지한다. |
| 안전 경계 | main은 헌법 14.0.0이고 작업 브랜치는 오너 승인 헌법 15.0.0이다. 일반 장중 배포 금지는 유지하고, 저장소 오너·정확한 main·15분 이내·단회 요청만 주문 잠금과 KIS 미체결 0건 확인 뒤 허용한다. 수동 주문, 자본 증액, 전략 승격, 허용목록·위험 관문 우회는 허용하지 않는다. |

## 현재 진행 — 2026-09-03 KST (스펙 179 오너 단회 장중 긴급 배포)

운영자가 자동 실거래 가동성 보정을 장 마감까지 미루지 말고 즉시 배포하라고 명시 승인했다.
헌법 전용 커밋 `b94e37cf`는 일반 장중 배포 금지를 유지하면서 저장소 오너·정확한 현재
`main`·최대 15분·단회 요청에만 예외를 허용한다. 구현 커밋 `133b9cef`는 승인 감사를
먼저 남기고 루트 유지보수 잠금과 중개사 쓰기 배타 잠금을 잡은 뒤, 이전 scheduler
timer·service·worker를 중지하고 KIS 미체결 주문 0건을 확인한 다음 기존 배포 상태기계를
실행한다. 두 커밋 모두 `this changes the safety perimeter`를 기록한다.
