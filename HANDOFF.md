...
## 최근 마일스톤 — 2026-05-29 (스펙 017 슬라이스 3: 상관 헤어컷)
...
* **스펙 017 (변동성 기반 포지션 사이징)** — **슬라이스 1·2·2b·3 출시 완료** (2026-05-28~29). 측정 토대 위에 리스크 사이징(변동성·역변동성·상관)을 한 바퀴 완성. 슬라이스 1(PR #83 `c291d75`): 실현 변동성이 타깃 초과 시 기준 수량을 줄이는 **변동성 throttle**(하향 전용). 슬라이스 2(PR #85 `ab4a140`): 잔잔한 구간에서 기준 수량 위로 **확대**하는 양방향 타깃팅(룰의 `max_scale`, K1이 진짜 천장). 슬라이스 2b(PR #87 `b8fb7e9`): 여러 종목을 한 바구니(`sizing_group`)로 묶어 변동성 높은 종목을 줄여 리스크 기여도를 균형화하는 **역변동성 그룹 리스크 패리티**(`mode="inverse_vol"`). 슬라이스 3(PR #89 `33d3926`): 그룹 멤버 간 양의 상관에 비례해 집중 베팅을 추가로 줄이는 **상관 헤어컷**(옵트인 `correlation_haircut`, 하향 전용). 새 비커널 모듈 `strategy/sizing.py`(`build_sizing_groups`·`inverse_vol_group_scale`·`pearson_correlation`·`average_correlations`·`correlation_haircut`·`group_scale_for`). 백테스트 `replay`와 라이브 `OrderRouter` 양쪽이 K1 게이트 **전에** 같은 함수로 가중치 계산(상관은 공통 거래일 날짜 키로 정렬) → 단일 잣대(헌법 X.2). **안전 경계: 전부 하향 전용 또는 K1이 천장이라 노출 증가 불가. 그룹/사이징/헤어컷 전부 옵트인이라 미설정 룰은 byte 동일(회귀 무손상). Kernel 터치 0건(전부 `strategy/sizing.py`·`config/rules.py`·`backtest/replay.py`·`execution/order_router.py`·`worker/loop.py` 비커널 — 커널인 `worker/schedule.py` 아님). 감사 K4 무변경. 결정론적·LLM 미사용. dry-run 그대로.** 테스트 슬라이스 1 신규 18 + 2 신규 9 + 2b 신규 8 + 3 신규 7건. 자세히는 `HANDOFF-025`·`HANDOFF-026`·`HANDOFF-027`·`HANDOFF-028`.
...
| 마지막 main 커밋 | `33d3926 Merge PR #89 — feat(017): 상관 헤어컷 (슬라이스 3)` |
| 활성 작업 | **없음 — 스펙 017 슬라이스 3(상관 헤어컷) 완료·머지(2026-05-29, PR #89 `33d3926`).** 라이브 worker dry-run 가동 중. 포지션 변동성 타깃팅(1·2)·그룹 역변동성 패리티(2b)에 이어, 그룹 멤버 간 양의 상관에 비례해 집중 베팅을 추가로 줄이는 상관 헤어컷(하향 전용, 옵트인)을 더했다. 이로써 사이징 토대(변동성·역변동성·상관)가 한 바퀴 완성됐다. 다음 후보: 신호/알파 과학(다요인·레짐 인식 — 이제 리스크 사이징 위에서 안전하게), 완전 공분산 ERC/상관 하드 합산 캡, 양방향 그룹 budget-split, 또는 사이징 결정 감사 기록(K4). 전부 워크포워드로 표본 외 검증 받으며 진행. 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015, 016(슬라이스 1·2·3 전부), 017(슬라이스 1 변동성 throttle 하향 + 2 양방향 타깃팅 상향 + 2b 역변동성 그룹 리스크 패리티 + 3 상관 헤어컷) |
| 진행 중 스펙 | 스펙 017 — 슬라이스 1·2·2b·3 출시. 후속(미착수): 완전 공분산 ERC/상관 하드 합산 캡, 양방향 그룹 budget-split, 사이징 감사 기록(K4) |
| 골격 스펙 (즉시 착수 가능) | **신호/알파 과학 또는 스펙 017 후속** — 리스크 사이징(변동성·역변동성·상관)이 한 바퀴 깔렸으니, 다음은 신호 자체 개선(다요인·레짐 인식)을 리스크 사이징 위에서 진행하거나 스펙 017 후속(ERC/budget-split/K4 감사). 전부 워크포워드 검증 필수 |
...
| main 테스트 | 1110 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
