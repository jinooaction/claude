...
## 한눈 요약표 — 2026-08-28 KST 최신 코드 main 기준

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `32807f4` — Merge pull request #695 from jinooaction/codex/171-observer-cleanup-truth |
| main 테스트 | #695 머지 직전 `uv run pytest` → 3132 passed, 6 skipped. 건너뛴 6개는 KIS 실연결 전용이다. |
| main 린트 | #695 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 2026-08-28 사실 확인 시 열린 기능 PR은 없다. 스펙 171의 전략·운영 보정·생산 확인은 완료됐고 문서 전용 마감만 진행한다. |
| 출시 완료 스펙 | 최신 기능: #692~#695(스펙 171, 독립 저회전 레짐 후보 검증·KIS 토큰 재사용·배포 시차·0자본 증거 수명주기 보정), #689·#690(스펙 170, 소액 계좌 실행 ETF 동등성·주문 구현성·실패 증거 거짓 승인 차단), #687(스펙 167, 실제 17개 전략군과 검정력 보정 연구 진입), #686(스펙 166, 소비자 독립 v3 증거와 정확한 소액 자금 구현성). |
| 골격 스펙 | 없음. 스펙 171의 코드·생산 확인은 완료됐다. 다음 신규 연구는 기존 추세 전략에 작은 조건을 더하는 방식이 아니라 수익 발생 시점이 여러 구간에 분산되는 독립 원천만 사전등록한다. |
| 최근 출시 작업 | 16개 레짐 후보는 개선이 3개월에 집중돼 `NO_RESEARCH_EDGE`다. 생산 동등성 세 쌍, 실행 준비, 144달러 주문 구현 가능성은 모두 통과했고 관찰 오류 세 종류를 고쳤다. |
| 활성 작업 | 최종 자동 판정 `33093842870`은 오류 0건, `WAIT_EDGE`, 단 0→0, 자본 이동·실주문 0건이다. 실행 준비는 참이지만 앵커드 `NO_EDGE`, 탐색·연구 캐너리 미확정이라 자본을 열지 않았다. |
