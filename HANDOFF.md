...
## 최근 마일스톤 — 2026-05-29 (스펙 020: 레짐 배율 + ERC 가중치 거래 루프 실배선)
...
- `HANDOFF-029-SPEC-020-REGIME-ERC-WIRING.md` — 스펙 020 레짐 배율 + ERC 가중치 거래 루프 실배선 (2026-05-29, PR #95 `cb5dcae`). `order_router`·`replay` 양 경로에 레짐 배율·ERC 가중치 실제 적용. Kernel 터치 0건. 신규 테스트 5건. 스펙 019 유틸리티를 실거래 루프에 연결 완료. **다음 세션 참고.**
...
| 마지막 main 커밋 | `cb5dcae Merge pull request #95 from jinooaction/claude/spec-020-regime-erc-wiring` |
| 활성 작업 | **없음 — 스펙 020(레짐 배율·ERC 가중치 거래 루프 실배선) 완료·머지(2026-05-29, PR #95 `cb5dcae`).** 라이브 worker dry-run 가동 중. `order_router`·`replay` 양 경로에 레짐 배율·ERC 가중치가 K1 캡 전에 실제 적용됨. 다음 후보: ERC 모드 YAML 룰 예제, 교차 단면 모멘텀 랭킹, 양방향 그룹 budget-split. 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015, 016(슬라이스 1·2·3 전부), 017(슬라이스 1·2·2b·3 전부), 018(슬라이스 1 다요인 신호 + 슬라이스 2 사이징 감사 기록), 019(레짐 인식 + 공분산 ERC), 020(레짐·ERC 거래 루프 실배선) |
| 진행 중 스펙 | 없음. 스펙 020 출시 완료. 후속(미착수): ERC 모드 YAML 예제, 교차 단면 모멘텀 랭킹, 양방향 그룹 budget-split |
...
| main 테스트 | 1166 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
