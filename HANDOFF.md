...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `e5aa0ef` — Merge pull request #717 from jinooaction/Codex/176-schedule-redundancy |
| main 테스트 | `Codex/176-schedule-redundancy-handoff`에서 현재 main을 포함해 `uv run pytest` → 3228 passed, 7 skipped, 실패 0. 6개는 `KIS_LIVE_TEST=1` 전용이며 생산 KIS 스모크에서 6/6 통과했다. 나머지 1개는 사다리 가동 전 상태 전용 시험이라 현재 단 1에서 건너뛴다. |
| main 린트 | `Codex/176-schedule-redundancy-handoff`에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. |
| 출시 완료 스펙 | 최신 기능: #717(스펙 176, 같은 장중 네 번의 예약 기회·거래일 1회 주문 선점·최초 실행 증거 보존), #716(스펙 176, 시작 시점과 각 중개사 쓰기 직전 XNYS 정규장 실패 폐쇄·부분 실행 증거 보존), #715(스펙 176, 선언한 NAV 10%와 실제 주문 자본 일치), #714(스펙 176, 정확한 main 사전점검 인계), #712(스펙 176, 뉴욕 현지 10:17 비정각 실거래 예약), #709(스펙 176, 역할 분리 운영 증거와 NAV 10% 실거래 검증 캐너리), #706(스펙 175, 21가족 교정·PEAD 진단), #701(스펙 174, 회계 기반 횡단면 팩터), #699(스펙 173, 판정 경로 교정·월말월초 전략). |
| 골격 스펙 | 없음. 스펙 176 코드·단 1 센티넬·정규장 주문 방어·거래일 1회 선점은 main에 출시됐다. 남은 완료 관문은 장 마감 뒤 정확한 main 배포와 다음 거래일 실제 KIS 주문·체결·감사·정합 확인이다. |
...
| 활성 작업 | #717 merge `e5aa0ef`로 뉴욕 10:17·11:17·12:17·13:17 다중 기회와 거래일 1회 주문권이 main에 들어갔다. 배포 run `33408124750`은 장중 배포 금지에 따라 성공적으로 연기됐고 서버는 아직 안전한 기존 exact main `3d2592b`다. main money-path는 오늘 다음 후보를 16:17 UTC로 표시하지만 배포 전이면 서버 코드 일치 게이트가 주문 전에 차단하는 것이 정상이다. 서버 타이머의 다음 허용 배포는 2026-08-31 20:00 UTC 이후이며, 배포와 다음 거래일 최초 실행·후속 중복 차단·실제 체결을 계속 추적한다. 실제 전략 주문·체결은 아직 0건이다. |
...
## 최근 진행 — 2026-09-01 KST (#709~#716 운영 검증 캐너리 무장·예약·보고·자본·정규장 계약 보정, 실제 체결 대기)
