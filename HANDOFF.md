...
## 최근 마일스톤 — 2026-05-27 (스펙 001 T050/T052: 장 마감 정합성 자동 실행)
...
- `HANDOFF-021-RECONCILE-AT-CLOSE.md` — **최신**. 스펙 001 T050/T052 장 마감 정합성 자동 실행 (2026-05-27, PR #75 `4319535`). 구현·테스트는 됐으나 자동 호출 배선이 빠져 테스트만 호출하던 정합성 검증을, 워커 장 마감 전이마다 자동 대조 + `auto-invest reconcile` 수동 명령으로 연결. 라이브 전용·인-틱·오류 격리·Kernel 터치 0건. 다음 세션 참고.
...
| 마지막 main 커밋 | `4319535 Merge PR #75 — feat(001): 장 마감 정합성 자동 실행` |
| 활성 작업 | **없음 — 스펙 001 T050/T052(장 마감 정합성 자동 실행) 완료·머지(2026-05-27, PR #75 `4319535`).** 라이브 worker dry-run 가동 중. 다음 후보: 체결 가격 정밀도(누적 한계가), 미체결 명시 조회(`inquire-nccs`), 헬스 연동(T014 보류), 모델 교체 노브. 실거래 전환은 운영자 명시 지시 필요(이제 라이브 측정·체결·정합성 안전 토대 작동) |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015 |
| 진행 중 스펙 | 없음 |
...
| main 테스트 | 1031 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
