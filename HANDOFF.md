...
### 다음 세션 최우선 (이번 발견의 직접 귀결 — "진짜 돈"으로 가는 길)

1. **성장 최적 레버리지를 라이브 전략에 정량화** — 라이브 역변동성 3자산의 5.3% 낙폭 + 샤프
   1.81 에 운영자 낙폭 예산(20%)을 대면 안전 레버리지 배수가 나온다(단순 보유 20.7% 대비 약
   4배 여유). 스펙 044 엔진 재사용. 자본 사다리가 그 배수를 단계적으로 적용. **이게 같은
   위험에서 복리 천장을 올리는 핵심.** 단 레버리지·자본·라이브 사이징은 운영자 게이트(헌법 X.4).
2. (선택) 이 깊은 검사를 사이드카로 상시화 — `regime-stratify.yml` 패턴으로 주기 발행해 라이브
   전략 레짐 강건성 지속 감시(공개 데이터라 인스턴스 비용 0).
3. 비상관 차원 추가 연구(추세추종 외)는 여전히 유효하나 **긴급하지 않다** — 현재 전략이 정당화됨.
...
| 마지막 main 커밋 | `4a7b78c Merge pull request #307 — feat(047) 깊은 OOS walk-forward 엣지 재검증` |
...
| main 테스트 | 2038 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
