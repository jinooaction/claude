...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `194b7ed` — Merge pull request #676 from jinooaction/Codex/162-capital-rung-reporting-parity |
| main 테스트 | #676 기능 머지 전 `uv run pytest -q` → 3050 passed, 5 skipped. 건너뛴 5개는 KIS 실연결 환경 전용이며 KIS 생산 검사는 별도로 5/5 통과했다. |
| main 린트 | #676 기능 머지 전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. 기능 구현과 생산 재검증이 완료됐고 이 인계 갱신만 진행 중이다. |
| 출시 완료 스펙 | 최신 기능: #675(스펙 161, 16/16 연구 캐너리 증거 계약 정합), #674(스펙 160, 무레버리지 전진 장부 무결성), #673(스펙 159, 쌍체결 활성수익 PSR·전략 사용성), #671(스펙 158, 전체 다섯 관문 현실 감사·독립 EIA 공급/수요 16개·704개 고유 감사 장부), #669(스펙 157, 현실 통계 대조군·독립 원자재 재고/포지셔닝), #667(스펙 156, 독립 원자재 기간구조), #665(스펙 155, 독립 외환 금리차·관문 검출력), #663(스펙 154, 회사채 스프레드). |
| 골격 스펙 | 없음. 현재 우선순위는 새 전략 탐색보다 이미 출시된 검증·장부·운영 보고 계약의 생산 정합성을 닫는 것이다. |
| 최근 출시 작업 | 16/16 전략 공장 계약 오류와 무레버리지 전진 장부를 고친 뒤, 자본 사다리 계산 10%를 25%로 잘못 표시하던 운영 보고도 공통 사다리 원천으로 교정했다. 현재 EIA 공급·수요 후보는 PSR 0.705637로 `NO_FACTORY_EDGE`다. |
| 활성 작업 | 기능 구현 없음. 최신 돈 경로는 `PREVIEW_ONLY`, `ACCUMULATING_EDGE`, 단 0/5, 자본 0, 실주문 불가다. 깨끗한 `globalfixed` 전진 관측은 0개이며 자동 누적을 시작했다. |
...
## 최근 완료 — 2026-08-25 KST (#673~#675 판정·장부·연구 캐너리 계약 교정)
