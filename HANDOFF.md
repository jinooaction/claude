...
2026-09-21 후속194 설계 중: specs/194-sparse-opening-research/tasks.md에서 이어받는다.
193 관측 입력을30후보 파일의 시초 거래량 돌파 연구에 연결한다.
기존177의 일별 보유 초기화/현금 미차감 실행기는 재사용하지 않는다.
기간 전체 현금·예약·결제·미청산 장부를 새로 구현한다. 성과는 아직 평가하지 않았다.
T001~T005완료, 계약/원본목록은cc56bd6으로성과조회전에봉인했다.
신규 입력·신호12시험 및193포함28시험 통과,관련ruff·하네스·인계검사 통과.
T006 현금/잔량 반례와T007 시간순계좌부터진행한다. 전체회귀/실자료성과는아직미실행이다.
...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `510022f` — Merge pull request #846 from jinooaction/codex/193-release-evidence |
| main 테스트 | 코드4873dd9 전체4864 passed/13 skipped(959.91초), 관련23개. 이후 문서만 변경. 마지막 실제KIS는 이전main6580c05의35541770144 12개 통과;193은 브로커 코드 미변경. |
| main 린트 | 코드4873dd9의 `uv run ruff check src tests`와 신규 연구CLI, 하네스14/14·인계 사실 검사 통과. |
| 열린 PR | #827 유료 공급자 초안 보류.193 출시 증거 문서 후속 작업. |
| 출시 완료 스펙 | 184 고정 예산 시작·중지·재시작 프로그램과 관측 구간 체결·정산 연결 출시. 기존182 계좌 조회와183 모바일 운영판 유지. 기존 전체 NAV 인증과 실제 투자 가동은 별도이며 이번에 수행하지 않았다. |
...
| 최근 출시 작업 |193 누락 보존 입력 PR845, 배포35554975946·감사35555021310 성공. target83c6d20 완료 확인. 기존 수집기·주문 권한 유지. |
| 활성 작업 |193 완료 후 당시30종목 후보 원본 수집·달력검사·공개일 확인 완료. 가격 연결 조사와 새 연구 계약이 남음. 기존29전략 미통과와 추가 거래량 결합 가설 실패를 보존해 향후 최소30가설을 계상한다. 최종확인495일 성과 미개봉. 통과전략·전진관찰·실행 동등성·실제 운영 검증이 남음. |
