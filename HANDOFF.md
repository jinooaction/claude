...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `0365b3b` — Merge pull request #693 from jinooaction/codex/171-parity-helper-deploy-skew |
| main 테스트 | #693 머지 직전 `uv run pytest` → 3131 passed, 6 skipped. 건너뛴 6개는 KIS 실연결 전용이다. |
| main 린트 | #693 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 단 0 자본값 `0`에서 불필요한 라이브 성과 조회가 오류 문구를 남기는 후속 보정을 검증 중이다. 판정·자본·주문 동작은 바꾸지 않는다. |
...
| 골격 스펙 | 없음. 스펙 171 연구와 생산 동등성 복구는 완료됐고, 단 0의 불필요한 읽기 전용 성과 조회만 후속 보정 중이다. |
| 최근 출시 작업 | 16개 레짐 후보는 개선이 3개월에 집중돼 `NO_RESEARCH_EDGE`다. #693 뒤 생산 동등성 세 쌍은 모두 통과했고 실행 준비와 144달러 주문 구현 가능성도 확인됐다. |
| 활성 작업 | 자동 판정 `33090550150`은 `WAIT_EDGE`, 단 0→0, 자본·주문 0건이다. 실행 준비는 참이지만 최근 앵커드 판정 `NO_EDGE`와 연구 캐너리 미확정이 남았다. 자본 0에서 불필요한 성과 조회가 남긴 오류 문구만 제거 중이다. |
...
- **돈 경로**: 자동 판정 `33090550150`은 계좌 NAV `$1447.51`, IAUM 1주와 SCHX
  ...
  단 0, 자본·주문 0건이다.
- **남은 작은 오류**: 단 0의 자본 문자열 `0`을 비어 있지 않다고 보고 라이브 전략 성과를
  조회해 `capital must be positive`를 남겼다. 판정에는 쓰이지 않았지만 양수 자본에서만
  호출하도록 후속 보정 중이다.
