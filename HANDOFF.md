...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `4f59ef7` — Merge pull request #678 from jinooaction/Codex/162-usda-crop-supply-demand |
| main 테스트 | #678 머지 직전 `uv run pytest -q` → 3060 passed, 5 skipped. 건너뛴 5개는 KIS 실연결 환경 전용이며 merge 커밋 생산 스모크에서 5/5 통과했다. |
| main 린트 | #678 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. 기능 구현과 생산 재검증이 완료됐고 이 인계 갱신만 진행 중이다. |
| 출시 완료 스펙 | 최신 기능: #678(스펙 162, USDA 작물 수급 16개·720개 고유 감사·기준 검출력), #675(스펙 161, 16/16 연구 캐너리 증거 계약 정합), #674(스펙 160, 무레버리지 전진 장부 무결성), #673(스펙 159, 쌍체결 활성수익 PSR·전략 사용성), #671(스펙 158, 전체 다섯 관문 현실 감사·독립 EIA 공급/수요), #669(스펙 157, 현실 통계 대조군·독립 원자재 재고/포지셔닝), #667(스펙 156, 독립 원자재 기간구조), #665(스펙 155, 독립 외환 금리차·관문 검출력). |
| 골격 스펙 | 없음. 다음 신규 전략 우선순위는 결과를 보기 전에 별도 SDD로 고정할 `independent_energy_cross_market`(독립 에너지 교차시장)이다. |
| 최근 출시 작업 | USDA 공식 시점별 발표 192개로 작물 수급 후보 16개를 검증하고 누적 720개 감사를 생산화했다. 사전 선택 후보는 PSR 0.204521, 50bp 후 연 -1.710304%로 `NO_FACTORY_EDGE`다. 기준은 양성 대조군 통과·무효 대조군 탈락으로 정상이다. |
| 활성 작업 | 기능 구현 없음. 최신 돈 경로는 `PREVIEW_ONLY`, `ACCUMULATING_EDGE`, 단 0, 자본 0, 실주문 불가다. 깨끗한 `globalfixed` 전진 관측은 0/20이다. |
...
## 최근 완료 — 2026-08-25 KST (#678 USDA 작물 수급 전략·720회 판정)
