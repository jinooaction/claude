...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `7c28b127` — Merge pull request #724 from jinooaction/Codex/176-independent-live-scheduler |
| main 테스트 | PR #724의 exact 코드 트리에서 `uv run pytest` → 3251 passed, 7 skipped, 실패 0. exact-main 생산 KIS 스모크 `33552911287`도 6/6 통과했다. |
| main 린트 | exact main `7c28b127`에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. #724는 장 마감 뒤 merge됐고, 이 문서 갱신 전 원격 열린 PR 목록은 0건이었다. |
| 출시 완료 스펙 | 최신 기능: #724(스펙 176, 독립 서버 예약과 정수주 표현 가능 목표 기준의 자금성 계약), #722(스펙 176, 거래소별 5xx 뒤 제한된 시세 탐색과 전 후보 장애 보존), #721(스펙 176, 고정 읽기 전용 배포 감사 명령), #719(스펙 176, 10:17~13:53 최대 12분 간격 19개 예약 기회), #717(스펙 176, 거래일 1회 주문 선점·최초 실행 증거 보존), #716(스펙 176, 시작 시점과 각 중개사 쓰기 직전 XNYS 정규장 실패 폐쇄·부분 실행 증거 보존), #715(스펙 176, 선언한 NAV 10%와 실제 주문 자본 일치), #714(스펙 176, 정확한 main 사전점검 인계), #712(스펙 176, 뉴욕 현지 비정각 실거래 예약), #709(스펙 176, 역할 분리 운영 증거와 NAV 10% 실거래 검증 캐너리), #706(스펙 175, 21가족 교정·PEAD 진단), #701(스펙 174, 회계 기반 횡단면 팩터), #699(스펙 173, 판정 경로 교정·월말월초 전략). |
| 골격 스펙 | 없음. 단 1 센티넬·정규장 주문 방어·거래일 1회 공유 선점·GitHub 12분 예약 복구·독립 서버 예약·읽기 전용 배포 감사·거래소별 5xx 시세 탐색·정수주 자금성 계약은 main과 생산에 출시됐다. 스펙 176의 남은 완료 관문은 실제 KIS 자동 주문·체결·감사·정합 및 다른 예약 출처의 중복 주문 0건 확인이다. |
| 최근 출시 작업 | 정확 배포 전략 `globalfixed-ensemble-3-6-9-12`를 알파 확정이 아닌 운영 검증으로 분류하고, 최근 실계좌 NAV `$1434.07`의 10%를 내림한 `$142`만 단 1에 무장했다. exact-main 주문 없는 사전점검 `33553034421`은 1주 미만인 IAUM을 진단에 유지하면서 SCHX 2주를 약 `$30.09` 지정가에 매수하는 안전한 부분 구현을 `ENTRY_READY`로 판정했다. ORANY 28주는 비관리 보유로 자동 매도하지 않으며 실제 주문은 0건이었다. |
| 활성 작업 | exact main `7c28b127` 배포 run `33552760635`의 2차 실행이 독립 `auto-invest-live-canary.timer`를 검증·활성화했고 다음 발화는 2026-09-02 14:35 UTC다. 첫 유효 GitHub 또는 서버 예약에서 실제 주문 접수·체결·전략 추가 전용 감사·같은 실행의 계좌 정합을 확인하고, 다른 출처가 같은 최초 run/source를 반환하면서 중복 중개사 쓰기 0건인지 확인해야 한다. 이 생산 증거 전에는 완료가 아니다. |
