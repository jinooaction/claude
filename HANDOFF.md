...
## 최근 마일스톤 — 2026-05-24 (스펙 005 자율 튜너 출시)
...
* **main의 테스트**: 887 통과, 4 스킵 (라이브 KIS 스모크 4건은 `KIS_LIVE_TEST=1` 환경변수로 게이트).
...
**다음 후보 (빌드 가능한 스펙 001~011 전부 출시 완료 — 아래는 후속 확장 후보)**:

* **L1 적용 표면 확장** — 스펙 005 튜너가 현재 `config/llm_kpi_thresholds.toml` 임계값만 자동 적용. 모델 라우팅·캐시 TTL 을 튜닝 가능 노브로 만들면(cost/cache/latency drift 후보는 이미 감지 중, 적용 노브만 없음) 자동 적용으로 승격. K3 인접 표면이라 스펙 005가 의도적으로 범위 밖에 둠.
* **튜너 자동 호출** — `auto-invest tune` 을 세션 마감 후 타이머/워커 스케줄에 연결(현재는 수동/cron 1회).
* **L2/L3 → 캐너리 자동 연결** — 튜너의 L2/L3 후보를 스펙 007 캐너리 엔진에 자동 투입하는 큐.
...
- `HANDOFF-016-SPEC-005-AUTONOMOUS-TUNER.md` — **활성**. 스펙 005 자율 튜너 출시 (2026-05-24, PR #60 `0a176fb`). 다음 세션 필독.
...
| 마지막 main 커밋 | `b5a6946 Merge PR #61 — docs(handoff): 스펙 005 자율 튜너 출시 반영` (스펙 005 코드 머지는 `0a176fb` PR #60) |
| 활성 작업 | **없음 — 스펙 005(자율 튜너) 완료·머지(2026-05-24).** 라이브 worker dry-run 가동 중 (2026-05-23 시작). 다음 후보: L1 적용 표면 확장(모델 라우팅·캐시 TTL 노브), 튜너를 세션 마감 타이머에 연결, L2/L3→캐너리 자동 연결. 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001, 002, 003, 004, 005, 006, 007, 008, 009, 010, 011 |
| 진행 중 스펙 | 없음 |
| 골격 스펙 (즉시 착수 가능) | 없음 — 빌드 가능한 스펙 전부 출시. 후속은 L1 적용 표면 확장 등(스펙 005 후속 후보, HANDOFF-016 참조) |
...
| main 테스트 | 887 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
