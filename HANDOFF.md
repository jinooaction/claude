...
2026-09-25 후속196 출시: PR853/main dda232f에 과거 보관 원문·수집 기록 검사기를 병합했다.
관련62개, 전체ruff, 하네스14/14 및 인계 사실 검사를 통과했다. 실제 WMT 응답·metadata·warcinfo
연결과 인용이 통과했고, 올바른 파일 지문을 가진 잘못된 연결 사본은 실제 CLI에서 거부됐다.
전체 pytest는4989 passed/13 skipped(793.89초), 실행 핸들75776 종료 코드0으로 완료됐다.
로그는 `/tmp/auto-invest-196-pytest.log`다. 종료된 검사를 진행 중으로 오인하지 않는다.
배포36084456885·읽기 전용 감사36084571623 성공. 상관키0279dd91abe3af75fda78723d8aa9382의
시작19372/완료19377, target dda232fbe076, DEPLOY_COMPLETED·기존모드live를 확인했다.
현재 명세는 specs/196-archived-event-evidence/tasks.md이며 T001~T011 완료다.
실제 자료·보고서 지문은 release-evidence.md에 있다. 조건부 공급자 구간과 현재 검사 시각을
분리하며 기존195 관측을 소급하지 않는다. WMT 기준점은2014-03-10T00:43:43Z이고 수집 소요
1470ms를 중복 가산하지 않는다. 전략 승인·단타 주문·자금배정 변화 없음. 전체181 T013~T016은 남는다.

2026-09-21 후속195 출시: 예정/실제 실적 사건과 로컬 관측 시각을 분리하는 입력 도구 구현·검증·배포 완료.
코드522ac3d의 관련42개·전체4927 passed/13 skipped(817.90초), ruff·하네스14/14·인계 검사 통과.
전체 검사71555는 종료 코드0이며 /tmp/195-full-pytest.log에 보존했다. PR851/main da11ad8 병합.
배포35562612202·읽기 전용 감사35562644448 성공. 상관키 c3f3a5c7230bc610d956434891748403의
시작19221/완료19226, target da11ad893573·DEPLOY_COMPLETED·기존모드live를 확인했다.
실제 두 원문은 과거2014년 조회0건/현재 관측 이후2건이었다. SEC 역사 접속 로그와
날짜별 공시 원문도 확보했으며 archival-source-pilot.md에 지문·범위·미검증 의미를 기록했다.
specs/195-earnings-event-inputs/tasks.md에서 이어받는다. 현재 사본을 과거에 알려진 정보로
소급하지 않는다. 원문 진위·과거 버전 공증은 아니며 전체 자료 범위와 통과 전략은 미확정이다.
단타 주문·자본·기존 안전 관문 변경 없음. 아래194 전체 검사·배포 기록은 이전 출시 근거다.

2026-09-21 후속194 연구 도구 출시·배포 확인 완료, 후보 탈락: specs/194-sparse-opening-research/tasks.md.
T001~T010 완료. 공유 현금·예약·결제·익일 잔량·별도 장부 검사를 4d22e7b까지 구현했다.
계약/원본 목록은 cc56bd6, 변환 manifest는 f1af197로 성과 조회 전에 봉인했다.
30종목×1515거래일의 17,630,638분봉과 누락 19262분을 보존했다. v3 재생8683은 exit0/verified=true다.
...
T001~T012 완료. PR848/main9df3a25, 배포35559689034·읽기 전용 감사35559747577 성공.
상관키 a8797f66605b70be0d881ceaf39b3f65의 시작19208/완료19213과 target9df3a25f02a6,
DEPLOY_COMPLETED·기존 모드live를 확인했다. 종료된 재생과 전체 검사를 재시작하지 않는다.
...
전체목표/181 T013~T016은 미완료이며 실제주문·자본·승격 변경없음.

2026-09-21 출시: PR845/main83c6d20은 누락 보존 연구 입력과 읽기 전용 감사 CLI를
...
T001~T013완료. 배포35554975946 성공, 읽기 전용 감사35555021310에서
...
관측 누락 처리 도구 완성은 통과 전략이나 전체 실사용 목표 완성이 아니다.

2026-09-21 전체 작업표 정합:181 T012(756거래일 실제 자료 확보)를 완료로 갱신했다.
승인된 HF Data Library/189 오프라인 경로의 원본5개 해시를 다시 확인했고 정식 입력
검사74509 exit0에서 공통1645세션·종목별127734봉·quality_reasons=[]·기존 지문이
일치했다.181 API 수집기에 공급자를 추가한 것이 아니며 KIS 체결 동등성도 미인증이다.
181의 남은 전체 완료 작업은 T013~T016이다. 자세한 원본 근거는181 research.md에 있다.

2026-09-21 후속:192 고정 매수2540건은 미래의 최적30분봉 시가 청산을 가정해도
양비용 합계손실이다. 기준2231건/가혹2373건은 개별 상한도0 이하다.
따라서 청산 규칙만 바꾸는 후속 반복은 제외한다. 새 가설에는 매수 전에 관측할 수 있는
거래 상황 구별 근거가 필요하다. 산술 교차검증과 재현 지문은
specs/192-noise-band-research/exit-ceiling-diagnostic.md에 있다. 이는 미래 가격을
이용한 진단이며 실행 가능한 전략·합격·새 실주문 증거가 아니다.

2026-09-21 최신:192 과거 같은 시각 변동 폭 연구를 PR841/main230383a로 병합했다.
코드dfced27 전체4841 passed/13 skipped(1093.55초), 관련79개·ruff·하네스14/14 통과.
전체검사9308은 exit0으로 종료됐으며 재시작할 작업이 아니다.
실제 입력1645일/평가1631일, 단일후보 양비용2540청산/미청산0이나 순손실로 탈락했다.
개발21316/원본재계산51925 exit0,10160장부행/결과 일치.495일 확인 자료는 미개봉이다.
기존28+신규1=29후보 실패를 보존한다. 고정금액 누적 손익은 자본소진 후도 계속 계산해
실제 현금계좌 수익률로 해석할 수 없다. 동일체결 수수료 제거 후에도 양비용 손실이다.
배포35551371254 성공, 읽기 전용 감사35551419380에서 상관키
bda57833abeee91e9eb7ce9059ece016의 시작19182/완료19187,
DEPLOY_COMPLETED·target230383a5e251·기존모드live를 확인했다.
실제 단타 주문0·자금배정0이며 통과전략·독립확인·전진관찰·실행/운영 검증은 남았다.
자세한 증거는 specs/192-noise-band-research/results.md와 tasks.md다.
이번 연구의 체크 완료를 전체 목표 완료로 해석하지 않는다. 같은 실패 조건 재실행이나
수수료 확인 대기만으로 진전했다고 보고하지 않는다.

2026-09-21 최신: PR839를 mainca8e1c1로 병합했다.191 급락 후 회복 연구와
부분/미체결 이후 잔량 청산 지속 보정이 포함됐다. 코드91a2b55 전체4820 passed/
13 skipped(947.25초), 관련67개·ruff·하네스14/14·인계 사실 검사 통과.
전체 검사7491은 exit0으로 종료됐으며 재시작할 진행 작업이 아니다.
실제1645일4후보 모두 비용 후 손실로 DEVELOPMENT_REJECTED, 선택 없음.
원본 재계산83436도 exit0/valid=true,4208장부행과 결과 일치. 새495일은 미개봉이다.
배포35546033949 성공. 읽기 전용 감사35546215493에서 상관키
ac842685b4b95414efcd5e8b527b0a2f의 시작19169/완료19174와 targetca8e1c1deb12를
확인했다. 기존 서버 모드는 live이며 연구 후보의 승인·자금 배정·주문은 수행하지 않았다.
기존18+190의6+191의4=28후보 실패를 보존한다. 자료는 확보했으나 통과 전략·동일 후보
60세션 전진관찰·실행 동등성·실제 단타 체결 증거가 없어 전체 목표는 미완료다.
현재 근거는 specs/191-shock-recovery-research/results.md와 tasks.md다.
후속 원문 적용 조건 점검은 로컬 자료 폴더의 next-research-source-audit.md에 있다.
공매도·레버리지·상이한 비용을 쓰는 논문 성과를 현 계좌의 합격 증거로 옮기지 않는다.

2026-09-21 연구 경로 출시 확정: PR837 main9b8c9e1 병합·배포35543697028 성공.
감사35543740327도 시작/완료2행·target9b8c9e1089a8·DEPLOY_COMPLETED를 확인했다.
코드8ca0c9c 전체4797 passed/13 skipped, 관련55개·ruff·하네스14/14·인계 검사 통과.
개발 원본 재계산 valid=true, 장부4616행.190 신규6후보는 모두 비용 후 손실로 탈락했고
최종확인495일은 열지 않았다.190 연구 프로그램 완료와 전체 단타 실사용 목표 미완료를
구분한다. 통과 전략·동일 후보60세션 전진 관찰·실행 동등성·실주문 증거는 없다.
다음 작업은 후속 가설 또는 실제 계좌 비용의 근거를 먼저 정하고 별도 사전등록하는 것이다.
기존18+6후보 실패를 덮어쓰거나 확인 기간에서 재선택하지 않는다.

2026-09-21 후속: PR836을 main6580c05로 병합했다. 전체4776 passed/13 skipped,
배포35541701802 성공, 동일main의 실제KIS35541770144는12 passed(111.83초).
현재190 비용 고려 단타 연구는 사전등록b85a074 뒤 코드6809afd로 실제 개발을 실행했다.
6후보 모두 비용 후 손실로 DEVELOPMENT_REJECTED, 선택 없음, 확인495일 미개봉이다.
관련55개/ruff 통과. 코드8ca0c9c 원본 재생 독립검사 valid=true/장부4616행,
confirm 실제 거부와 확인 자료 미접근 확인. PR837 검증 완료: 전체 회귀 세션64634
exit0, 4797 passed/13 skipped(828.14초), `/tmp/cost-aware-full-tests.log`에 보존.
실제 병합/배포 상태는 원격 기록으로 확인한다. 이 문장만으로 배포 완료를 주장하지 않는다.
specs/190-cost-aware-intraday/results.md와 tasks.md에서 이어받는다.
현재 구현 완료/전략 합격/실주문 가동으로 해석하지 않는다.

2026-09-21 최신(PR836 검증 결과): HF Data Library 무료 계정 연결·프로필 등록 후
5종목 Raw 원본 10,975,407행을 확보했다. 보간 없는 5분 집계로 1,645연속 세션을 확보해
18후보 재생 완료. 모의 자산 고갈 보고와 마지막 봉 신규 진입을 5535fdf에서 보정했고,
독립 검사 valid=true/장부480,239행/미청산0이다. 전략 판정은 INSUFFICIENT_EVIDENCE:
선택 후보80거래로200건 미달, 확인구간18후보 비용 후 손실. 관련34개/ruff 통과,
최신 전체 회귀 4,776 passed/13 skipped(1,150.92초), 하네스14/14·인계 사실 검사 통과.
PR836의 실제 병합 상태는 원격 기록으로 확인하며 이 기록만으로 배포 완료를 주장하지 않는다.
specs/189-kis-five-symbol-review/hf-revalidation.md와 로컬 원본/장부가 최신 근거다.
아래 자료 미확보·가입 미승인 기록은 과거 경과다. 실제 주문·자금 배정 없음.
...
정규장1분 누락TLT18/GLD3건, 라이선스·원천 수정 근거 미확인이다. 적격 자료로 미산입.
189/public-source-audit.md와 로컬claude-data-research/20260920-public-minutes에
표본/해시/품질 결과 보존. 가입·비용·주문·자금 배정·운영 코드 변경 없음.
다음은 원천/이용 조건/충돌행 수정 근거 또는 다른 추적 가능한 원본 확보다.

189 경계 재확인 완료: 공식 legacy/rest/get_ovsstk_chart_price.py 첫 줄에서
해외주식 분봉 최대 약1개월 설명을 새로 확인했다. 최신 함수 설명만 읽어 이 근거를
놓쳤다. PR832/main da57dde(실행3a0cb59)에서 빈 응답 경계를1분 전 방식으로
한 번 더 확인하도록 보완했다. 전체4769 passed/13 skipped(808.63초), 린트/하네스 통과.
실제34857636042에서12 passed(114.62초),5종목 각각 재시도1회 모두 EMPTY_PAGE.
최초2026-08-14 유지, 공통20완결일/누락0/부족736일/단기36회/장기 미통과 유지.
기존 보관은 겹치는5일로 재확인. 자료 부족은 코드 실패가 아니며 같은 호출 반복으로
736일을 확보할 수 있다고 말하지 않는다. 모든 한국투자 서비스의 불가능을 입증한 것은 아니다.
원본: data/kis-history-reviews/review-1129a519dd10419eb8bbc4b7234ed7f6.
배포34857634053은 장중 연기, 실제 읽기 검사는 새 코드 임시 작업 공간에서 완료.
신규 가입/비용/주문/자금 배정 없음. 자세한 근거는189/boundary-result.md.

189 실제 완료: 한국투자만 사용해5종목 공통2026-08-14~09-11의20완결 거래일을 확보했다.
예상20/완전20/누락0, 원본19242개, 오늘 미완결 구간 제외. 공식 장기 연구는
INSUFFICIENT_EVIDENCE(756일 중736일 부족), 독립 증거 검사 통과. 별도 단기 진단은
18후보×2비용=36회 REPLAYED이며 승격 근거로 쓰지 않는다. PR830/main3c86569,
실행b55f480, 실제34854081485에서12 passed(109.94초). 개별 후보 손익은 로그 미조회.
원본/연구/단기 장부는 서버 data/kis-history-reviews/review-a87b31f169494a45acf635dd94cf57f1.
신규 계정 연결·결제·실제 주문·자금 배정 없음. 189/live-result.md의 최신 결과를 따른다.
배포34854081522는 장중 거절이며 워커 교체와 별개로 임시 작업 공간의 실제 분석은 완료됐다.

189 후속: 운영자가 가입 없이 한국투자로 계속 진행하도록 지시했다. Alpaca 연결 선택
대기를 해제하고 KIS5종목 과거 수집·공통 완결 구간 연구 연결을 구현했다. 관련25개,
전체4767 passed/13 skipped(816.51초)·린트·하네스 통과. /tmp/claude-189-full-final.log.
오늘 미완결 구간과 범위 밖 원본은 연구에서 제외하되 보관한다. 완전한 공통 자료에는
18후보×2비용 별도 단기 재생과 장부를 보관하며 승격 증거로 사용하지 않는다.
756세션 기준·후보·비용·독립 증거 검사·주문 관문 유지. 실제 결과는 위 최신 기록을 따른다.

188 실제 확인 완료: PR828/main5697931, 실행 코드152828c. 실제34850908418에서
12 passed(38.87초). SPY5분봉34페이지/3908고유 원본, 최초2026-08-14T08:00Z,
최종2026-09-14T13:40Z. 정규장1562봉/21개 날짜(오늘 미완결 구간 포함), EMPTY_PAGE 종료.
80페이지 한도 도달이 아니다.30일 이전 자료를 실제 받았지만756세션은 확보하지 못했다.
서버 원본은 data/kis-history-depth/run-ab675873d6364711af75b1769135ac68에 보관했다.
모든 KIS 조회 방법의 보관 한도를 입증한 것은 아니다. 188/live-result.md 참고.
무료 Alpaca 계정 연결 선택을 요청했다. 한국투자 매매 계좌 유지, 실제 주문·자금 배정 없음.
배포34850908518은 장중 거절이며 워커 교체는 미수행. 임시 작업 공간에서 새 진단은 실행 완료.

188 무료 경로 확인: 기존 KIS 인증으로 SPY 과거 분봉을 최대80페이지 읽는 독립 진단을
구현했다. 원본은 서버 토큰 캐시 부모의 kis-history-depth/run-*에 비공개 추가 저장한다.
빈 응답·커서 진행 중단·설정 한도 도달을 구분하며80페이지를 공급자 보관 한도로 해석하지
않는다. 관련22개, 전체4764 passed/13 skipped(894.35초), 린트·하네스 통과.
/tmp/claude-188-full.log. 실제 서버 결과는 위 최신 기록과 live-result.md를 따른다.
187 유료 공급자 PR827은 보류이며 새 진단만 별도 분리했다. 실제 주문·자금 배정 없음.

2026-09-14 전체 보관 재고 확인: PR825/main5fa2e20, 실행 코드f253182.
실제34790306811에서KIS11개 통과(28.40초), 코드별 보관2묶음(각5일/1일),
중복 제거5일(2026-09-04,08,09,10,11), 모두 실제 KIS 자료다. 각 연구 판정은
INSUFFICIENT_EVIDENCE이며 통과 전략 없음. 이전의1일은 최신 묶음만 본 값이다.
5일 묶음도756일 기준에751일 부족하다. 배포34790306805 성공.
감사34790395627에서 같은 SHA의 DEPLOY_COMPLETED 확인(f74bdc6b79d0b0846e1c6424e845f6e0).
과거 자료 존재를 확인한 것이며 합친 전략 통과나 등록 후 전진 실적은 아니다.
...
실제 서버 재고는 위 최신 결과로 확인했다. /tmp/claude-185-inventory-full.log.
실제 주문·자금 배정·운영 권한 발급은 수행하지 않았다.
...
main8966780의 DEPLOY_COMPLETED 확인(0299cde6974d4c97fa14fe80217d5f1b).
같은 코드 KIS 검사34759932173도 key_valid=true/smoke_state=success.
실제 투자 가동/자금 배정/승인 발급은 하지 않았다. 아래 이전 개발 기록의 미완료
표현보다 이 머지 결과와 completion-audit.md의 현재 고정 예산 판정을 우선한다.

184 FR070 고정 예산 경로 구현: 사용자는 대안 제안 뒤 이어서 진행을 지시했다.
600달러를 NAV/현금으로 위장하지 않는 BudgetObservation, 모든 intraday 지문의
매수·미체결 예약·보고 정산 재계산, 주문 직전 종목별 여력과 쓰기 직전 신선도 검사를
기본 사용자 시작에 연결했다. 전체 보유480달러/종목·주문120달러 제한 유지.
기존 계좌 NAV 모드의 미검증은 보존하며 새 운영 식별자/코드 지문으로 이전 승인
재사용을 막는다. 모의 KIS HTTP→매수→부분 체결→재시작→정리→STOPPED 통과.
전체 회귀4751 passed/12 skipped(847.89초), 종료0. 린트·하네스14/14·인계 통과.
`/tmp/claude-184-fixed-budget-full.log`. 실제 KIS11개와 가동 전 전용1개는 제외했다.
실제 주문/승인 발급/자금 배정/가동 없음. T009/T010의 새 모드 구현·모의 통합은
확인했고 T080의 소프트웨어 검사를 완료했다. 원격 머지/배포 확인은 별도다. 아래 FR069 이전 미완료 기록을 새
고정 예산 경로의 미구현으로 다시 해석하지 않는다. 완료 판정은 completion-audit.md 참고.

## 고정 예산 경로 이전의 진행 기록

184 최신 전체 회귀: ef44227(실행 코드40ab005, FR069 포함)에서4702 passed/12 skipped
(781.94초), 종료0. `/tmp/claude-184-cancel-only-full.log`. 실제 KIS11개와 가동 전
상태 전용1개 제외. 아래 최신 검사 실행 중 기록은 이 결과로 갱신한다.
전체 계좌/거래 후 순현금 제공자와 T009/T010은 미완료다. 현재 원본으로 확정할 수
없는 포함 범위·반영 시점이 남아 있다. 완성도 점검을 현재 코드와 검증으로 갱신했다.
새 근거 없이 같은 조회/검사를 반복하거나 전체 계좌/현금 검증값을 올리는 것은
다음 조치가 아니다. PR793 초안 유지. 실제 주문·취소·자본 배분·활성화 없음.

184 FR068 전체 회귀: 코드 cbd7d1f에서4686 passed/12 skipped(788.88초), 종료0.
`/tmp/claude-184-exact-notional-full.log`. 실제 KIS11개와 가동 전 상태 전용1개 제외.
후속40ab005의 취소 관리 분리는 이 결과에 포함되지 않으며 해당 전체 검사는 후속이다.
아래 FR068 전체 검사 실행 중 기록은 이 완료 결과로 갱신한다.

184 FR069/T077: 관리 호출은 계좌 범위/현금/NAV 미검증 시 이번 성공한 체결 조회로
우리 주문의 신원·시장·원래 수량·체결/잔여 수량을 대조해 중지/만료 취소만 처리한다.
별도 관측·실제 경과30초와 권한을 잠금 뒤 전송 직전에도 확인한다. 신규 주문·청산
매도·STOPPED 판정에는 사용하지 않고 원래 전체 계좌 검증값은 유지한다. 관련93개
(2.66초), 만료 사례 포함 단타1108 passed/4 skipped(48.86초), 린트·하네스14/14·
인계 통과. 정상 전체 계좌/거래 후 순현금/T009/T010은 여전히 미완료다.
새 전체 회귀는 후속이며 아래 cbd7d1f 실행은 FR068까지만 포함한다. 실제 주문 없음.

184 FR068/T076 구현: 부분 체결의 원래 누적 평균가격/수량과 추가 거래대금을 별도
추가 전용 표·감사 기록에 보존했다. 재조회·비용 대조·단타 손익이 이 값을 소비하며
기존 행은 수정하지 않는다. 관련76개(1.37초), 회귀1348 passed/4 skipped(158.80초),
추가7개(0.21초), 린트·하네스14/14·인계 통과. 새 전체 회귀는 후속이며 T009/T010은
미완료다. 아래 FR068 구현 전 기록은 이 결과로 갱신한다. K4 감사 필드 추가가 있다.
이전 c342e63 전체 검사는4675 passed/12 skipped(785.98초), 종료0으로 완료됐다.
`/tmp/claude-184-repoll-full.log`. 새 FR068 전체 검사와 구분한다. 실제 주문/자본 배분 없음.

184 후속 FR068/T076: 부분 체결 단가를 나눈 뒤 다시 곱하면 원래400.04가
400.039999…로 바뀌는 반례를 재현했다. c342e63의 동일 수량 금액 대조에서도 이를
차단할 수 있으므로 정확한 원래 거래대금을 별도 보존하는 후속 연결이 필요하다.
명세/계획만 추가했고 실행 코드는 아직 c342e63이다. 해당 전체 회귀가
`/tmp/claude-184-repoll-full.log`에 실행 중이며 성공 여부는 종료 결과로 확인해야 한다.
FR068 구현 전에는 FR067을 전체 현금 입력 보정 완료로 주장하지 않는다.

184 FR067: 체결량이 같아도 평균가격×수량과 기존 장부 금액을 대조한다. 누락·변경은
경고로 전달하고 해당 주문의 종료를 보류한다. 체결/감사 원본은 수정하지 않으며,
값이 다시 일치하면 새 체결 없이 종료할 수 있다. 실제 SQLite 반복 조회/복구 포함
관련39 passed(1.14초), 단타1090 passed/4 skipped(46.31초), 린트·하네스14/14·인계 통과.
기존 실행 지문에 fill_sync.py가 포함되어 이전 자격을 그대로 재사용할 수 없다.
전체 회귀는 후속이며 아래4665개는 이전 코드 기준이다. T009/T010은 미완료다.

184 통합: mainf69c4b0(PR821 거래내역 원본 수집과 PR822 실제 결과 인계)를 연결했다.
최근3개 등록일0행과11응답·순번13 COMPLETE·주문0을 실제 확인했다. 기존 기간 지정
비용 대조 기능은 보존하고 명시적 기간이 있으면 최근 내역을 중복 조회하지 않는다.
통합 관련 검사는129 passed(14.22초)다. 아래4665개 전체 검사는 이번 통합 이전
03f8500 기준이다. 전체 계좌 입력 연결과 정상 제공자의 가동 검증은 T009/T010에 남아 있다.

184 최신 전체 회귀: 코드03f8500에서4665 passed/12 skipped(784.88초), 종료0.
`/tmp/claude-184-transaction-scope-full.log`. FR066과 main7b288b7 통합까지 포함한다.
실제 KIS11개와 가동 전 상태 전용1개 제외. 아래 이번 전체 회귀 후속 기록은 이
결과로 갱신한다. 실제 거래내역 수집은186 PR821 배포 후 실행34700917080에서
확인했으며 T009/T010은 미완료다.

184 FR066: 공식 CTOS4001R 요청 명세의 공백 거래소 필터를 거래내역 공통 읽기에
적용했다. 기존 NASD 기본값/미국 거래소 제한은 제거하고 미지원 명시 필터는 요청
전에 거절한다. 모든 통화 원본/전체 페이지/통화별 산술과 기존 자격·비용 관문은
유지한다. 관련100개(13.59초), 단타1085 passed/4 skipped(46.05초), 린트·하네스14/14·
인계 통과. 실제 서버 거래내역과 이번 전체 회귀는 후속이다. 이 원본 범위 보정을
전체 현금·거래일 범위·수수료 포함 인증으로 취급하지 않는다. T009/T010 미완료,
주문/자본 배분/활성화 없음. 아래4655개 전체 검사는 이번 변경 이전 결과다.

184 통합: main7b288b7(PR819 자산 분류 대조와 PR820 실제 결과 인계)를 연결했다.
실제34699060401에서 현금 CALCULATED,3종목/자산19분류 MATCH, 주문0 확인.
관련133 passed(6.28초), 린트·하네스14/14·인계 통과. 최초 검사 명령의 잘못된 파일
경로는 실행 전 오류였고 실제 경로로 수정해 검사를 완료했다. 아래4655개는 이번
공개 판정 연결 통합 이전b7e2935의 전체 검사다. 거래명세/비용 기능은 보존했다.

184 최신 전체 회귀: 코드b7e2935에서4655 passed/12 skipped(792.29초), 종료0.
`/tmp/claude-184-asset-scope-full.log`. FR065와 main675fe93 통합까지 포함한다.
린트도 통과했으며 아래 이번 전체 실행 중 기록은 이 결과로 갱신한다. 실제 KIS
조건11개와 사전 가동 상태 전용1개 제외. 현재 전체 계좌 인증/T009/T010은 미완료다.

184 FR065: 같은 계좌 관측 구간의 자산표19분류와5열 합계, 별도 채무/청약/담보,
현재 원화 현금과 해외 보유의 존재를 대조한다. 가격 변화는 허용하고 매입 구성
변경·누락·지원 외 자산은 거절한다. 정상 계좌 입력과 실행 지문에 연결했다.
관련106개(1.36초), 단타1075 passed/4 skipped(45.13초), 린트·하네스14/14·인계 통과.
전체 회귀는 실행 중이며 아래4592개는 이번 변경 이전 결과다. 결제 기준 분류 대조를
현재 전체 계좌/비용 인증으로 올리지 않는다. T009/T010과 PR793 초안은 유지한다.
제거한 기존 기능은 없으며 실제 주문·자본 배분·활성화도 없다.

184 최신 전체 회귀: 코드a8beac7에서4592 passed/12 skipped(810.45초), 종료0.
`/tmp/claude-184-a8beac7-full.log`. FR061~064 원화/달러·미결제·보유·관측 구간을
모두 포함한다. 실제 KIS 조건11개와 사전 가동 상태 전용1개 제외. 후속627a9ad는
남은 원본 계약 문서의 오래된 설명을 현재 구현에 맞춘 수정이며 실행 코드는 동일하다.
아래의 이번 전체 회귀 미완료 기록은 이 결과로 갱신한다. 실제 전체 계좌 인증과
T009/T010은 여전히 미완료이며 PR793은 초안이다.

184 FR064: 정상 계좌 읽기를 하나의 CURRENT 전후 구간으로 묶었다. 미국4개와
국내/자산표2개를 사이에 수집하며 증거금 중복을 제거해 보통9GET이다. 요청마다
동일 연결·실제 시작/수신 시각·전체30초를 확인하고 기존 현금/수량 정규화기는
전후 구간이 감싸는 중간 조회도 대조한다. 보고 평가 시각을 새로 만들지 않는다.
관련121개(1.27초), 단타1012 passed/4 skipped(45.36초), 린트·하네스14/14·인계 통과.
원본 자산표는 비공개 입력으로만 보존하고 공개 판정과 분리한다. 전체 계좌 검증
표시는 false 유지다. 다음은 이 일관된 관측 구간과 남은 전체 자산 범위/수수료
포함 관계의 연결·실제 원본 검사다. T009/T010과 이번 전체 회귀는 미완료이며
실제 주문/자본 배분/활성화는 없다. 아래 FR063의 다음 조회 통합은 이번에 구현됐다.

184 FR063: 기존 국가 전체 CURRENT 원본의 체결 기준 보유를 별도로 정규화하고,
일반 미국 잔고와 양수 수량/통화/시장 범위를 대조한다. 전후 보유/매도가능/대출/시장
변화는 거절하며 가격 변화는 허용한다. 소수/0수량과 누락 평가를 보존하고 조회 시각을
가격 시각으로 만들지 않는다. 관련103개(1.48초), 단타987 passed/4 skipped(45.75초),
린트·하네스14/14·인계 통과. 해외 주식 범위 일치는 전체 계좌 인증이 아니며 기존
full_account_scope/cash/nav 검증값은 false 유지다. 다음은 전체 현재 조회가 국내/
미국 잔고와 자산 보고를 감싸는 하나의 관측 구간으로 묶고 남은 전체 범위를 연결하는
작업이다. 수수료 포함 관계/T009/T010도 미완료. 추가 GET·실제 주문은 없다.

184 FR062: 같은 CURRENT→MARGIN→CURRENT 원본에서 독립 단일 USD 보고의
사용가능금액+증거금과 반복 예수금/증거금을 대조하고, 공통 미결제 구성에 대해서만
예수금+미결제매도-미결제매수 산술을 연결했다. 증거금을 자산에서 다시 차감하지 않고
원래 사용가능/증거금/미결제 값을 보존한다. 기존0조정 결과는 유지하며 새 조건에 맞는
보고 현금만 별도 범위로 원화/달러 평가에 연결한다. 관련99개(1.37초), 단타960
passed/4 skipped(45.55초), 린트·하네스14/14·인계 통과. 원본 미수/누락/다른 통화/
불일치/변화는 거절한다. fees_inclusion_verified=false와 전체 계좌 false를 유지한다.
이 조건 밖의 증권사 표시 방식과 수수료 포함 관계, 전체 자산 범위/T009/T010은
후속이며 아래4474개 전체 검사는 FR061/FR062 이전이다. 추가 GET·실제 주문 없음.

184 FR061: 전후 USD 보고에서 같은 KRW 총예수금과 최초고시 USD-KRW 환율을
보존하고, 국내 예수금/정산액/순자산과 대조해 원화·달러 현금 입력을 연결했다.
같은 원화 값을 반복 합산하지 않으며 원화/달러 원본을 분리해 남긴다. USD 평가는
명시적 최초고시환율 정책·12자리 내림이며 실시간 환전 시세/주문가능금액이 아니다.
관련91개(1.69초), 단타938 passed/4 skipped(47.69초), 린트·하네스14/14·인계 통과.
누락/변화/정산 차이/대출/시각을 거절하고 음수 원화 현금도 보존한다. 전체 계좌 범위와
거래 후 미결제 포함 순현금/T009/T010은 미완료다. 아래4474개는 이 변경 이전이다.

184 통합 후속: main e79cb01(PR813 현금 대조/PR814 실제 결과 인계)을 통합한다.
독립 현금 대조의 실제 관계를 아래 기록에서 확인하고, 원화/달러 잔고 계산 연결을
이어간다. 아래4474개 전체 검사는 이18개 대조 시험 추가 전 실행 코드8658792 기준이다.

184 최신 전체 회귀: 실행 코드8658792에서4474 passed/12 skipped(829.41초), 종료0.
FR058~FR060의 보고 평가/OTC 구독/국내 현재잔고 연결을 포함한다. 검사 중 실행 소스는
변경하지 않았고 e09be5d는 원본 순자산 대안 검토 문서만 추가했다. 로그는
`/tmp/claude-184-domestic-final-full.log`다. 실제 KIS 조건부11개와 가동 전 사다리 검사1개
제외. 정상 전체 계좌 계산/T009/T010은 여전히 미완료, PR793 초안 유지.
186 PR813의 국내·해외 현금 대조기는 별도 전체 검사 중이며 아직 이 결과에 포함되지 않는다.

184 통합 후속: main ec493ce(PR812 인계)를 병합했다. 국내 원본 수집의 전체 검사와
실제 서버 확인은 아래186 FR009 출시 기록으로 확정됐다.184 FR058~FR060을 포함한
최신 전체 회귀를 수행하며 전체 계좌 계산/T009/T010은 미완료로 유지한다.

184 FR060: 186/85db517을6062e0c로 통합해 원본 수집기의 국내 현재잔고 추가를
연결했다. 충돌은 거래명세/비용 조회와 양쪽 인계 기록을 모두 보존해 해결했다.
KISExecutionObserver._read에도 같은 계좌/인증/제한 클라이언트의 국내 읽기를 연결했다.
KRW 보고와 전체 계좌/현금 검증을 분리하고 새 소스를 실행 지문에 넣었다. 관련71개
(1.65초), 단타912 passed/4 skipped(49.37초), 린트 통과. 국내 조회 실패는 계좌 부분
성공으로 바뀌지 않는다.186/PR811 전체 검사/실제 서버 확인은 아래 출시 기록과 같으며
T009/T010과 전체 계좌 범위·거래 후 순현금 판정은 계속 미완료다.

184 FR059: 실제 KISExecutionObserver._read가 OTC의 명시적 보고 평가/양수 정수 수량을
reported_asset_values로 변환한다. 기존 unverified_assets와 전체 계좌/현금 검증 false는
유지한다. _ValuationFeed는 같은 계좌 읽기(30초 제한)를 사용해 보고 평가 자산을 시세
발견/구독에서 분리하고, 나머지는 기존 원본 시각 구독을 유지한다. 알려진 미지원 자산에
보고 평가가 없으면 구독 준비를 거절한다. 관련61개(1.19초), 단타910 passed/4 skipped
(46.59초), 린트·하네스14/14·인계 통과. 최신 전체 검사는 여전히8364f76의4429/12다.
T009/T010·거래 후 순현금·전체 계좌 범위 제공자는 미완료다. 다음은 전체 국가 현재잔고와
일반 보유/전체 자산표의 포함 관계를 확인해 실제 전체 계좌 입력을 완성하는 것이다.

184 FR058: 단타 허용 종목 밖의 기존 자산은 별도 보고 평가 형식으로 NAV와 전체
노출에 포함한다. 수량/조회 구간/중복을 검사하고 주문 대상에는 사용할 수 없다.
실제 주문 시세의 원본 시각은 유지하며 보고 평가를 시세로 변환하지 않는다. 한도
판단의 보고 금액/수량/조회 구간은 비공개 사건 장부에 추가 기록한다. 관련93개(1.00초),
단타905 passed/4 skipped(46.94초), 린트·하네스14/14·인계 통과. 실제 전체 계좌 공급자,
기존 보유 구독의 새 형식 연결, 거래 후 순현금 및 T009/T010은 미완료다. FR058의 전체
회귀는 아직 하지 않았다. 직전 FR057 완료 코드8364f76의 전체4429 passed/12 skipped
(795.37초)는 정상 종료0이며 새 평가 형식을 포함한 결과가 아니다. PR793 초안 유지.

FR057 통합 회귀 후속: 전체 검사에서 기존 KIS 프로그램 시험2개가 추가된 현재잔고
조회 응답을 준비하지 않아 실패했다. 시험 응답과 GET7회 기대값을 실제 호출 경로에
맞췄으며 관련48개(1.02초)·린트 통과. 미검증 계좌의 주문 차단 기대값은 유지했다.
진행 중 전체 검사의 실패를 숨기거나 전체 통과로 표시하지 않는다.

184 FR057: 동일 인증 연결로 현재 USD 보고→외화 예수금→현재 USD 보고를 수집한다.
후속 검토에서 비페이지 조회의 연속 조회 헤더 생략/공백을 기존 계좌 읽기와 같은
빈 상태로 해석하도록 보정했다. 관련29개(0.67초)·린트 통과. 전체 회귀는587deb7에서
시작한 검사이며 이 헤더 보정의 추가2개 시험을 포함한 결과로 보고하지 않는다.
단일 USD 행을 기준으로 반복 예수금을 대조하며 합산하지 않는다. 미결제/미수/증거금/
다른 현금/대출이 없고30초 안에 전후 값이 같을 때 보고 범위 시작 현금을 반환한다.
정상 KISExecutionObserver의 reported_cash_baseline에 연결했고 전체 범위/NAV 검증은
승격하지 않는다. 입력기·계좌 파서·공유 숫자 파서 변경은 실행 지문에 반영된다.
관련294개(1.77초)·린트·하네스14/14·인계 검사 통과. 신규 입력기의 서버 검사와
전체 회귀는 미실행이다. T009/T010 미완료, PR793 초안 유지. 다음은 전체 계좌 범위와
거래 후 미결제를 포함하는 순현금 입력 연결이며 실제 주문·승인·자본 배분은 수행하지 않았다.

184 FR056: cfc0022로 main5b72039의 실제 자산 구성 분석을 통합한 후 OTC 보고
가격/평가/매도가능 수량을 무조건None으로 버리던 동작을 수정했다. 원본 제공값을
보존하고 USD 일반잔고 보고 평가를 합산한다. 누락 값은 합계None, 비정상 제공값은
오류이며 원본 가격 시각은None이다. 관련189개(1.91초)·린트·하네스14/14·인계 통과.
실행용 시세나 전체 계좌 검증으로 승격하지 않는다.4378/12는 이전f0ffc85 기준이고
이번 변경 전체 회귀는 아직 미실행이다. 다음은 실제 현금 정규화와 전체 계좌 입력 연결이다.

184는4ccc4d4로 main68f0949의 원본 구조 분석·독립 현금 대조를 통합했다. 거래명세와
비용 대조 옵션,184 활성 명세 포인터는 유지한다. 통합 검사에서 과거 주문과 실제 현재
수신 시계를 섞은 시험3개가 실패해 시험 수신 시계를 고정했다. 실행 시각 검증은 유지하며
관련116개(2.22초), f0ffc85 전체4378 passed/12 skipped(806.20초), 린트·하네스14/14·
인계 검사는 통과했다. 실제 KIS11개와 가동 전 상태 전용1개는 제외다.
실제 USD 독립 보고 일치를 정상 계좌 입력에 연결하는 작업과 T009/T010은 미완료다.
...
186 FR014 진행:184 03f8500의 공백 거래소 거래내역 조회/산술을 가져오고 기존
--history-db 명령이 최근3개 UTC 등록일 원본을 함께 보관하도록 연결했다. 거래내역은
현재 잔고 전후 구간 전에 읽어 잔고 구간30초를 소모하지 않는다. 설치된 root 소유
도우미 수정 없이 기존 승인된 명령 진입점으로 적용한다. 관련64개(0.54초), 린트·
하네스14/14·인계 통과. 전체 검사·배포·실제 원본 수집은 T023 미완료다. 조회 범위가
모든 현금/미결제 거래를 포함한다고 주장하지 않으며 주문/자본/가동 변경은 없다.

186 FR013 실제 판정 완료: PR819/main923a8be, 실행34699060401 성공·KIS11개 통과。
장부 순번12·10응답 COMPLETE, 전후 안정, 추가 GET0·주문0. 새 자산 분류19개와
현재3종목 존재 대조 MATCH. 기존0조정 USD/보고 미결제 현금/원화달러 평가도
CALCULATED, 국가 전체 보유 대조 MATCH다. 로그는
`/tmp/claude-186-asset-scope-live.log`. 테스트 시간 일부는 GitHub 비밀값 마스킹으로
가려져 있어 초 단위 소요시간을 복원하거나 추정하지 않는다.
새 분류 판정은 결제 표와 현재 입력의 대조이며 전체 현재 계좌/거래 후 비용
인증은 아니다.184 T009/T010을 이 결과만으로 완료 처리하지 않는다.

186 FR013 전체 검사: 코드a61690f에서4066 passed/12 skipped(759.50초), 종료0.
`/tmp/claude-186-asset-scope-full.log`. 린트·하네스14/14·인계 통과. 아래 이번 전체
검사 미실행 기록은 이 결과로 갱신한다. 실제 원본의 새 분류 판정은 main 반영 후
확인하며 현재 전체 계좌/수수료 포함 인증과184 T009/T010은 미완료다.

186 FR013 진행:184 FR065와 같은 자산 분류 대조를 기존10GET 원본 수집 결과에
연결했다. 공개 값은 고정 판정/이유/건수뿐이며 추가 조회와 인증값 승격은 없다.
관련148개(0.53초), 린트·하네스14/14·인계 통과. 전체 검사·main 반영·실제 판정은
T021로 남아 있다. 아래4011개는 이번 변경 이전 검사다. 기존 기능 제거와 실제
주문·자본 배분·활성화는 없다.

186 FR012 실제 판정 완료: PR817/main675fe93, fe08a91 전체4011/12(749.77초).
실제34697142253 KIS11개(24.69초), 원본10응답·순번11 COMPLETE·주문0·추가조회0.
현재0조정 상태에서 USD 기본 현금/보고 미결제 현금/원화·달러 평가 모두 CALCULATED,
국가 전체 현재 보유 OBSERVED, 일반 미국 보유와 양수3종목 수량 대조 MATCH다.
`/tmp/claude-186-models-live.log`. 실제 현재 입력의 조건부 계산 성공을 확인했으며
비영 미결제 발생 후 수수료 포함 관계·전체 자산 범위 검증을 대신하지 않는다.
184에는954a876으로 통합, 거래내역/비용 기능을 보존하고 관련204개(1.60초)·린트·
하네스14/14·인계 통과.184 전체4592/12는 직전a8beac7 기준이다. 다음은 확인된
현금과3종목 보유를 남은 자산 분류·거래 후 비용에 연결하고 T009/T010을 검증하는 일이다.

186 FR012 구현:184의 조건부 USD 현금/미결제/원화·달러 평가/국가 전체 보유 대조
계산기를 동일 원본 장부와 직접 수집한 파서 결과에 적용한다. 추가 GET 없이 결과의
상태·닫힌 이유·건수만 공개하고 금액/원문은 비공개다. 관련155개(0.46초), 린트·
하네스·인계 통과. fe08a91 전체4011 passed/12 skipped(749.77초), 종료0.
`/tmp/claude-186-models-full.log`. 실제 서버 계산 결과는 후속이며 전체 계좌/현금/NAV
검증값은 false 유지다. 이 결과로184의 실제 원본 조건 통과 여부를 확인한다.

186 FR011 실제 확인 완료: PR815/main965ecee. 전체3913 passed/12 skipped(765.67초),
실제34696061360 KIS11 passed(25.60초), 원본10응답·장부 순번10 COMPLETE·주문0.
CURRENT 전후 보고는 안정적이며 국내 예수금은 현재/결제 총예수금과 자산표 예수금
분류/요약에 계속 일치한다. 국내 보유/대출/기타 구성0, 예수금·정산액·순자산 양수.
로그 `/tmp/claude-186-frame-live.log`. 전체 계좌/현금/NAV 검증은 false 유지다.
184에는9b8ce2a로 통합했고 거래내역/비용 기능을 보존해 관련153개(1.49초), 린트·
하네스14/14·인계 통과.184 직전 실행 코드a8beac7의 전체4592/12(810.45초)와
이번 결합의 관련 검사는 구분한다. 다음은 이 원본 구간에 전체 자산/비용 계약을
연결하는 작업이며184 T009/T010은 미완료다.

186 FR011 구현: 같은10개 원본 조회를 CURRENT→결제→미국4개→국내→자산표2개
→CURRENT 순서로 수집한다. 기존 결제 진단과 원본 장부는 보존하며 중간 실패/취소/
30초 초과 시 성공으로 기록하지 않는다. 관련60개(0.31초), 린트·하네스14/14·인계
통과. 첫 전체 검사는3912 passed/12 skipped/1 failed(780.96초)였다. 실패는
개인 장부 없이 실행하는 기존 시험의 이전 호출 순서 기대값이며 c13f773에서 새
전체5회 순서를 확인하도록 보정했다. 관련104개(0.40초)·린트 통과, 실행 코드는
동일하다. 최종 전체 c13f773은3913 passed/12 skipped(765.67초), 종료0이다.
`/tmp/claude-186-frame-final-full.log`. 실제 KIS 조건11개/사전 가동 상태 전용1개 제외.
실제 서버 확인은 main 반영 후이며184의 전체 계좌 인증/T009/T010과 구분한다.
이 원본 순서가184의 전후 관측 모델을 실제 자료로 검사할 기반이다. 실제 주문 없음.

186 FR010 출시: PR813/main cd1bec9, 코드adfb510 전체3908 passed/12 skipped
(788.78초), 관련62개·린트·하네스14/14·인계 통과. 실제34691717131 KIS11개 통과,
10응답·순번9 COMPLETE·주문0. 실행 시간은 공개 로그에서 일부 마스킹돼 재구성하지 않는다.
국내 예수금은 현재 전후/결제 요약의 tot_dncl_amt와 EQUAL, 자산표 예수금 분류17의
evlu_amt 및 같은 응답의 dncl_amt/tot_dncl_amt와 EQUAL이다. 국내 익일/D+2 정산액,
순자산/총평가도 예수금과 EQUAL이고 나머지 확인한 구성은0이다. 반면 현재 요약
dncl_amt와는 DIFFERENT이며 국내+frcr_evlu_tota를 tot_dncl_amt와 비교하면 DIFFERENT다.
해외 dncl_amt=0만으로 원화 현금0이라고 판단하지 않는다. 같은 원화 항목을 여러 API에서
중복 합산하지 않는다. 다음은 이 원본 관계를 기준으로 원화/달러 입력을 분리하는184
계산 연결이다. 동시 시점·전체 계좌·미결제 계산 인증은 별도이며 T009/T010은 미완료다.

186 FR009 출시: PR811/main aeace15, 코드85db517 전체3890 passed/12 skipped
(763.73초), 관련72개·린트·하네스14/14·인계 통과. 실제34690683642는 KIS11개
(24.44초), 원본10응답·장부순번8 COMPLETE·주문0. 국내 현재잔고1페이지 완료,
보유/수량0행 모두0, 증권평가/CMA/대출/대주/전일·당일 매매 모두0이다. 반면 예수금,
익일/가수도 정산액, 총평가/순자산은 양수다. 해외 현재 보고의 dncl_amt=0과 달라
국내 예수금을 USD 현금에 곧바로 더하거나0으로 무시하지 않는다. 금액 범위는 후속
확인이며 원본은 비공개 장부에 보존됐다.184의 da1435e는 국내 조회를 정상 읽기에
연결하고 실패 시 부분 성공을 차단한다. 관련71개/단타912개 통과·4개 조건부 제외.
184 전체 계좌 공급자/T009/T010은 미완료다. 원래 해외 조회·금액 비노출·기존 장부와
실제 주문/취소/자본 배분 경계는 유지한다.

186 FR008 출시: PR809/main1b901ab, 코드1c9472b 전체3873 passed/12 skipped(772.80초),
린트·하네스14/14·인계 통과. 실제34686417861 KIS11개(23.59초),9응답·순번7 COMPLETE。
OTCB1행은 수량·매도가능 수량·보고 가격·보고 평가액 모두 양수다. 가격/평가액이
원본에 없다고 말하거나0수량 표시 행으로 제외하면 안 된다. 원본 가격 발생 시각과
실행 시장 지원은 별도 문제다. 미국 상장2행도 네 항목 양수이며 다른 시장 행은0개다.
전체 자산표의 양수 분류는8(해외주식),16(외화),17(예수금)이며 다른 분류의5열은0이다.
두 표 모두 대출 열0. 현재 dncl_amt/CMA/대출/미결제는0인데 tot_dncl_amt는 양수다.
따라서 총예수금에 외화 총액을 다시 더하는 식은 중복 가능성이 있어 채택하지 않는다.
현재 표와 분류표의 포함 관계는 후속 확인이며 값이 같은 식을 임의 선택하지 않는다.
실제 주문0, 계좌 인증/184 정상 입력기는 미완료다. 다음은 OTC 보고값 보존과 실행용
시세 검증의 분리, 독립 USD 현금·원본 보유를 사용하는 명시된 계산 범위 구현이다.

186 FR007 출시: PR807/main2f7c11f, 코드772915d 전체3853 passed/12 skipped(761.84초),
린트·하네스14/14·인계 통과. 실제34685116089 KIS11개 통과(26.44초), 원본9응답·
장부순번6 COMPLETE, 실제 주문0. 공통 USD 예수금은 현재 조회 전후와 결제 잔고의
USD 값에 모두 정확히 일치하고 미결제/미수/증거금 조정은0이다. 현재 보고의 USD
환산액/외화 총액과 HTS 총자산식 후보는 DIFFERENT, 결제 보고의 두 비교는 필요한
항목이 없어 UNAVAILABLE다. 다른 통화의 보고 금액은0이다. 일치는 행 합산 계약이나
전체 계좌 인증이 아니며 차이는 임의 보정하지 않는다.184 후속은 조회 단위·보고
합계의 포함 범위를 구분해 정상 계좌 입력기를 구현하는 일이다. T009/T010 미완료.

186 FR006 출시: PR805/main6b92ae1, 코드bc35254 전체3839 passed/12 skipped(774.73초),
린트·하네스 통과. 실제34684108577은 KIS11개 통과(25.09초), 원본9응답·장부순번5 저장.
USD10행은 국가명10종, 현금5개 구성은 모두 동일하고 예수금만 양수이며 나머지0이다.
일반주문가능금액은5종으로 다르다. 다른 통화6행/미분류34행의 조사한9개 숫자 필드는
모두 유효한0이며 미분류 국가명은 전부 공란이다. 이 관측은 국가별 반복 표시 가설을
뒷받침하지만 동일 값만으로 합산/중복 제거 계약을 확정하지 않는다. 다음은 동일 배치의
독립 잔고 총액·통화 행과 공통 USD 예수금 대조, 공식 HTS 총자산식 대조다.
전체 계좌 인증과184 정상 시작은 여전히 미완료이며 실제 주문0이다.

186 FR006 후속: 저장에 성공한 동일 응답을 USD/다른 통화/미분류 그룹의 숫자 유효성,
0값·음수·현금 구성 중복 건수로 분석하는 진단을 연결했다. 원본 숫자·국가명·임의
필드는 공개하지 않으며 합산 규칙/계좌 인증으로 승격하지 않는다. 관련36개·린트·
하네스14/14·인계 통과. 전체 회귀와 실제 서버 분석 결과 확인은 진행 전이다.

186 원본 수집 출시: PR803이 main8ec597b에 반영됐다. 실제 서버 검사34630508745에서
KIS11개 통과(23.65초), 비공개 data/account-source.db에 응답9개·관측순번1·COMPLETE를
확인했다. 실제 주문0, 계좌 검증은 false 유지다. USD 구성10행·미분류34행·미해석
자산1개·일반 보유2개·미체결0이 현재 진단 결과다. 원본 수집 성공은 이 항목의
순현금/전체 계좌 정규화 성공이 아니다. 이제 사용자 파일 위치 대기 대신 서버에 남긴
원본을 기준으로184 계산 연결을 이어간다.
배포34630458624의 작업 결과는 success지만 운영 워커 교체는 미국 장중으로20:00Z
이후로 연기됐다. 준비 단계에서 읽기 도구는 갱신돼 별도 격리 조회가 성공했다.
두 번째 실행34630708820도 KIS11개 통과(20.11초), 응답9개·관측순번2·COMPLETE로
끝나 기존 파일을 다시 열고 추가 기록한 경로까지 확인했다. 실제 주문0.

186 코드513b33a 전체3821 passed/12 skipped(814.99초), 종료0. 실행 중 소스 변경 없음.
제외는 실제 KIS11개와 가동 전 상태 전용1개다. PR803 구현 검증 완료, 독립 반영과
서버 실제 수집 확인은 후속 단계다. 전체184 단타 완성/계좌 검증을 뜻하지 않는다.

186 진행 중: 미완료184에서 비공개 계좌 원본 수집기만 분리했다. 기존 main 전용
KIS 읽기 검사가 성공하면 같은 격리 checkout으로 수집하며 운영 data/account-source.db에
0600으로 추가 기록한다. 실제 설치/서버 실행 전이며 전체 단타 완성이 아니다.
관련24개·린트·하네스14/14·인계 통과. 전체 회귀와 PR 검증 후 독립 반영하고
확보된 원본의 통화/채무/미결제 범위를184 정상 입력기에 연결하는 것이 다음 단계다.

2026-09-12 원본 확보 대안 진행: 기록기만 main 기반186으로 분리해 PR803을 열었다.
별도 작업 디렉터리는 /Users/mason/Projects/claude-worktrees/codex-account-source-collection,
코드513b33a다. 기존 main 전용 KIS 검사 성공 후 비공개 account-source.db에 원본을
남기는 경로를 구현했고 관련24개·린트·하네스 통과, 전체 검사는 진행 중이다.
서버 설치·실수집 전이므로 아직 원본 확보로 판단하지 않는다. 다음 단계는186 검증·
반영·서버 수집 확인 후 이184의 전체 계좌 계산 계약을 연결하는 것이다.
아래 잔고 파일 위치 질문만 반복하며 대기하던 진행 방식은 이 수집 경로로 대체한다.

2026-09-12 최신 전체 회귀: 실행 코드f66840c에서4342 passed/12 skipped(809.50초),
종료0. FR053~055와 날짜·종료 확인 보완을 포함하며 실행 중 소스 변경 없음.
사용자 self-test·린트·하네스14/14·인계 검사 통과, 실제 주문0. 제외는 실제 KIS11개와
가동 전 상태 전용1개다. 전체 계좌 제공자는 여전히 미구현이므로 T009/T010과 프로그램
전체는 미완료다. 현재 요구별 구현·근거·막힌 연결은
`specs/184-intraday-operator-program/completion-audit.md`를 먼저 읽는다.
다음 작업은 실제 전체 잔고 원본의 위치/조회 경로를 확인해 계좌 정규화 제공자를
구현하는 것이다. 기존 서버의 일부 체결 이력 부재를 주장하거나 화면을 필수화하지 않는다.

FR055 최종 검토: 날짜 누락/불일치 미체결 주문의 잘못된 모델 일치2개를 재현·수정했다.
모든 주문 날짜를 신호의 뉴욕 거래일에 연결하고 원래 수량보다 적은 체결은 종료 확인을
요구한다. 관련91 passed(1.38초), 린트 통과. 실제 주문이나 승인 파일 발급은 없다.

2026-09-12 FR055: 계산한 모든 실행 모델 조건을 최종 자격 검사에 결합했다. 모의
조회/장부/계산/모의 승인 정상 통과와 각 조건 누락/변조·승인 철회 거절을 확인했다.
단타/해외/체결/비용930 passed/4 skipped(45.41초), 린트·하네스14/14·인계 통과.
기존 고정 미구현 표시는 검증된 종합으로만 대체하고 새 미지원 조건은 계속 거절한다.
이것은 계좌 인증/전략 승격이 아니며 실제 승인 발급/주문은 없다. 전체4303개는
ddff134 기준으로 FR053~055 이전이다. 정상 전체 계좌 입력 및 T009/T010은 미완료다.

2026-09-12 FR054: 같은 종목의 반복 주문을 날짜/방향별 수량·대금으로 대조하고
수수료는 주문별 배분 없이 묶음 기준으로 검사한다. 단타/해외/체결/비용930 passed/
4 skipped(49.74초), 린트·하네스14/14·인계 통과. 날짜 간 합계 상쇄·날짜 누락·묶음
중복/누락을 거절한다. 전체4303개는ddff134 기준으로 FR053/054 이전이다.
전체 모델 자격의 최종 종합과 정상 계좌 입력/T009/T010은 미완료다. 실제 주문 없음.

2026-09-12 FR053: 원본 주문 수량을 장부와 대조하고 미체결을 포함한 모든 조회 주문을
다음 봉의 정수주 모델 수량과 비교한다. 단타/해외/체결/비용921 passed/4 skipped
(47.03초), 린트·하네스14/14·인계 통과. 아래 수량 연결 대기는 이 구현으로 대체한다.
전체4303개는ddff134 기준으로 이 변경 이전이다. 각 부분 결과를 최종 자격으로
종합하는 연결과 정상 전체 계좌 입력/T009/T010은 미완료이며 실제 주문은 없다.

2026-09-12 최신 전체 회귀: 코드ddff134에서4303 passed/12 skipped(801.30초),
종료0. FR051/052 가격·수수료·보고 날짜 변경을 포함하며 검사 중 실행 소스 변경 없음.
사용자 self-test와 린트도 통과, 실제 주문0. 아래969eb1c 기준 전체 회귀 대기는 이
결과로 대체한다. 정상 전체 계좌 입력과 전체 실행 자격/T009/T010은 미완료다.
다음 구현은 원본 주문 수량을 보존해 실제 체결 수량을 연구의
min(주문 수량, floor(다음 봉 거래량×참여율))과 대조하는 경로다. 기존 상한 검사만으로
부분 체결 결과까지 일치한다고 간주하지 않는다. 잔고 원본 위치 질문에는 아직 답이 없다.

2026-09-12 FR052: 엄격한 체결 파서가 주문 날짜를 시각과 분리해 보존하고 실제 비용
조회에서 보고 매매일과 대조한다. 단타/해외/체결/비용894 passed/4 skipped(45.86초),
린트·하네스14/14·인계 통과. 날짜 누락은 미검증이고 충돌은 거절하며 UTC 시각을
추정하지 않는다. 전체 거래 범위/모델 동등성/계좌 입력/T009/T010은 미완료다.
전체 저장소 최신4259개는969eb1c 기준으로 FR051/052 변경 이전이다.

2026-09-11 FR051: 같은 원본 다음 봉 시가 기준으로 실제 평균 체결 가격과 보고 수수료를
연구 한도에 대조한다. 단타791 passed/4 skipped(47.15초), 추가 비용/자격 소비75 passed
(1.16초), 린트·하네스14/14·인계 통과. 아래 다음 봉 비용 연결 대기는 이 구현으로
대체한다. 전체 저장소4259개는969eb1c 기준으로 이 변경 이전이다. 전체 계좌 입력과
주문/거래 범위·전체 실행 동등성/T009/T010은 미완료이며 실행 승인으로 승격하지 않는다.

2026-09-11 최신 전체 회귀: 코드969eb1c3d6c458a8a02c99f0f1e8ecb520ed6b1d에서
4259 passed/12 skipped(809.77초), 종료0. FR048~050의 기간 범위/수집 증명/신호 연결을
포함한다. 사용자 self-test도 SELF_TEST_PASSED(실제 주문0), 린트 통과. 아래93ee2fa
기준 전체 검사 대기는 이 결과로 대체한다. 정상 전체 계좌 입력/T009/T010은 미완료다.
다음 검증 연결: 연구기 _cost_values는 다음 봉 시가로 비용을 계산하지만 현재 비용
대조는 실제 체결대금 기준이다. 원본 다음 봉 시가에 대한 가격/수수료 상한을 연결해야
하며, 현재 구간 통과만으로 전체 실행 동등성이나 승격을 허용하지 않는다.

2026-09-11 FR050: 주문 의도와 전략 청산 사건에 원래 신호 봉 종료 시각을 보존한다.
부분 체결/취소/재시작 후에도 원래 시각으로 다음5분 봉 구간 포함 여부를 자격 모델에서
검사한다. 단타766 passed/4 skipped(45.63초), 린트 통과. 실제 체결 시각을 추정하지
않으며 전체 계좌 입력/전체 실행 동등성/T009/T010은 미완료다. 전체 저장소 최신 검증은
아래93ee2fa 기준이며 FR048~050은 포함하지 않는다. 실제 주문/키 설치 없음.

2026-09-11 FR049: --attest-market-data로 직접 KIS 수집한 봉과 수신 시각을 별도
서버 키로 서명하고 모의 장부→독립 재생→자격 모델에 전달한다. 단타741 passed/
4 skipped(45.09초), 린트 통과. 기존 무서명 진단은 출처 인증으로 승격하지 않는다.
서버 키/시계/수집기를 신뢰하며 KIS 전자서명은 아니다. 키 설치·실접속은 미수행이다.
현재 구간 거래량·수집 출처만 확인 가능하고 전체 실행 모델 동등성/전체 계좌 입력과
T009/T010은 미완료다. 최신 전체 회귀4215개는93ee2fa 기준으로 FR048/049 이전이다.

2026-09-11 FR048 후속: 비용 검증 지문을 명시된 검증 기간의 주문/체결/상태/감사로
한정했다. 기간 밖 기록 추가로 재시작 승인 참조가 달라지는2개 실패를 재현·수정했고
관련86개(13.37초)·린트 통과. 기간 안 혼입/조회 중 장부 변경 차단은 유지한다.
아래4215개 전체 회귀는 수정 전93ee2fa 기준이다. 정상 계좌 입력/T009/T010은 미완료다.

2026-09-11 최신 전체 검증: 실행 코드93ee2fa에서4215 passed/12 skipped(804.63초),
종료0. FR044~047 현금 범위 계산·구간 자동 소비·수신 시각 보존을 포함한다.
사용자 명령 self-test도 SELF_TEST_PASSED, 실제 주문0으로 확인했다. 린트 통과.
아래 과거의 전체 회귀 미실행 표기는 이 결과로 대체한다. 정상 전체 계좌 입력과
시장 원본/전체 실행 자격·최종 정상 시작 T009/T010은 여전히 미완료다.

2026-09-11 FR047: 실제 체결 조회가 끝난 시각을 신규 감사 필드에 저장하고 재시작 후
모든 체결번호/수량/가격/주문/전략과 대조해 구간 상한을 좁힌다. 기존 논리적 관측 시각은
수신 완료로 해석하지 않는다. 단타/체결/감사975 passed/4 skipped(113.56초), 린트 통과.
현재 시장 원본 인증·정상 전체 계좌 입력·T009/T010은 미완료이며 목표는 계속 진행 중이다.
최근 전체 저장소 회귀는 eb22ad4 기준으로, 이번 변경의 전체 회귀와 구분한다.

2026-09-11 FR046: 전진 관찰 장부의 동일 독립 재생 스냅샷에서 완결 세션 봉을
실제 자격 준비의 구간 거래량 계산으로 전달했다. 단타710 passed/4 skipped(44.53초).
장부 재생 일치와 시장 원본 인증은 구분하며 후자는 아직 미입증이다. 전체 회귀와
정상 순현금 공급자/최종 생산 시작 T009/T010은 미완료다. 실제 주문 없음.

2026-09-11 FR044/045: 단일 USD 원본 행의 미결제/미수/증거금0 조건에서 보고 결제
현금을 계산하고 계좌 GET·공개 진단에 연결했다. SUBMITTING 장부 기록→체결 GET 응답
수신으로 구간을 생성하고 가능한 모든5분 봉에 겹치는 동일 종목 주문 수량을 합산해
거래량 한도를 검사한다. 정확한 체결 시각은 필요하지 않지만 봉의 원본 인증과 전체
연구/실행 동등성은 별도다. 단타707개/실접속4개 제외·린트·하네스14/14 통과.
최신 전체 저장소 회귀는 미실행이며 아래 eb22ad4 결과와 구분한다. 실제 주문 없음.

2026-09-11 최신 검증: 실행 코드eb22ad4 전체4168 passed/12 skipped(797.47초), 종료0.
추가 시작 통합2개는 별도10개 검사(13.05초)에 포함해 통과. 실제 시작 조립→인증→GET→
자료 부족/요청 취소→DB 닫기→재시작을 검증하며 연구/전진 합격은 시험 격리다.
아래의 최신 전체 회귀 미완료 표기는 이 결과로 대체한다. 정상 순현금 원본 공급자와
시간/거래량 모델 증명, T009/T010 정상 생산 시작은 아직 미완료다. 실제 주문/활성화 없음.

2026-09-11 기능184 후속: 인증된 주문/거래 GET→기존 SQLite 체결·비용 재계산→실제
prepare_qualification 호출을 연결했다. 계산 지문과 운영 승인 참조를 비교하며 시간/거래량/
주문-거래일 연결 미입증은 별도 차단한다. 공용 current_positions를 평가 구독 합집합에
포함하되 외부 기준 보유에는 복제하지 않는다. 관련59개·단타679개/실접속4개 제외·린트·
하네스14/14 통과. 최신 전체 회귀와 T009/T010은 미완료다. 실제 주문/취소/활성화 없음.
...
## 한눈 요약표 — 2026-09-09 KST 실제 보관 자료 연구 연결

2026-09-11 후속: 아래4cfcb5f의 두 물리 시세 연결은 단일 연결+전략/평가 뷰로 대체했다.
전략5개를 포함해40개 한도를 적용한다. 관련73개(1.01초)·린트 통과. 공식 원본을 다시
조회해 순현금 구성/수수료 포함/중복 여부의 설명 공백과 비용 명세의 주문번호 부재를
specs/184-intraday-operator-program/contracts/unresolved-source-fields.md에 기록했다.
모든 계좌를 지원할 수 없다는 증명은 아니며, 현재 코드의 정상 공급자/체결 자격/최종 통합
T009/T010은 미완료다. 외부 자료 부족과 코드 미구현을 혼동하지 않는다. 전체 회귀/머지/
서버 서비스 시작·실제 주문·취소·자본 배분은 수행하지 않았다.

FR042/T038 (2026-09-11): 전략5종목 시세와 외부 기존 보유 평가 시세를 분리했다.
실제 가격 GET의 거래소만 사용해 별도 원본시각 구독을 만들고 관측 가격에서 합친다.
전략 진입의 정확한5종목 검사/주문 허용목록/기존 보유 소유 경계는 유지한다. 두 연결은
실행 잠금 뒤 시작하고 함께 닫으며 미지원ORANY를0원/조회시각 가격으로 대체하지 않는다.
단타670 passed/4 skipped(26.36초), 린트·하네스14/14·인계 통과. 시세 해석기와 거래소
탐색 소스를 실행 지문에 포함했다. 전체 회귀·서버 검증·ORANY 평가·시작 순현금/이후
변동 대조·체결 자격/T009/T010은 미완료다. 되돌림은 코드 복구이며 원본 장부 변경 없음.

FR041/T037: 전체 과거 내역을 개발 선행 조건으로 요구했던 판단을 철회했다. 최신 서버
보고서(2026-09-09T21:52:22Z)는 기존 체결5건/현재 전략 성과 체결2건을 보여 준다.
--history-db로 지금부터 계좌 GET 원본·보유·미체결·외화 구성과 기존 장부 전후 순번을
비공개 추가 기록한다. 날짜 없이 시작하고 재실행하면 이어 쓴다. 실패도 FAILED로 보존하며
인증 헤더/자유문구는 저장하지 않는다. 신규13개 포함 관련195개(1.35초)·린트 통과.
원본 보관 기능의 서버 설치, 시작 순현금 인증, 이후 현금 대조, 자격/T009/T010은 아직
미완료다. 원본 기록을 검증된 잔고로 승격하지 않는다. 기존 주문 장부/안전 경계 유지,
되돌림은 --history-db 사용 중지다. 전체 회귀는 핵심 연결 후 수행한다.

FR040/T036: 정상 전략의 매도 신호가 미체결 취소 뒤 비판단 봉에서 사라지는 오류를
수정했다. 첫 매수 체결 순번으로 보유 회차를 구분해 청산 의도/지정가를 추가 기록하고,
새 봉 없이 관리하거나 DB를 재열어도 남은 단타 수량만 원래 가격으로 처리한다.
같은5분 구간 중복과 새 보유 회차에 옛 청산 적용을 차단한다. 기록 오류는 거절하지만
명시적 중지/마감/손실 정리 경로는 보존한다. 신규19개 포함 단타650개(24.99초)·린트
통과. 전체 계좌 원본·체결/비용 자격·T009/T010은 여전히 미완료이며 전체 회귀는
이 핵심 연결 이후 수행한다. 실제 주문/취소/자본 배정/서비스 활성화는 하지 않았다.

FR038/FR039 후속: 첫 종목의 주문 대기를 신호 전체 완료로 처리해 나머지 종목을
놓치던 운용기 오류와 반복 수집 실패 시 종료 취소를 삼키던 오류를 수정했다. 실제
전략/엔진/모의 KIS로 전체·부분 체결 후 나머지 처리, 중지/노후 신호/수집 실패 차단,
종료 대기 중 취소·지속 정리·잠금 반환을 검사했다. 관련25개(3.02초), 단타631개
(24.82초)·린트 통과.
이 수정의 전체 회귀는 아직 미완료이며 전체 계좌/최종 자격·T009/T010도 남아 있다.

FR037/T033: 내부 공급자의 원본 순현금 사건을 시작 잔액부터 재계산하고 매 사건 후
잔액과 최종 잔액을 대조해 전체 보유 NAV 계산에 연결했다. 원본 순서 번호/통화/시간/
잔액 불일치를 거절하고 새 계산기 소스를 실행 지문에 포함한다. 단타624개(22.98초)
이후 지문 추가 관련72개(0.92초)·린트·하네스14/14·인계 통과. 코드f86514e 전체
4114 passed/12 skipped(761.15초), 종료0. FR038/FR039는 이 전체 결과에 포함되지 않는다.
이 계약은 KIS 원본 형식이나 원본 범위 증명이 아니다. 실제 KIS 자료 변환 제공자와
최종 자격·T009/T010은 계속 미완료이며 사용자 파일로 검증 상태를 발급하지 않는다.

FR036/T032 자체 순자산 계산: 검증된 내부 공급자의 순현금과 모든 보유의 원본 가격을
더하는 경로를 실제 계좌 변환기에 연결했다. 이 경로는 보고 NAV와 nav_verified에
의존하지 않으며 주문가능 현금과 순현금은 분리한다. 기존 검증된 보고 NAV 경로와
전체 범위/현금 검증·미확인 자산 차단·가격 신선도 검사는 보존했다. 단타603개(23.67초)·
린트 통과. 실제 엔진의 부분 체결→중지→같은 DB 재시작→청산과 비용 포함 현금/NAV를
검사했다. 코드e068ca5 전체4092 passed/12 skipped(778.61초), 종료0으로 완료됐다.
실제 KIS 순현금/전체 범위 제공자와 T009/T010은 계속 미완료다. 화면 접근은 선행 조건이 아니다.

2026-09-10 후속 T031: 같은 모의 계좌의 실제 GET 파서로 받은 주문체결/거래 명세를
기존 SQLite 장부와 읽기 전용으로 대조한다. 주문번호별 종목/방향/누적 수량/금액과
명세 그룹 합계를 비교하고, 주문별 수수료는 배분하지 않는다. 관련87개(1.05초)·린트·
하네스14/14·인계 통과. 결과는 제공된 조회 범위의 일치이며 계좌 전체/현금 인증이 아니다.
코드6a30340 전체4079 passed/12 skipped(771.87초)로 완료됐다.
T030/T031까지 이 전체 시험에 포함한다. 코드 연결/전체 계좌 계산·체결 자격·최종
통합은 계속 미완료이며 실제 주문/취소/자금 배정은 수행하지 않았다.

2026-09-10 후속 T030: 거래 명세의 정산 산술을 실제 조회/점검 경로에 연결했다.
모든 행의 거래금액·두 보고 수수료·정산금액이 맞을 때만 통화별 합계를 만든다.
불일치/무거래는 검증 합계가 없고 공개 보고서는 판정/건수만 포함한다. 관련72개
(0.96초)·린트·하네스14/14·인계 통과. 실제 현금·체결 자격은 여전히 미검증이다.
이 후속은 전체 시험의 코드를 보존하려고 별도 codex-184-fill-time-provenance 폴더에서
검증했다. 기존 전체 시험은 종료했고 원래 작업 브랜치로 변경을 합친다.
T030/T031 전체 회귀는 위 코드6a30340 결과로 완료됐다.

FR019 시험 환경 보정: 주문 수명/재시작 시험의 가짜 계좌도 전진 모의 운용의0.25%
가정 수수료를 매수·매도에 차감한다. 실제 운용기의 부분 체결→중지→늦은 체결→
재시작→청산에서 현금/순자산과 반복 조회 비용 중복 차감 방지를 확인했다.
관련73개(2.79초)·단타590개(22.36초)·린트 통과. 코드7176a14의 후속 전체 시험은
4079 passed/12 skipped(773.42초), 종료0으로 완료됐다. 실제 계좌 수수료 인증이나
T009/T010의 완료가 아니다. KIS 문의는 개발의 선행 조건이 아니다.

진행 판단 정정: 화면 접근은 프로그램 구현의 필수 조건이 아니다. permission_denied는
원본을 살펴보려던 한 경로의 실패이며 전체 개발 중단의 근거로 삼지 않는다. 화면 접근
복구를 기다리도록 했던 앞선 판단은 철회한다. 기존 API 계약·저장된 원본·장부와 모의
증권사 응답으로 계좌 입력·자격 제공자·시작/중지/재시작 통합을 계속 구현·검증한다.
실제 계좌에 필요한 입출금·환전·배당 등의 자료 범위가 확인되지 않은 부분은 해당 입력의
미검증 사유로 남긴다. 모의 검증을 실제 계좌 인증으로 승격하거나 미구현 연결을 자료
대기 탓으로 숨기지 않는다. KIS 문의와 화면 권한을 일괄 선행 조건으로 요구하지 않는다.
이미 끝난 위 전체 시험은 코드 변경 없이 다시 실행하지 않는다.

2026-09-10 후속 T029: 주문일/시각을 체결 시각으로 복사하던 오류를 제거했다. 실제
체결 시각이 없는 KIS 누적 조회는 관측 시각으로 기록하고 FILL 감사에 OBSERVED 출처를
남긴다. 기존 장부/감사는 재작성하지 않는다. 관련49개·단타567 passed/4 skipped
(32.09초)·린트·하네스14/14·인계 통과. K4 감사 모델 터치가 있다. 전체 회귀는 미완료이며
663870f 시험은 최신1e5b6a9 수정 검증으로 교체하기 위해 중단했다(종료2,
1682 passed/12 skipped,483.87초). 이는 전체 통과가 아니다. 실제 체결
발생시각·비용·전체 계좌 계산은 미완료다. 주문 복구의 시간대 해석은 추가 조사 대상이다.

2026-09-10 후속 T028: 공식 거래/수수료 원본 GET과 기존 잔고 점검의 선택적 등록일
구간 조회를 구현했다. 관련93개·린트·하네스14/14·인계 통과. 원본 행·중복·소수 수량과
수수료 항목을 보존하고 중간 조회 실패·멈춘 호출은 부분 성공으로 보고하지 않는다.
실제 조회는 실행하지 않았다. 내부 체결 대응·현금 계산·체결 동등성과 최종 통합은 여전히
미완료다. KIS 문의는 선행 조건이 아니다. T028 이후 전체 회귀는 아직 미실행이다.

2026-09-10 직전 전체 검증: 코드9a99abb,4033 passed/12 skipped(892.09초), 종료0.
인증·시세·자격 재검사·사용자 시작 명령(T024~T027)을 포함한다. 실제 KIS11개와
가동 전 상태 전용1개는 제외다. 아래 단계별 '전체 검증 대기/이전 코드 결과' 문구보다
이 결과를 우선한다. 전체 계좌 계산·체결 동등성 검증과 최종 통합은 미완료이며 머지하지 않는다.

2026-09-10 후속 T027: execution-start를 기존 장부/설정/보유 기준표·시세·봉·실제
운용기·정리 신호에 연결했다. 첫 체결 조회 전 인증을 갱신하고 라우터/주문 권한에
같은 토큰을 반영한다. 서버 승인은 DB/중지 경로·한도·허용목록·기준표 지문에도 묶인다.
관련58개·단타549개(실제 KIS4개 제외,26.53초)·린트·하네스14/14·인계 통과.
모의 계좌/자격으로 실제 엔진 중지·같은 장부 재개를 확인했으며 실제 활성화는 하지 않았다.
전체 계좌 계산·체결 동등성 검증은 여전히 미완료다. 아래 T026 이전의 '사용자 시작
명령 미완료'는 당시 기록이며 현재 시작 명령은 구현됐다. 전체3988개 검사는 이전 코드다.

2026-09-10 후속 T026: KIS 구성은 외부 자격 콜백 대신 내부 역사/전진 재검사와
별도 root 소유 서버 운영 승인 검사를 사용한다. 관련52개·단타540개(실제 KIS4개 제외,
23.98초)·린트·하네스14/14·인계 통과. 운영 승인 지문은 검토 참조이며 계산 검증이 아니다.
현재 전진 검사의 체결 동등성 false는 별도 거절하고 승인으로 덮지 않는다.
최종 자격 검증·전체 계좌 계산·사용자 시작 명령은 미완료다. 승인 발급/설치·실거래는 없다.

2026-09-10 후속 T025: KIS 구성이 같은 계좌의 시세 인증·구독 수명도 소유한다.
장부 잠금 후 구독, 중복 구독 차단, 시세 종료 시 진입 차단·기존 관리 유지,
실행 취소 시 연결 닫기·정리 재개를 실제 엔진/모의 구독으로 검증했다.
관련25개·단타516개(실제 KIS4개 제외,23.63초)·린트·하네스14/14·인계 통과.
생산 사용자 시작 명령·자격 제공자·전체 계좌 계산은 미완료이며 실제 활성화는 없다.

2026-09-10 후속 T024: 같은 실행 계좌의 인증 갱신·잔고 읽기를 구성 함수에 연결했다.
신규9개 포함 관련51개·단타513개(실제 KIS4개 제외)·린트·하네스14/14·인계 검사 통과. 모의 HTTP로 실제 인증/잔고
파서/엔진의 연결과 미검증 범위 차단을 검증했다. 아래 전체3988개 결과는 이 변경 이전이다.
전체 계좌 검증 공급자·전략 자격 공급자·사용자 시작 명령은 계속 미완료다.

2026-09-10 최신184 검증: 코드4c989eb 전체3988 passed/12 skipped(786.05초), 종료0.
전략 서명 등록과 공통 가격 계산을 포함한다. 제외는 실제 KIS11개와 가동 전 상태 전용1개다.
아래 단계별 과거 기록보다 이 전체 결과를 우선한다. KIS 문의 답변은 개발의 선행 조건이
아니다. 스펙182 US8~US11에 따라 공식 잔고 직접 조회·자체 산술 대조·자산 분류가 이미
구현됐다. 문의 접수나 개인정보 입력을 프로그램 완성의 필수 작업으로 안내하지 않는다.
T009/T010의 남은 작업은 생산 계좌 입력·전략 자격 제공자·사용자 실행 시작의 구현과 통합
검증이다. 보고서 산술 일치가 장중 가용현금·전체 계좌 순자산의 실행 적합성을 자동으로
증명하지는 않는다(FR-028/FR-037). 남은 입력 계약과 검증을 구현하되 임의 현금·자산 값이나
시험용 자격을 생산 입력으로 바꾸지 않는다. 이전 문의 대기 기반 막힘 판단은 철회한다.
프로그램 개발 지시와 실제 투자 실행은 계속 구분하며 실제 주문·자금 배정은 수행하지 않았다.

목표 기능으로184 프로그램 완성을 추적 중이다. 운영자는 실제 투자 실행이 아니라
사용자가 운용할 프로그램 개발·검증을 요청했다. 미검증 거래 자격과 미구현 프로그램을
구분하며 실제 주문·자금 배정·실거래 활성화는 수행하지 않는다.
FR019/T017 self-test는 계좌 키 없이 실제 엔진·운용기·임시 DB로 부분 체결·재시작·
중지 정리를 검증한다. 신규4개 포함 관련32개(7.02초)·린트 통과, 전체는 후속 구현 후
검증한다. T009/T010의 계좌·자격·시작 연결은 계속 구현 대상이며 완료 표시하지 않는다.

FR020/T018은 프로그램 내부 계좌·발생시각 시세 공급자를 기존 Observation으로
연결했다. 신규29개 포함 관련124개(2.57초)·린트 통과. 모의 공급자에서 실제 엔진의
접수·부분 체결·오래된 시세 중 취소 허용/가격 주문 차단·최종 정리까지 검증했다.
현재 KIS observe_account의 미검증 상태는 ACCOUNT_SCOPE_UNVERIFIED로 차단된다.
이 소비자는 검증 플래그를 발급하지 않으며 현금/NAV 생산 인증·실행 시작을 완료한 것이
아니다. 다음은 계좌 공급자의 실제 의미 계약과 시작 구성·전략 자격을 연결하는 작업이다.
실행 구성에서 새 관측/권한 소스를 실행 식별자에 포함하는 검증도 함께 수행해야 한다.

FR021/T019는 이전 합격 보고서 대신 원본 보관 자료를185/177 경로로 재계산해 후보를
선택한다. 신규8개 포함 관련97개(3.94초) 통과. 실제 부족 자료가 위조된 합격 보고서를
무시하는 경로, 사전등록 고정·타 공급자 미변환·원본 보존을 검사했다. 성공 선택 단위 시험은
모의 계산 결과이므로 실제 전략 합격으로 해석하지 않는다. 실행 지문에는 관측/선택 소스와
후보 실제 매개변수 전체를 포함했다. 전진 관측·체결 동등성·생산 자격·시작 구성은 남았다.

FR022/T020은 연구 선택·계좌 관측·명시적 자격 검사·라우터·운용기의 내부 시작 조립을
구현했다. 신규13개 포함 관련101개(3.06초)·린트 통과. 실제 엔진/모의 전송에서
시작→부분 체결→중지→DB 재열기→늦은 체결→정리를 검증했고 계좌·자본·지문 변경과
승인 검사 실패를 차단한다. 실제 자격 제공자·CLI 시작은 여전히 미구현이며 이 조립 함수는
승인을 발급하지 않는다. 다음은 자격 제공자의 검증 계약과 사용자 시작 명령 연결이다.

FR023/T021은 모의 장부를 읽기 전용 복사해 원본 봉과 수신시각으로 독립 재생한다.
고정 이후의 완결·제때 수신·중단/마감 잔여 없는 거래일만 센다. 신규8개 포함 관련38개
(5.10초)·린트·하네스14/14·인계 통과. 현금 조작 후 해시를 다시 만든 기록도 거절했다.
고정 시각 인증·실제 체결 동등성·생산 자격 발급은 수행하지 않으며 사용자 시작 연결은
계속 미완료다. 2026-09-10 코드89eea0e의 전체 회귀는3969 passed/12 skipped,
774.75초·종료0이다. 제외는 실제 KIS11개와 가동 전 상태 전용1개이며 실제 계좌 검증
증거가 아니다. 린트·하네스14/14·인계 사실 검사도 통과했고 PR793은 초안으로 유지한다.

FR024/T022는 서버 키에 묶인 연구 고정 등록과 서명 검증을 구현했다. 등록자는 원본
연구를 재검사하고 실제 현재 코드/시각을 서명한다. 별도 고정 키 경로의 root 소유·권한·
32바이트를 확인하며 키 설치/실제 등록은 실행하지 않았다. 신규11개 포함 관련40개
(4.11초)·린트·하네스14/14·인계 통과. 이 변경은89eea0e 전체 시험 이후이며 새 전체
결과를 주장하지 않는다. 서명된 서버 시각은 검증하지만 체결 동등성·주문 권한은 별도다.
판단 정정: 헌법 IX.D는 운영자 지시 아래 절차 규칙 개정을 허용한다. X.4의 기존 하루1회
권한을 단타에 재사용하면 안 되지만, 그 규칙 자체를 새 프로그램 개발 금지로 삼지 않는다.
생산 계좌 공급자·키 배치·단타 권한 제공자·시작 명령은 계속 미완료다.

FR025/T023은 모의/실행 가격 차이를 재현해 공통 Decimal 계산으로 보정했다.
100달러 매수100.05→100.06, 25달러 강제 정리24.98→24.99이며 기존6bp 계약에 맞춘다.
신규8개 포함 단타504 passed/4 skipped(32.18초)·린트·하네스14/14·인계 통과.
공통 가격 소스도 모의/실행 지문에 포함해 이전 모델 장부와 섞지 않는다. 기존 장부는
보존하고 새 모델은 새 장부에서 관찰해야 한다. 이 보정은 수수료·실제 체결의 전체
동등성이나 생산 시작 완료가 아니다. 최신 전체 시험은 여전히 이전89eea0e의3969/12다.
...
실제 서버 검증은185/main8e2e5ce에서 완료했다. 완결2일·부족754일·누락0일이며
실제11개 읽기 시험·배포·감사를 통과했다. 아래 현재 결과에 실행 기록을 남겼다.
...
린트·하네스14/14·HANDOFF 검사는 통과했다. 코드`3756aee`의 전체 검사는
3767 passed/10 skipped, 실패0(762.39초)다. 제외는 실제 KIS9개와 가동 전 조건1개다.
...
운영자는 기존 한국투자 계좌 계속 사용을 확정했다. 계좌 선택을 다시 묻지 않는다.
후속 구현은 기존 외부 보유 기준표를 단타 실행 엔진의 공용 체결 장부 대조에 연결한다.
기존 보유는 전체 계좌 노출에 포함하지만 단타 소유·손익·청산 수량에서는 제외한다.
부분 체결·취소 후 재시작에서도 기존 주식을 남기는 신규25개를 포함해 관련196개 검사를
통과했다. 후속 수정본 전체 검사는3792 passed/10 skipped(796.09초), 실패0이다.
기존 계좌의 전체 현금/NAV 관측과 실주문 운용 연결이 완료된 것은 아니다.
현재 전체 현금/NAV 대신 시작 잔고를 추정하는 우회는 없다.
기존 계좌 실제 읽기 재검증34240823757(main292805f)은9개 통과(16.77초)다.
미체결0, 미해석 자산1개, 분류되지 않은 증거금34행이며 전체 현금/NAV 검증은false다.
단일 USD 비교EQUAL·조회 전후 통화 값 안정성은 확인됐지만 전체 계좌 관측 완료를 뜻하지 않는다.
자본 변경·실주문0이며 개발 브랜치의 시세 입력기를 인증한 검증과도 구분한다.

2026-09-09 후속 구현은 `execution/intraday_runtime.py`에 실제 주문 엔진의 운용 루프를
연결했다. 봉 수집과 독립된 주문 관리, 추가 전용 중지·정리 기록, 중지 직전 신규 주문 차단,
DB 경로에 묶인 잠금과 재시작 복구를 추가한다. `execution-status/execution-stop --db`는
이 운용 루프를 제어하며 기존 `run/status/stop`은 계속 모의 운용이다.
시험 KIS 응답과 실제 주문 엔진을 연결한 신규 통합19개 및 소스 식별자 회귀1개를 추가했다.
최종 관련51개가 통과했다(2.51초). 전체3812 passed/10 skipped, 실패0(806.13초),
린트·하네스14/14·HANDOFF 사실 검사도 통과했다. 실제 계좌 주문·취소·자본 변경은0이다.
생산 KIS 관측 공급자와 적격 권한을 갖춘 엔진을 생성하는 연결은 여전히 없다.
따라서 명령줄 실주문 시작은 아직 제공하지 않으며 T009/T010을 완료 표시하지 않는다.

main2d79712의 US9 산술·진단과 US10 전체 자산 조회/실제 인계를 통합했다.
통합 코드90c7dfc에서 관련254 passed(12.83초), 전체3886 passed/11 skipped,
실패0(790.54초), 린트·하네스14/14·인계 검사를 통과했다. 제외11개는 실제 KIS10개와
가동 전 조건1개다. 새 읽기 코드는 main d2528ad의 실제34298665338에서10개 조회
시험을 통과했다.19개 자산 분류·5개 열 합계는 일치했지만 별도 순자산 요약과 차이가
있으며 해외 원화 합계3개도 불일치다. 이 기록은 통합 코드 검증 완료이며 생산 관측·
적격 권한 연결(T009/T010)이나 실제 자금 배정·단타 가동 완료가 아니다.

후속 main6e66d04(PR799)의 US11 보고서 버전2를 통합했고 관련305개(9.03초),
린트·하네스14/14·인계 검증을 통과했다. 이 통합에서 전체3886 검사를 다시 실행하지는
않았다. 변경된 main 코드30d27ed는 전체3787/11(819.92초) 통과이며 기존184 전체
증거와 이번 관련 통합 검증을 구분한다. 실제34307196553은10개 조회 시험 통과,
자산표6일치·해외 내부15일치/1미확인, 미입증 교차 숫자 비교4개 DIFFERENT다.
T009/T010과 현재 진단 수집기의 정식 전진 검증 부재는 이 보정으로 완료되지 않는다.

185 후속: 날짜별 보관 자료를 기존177 연구 검증기로 연결하는 독립 읽기 명령을 구현했다.
실제 세션 수·부족분·중간 누락을 원본과 CSV에서 계산하고 원본을 바꾸지 않는다.
실제34353848847에서 현재 수집기의 완결2일·부족754일·기간 내 누락0일·불완전 폴더0개,
합성 아님·KIS 부분 시장 자료와 INSUFFICIENT_EVIDENCE 판정을 확인했다.
실주문·정식 전진·NAV 승격이 아니며 과거2일을 정식 전진 실적으로 세지 않는다.
신규17개·전체3804 passed/12 skipped(781.43초), 린트·하네스14/14·인계 통과.
184 통합195c23d는 별도 전체3906 passed/11 skipped(796.72초)를 통과했다.

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `dda232f` — Merge pull request #853 from jinooaction/codex/196-archived-event-evidence |
| main 테스트 | 코드1b02509 전체4989 passed/13 skipped(793.89초), 관련62개. 이후 문서만 변경. 생략은 실제KIS12개·가동 전 전용1개.196은 브로커 코드 미변경. |
| main 린트 | 코드1b02509의 `uv run ruff check src tests`와 신규 보관자료CLI 통과. 하네스14/14·인계 사실 검사 통과. |
| 열린 PR | #827 유료 공급자 초안 보류.196 PR853은 병합 완료; 출시 근거 문서 후속 반영. |
| 출시 완료 스펙 | 196 과거 보관 원문·수집 기록·조건부 시각 검사기와195 로컬 관측 도구를 main에 병합.194 실제 후보는 탈락.184 고정 예산 운영·관측 구간 체결·정산,182 계좌 조회,183 모바일 운영판 유지. 전체 단타 실사용 목표는 별도다. |
| 골격 스펙 | 스펙 178은 코드·계약·시뮬레이터와 실제 iPhone 설치·실행까지 완료됐다. 물리 화면 픽셀 캡처와 터치 탐색은 남았다. 스펙 177은 실제 1,645세션 연구 재생을 완료했으나 거래 수와 비용 후 성과가 미달이다. 스펙 176은 실제 KIS 자동 주문·체결·감사·정합과 중복 주문 0건 확인이 남았다. |
| 최근 출시 작업 |196 PR853, 배포36084456885·감사36084571623 성공. target dda232fbe076 완료 확인. 기존 수집기·주문 권한 유지. |
| 활성 작업 |196 입력 검사 출시 완료 후 과거 사건 자료 범위·처리 지연·종목 연결을 연구 입력에 연결. 최소31가설 실패를 보존하며 새 성과 가설은 미선정. 최종확인495일 미개봉. 통과 전략·전진 관찰·실행 동등성·실제 운영 검증이 남음. |
...
## 현재 결과 — 실제 보관 자료의 연구 검증

PR801/main8e2e5ce의 실제34353848847은11 passed/24.82초다. 원본과 CSV를
검증한 완결2일이 기존177 연구기로 전달됐고 부족754일·기간 내 누락0일이었다.
불완전 폴더0개, synthetic=false, provider=kis-nasdaq-partial-unadjusted다.
판정은 INSUFFICIENT_EVIDENCE이며 actual orders_submitted=0, live_eligible=false다.
배포34353848826 성공, 감사34353974622에서 b54c103→8e2e5ce의 시작·완료2행과
DEPLOY_COMPLETED(live,2026-09-09T12:55:46.917Z)를 확인했다. 이 배포 상태는
단타 실주문 시작을 검증한 것이 아니다. 계좌 생산 입력·적격 권한·전진 증거는 그대로 남았다.
184는0471431로 통합했고 두 실행 명령을 포함한 관련38개·린트·하네스14/14를 통과했다.
184 전체3906/11은195c23d 기준이며 이후 독립 명령 통합에는 관련 검증을 적용했다.

## 이전 결과 — US11 검증과 숫자 비교 분리

원본 숫자 차이를 계좌 오류로 해석할 수 있는 분류를 수정한다. 공식 명세로 동등한
집계식이 입증되지 않은 해외 원화3개·분류표/별도 순자산1개는 comparisons로 옮긴다.
보고서 내부 합계·필드·안정성은 checks와 status로 유지한다. 버전2의 EQUAL/DIFFERENT는
수치 관계일 뿐 계좌 오류도 실시간 NAV 인증도 아니다. 과거 실행 기록은 당시 버전1 결과다.
금액·계좌·원문 비노출, 전체 행 보존, 실제 합계 오류·조회 변화 거절은 유지한다.
관측 보고서만 고치며 실제 주문·취소·자본·서비스 수동 시작은 수행하지 않는다.

관련119개와 전체3787 passed/11 skipped(819.92초), 린트·하네스14/14·인계 검증 통과.
서버 진단 수집기는 관측34306360635에서 active·신선도3초·WAIT_SESSION,
2026-09-08 자료 보관 COMPLETE였다. qualified_forward_sessions=0은 진단용 상태의
고정값이며 측정한 누적 실적이 아니다. 수집기를 켜 두는 것만으로 정식60세션을 채우지 않는다.

PR799/main6e66d04를 정상 병합했다. 실제 자동34307196553은10 passed/21.49초다.
버전2 자산표는19분류·비영3분류, 안정성+5개 열 합계6개 모두 MATCH다.
해외 내부 검증은15 MATCH/1 INCOMPLETE/0 MISMATCH이며 요약 손익의1원 이내 차이를
합격시키지 않았다. 별도 comparisons의 해외 원화3개와 순자산1개는 DIFFERENT,
equivalence_verified=false다. 원래 숫자 차이를 삭제하거나 회계 원인을 확정하지 않았다.
배포34307196519 성공, 감사34307267412에서2d79712→6e66d04의 시작·완료2행과
DEPLOY_COMPLETED(live,2026-09-09T03:28:07.744Z)를 확인했다. 실주문 가동 증거는 아니다.
#793은 f737923에 통합했고 관련305개(9.03초)·린트·하네스14/14·인계 검증을 통과했다.
184 전체 검사는 기존90c7dfc의3886/11이며 이 통합에서 전체 재실행은 하지 않았다.

## 이전 진행 — 2026-09-09 KST (스펙182 실제 산술 대조)

기존 공개 결과의 cash/nav=false는 실제 수치 불일치를 검사한 결과가 아니라
전체 현금 합산·실행 시각을 증명하지 않는다는 고정 제한이었다. 새 report_audit는
체결수량×가격, 외화 평가−매입, 외화 금액×환율의 원화 합계와 요약 손익을 실제 비교한다.
현금·미결제·대출·총자산 필드의 누락과 숫자 오류도 구체적으로 표시한다.
모든 행을 검사하고 요약에는 최대100개 항목과 상태별 총개수만 공개한다.
금액·종목·계좌·원문은 공개하지 않는다. OTCB와 소수 수량을 제외하지 않는다.
MATCH는 보고서의 검사 대상 산술 관계 일치이며 전체 실행 NAV의 합격이 아니다.
공식 명세에 없는 합산식·환율 단위·반영 시각은 추정 통과시키지 않는다.
현재 기능은 독립 읽기 전용 변경이며 실제 주문·취소·자본 변경은0이다.

PR794/main cd17fb9 이후 실제 KIS34291826106은9 passed/21.48초다.
2개 자산(OTCB 포함)의 수량×가격·평가−매입은 모두 일치했고 세 출력은 안정됐다.
전체19개 산술 검사 중15일치·3불일치·1미확인, 숫자 누락0이다. 원화 매입·평가·
손익 합계가 불일치하며 요약 손익은1원 이내 반올림 차이로 미확인이다. 이는 새로운
수치 증거이며 기존 상수 cash/nav=false와 다르다. 증권사 오류로 단정하지 않는다.
배포34291826077 성공, 감사34292367400에서292805f→cd17fb9의 시작·완료2행과
DEPLOY_COMPLETED(live,2026-09-08T23:42:56.157Z)를 확인했다. 단타 가동 증거가 아니다.
#793에는 main cd17fb9를91a9a68로 반영했고 함께 실행한 관련192개가 통과했다.

후속 PR795/main478a2f6은 단위·정밀도 가설 진단을 추가했다. 코드14ddfe2의 전체
3741/10(755.31초)·관련73개·린트·하네스14/14·인계 검사를 통과했다.
실제 자동34293409761은9 passed/27.62초, 결과 게시 성공이다. 원화3개 모두 환산
계산보다 작으며 외화 원합과도 다르고 각 행1센트 환산 합 범위를 넘는다. 이를 증권사
오류로 확정하지 않는다. 배포34293409755 성공. 수동 중복34293410684는9 passed였지만
결과 게시에서 동시 갱신 충돌했다. 자동 실행이 이미 있으므로 수동 재검사를 중복 예약하지 않는다.

## 이전 진행 — US10 선택 계좌 전체 자산 분류

기존 해외잔고와 다른 공식 CTRP6548R을 추가한다. 공식 일반20행/21번 상품17행
분류표의 국내외 자산·외화·예수금과 마지막 합계를 두 번 GET으로 읽는다. 금액5개
열의 합·요약 순자산·안정성7항목을 대조하고 공식 분류 수와0이 아닌 분류 수를 공개한다.
기존 balance-check 명령에 별도 account_assets를 연결하고 기존 서버 읽기 시험에도
추가했다. 새43개 포함 관련116개, 전체3784 passed/11 skipped(776.90초), 린트,
엄격 하네스14/14와 인계 검사를 통과했다. 첫 전체 실행 중 인계 기준이 이전 main을
가리켜 실패한2개는 문서 수정 후 전체 재실행으로 해소했다. 동일 선택 계좌의
결제기준 보고서이며 다른 계좌 전체·실시간 USD 현금/NAV를 증명하지 않는다.
추가 자본·주문·가동은0이고 금액·계좌·원문은 공개하지 않는다.

PR796/main d2528ad의 실제 자동34298665338은10개 읽기 시험을 모두 통과했다.
소요시간 앞부분은 마스킹되어 복원하지 않는다. 공식19개 분류·금액이0이 아닌3개
분류를 확인했다. 두 조회가 안정적이고 매입·평가·손익·신용대출·순자산의 분류 합5개가
합계 행과 정확히 일치했다. 마지막 table_net_assets_vs_summary는 MISMATCH다.
이는 분류표의 real_nass_amt 합계와 별도 output2.nass_tot_amt의 숫자가 다르다는
관측이며 증권사 오류나 같은 집계 기준을 확정한 결론이 아니다. 전체 보고서는6일치·1불일치다.
기존 해외 보고서는15일치·3불일치·1미확인을 유지한다. 공개 명세는 두 요약값의 상세
집계식·장중 평가 발생시각을 제공하지 않아 실시간 USD NAV로 승격하지 않았다.

배포34298665322 성공. 감사34298828715에서478a2f6→d2528ad의 시작·완료2행과
DEPLOY_COMPLETED(live)를 확인했다(2026-09-09T01:19:14의 소수초 일부 마스킹).
이번 후속 기록은 같은 실행 코드를 보존하며 실제 자금 배정·주문·단타 가동을 뜻하지 않는다.

## 이전 진행 — 2026-09-08 KST (스펙182 직접 잔고 실제 검증 완료)

공식 체결잔고 CTRP6504R과 결제잔고 CTRP6010R을 체결→결제→체결 순서로
읽는 명령 `uv run python scripts/intraday_balance_check.py`를 추가했다.
기존 서버 KIS 검사에도 같은 GET 수집기를 연결했다. 금액·계좌·보유종목은 공개하지
않고 출처별 행 수, 페이지 완료, 통화 응답 안정성, 단일 USD 필드 수치 비교만 남긴다.
누락된 종료 헤더, 반복 페이지, 30초 초과는 조회 실패이며, 조회 중 통화행 변경은
관측 사실을 보존하되 수치 비교를 불가로 남긴다.
코드08ef16b에서 관련116개·전체3707 passed/10 skipped(1079.30초), 린트,
하네스14/14·인계 검사를 통과했다. 제외10개는 실제 KIS9개와 가동 전 조건1개다.
종료 보정 코드344a352의 전체3710/10·관련119개·린트·하네스14/14·인계를 통과했다.
PR791/main8196656 병합 후 실제 KIS34201399851은9/9 통과했다.
세 조회 모두 각 output1/2/3은1행, USD1행, 공란 통화0, 종료는
NO_CONTINUATION이다. current_read_stable=true, 수치 비교EQUAL을 확인했다.
cash/nav/full_account_scope/live는 모두false다. 실행시간 숫자의 일부는 Actions에서
마스킹되어 복원하지 않는다. 금액·계좌·보유종목은 공개하지 않는다.

배포34201399881 성공, 감사34201503586은 1cb1af0→8196656의
시작→완료(live,2026-09-08T07:51:29.851Z)2행을 확인했다. 상관값은 일부 마스킹됐다.
서비스34201506372는 생산8196656, timer active/service success,
WAIT_SESSION/20.8초, 원본 COMPLETE/2026-09-04, 실제 주문0·정식 전진0이다.
이 인계 갱신은 실행 코드와 설정이 같은 문서 변경이며 새 배포를 요구하지 않는다.

### 직접 조회 첫 검사와 보정 경과

PR790/main1cb1af0 배포34193363952 성공. 실제34193364010은 기존8개 통과,
새 잔고1개 BALANCE_CONTINUATION_HEADER 실패다. 공식 구현이 성공 응답의 M/F만
다음 조회하고 나머지를 종료하므로 명시적 공란을 NO_CONTINUATION으로 수용하도록
보정한다. 누락 헤더는 계속 실패한다. 이전 오류만으로 실제 공란임을 단정하지 않고
후속 실제 결과의 종료 분류를 확인한다. 코드344a352에서 관련119개,
전체3710 passed/10 skipped(1323.66초), 린트 통과.
병합 전 서비스34193202991은 active/success, WAIT_SESSION/17.7초,
원본 COMPLETE/2026-09-04, 실제 주문0·정식 전진0이다.

수치가 일치해도 두 API의 보고 기준과 반영 시점은 다르다. 공식 문서가 미국장 중
일부 계좌의 잔고 반영 지연을 명시하므로 cash/NAV/live 판정은 false로 유지한다.
문의 발송은 선택 사항이며 기다리지 않는다. 실제 자금 배정·주문·위험 관문·기존
실행기는 바꾸지 않았다. 문제가 생기면 새 읽기 명령과 선택 검사를 제거하면 되며
기존 계좌 조회와 주문 방어는 독립적으로 남는다.

### 이전 진행 — 스펙183 모바일 자금·전략 운영판 출시

백엔드 PR #788은 main `7a915c19`로, 비공개 Flutter 앱 PR #3은
`2801cd1bee83a1bd9a08675611b62379cf08174d`로 병합됐다. 공개 상태 발행
34186033060과 건조 실행 워커 배포 34186033089가 성공했다. 공개
`status.json`에서 자금·전략 필드와 생성 시각 `2026-09-08T04:10:23.836997Z`를
직접 확인했다. 현재 공개 값은 저빈도 캐너리 운용 한도 143달러·NAV 대비 10%·단 1,
확인된 실제 투자금/손익 0달러, 준비 전용 단타 예산 600달러·하루 손실 정지 12달러다.
계좌 총자산·현금·평가액은 집계 계약이 미검증이므로 null/확인 대기로 유지한다.

활성 전략 `globalfixed-ensemble-3-6-9-12`는 운영 배관 검증용 캐너리이며 수익 우위는
미확정이다. 전진 관측은 9/40, 요구 PSR은 0.8이고 대상은 IAUM·SCHX다. 단타는
`PREPARATION_ONLY`, 주문 0건이며 여섯 차단 조건을 그대로 표시한다. 연구 후보
`pead-ann06of07-scale100-fd242e072b8b`는 과거 판정만 남았고 보고가 135.1시간 지나
`STALE`로 분리 표시된다. 앱 전체 상태가 주의인 이유도 이 비핵심 연구 보고 지연이며,
핵심 돈 경로는 막히지 않았다는 설명을 홈에 유지한다.

Flutter 24개 시험, 정적 분석, 무서명·서명 iOS 출시 빌드를 통과했다. iOS 26.5
`Codex iPhone 17` 시뮬레이터에 설치·실행하고 실제 공개 자료로 홈·자금·전략·시스템
네 탭을 화면 확인했다. 실물 `꾸꾸까까의 iPhone`은 CoreDevice에서 `unavailable`이며,
잠금 해제 후 케이블 또는 같은 네트워크 연결·신뢰·개발자 모드가 필요하다. 따라서
실기기 설치·프로세스·네 탭·잠금/보호막 검증은 완료로 바꾸지 않는다.

이 출시는 기존 자료를 읽어 보여주기만 한다. 주문 제출, 자본 배정, 전략 승격,
허용 종목, 비밀값, 감사 로그를 변경하지 않았다. 배포 성공도 실거래 전환을 뜻하지 않는다.

### 이전 진행 — 스펙182 다중 USD 조회 보정

후속 실행 관측 보정: 생산 연결을 비활성 factory로 감추지 않고 실제 빠진 두 입력을
수정한다. Observation의 매도가능수량과 증권사 가격 발생시각이 필수이며, 새 주문은
최종 전송 직전에도 시세 나이를 검사한다. 기존 취소는 시세 나이와 분리하고 계좌
신선도·대사·권한을 유지한다. a18ebdb에서 관련81개, 전체3660 passed/9 skipped
(767.15초), 린트·모의600/12·하네스14/14·인계 검사 통과. PR785/main2243b8b 병합,
배포34180701901 성공. 감사34180755506은 상관값50eed028b5d55c73b045471c989d4978의
시작→완료(live,2026-09-08T02:37:58.188Z)를 확인했다. 서비스34180756933은
timer active/service success, WAIT_SESSION/7.7초, 원본 COMPLETE/2026-09-04,
주문0·정식전진0이다. 서버 커밋 출력 앞부분은 자동 마스킹됐으며 표시된 뒷부분이
배포 대상2243b8b2c2bf849cc4b5718b1144b8446bb926f6과 일치한다.
T030 구현은 완료했지만 T011 생산 adapter가 완료된 것은 아니다.

KIS 문의는 [문의 초안](specs/182-intraday-kis-execution/capital-review/kis-cash-contract-inquiry.md)의
문의 내용6개 질문 발송을 운영자가 2026-09-08 명시 승인했다. 같은 허락을 다시 묻지 않는다.
계좌번호·키·금액·보유종목은 보내지 않는다. 구 Q&A 게시판은 공식 FAQ상 운영 종료이며,
현재 창구는 한국투자증권 고객의 소리다. 회원 경로는 보안프로그램 화면으로 이동했고,
비회원 경로는 이름·생년월일·휴대폰번호 수집 및 5년 보유 동의를 요구한다.
개인정보 단계는 운영자 직접 처리를 요청했으며 아직 본문 입력·발송·접수번호는 없다.
개발자센터 로그인 요청은 잘못된 접수 경로여서 철회했다.
이 문의는 조회 완료를 다시 막는 조건이 아니라 실제 자금 계산의 필드별 의미·집계·시점 확인이다.
T010(실자료·전략합격·전진60세션), T011(생산 연결), T012(실제 체결), T027(집계/NAV)은
현재 근거 없이 완료로 바꾸지 않는다. 문의 승인만으로 이 조건이나 개인정보 동의를 대신하지 않는다.

### 완료된 조회 보정

USD 행이 하나여야 한다는 근거 없는 조건을 제거하고 모든 구성금액 행을 검증·보존한다.
집계 금액은 추정하지 않으며 단일 금액 필드는 여러 행일 때 None이다. 공개 결과에
조회 완료와 미검증 사유 목록을 분리했다. 자산 경고가 현금 경고를 숨기지 않는다.
41d5770 코드에서 관련113개, 전체3629 passed/9 skipped(805.92초), 린트,
하네스14/14·인계 검사 통과. PR783 병합·배포와 실제 GET 재검증까지 완료했다.

**최종 확인:** PR783/main a44794d, KIS34170209460 실제8/8(21.32초).
USD10행·공란통화34행·미검증 자산1개를 구분하여 조회 완료를 확인했다.
여러 행을 합산하거나 계좌 순자산으로 인정하지 않는다. T020/T028 완료이며
T027 집계/NAV와 T010~T012 전체 실거래는 별도다.
배포34170209451 성공, 감사34170285836은 상관값8f841e96c62eafbfe1fc4b57adedac2f의
시작→완료(live,2026-09-07T23:30:03.644Z,a44794d)를 확인했다.
서비스34170287557은 생산a44794d, timer active/service success,
WAIT_SESSION/45.6초, 원본 COMPLETE/2026-09-04, 주문0·정식전진0이다.
이 후속 인계는 문서만 변경하며 전체 검사를 통과한 src/tests와 동일하다.

### 이전 준비 단계 기록 (2026-09-07)

600달러·하루 손실 정지12달러는 준비 기준으로 확정됐고 같은 금액을 다시 묻지 않는다.
확정 설정·600달러 모의 주문 수명·GET 계좌 조회와 진단 코드는 PR775~781로
병합됐으며 생산 d602525까지 정상 배포됐다. 실제 자금 변경·주문은0이다.
마지막 코드02191df의 전체3619 passed/9 skipped(767.73초), 관련103개,
린트·하네스14/14·인계 검사를 통과했다. 제외9개는 실제 계좌 읽기8개·가동 전 조건1개다.

실제 KIS34077759009는7개 통과/새 계좌1개 실패(21.96초)다. 페이지·OTCB·공란통화
처리를 넘어 USD10행을 확인했고 USD_MARGIN_ROW_COUNT로 차단했다. 공식 출력의
국가명/통화는 유일성이 보장되지 않으며 합산·중복 제거·대표 선택 규칙도 없다.
API 통신 실패가 아닌 금액 해석 미확정이다. 첫 행 선택이나 단순 합산으로 통과시키지 않는다.
KIS 확인 요청 초안: specs/182-intraday-kis-execution/capital-review/kis-cash-contract-inquiry.md.
초안은 외부에 보내지 않았다. T020/T027 및 전체 실거래 T010~T012는 미완료다.

배포34077759027 성공, 읽기 감사34077931381은 상관값
c4fd51c6a4d6538bac80336dd845d559의 DEPLOY_STARTED→DEPLOY_COMPLETED
(live,2026-09-07T02:53:36.818Z,대상d602525)를 확인했다. 서비스34077932147은
생산 d602525, timer active/service success, 원본 COMPLETE/2026-09-04,
WAIT_SESSION/13.9초, 주문0, qualified forward0이다. 서버 진단 수집은 계속 동작한다.
이를 정식 전략 전진60세션이나 새 단타 실주문 가동으로 보고하지 않는다.

### 이전 보정 경과

최신 PR780/d05b92e는 전체3615 passed/9 skipped(779.63초), 관련99개와
린트·하네스14/14·인계를 통과했고 배포34076749564도 성공했다. 실제 읽기
34076749615는 INVALID_MARGIN_CURRENCY에서 거절됐다(기존7개 통과,20.82초).
공식 동일 endpoint/TR 예제가 빈 crcy_cd 행을 제외하므로 FR-021에서 이를
별도 집계하고 USD/합계/현금0으로 추정하지 않는다. 누락/null/형식 오류는
계속 거절하며 비밀값 없는 구조 진단만 추가한다. 관련103개 통과,
전체 회귀·병합·실제 서버 읽기는 진행 중이다.

최신 PR779/0b8992b는 전체3605 passed/9 skipped(799.88초), 관련89개,
린트·하네스14/14·인계를 통과했고 배포34075774244도 성공했다. 실제 읽기
34075774149는 OTCB 처리를 넘어 USD_MARGIN_ROW_COUNT에서 실패했다
(기존7개 통과,19.77초). FR-020에서 유효한 USD 내역 미제공은 None/경고로,
중복/잘못된 코드·금액은 오류로 구분하고 통화 공백을 정규화한다. 현금0이나
구매가능금액으로 치환하지 않는다. 관련99개 통과, 전체 회귀·병합·실제 재검증 중이다.

최신 PR778/e6674c8은 전체3593 passed/9 skipped(801.91초), 관련77개와
린트·하네스14/14·인계를 통과했다. 실제34074654444에서 inquire-balance가
OTCB를 반환해 거절된 것을 확인했다(기존7개 통과,20.36초). 공식 응답 코드의
정확한 매핑은 미확인이다. 스펙182 FR-019로 이 행을 별도 미검증 자산에
보존한다. 정상 보유나 주문 대상에 합치지 않고 가치0/NAV를 합성하지 않는다.
공개 결과도 INTRADAY_ACCOUNT_READ_WITH_UNVERIFIED_ASSETS로 표시한다.
수량·통화·중복·미체결 시장 차단은 유지했고 관련89개 통과, 전체 회귀·병합·
실제 서버 읽기는 진행 중이다.

최신 PR777/2745f99는 전체3591 passed/9 skipped(762.39초), 관련75개,
린트·하네스14/14·인계를 통과했고 배포34073693771도 성공했다. 실제 읽기
34073693815는 NON_US_MARKET/OTHER로 여전히 거절됐다(기존7개 통과,20.70초).
NAS/공백 보정이 실제 원인을 해결했다고 보고하지 않는다. 허용 범위를 유지하며
실패 endpoint와 제한된 공개 거래소 코드만 진단하는 후속을 추가했다. 숫자 계좌번호·
긴 토큰·자유문은 숨기며 관련77개 통과, 전체 회귀·병합·실제 진단은 진행 중이다.

최신 보정 PR776/72e8b03은 전체3581 passed/9 skipped(801.36초), 관련65개,
린트·하네스14/14·인계·PR 검사를 통과했다. 정상 배포34072779467도 성공했다.
실제 읽기34072779392는 페이지 오류를 넘었지만 NON_US_MARKET에서 실패했다
(기존7개 통과,16.39초). 공식 NAS=나스닥 코드와 고정 문자열 공백 처리를
`codex/182-account-exchange-code`에서 보정했다. 다른 시장/빈값/잘못된 자료형
거절은 유지하며 원문 대신 닫힌 분류만 진단한다. 관련75개 통과, 전체 회귀·
병합·서버 실제 읽기는 진행 중이다. 실제 실패 원문 코드는 공개하지 않았으므로
정확한 응답 형태를 확인한 것처럼 말하지 않는다.

최신 후속: 운영자가 "응 확정하고 완성해줘"로600달러·하루 손실 정지12달러를
다음 준비 단계 기준으로 확정했다. 같은 금액을 다시 질문하지 않는다.
PR775/224558c에서 confirmed-budget.json과
`scripts/intraday_execution.py preflight`를 구현했다. 실제 실행기에600달러를
적용한 가상 주문 수명, 비용을 포함한12달러 손실 정지 경계를 검증했다.
KIS 미국 정수주 보유·미체결·USD 외화구매가능금액·외화증거금GET 수집기를 추가했다.
모든 페이지와 숫자/통화/전체30초 신선도를 검사하며 NAV는 미검증으로 남긴다.
관련62개와 전체3578 passed/9 skipped(749.00초), 린트·하네스14/14·HANDOFF 검사가
통과했다. 9개 제외는 실제 계좌 읽기8개와 가동 전 사다리 조건1개다.
정상 배포34067977033은 성공했고 읽기 감사34068023401은 상관값
e57d5076d899407d81a8611863c25edf의 DEPLOY_STARTED→DEPLOY_COMPLETED
(live,2026-09-06T23:50:31.717Z,대상224558c)를 확인했다.
서비스 관측34068023596은 생산224558c, timer active/service success,
원본 COMPLETE/2026-09-04, WAIT_SESSION/24.7초, 주문0, qualified forward0이다.
실제 KIS34067976976은 기존7개 통과/새 계좌 조회1개 실패(ACCOUNT_CURSOR_STALLED)다.
공식 동일 API 예제가 D/E를 최종 페이지로 취급하는 근거에 따라
`codex/182-account-pagination-terminal`에서 미체결 조회에도 같은 종료 계약을
적용했다. 빈 헤더/추가 페이지의 키 누락·반복 차단은 유지하고 비밀값 없는 형태
진단만 추가했다. 관련65개는 통과했고 전체 회귀·병합·서버 재검증은 진행 중이다.
준비 금액 확정은 실제 자본 배정·주문 권한이 아니며 기존 저빈도 자본은 불변이다.

후속 자본 검토는 PR773/a9d7e41로 병합됐다. 운영자는
배정안을 제안하고 절차대로 진행하라고 했으며 당시에는 구체적인 금액을 결정하지 않았다.
검토 CLI와 100/600달러 보고서를 추가했다. 600달러에서 참고 시세상 TLT1주만
계산되며 100달러는 전 종목0주다. 현재 금액 확정은 위 최신 후속을 따르며 실제 배정·매수 신호는 아니다.
코드0f3dbda에서 관련41개와 전체3541 passed/8 skipped(841.59초), 실패0을 확인했다.
린트·하네스14/14·HANDOFF·PR 품질 검사가 통과했다.
...
정상 배포34066010522가 성공했다. 읽기 감사34066095277은 상관값
2565ba7d7ea3cdfb2770ca3f6e1c83c5의 DEPLOY_STARTED→DEPLOY_COMPLETED
(live, 2026-09-06T23:08:26.939Z, 대상a9d7e41)를 확인했다.
서비스 관측34066095424도 생산a9d7e41, timer active/service success,
원본 COMPLETE/2026-09-04, WAIT_SESSION/57.3초, 주문0, qualified forward0이다.
검토 기능은 생산 코드에 포함됐지만 기존 진단 서비스가 자본을 배정하거나 주문하지 않는다.

PR771은18521b9로 병합됐다. 정상 배포34052281701이 성공했고,
감사34052386141은 ef018f9332f6c8d06c904d130d83cc7f의
DEPLOY_STARTED→DEPLOY_KERNEL_TOUCHED→DEPLOY_COMPLETED(live,18:37:30.640UTC,
대상18521b9)를 확인했다. 장중 긴급 경로나 실제 주문은 실행하지 않았다.
KIS34052281711은7/7(20.41초), 2026-09-04 다섯 ETF 각78봉 계약을 통과했다.
기존 단타 서비스 관측34052387436은 timer active/service success,원본 보관 COMPLETE,
상태 WAIT_SESSION/나이46.6초,실주문0,qualified forward0을 확인했다.
새 주문 실행 라이브러리는 배포됐지만 승인된 생산 단타 주문 진입점은 아직 없다.
따라서 서버의 LIVE_ADAPTER_NOT_IMPLEMENTED·PRODUCTION_FILLS_NOT_VERIFIED
전체 운용 차단을 해제하지 않았다. spec182 T002~T009는 끝났고 T010~T012는 미완료다.

Alpaca 계정은 없으며 필수 증권사로 취급하지 않는다. 기존 KIS 수집을 유지한다.
스펙182는 사전등록 신호를 실제 귀속 보유와 연결하는 실행기, 봉별 영속 선점,
매도 우선·현금/노출 제한·종료 청산·손실 정지·응답 유실/재시작 방어를 구현한다.
새 생산 실주문 진입점은 설치하지 않았다. 권한은 기본 차단이고 검증 CLI는
가짜 계정·격리 장부·MockTransport만 사용한다. 실제 주문을 한 것으로 보고하지 않는다.

기존 취소 경로의 종목/수량 누락과 업무거절 미확인을 고쳤다. 즉시 취소확정과
재호가를 제거하고 최종 증권사 상태를 기다린다. 취소 결과 불명확이면 재전송하지
않으며, 잠금 획득 전에 막힌 경우만 요청 미발행으로 재시도한다. 체결 다중 페이지와
누적 평균금액을 처리하고 새 단타 경로는 원주문 연결·취소완료·수량을 함께 확인한다.
원문 시각의 UTC를 추정하지 않고 체결 관측 시각을 남긴다. 감사·제한·원래 선점은 보존한다.

실행 코드600a161에서 전체3500 passed/8 skipped(799.45초), 실패0을 확인했다.
관련67개와 오프라인 매수→부분체결→취소→추가체결→청산 재현도 통과했다.
8개 제외는 실제 KIS 읽기 시험7개와 가동 전 사다리 조건1개다. 린트·하네스14/14·
HANDOFF 사실 검사도 통과했고 PR771 병합·정상 배포까지 확인했다.
K4 감사 기록과 K6 실행 권한 경로를 수정했으므로 정상 강화캐너리·장외배포를 거친다.
되돌릴 때 새 실행기 호출을 중단하고 주문·체결·감사·선점 장부를 보존한다.

전체완료에는 실제756세션 자료, 합격 후보, 동일 후보60세션, 공급자/체결 동등성,
단타전략·자본 승인과 별도 반복 실주문 경계·생산 관측/권한 진입점·실제 체결증거가
남았다. 무료 장기 시세 계정 허용 여부를 운영자에게 질문했으며 아직 답이 없다.
이 조건을 가짜 자료·시험·기존143달러 승인으로 대체하지 않는다.

## 이전 진행 — 2026-09-07 KST (스펙181 서버 진단 운용 가동, 실주문 미완료)

PR769는 a8fb91a로 병합됐다. 배포34045229510이 성공했고 감사34045311817은
상관값5f87e0b22d242faac629b974687af343의 DEPLOY_STARTED→DEPLOY_KERNEL_TOUCHED→
DEPLOY_COMPLETED(live,16:23:09UTC,대상a8fb91a)를 확인했다. 최초 유닛 동기화는
구버전 helper라 단타 유닛이 빠졌고, 정상 배포34045310271의 갱신된 sync가
단타 service/timer를 설치하고 활성화했다. 장중 긴급 경로나 수동 주문은 쓰지 않았다.

실제 KIS34045313477은7/7(16.97초), 2026-09-04 SPY/QQQ/IWM/TLT/GLD 각78봉,
총390봉 계약을 통과했다. 이 smoke 원본은 임시 검증 자료다. 별도 생산 관측
34045385754는 timer active,service result success,실제 생산 a8fb91a와 소스 지문 일치,
원본 archive COMPLETE/2026-09-04,상태시각16:25:21UTC/나이11.9초를 확인했다.
정식 영구 보관 경로는 `/var/lib/auto-invest-intraday/<소스지문>/sessions/2026-09-04`다.
현재 휴장 WAIT_SESSION,실주문0,live/forward promotion=false,qualified sessions0이다.
재관측34045564976도 성공했다. 동일 생산 코드/원본 보관 상태를 유지하면서 상태 시각이
16:25:21→16:28:24UTC로 전진했고 나이31초였다. 단회 설치 성공만이 아니라 반복 갱신을
확인했다. 장중 모의 체결이나 정식 전진60세션이 확인된 것은 아니다.

T018~T022의 독립 운용 단계는 검증됐지만 전체 단타 요청 T012~T016은 끝나지 않았다.
Alpaca 역사 자료 계정 접근,756세션 자료,실자료 후보 합격,동일 후보60세션,
공급자/체결 동등성,단타 반복 실주문 경계와 생산 접수·청산 증거가 필요하다.
기존18후보 진단 관측을 정식60세션으로 바꾸거나 저빈도143달러 승인을 재사용하지 않는다.
다음 유효 거래일의 장중 모의 판단은 아직 생산에서 관찰하지 않았다.
되돌릴 때는 단타 timer만 비활성화하고 단회 종료를 기다린다. 원본·장부는 보존한다.

### 구현·검증 경과

운영자가 단타 전체 완료를 다시 요청했다. T012~T016을 막는 실제 자료·후보 합격·
60세션·단타 실주문 구현을 확인했다. 별도 서버 예약이 없어 자료가 쌓이지 않던
부분은 `codex/181-completion-20260907`에서 T018~T022로 구현한다.
매분 독립 단회 수집/진단 모의 실행, 장외 완결 세션 불변 보관, 소스 지문별 장부,
실패 복구·중복 잠금·상태 신선도·고정 읽기 관측을 추가했다. PR769의 실행 코드
ad8e32f에서 전체3449 passed/8 skipped(879.88초), ruff, 하네스14/14,
HANDOFF 검사가 통과했다. 후속 fa8f149는 느린 단회 종료 뒤에도 timer를 재예약하며
관련18개와 Linux systemd 실제 파서 검사가 통과했다. 생산 설치는 병합 뒤 확인한다.
기존 저빈도143달러·rung1·run_seq9·주문 권한·수익 판정 관문은 유지한다.
진단 관측은 T014의 합격 후보60세션이 아니며 단타 전체 완료로 표시하지 않는다.

## 이전 진행 — 2026-09-06 KST (스펙181 단타 수집·지속 모의 실행)

PR767은9f99501로 병합됐고 배포34007993006이 성공했다. 실제 KIS34007993027은
7/7(19.70초), 2026-09-04 SPY/QQQ/IWM/TLT/GLD 각각78봉을 수집·검증했다.
INTRADAY_DATA_CONTRACT_OK, 실주문0, live/forward promotion=false다.
자료 지문은 specs/181-intraday-runtime/research.md에 기록했다. 현금934.27,
NAV1432.67, 최근 주문 행0, 열린 미체결0이다. 단타 프로그램 코드는 서버에 있지만
상시 모의 서비스나 단타 실제 주문 서비스가 설치된 것은 아니다. T011은 완료,
T012~T016은 미완료다. 아래 초안·미배포 문구는 출시 전 경과 기록이다.
...
인계 문서 전체 검증도3439 passed/8 skipped(838.65초), 실패0으로 통과했다.

운영자의 재개 지시에 따라 독립 소프트웨어 출시와 전체 단타 실거래 완료를 분리한다.
과거의 "전체 요청 미완료라 PR767을 계속 초안 유지" 판단은 폐기한다. 실제 데이터·
전략 증거가 없어도 검증된 수집/모의 소프트웨어 단계는 출시할 수 있다. 기존 서버
KIS smoke의 토큰 캐시·main 계보·격리 checkout 경로에 최근 완결 세션의 5종목
분봉 격자 검사를 추가했다(T017). 코드592c106에서 관련31개와 전체3439 passed/
8 skipped(778.75초), ruff, 하네스14/14, HANDOFF 검사가 통과했다.
새 SSH 명령·임의 셸·키 복사·주문 권한은 추가하지 않았다. 출시 뒤 기존 smoke로
실계정 분봉 응답을 확인한다. 이는 T012의 756세션이나 T013~T016의 실제 전략/주문
증거를 대신하지 않는다. 아래 기존 로컬 키 부재 기록만으로 서버 검증을 미루지 않는다.

기존 main4a5f43a의 KIS 예약33952622567은9월5일 IAUM 전 거래소 시세500으로
5/6 실패했지만 새 읽기 전용34007623476은9월6일02:53UTC 6/6으로 회복했다.
IAUM AMS44.20, 현금934.27, NAV1432.67이다. 새 분봉 검사는 아직 이 실행에 없으며
merge 뒤 별도 확인한다. 일시 외부 오류를 숨기거나 전 거래소 실패 차단을 줄이지 않았다.

운영자 요청은 단타 자동매매 전체 구현이다. 기존 스펙177 CSV 연구 도구 완료를
전체 요청 완료로 취급했던 범위 혼동을 바로잡고, 스펙181에서 수집부터 실제 자동
매수·청산·감사·대사까지 T001~T016으로 연결했다. 현재 구현은 아직 main 출시가 아닌
`codex/181-intraday-runtime` 작업 브랜치이며 전체 완료를 선언하지 않는다.

- Alpaca 통합시장 역사와 KIS 부분시장 5분봉을 고정 읽기 전용 주소로 수집한다.
  전체 페이지·거래소·정규장·봉 완성·지문을 검사하고 공급자 자료를 섞지 않는다.
- `scripts/intraday_runtime.py`의 collect/paper/run/status는 기존 18후보 신호를
  별도 가상 계좌와 SQLite 추가 전용 장부에 연결한다. 다음 봉 고정 지정가·거래량
  제한·부분 체결·청산 잔량·후보별 손실 정지·재시작·중복 실행을 구현했다.
- 코드 commit7be42cf에서 전체 pytest 3435 passed/7 skipped(761.77초), 관련 시험48,
  ruff, strict harness 14/14, HANDOFF 사실 검사를 통과했다. 생략7개는 기존 실제
  KIS 시험6개와 사다리 가동 전 전용 시험1개이며 새 실패를 숨긴 것이 아니다.
- 반복 run 명령도 공급자 응답과 시계를 대체한 연결 시험에서 2회 반복·새 배치 2개·
  처리 봉 2개를 확인했다. 이는 외부 조회가 없는 시험이며 실자료 성공 증거가 아니다.
- 실제 로컬 collect 명령은 Alpaca/KIS 환경 자격 증명이 없어 각각
  DATA_ACCESS_REQUIRED로 종료됐다. HTTP 호출이나 실제 주문은 없었다. 이는 기존
  생산 서버의 KIS 계정 연결 부재를 뜻하지 않으며 키를 채팅에 요구하지 않는다.
- 새 실행기는 진단 모의 실행이다. 실제 데이터 성공·서버 상시 설치·전략 합격·
  60세션 전진 검증·단타 실주문 어댑터가 완료된 것이 아니다. 항상
  live_eligible=false, forward_promotion_eligible=false, orders_submitted=0이다.
- 실제 계정 데이터 접근 뒤 T012~T014를 검증하고, 별도 등급4/헌법 절차로 T015의
  단타 반복 주문 경계를 구현해야 한다. 기존 하루 한 번 저빈도 실주문 경로나
  globalfixed의 운용 캐너리 예외를 단타 승격에 재사용하지 않는다.

기존 저빈도 실거래·배포·자본·센티넬·허용목록은 변경하지 않았다. 되돌릴 때는
별도 단타 실행 프로세스만 중단하고 원본과 장부는 보존한다. `.specify/feature.json`과
AGENTS의 SDD 포인터는 스펙181을 가리킨다. T010/T011 또는 모의 체결만으로 전체 단타
요청을 완료 처리하지 않는다.

## 이전 진행 — 2026-09-05 KST (스펙180 예산 정합 보정)

PR765는 main7acd7093로 머지됐고 정확한 head7ba9f21에서 전체3414/7(727.92초),
ruff,하네스14/14,HANDOFF,본문과 원격 품질 관문이 통과했다. 실제 로컬Git의 main 진행·
조회 불능·오형식과 workflow 연결을 검사했고, 실제 원격 조회에서도 당시 최신fdb9149a는
통과하고 이전87c0095는exit75로 상태 변경 없이 차단했다. 배포33938162479와 최신 증거
33938162473은 성공했다. 감사33938198034의7d3e5a546bdbacfc6bb393e8c07e3764는02:07UTC
exact7acd7093 DEPLOY_COMPLETED를 확인했다. KIS33938189596은6/6,cash934.27,NAV1434.91,
ORANY28,open_unfilled0이다. 실제 자동 승인33938200698은새main 확인·운영증거를 통과해
STAY/rung1→1/센티넬무변경/새PR없음이었다. 무주문33938199313은exact7acd7093/143,
첫진입success,manual-no-order-preflight,postCLEAR/reconciliationOK/evidenceVALID/haltfalse/orders0이다.
T016 출시·생산 검증 완료. 실제 자동 신규 접수·체결T009는 미완료다. 서버타이머의 다음 달력
발화는Sep7 14:35UTC이나 휴장 차단 대상이며 다음 유효거래일은Sep8이다. 인계 후 중복
사전점검을 반복하지 말고 유효 자동 실행의 접수·체결·감사·대사·중복쓰기0을 추적한다.

autoarm33935656168은 전체3405/7·ruff·하네스14/14·HANDOFF·본문 관문을 통과하고
PR763을 fdb9149a로 자동 머지했다. 승인142→143, rung1/진입일2026-08-31 유지, run_seq9다.
deploy33937154453와 감사33937198586(4c6227d2241200efa30f3170f8df9a5d)은01:46UTC
실제 fdb9149a DEPLOY_COMPLETED를 확인했다. KIS33937196360은6/6이다.
배포 전에 연결된 no-order33936963890은 signed capital does not match sentinel로 주문 전에
차단됐으며 배포 후33937207081은143 첫진입 success/CLEAR/OK/VALID/주문0으로 통과했다.
T008/T011/T013 완료이나 실제체결T009는 미완료다. 자동 승인만으로 실제체결을 주장하지 않는다.
예약33936156498은 이전87c0095 기준에서 새fdb9149a 증거를 읽어 code_commit만 실패했고
강등PR764를 만들었으나 충돌로 병합되지 않았고 최신 검증 근거와 함께 닫았다. FR-010은 main 변경·조회 실패 시 낡은
실행을 상태 무변경으로 종료하고 기존 최신 진입·손실·주문 검증을 유지한다.

아래는 이전 생산 기록이며 현재 상태는 위 문단과 요약표를 우선한다.

PR761은 main21f63d64로 머지됐다. 최종 a17fabc에서 비-main Git 기본값·GitHub 색상 환경으로
전체3405 passed/7 skipped(729.17초),ruff,하네스14/14,HANDOFF,PR 관문을 통과했다.
실제 Git fixture·색상 CLI·생성 PR 본문 양쪽을 검사하며 기존 품질 기준과 주문·자본 경계는 유지한다.
deploy33934694258과 profit evidence33934694303이 성공했고, 감사33934744194의 상관값
3dc3cb3b85d6409722f32556290e5ba4에서00:59 UTC DEPLOY_STARTED→DEPLOY_COMPLETED를
확인했다. exact-main KIS33934742508은6/6,현금934.27,NAV1434.91,ORANY28,open_unfilled0이다.
이 인계 반영 후 autoarm을 새 main에서 한 번 실행한다. 승인143·최종no-order·실제체결은 아직
완료가 아니므로 T008/T011/T013/T009를 체크하지 않는다.
...
HANDOFF 기준으로 검증할 수 있다. 얕은 이력의 실제 실패와 전체 이력의 통과를 Git 회귀로 고정했다.
다음 단계는 이 인계 반영 후 최신 증거로 자동 승인을 재실행하는 것이다. 승인액은 아직142이며
RESIZE143 판단과 승인143 적용은 구분한다. T008/T011과 실제 체결T009는 아직 미완료다.
exact main aa6aa482의 deploy33931335050과 audit33931382855가 성공했다. 감사 상관값
10fc4c29934c0707520b7a89add952f2에서23:57 UTC DEPLOY_STARTED→DEPLOY_COMPLETED를
확인했다. KIS33931381162는6/6, cash934.27/NAV1434.91/open_unfilled0이다.

PR758은 main f2e4f708로 머지됐고 전체3400/7·ruff·하네스14/14·HANDOFF·PR 관문을 통과했다.
main f2e4f708 배포33929660221과 profit evidence33929638907이 성공했다.
새 코드854d81f9의 no-order33928968323은 현재 시세에서142도 ENTRY_READY로 통과했고
CLEAR/OK/VALID, haltfalse, orders0이었다. 이는143 승인 갱신이 검증됐다는 뜻은 아니다.
autoarm33929694159는 승인 전 전체 시험 도중 동일 환경의 depth1 clone이 정상 인계를
DEGRADED로 오판하는 것을 발견해 중단했다. 새 승인PR·예산 변경은 없었다. 전체 이력을
가져온 동일 clone에서는 기존 검사 그대로 OK였으므로 checkout fetch-depth0과 실제 Git 회귀로
보정한다. HANDOFF 검사 완화·수동 예산 변경 없이 새 main에서 T008 자동 승인을 다시 확인한다.
중단 실행이 남긴 보고서19460ee에는 실제 RESIZE142→143, NAV1434.91, rung1→1과
OPERATIONAL_CANARY_READY가 기록됐다. 진입 판단 자체는 생산에서 확인했지만 승인PR·main
센티넬 갱신은 아직 없으므로 T008은 미완료다.

PR #757은 23:13 UTC main854d81f9로 머지됐다. 최종 기능 커밋3443bd0의 전체3400 passed/7 skipped,
ruff, 엄격 하네스14/14, HANDOFF 사실 검사와 원격 PR 관문을 통과했다. 현재 NAV 검증과
과거 승인액의 작은 차이는 엄격한 전략0체결·운영1단에서만 기존 RESIZE 승인PR로 정합한다.
15%p/25%p 오차, 66% 표현 가능한 목표, NAV10% 비율, 주문·손실·중복·감사 관문은 그대로다.
기능 출시와 실제 승인 예산 갱신·첫 체결은 서로 다른 완료 항목이다.

exact main854d81f9 배포33928671285와 감사33928739275가 성공했다. 감사 상관값
79b06199af242253aa8e1a96791246a6의 DEPLOY_STARTED→DEPLOY_COMPLETED(23:14 UTC)가
실제 서버 코드 반영을 확인한다. KIS33928671315는6/6, cash934.27/NAV1434.91/ORANY28/
open_unfilled0이다. operator setup33928740976은 deploy_timer/live_canary_timer/worker active다.
이 인계의 main 기준을 반영한 다음 기존 autoarm으로 검증예산143과 승인예산142의 정합을
실행한다. 현재 인계 시점에는 아직 RESIZE 승인PR·최신 no-order 검증을 완료하지 않았다.

오너가 해외 ETP 신청 완료를 확인했다. 이는 사용자 확인이며 신청 후 브로커 수락 증거는 아니다.
보정 전 main c3c41ff8은 21:00 UTC 배포됐다. audit33925197841의 correlation
342a7958be990cb1a8a0a0852d1f9005가 DEPLOY_STARTED→DEPLOY_COMPLETED를 확인한다.
KIS33925193723은6/6, cash934.27/NAV1434.91/ORANY28/open_unfilled0이다.
operator setup33925263089는 deploy_timer/live_canary_timer/worker active다.

no-order33925261456은142에서 IAUM1/SCHX2, 가격45/30.4552의 비중오차
25.0848%p/15.1902%p가 기존25%p/15%p 한도를 넘어 차단됐다. 사후CLEAR/OK/VALID,
haltfalse/orders0이다. 이는 신청 전 overseas_etp 거절과 다른 새 차단이다.
autoarm은 current NAV10%의143으로 검증하지만 과거 sentinel142는10% drift 미만이라
STAY한다. 스펙180은 신뢰할 수 있는 전략0체결·current-NAV 진입검증이 있는 운영1단만
기존 RESIZE/승인PR 경로로 예산을 정합한다. 현재 재현에서143은 같은 한도를 통과하지만
가격·신호 변화로 현재 예산도 불가능하면 그대로 차단한다. 실제 체결 성공은 아직 아니다.

다음 유효 거래일은 Sep8(XNYS, Sep7 Labor Day 휴장)이다. 한국시간23:17 GitHub와
23:35 독립 서버 후보를 추적하되 수동 주문·서비스 시작·소비한 Sep4 복구슬롯 재개방은 금지한다.
기술·검증·승인·배포와 최신 no-order 준비를 먼저 완료한 뒤 실제 자동체결·감사·대사를 검증한다.

## 과거 진행 — 2026-09-05 KST 신청 확인 전 (외부 관문 기록)

PR #754 merge `7a69a5cb`는 root 전용 KIS 주문 거부 원문을 외부에 내보내지 않고
해외증권·해외 ETP·해외 변동성 ETN·범위 불명 서비스 신청만 닫힌 값으로 구분하는 진단 1.3을
출시했다. 관련 회귀 41개와 전체 `3373 passed, 7 skipped`, ruff, shell 문법, 엄격 하네스
14/14, HANDOFF 사실 검사와 PR 품질 관문이 통과했다.

일반 push 배포 run `33897995729`는 장중 관문에서 안전하게 연기됐다. 첫 오너 승인 긴급 배포
run `33898086663`은 서버 변경 전에 IAUM 일봉 조회의 일시적 HTTP 500 때문에 실패 폐쇄했다.
독립 exact-main KIS smoke run `33898208625`가 6/6, 현금 `$934.27`, NAV `$1434.91`,
ORANY 28주, 미체결 0건을 확인한 뒤 헌법의 시작 전 중단 복구를 사용한 정확한 단회 긴급 배포
run `33898309128`이 성공했다. 배포 감사 run `33898481924`는 상관관계
`6b5824a9979b1099fbeec79027e1c5d5`의 `DEPLOY_EMERGENCY_AUTHORIZED`부터
`DEPLOY_COMPLETED`까지를 확인했다.

exact-main 읽기 전용 observer run `33898481820`의 생산 진단 1.3은 IAUM·SCHX 모두
`message_topics=[account, service_registration]`,
`service_registration_scopes=[overseas_etp]`를 반환했다. 원문·부분 문자열·길이·해시·계좌·가격·
...
증거가 아니라 확인 위치를 정확히 좁힌 진단이므로 운영자가 완료 또는 활성 상태를 확인해야 한다.
본인 인증과 금융 약관 동의는 시스템이 대신할 수 없다. 완료 뒤 다음 자동 정규장 scheduler만
사용해 실제 접수·체결·전략 감사·계좌 대사와 중복 주문 0건을 확인한다.

## 현재 진행 — 2026-09-04 KST (#749 무접수 사고의 같은 장 1회 자동 복구)

PR #749 merge `f5084abb`는 첫 자동 실행의 모든 계획 주문이 중개사에서 명시적으로 거부되고
접수·부분체결·체결·열린 주문이 전혀 없을 때만, 같은 XNYS 정규장의 다음 root 소유 서버 예약이
별도 추가 전용 복구 슬롯을 한 번 소비하도록 출시했다. 원래 거래일 선점은 삭제하거나 수정하지
않고, 복구 실패도 슬롯을 소비해 세 번째 시도를 막는다.

복구 전에는 exact 사고 manifest, 첫 실행 완료·주문 제출 0건·계획과 거부 결과의 일대일 일치,
정상 체결 동기화·측정·계좌 대사, 정합 `OK`, 증거 `VALID`, 중지 없음, 현재 배포 코드의 새 KIS
점검과 미체결 0건을 모두 다시 확인한다. 접수 불명·접수·부분체결·체결·열린 주문·증거 누락·
GitHub 최초 출처·보정 계보 불일치는 모두 중개사 호출 전에 실패 폐쇄한다. 현재 과거 사고
manifest는 `enabled=false`이므로 닫힌 2026-09-03 세션을 다시 실행하지 않는다.

관련 120개와 전체 `3370 passed, 7 skipped`, ruff, shell 문법, workflow YAML 파싱, 엄격 하네스
14/14, HANDOFF 사실 검사와 PR 품질 관문을 통과했다. exact-main 배포 run `33817467201`이
성공했고 배포 감사 run `33817631524`는 상관관계 `bf0681bfcb1fd0c03196a79d7910ac74`,
대상 `f5084abb`, terminal `DEPLOY_COMPLETED`를 확인했다.

exact-main KIS 점검 `33817633554`는 6/6, 현금 `$934.27`, NAV `$1436.87`, ORANY 28주,
미체결 0건이다. 주문 없는 production 사전점검 `33817635538`은 `ENTRY_READY`, 자본 `$142`,
계획 IAUM 1주·SCHX 2주, fundability schema 1.1, 사후 측정 `CLEAR`, 계좌 대사 `OK`, 증거
`VALID`, 중지 없음, 주문 제출 0건으로 끝났다.

다음 첫 자동 GitHub 후보는 2026-09-04 14:17 UTC, 독립 서버 예약은 14:35 UTC다. 정상 첫
실행이 접수·체결되면 복구 미발동이 정상이다. 다시 전부 명시적 무접수일 때만 같은 장 다음 자동
서버 예약의 복구 1회와 이후 중복 중개사 쓰기 0건을 확인한다. 실제 자동 접수·체결·전략 감사·
같은 실행 계좌 대사 전에는 스펙 176과 사용자 목표를 완료로 표시하지 않는다. 상세는
`HANDOFF-178-BOUNDED-SAME-SESSION-RETRY.md`다.

## 현재 진행 — 2026-09-04 KST (스펙 176 자동 실행 주문 0건 원인 진단)

PR #744 merge `9f9ec55f`는 서버 timer의 고정 실행 상태 진단을 출시했고, 등록 오너가 승인한
긴급 배포 run `33776554180`으로 production에 정확히 반영됐다. 배포 감사 run `33776948519`은
상관관계 `12295284cda1ad96a1581424eba6ab5e`의 `DEPLOY_STARTED` → `DEPLOY_COMPLETED`를
확인했고, KIS smoke는 6/6·현금 `$934.27`·NAV `$1436.87`·ORANY 28주·미체결 0건이었다.

배포 뒤 2026-09-03 16:11 UTC의 root 소유 server timer가 수동 시작 없이 자동 발화했다.
읽기 전용 observer run `33777406564`가 회수한 실행 `20260903161106`은 exact code/deployed
commit `9f9ec55f`, `ENTRY_READY`, 거래일 선점, 주문 단계 종료 0, 체결 동기화·측정·계좌 대사
종료 0, `result=completed`를 보였다. 이전 `DEPLOY_HALTED` 유지보수 잠금은 성공한 긴급 배포가
검증 후 정리했고 독립 timer 가동성은 회복됐다.

이 자동 실행의 `orders_submitted=0` 원인을 #745의 정화 진단으로 확인했다. exact-main 긴급 배포
run `33779685492` 뒤 observer run `33779882654`가 회수한 결과는 계획 2건, 결과 2건이며 IAUM
BUY 1주와 SCHX BUY 2주가 모두 `REJECTED_BY_BROKER`였다. 내부 위험 gate는 없었고 ORANY 28주
매도 보류는 기존 `unmanaged_holding` 계약대로다. 즉 전략·자금성·예약은 통과했고 실제 KIS 주문
요청이 명시적으로 거부된 상태다.

PR #746 merge `020f8aa0`과 exact-main 배포 run `33811537163` 뒤 읽기 전용 observer
run `33811632918`은 같은 자동 실행의 IAUM·SCHX 거부가 모두 KIS HTTP 200,
`rt_cd=7`, `msg_cd=APBK1672`, `KisOrderResponseError`, 매수 TR `TTTT1002U`, 거래소
`AMEX`, 주문 구분 `00`임을 확인했다. exact-main KIS smoke `33811630111`은 6/6,
현금 `$934.27`, NAV `$1436.87`, ORANY 28주, 미체결 0건이다.

KIS 공식 `open-trading-api` 현재 main `b4e62497`과 비교하면 endpoint·TR ID·주문 본문·거래소는
우리 구현과 같고, 공식 공통 REST 래퍼가 항상 넣는 `custtype=P`, `tr_cont`만 우리 공유 헤더에
없었다. `APBK1672`의 공식 공개 의미는 확인되지 않았으므로 헤더 누락을 확정 원인으로 단정하지
않는다. PR #747 merge `f858fcfd`는 두 헤더를 고정하고 요청 캡처 회귀를 추가했다. 관련 61개와
전체 `3350 passed, 7 skipped`, ruff, 엄격 하네스 14/14, HANDOFF 사실 검사, PR 품질 관문을
통과했다. exact-main 배포 run `33813473967`이 성공했고 읽기 전용 감사 run `33813562353`은
상관관계 `e0553d0b09ce7312895aaeb391c339e5`의 `DEPLOY_STARTED` → `DEPLOY_COMPLETED`를
확인했다. exact-main KIS smoke `33813473991`은 6/6, 현금 `$934.27`, NAV `$1436.87`,
ORANY 28주, 미체결 0건이다.

주문 없는 production preflight `33813565139`는 `event=workflow_dispatch`이므로 실제 주문 0건이
정상이며, `ENTRY_READY`, 자본 `$142`, 계획 IAUM 1주·SCHX 2주, 사후 측정 `CLEAR`, 계좌 대사
`OK`, `evidence_quality=VALID`, halt 없음으로 끝났다. 이 사전점검은 헤더 보정의 실주문 해결
증거가 아니다. 다음 자동 정규장 실행의 실제 접수·체결·전략 감사·계좌 대사와 다른 scheduler의
동일 최초 run/source·중복 broker write 0건으로만 해결을 확정한다. 수동 주문·서비스 수동 시작·
기존 거래일 선점 삭제는 하지 않는다.

## 현재 진행 — 2026-09-03 KST (스펙 179 오너 단회 장중 긴급 배포)

운영자가 자동 실거래 가동성 보정을 장 마감까지 미루지 말고 즉시 배포하라고 명시 승인했다. #733 merge
`8a1bafc4`로 헌법 15.0.0과 단회 긴급 배포 경로가 main에 들어갔고, push run
`33667347236`의 일반 배포는 장중 관문이 안전하게 연기했다. exact-main KIS smoke
`33667347352`는 6/6, 현금 `$934.27`, NAV `$1436.31`, 미체결 0건을 확인했다.

첫 오너 긴급 run `33667656920`은 GitHub namespace 소유자 `jinooaction`만 허용해, 실제
시스템 오너가 사용하는 `masonoh-kidsnote`를 거부했다. SSH 키 설치·서버 변경·KIS 접속
전에 종료돼 계좌 영향은 없었다. 헌법 전용 커밋 `0649834a`는 등록 시스템 오너
신원을 명시한 15.0.1로 올렸고, 구현 `df9b1404`는 workflow 소스에 정확한 actor를
고정했다. 임의 입력·변수·secret·일반 협업자 역할로 권한을 늘리지 않는다. 두 보정 커밋은
모두 `this changes the safety perimeter`를 기록한다.

뉴욕 13:59 뒤에 배포가 끝나면 당일 자동 실행 후보가 없는 추가 회복 공백도 확인했다.
GitHub 예약과 수동 주문 표면은 늘리지 않고 root 소유 server timer만 10:35~15:35의
12분 간격 26개 자동 후보로 연장한다. 15:35 뒤 새 후보는 없고, 모든 후보는 기존
공유 거래일 1회 선점·XNYS 정규장·정확한 리비전·지정가·자본·위험·감사·대사 관문을
그대로 사용한다.

PR #734 merge `a852d0f6` 뒤 오너 긴급 run `33671389870`은 등록 actor 검사를 통과했고,
정확한 target에서 KIS smoke 6/6·미체결 0건을 확인했다. 그러나 root helper가 새 target의
배포 상태기계가 아니라 서버에 설치된 구버전 상태기계를 다시 호출해 승인 감사만 남기고
`DEPLOY_STARTED` 전에 실패했다. 생산 코드와 주문은 바뀌지 않았고 유지보수 잠금은 HALTED로
남았다. 헌법 15.1.0은 exact-target 격리 상태기계만 고정 production 경로에 실행하고, 이전
장부가 유일 승인 1건·시작 0건임을 증명한 경우에만 새 단회 요청이 이 잠금을 인계하도록 한다.

PR #735 merge `edd1d1c6` 뒤 오너 긴급 run `33673819722`은 이전 HALTED 인계와 exact-target
bootstrap, KIS smoke 6/6·미체결 0건을 통과하고 `DEPLOY_STARTED`까지 도달했다. 그러나 기존
DeployRunner의 config dry-run이 `RunnerConfig.env_path`를 loader에 전달하지 않아 systemd 밖
bootstrap 프로세스에서 KIS 비밀값 없음으로 실패했다. 기존 상태기계는 production HEAD를
`2d6054ee`로 복구하고 `DEPLOY_ROLLED_BACK`을 남겼다. read-only audit run `33674137761`은 같은
상관관계에 승인 1·시작 1·커널 변경 1·실패 1·롤백 1, terminal rollback을 확인했다. 주문은
0건이다. 다만 shell EXIT trap이 main의 지역변수 범위가 끝난 뒤 실행돼 요청과 QUIESCED 잠금
정리가 중단됐다.

헌법 15.2.0은 이 정확한 terminal rollback orphan만 자동 복구한다. 두 파일의 root 소유·0640·
닫힌 스키마·동일 신원, 배타 잠금, 정확한 장부 사건 수와 최신 롤백, production HEAD와 롤백
기준 일치를 모두 확인한 뒤 중개사 쓰기 잠금 아래에서 이전 요청을 제거한다. config dry-run은
...
PR #740 merge `68211963` 뒤 일반 push run `33686548933`은 새 root helper를 설치하고 기존
stale-target 상태를 정확히 실패로 유지했다. 등록 오너 run `33686633552`는 생산 `d43c55b0`의
양쪽 Git 계보와 배포 장부·worker/timer·두 잠금·첫 KIS 6/6·미체결 0건을 통과해
`DEPLOY_EMERGENCY_ORPHAN_RECOVERED`를 남겼다. 이어 새 요청으로 전환한 뒤 두 번째 KIS smoke가
같은 시각 새 OAuth 토큰을 다시 발급해 KIS 403을 받았고, `DEPLOY_STARTED` 전에 새 요청을
제거하고 HALTED 잠금을 남겼다. 생산 코드와 주문은 바뀌지 않았다. 보정은 smoke 격리 checkout이
...
2026-09-02 첫 GitHub 후보 뒤에도 예약 실행이 생성되지 않았고, 14:35 UTC의 독립 서버 예약도
실행 요약을 만들지 못했다. 실제 배포 커밋 `2d6054ee` 뒤 현재 main `e12b7e90`까지의 변경이
`HANDOFF.md`와 `specs/178-flutter-operator-mobile/**`뿐인데도 서버 경로만 두 커밋의 문자상 완전
일치를 요구한 것이 재현 가능한 차단 원인이다.

- **보정 계약**: 실제 배포 커밋이 현재 main의 조상이고 사이의 모든 변경이 기존 고정 비실행 경로
  `*.md`·`specs/**`·`.verify/**`·`.trigger/**`일 때만 운영 동등으로 판정한다. 그 밖의 실행 코드,
  배포 파일, workflow, 설정, 자동화, 계보 분기, diff 읽기 실패, 미분류 경로는 모두 실패 폐쇄한다.
- **이중 검사**: root 소유 scheduler가 선점 전에 확인하고, 실제 주문 경계의 `systemd-order`가
  현재 main 경쟁까지 다시 확인한다. 첫 진입 증거는 현재 main에, `DEPLOY_COMPLETED` 감사는 실제
  배포 커밋에 묶으며 실행 요약 1.1에 두 커밋과 동등성 판정을 함께 남긴다.
- **안전 의미**: GitHub 예약의 서명 경로, 거래일 1회 선점, 정규장, NAV 10%, 단 1, 지정가,
  허용 종목, 손실·노출 한도, 킬스위치, 추가 전용 감사와 수동 실주문 금지는 유지한다.
- **현재 검증**: 새 운영 리비전·서버 예약·주문 경계 관련 50개 시험과 쉘 구문 검사가 통과했다.
  헌법 버전 기대값과 기존 main의 낡은 HANDOFF 요약도 보정했으며, 최종 전체 검증은
  3291 passed/7 skipped, 린트, 엄격 하네스 14/14, HANDOFF 사실 검사가 모두 통과했다. PR #732는
  `MERGEABLE/CLEAN`이고 기능 커밋 `7b345a24`를 포함한 원격 PR 품질 관문도 성공했다.
- **남은 관문**: 장 마감 뒤 PR 병합과 정확 배포·읽기 전용 KIS 스모크·주문 없는 사전점검을 확인한다.
  그 다음 첫 자동 주문 접수·실제 체결·전략 감사·같은 실행 계좌 정합·다른 예약의 중복 쓰기 0건까지
  생산에서 확인해야 스펙 176과 목표를 완료할 수 있다.

## 최근 완료 — 2026-09-02 KST (#728 읽기 전용 Flutter 운영자 앱, #730 실제 iPhone 검증 마감)

자동매매 엔진을 휴대폰으로 옮기지 않고, 서버의 정제된 공개 운영 상태만 읽는 모바일 앱을
분리해 만들었다. 앱 장애가 worker·주문·기존 HTML·텔레그램 관측을 막지 않으며 모바일에서
실거래나 안전 중단을 해제할 수 없다.

- **공개 계약**: 기존 `status.html`·`index.html`을 유지하면서 같은 실행 자료를 담은
  `status.json`을 함께 발행한다. 최상위 형식은 `1.0`, `read_only: true`이며 기준 커밋,
  생성 시각, 생존 감시와 운영자 요약을 담는다. HTML 내 JSON과 외부 JSON 의미 일치,
  운영자 자료 누락 시 `null`, 금지 비밀값 부재를 통합 시험으로 고정했다.
- **현재 발행 확인**: Pages run `33632608454`와 Pages build가 성공했다.
  `https://jinooaction.github.io/claude/status.json`은 기준 `2d6054ee`, 생존 종합 `OK`,
  운영자 판정 `ATTENTION`, 자동화 25개를 반환했다. 이는 2026-09-02 21:54 KST의 시점 값이며
  이후 현재 상태는 앱·원본 실행 기록에서 다시 확인한다.
- **앱**: 비공개 저장소 `jinooaction/auto_invest_mobile` PR #1, merge `7fe13d6`.
  홈·자동화·설명 세 탭과 선택한 `Claymorphism + Vibrant & Block-based` 스타일,
  전용 앱 아이콘을 구현했다. 공개 자료에 없는 잔고·보유종목·손익은 만들지 않았다.
- **실패 폐쇄**: 지원하지 않는 버전, `read_only` 거짓, 손상·필수 필드 누락, 미래 시각,
  30시간 초과를 신선한 정상으로 해석하지 않는다. 네트워크 실패는 검증된 마지막 성공 자료만
  오프라인 배너와 함께 보여주며 캐시도 없으면 확인 불가로 끝난다.
- **검증**: auto-invest 전체 3279 passed/7 skipped, Ruff, 엄격 하네스 14/14,
  HANDOFF 사실 검사와 PR 품질 관문을 통과했다. 앱은 포맷·분석 0건·시험 11개,
  iOS 릴리스 서명 빌드 18.3MB와 iOS 시뮬레이터 실제 원격 자료 표시를 통과했다.
- **실제 iPhone 확인**: 운영자가 지정한 `대표실`(`com.jinooaction.ceooffice`)만 제거해
  무료 개발자 프로필의 앱 개수 관문을 해소했다. iPhone 14 Pro에 서명된
  `com.jinooaction.autoinvest`를 설치하고 전면 실행했으며 `Runner.app/Runner` 프로세스가
  실행 중인 것까지 확인했다. `Price Radar`와 `SayMemo`는 그대로 남겼다. QuickTime의 iPhone
  영상 소스가 선택되지 않아 물리 화면 픽셀 캡처와 터치 탐색은 별도 관찰 지점으로 남는다.
- **안전 의미**: broker·KIS·SSH·실거래 DB·감사 로그·자본·전략·허용 종목·한도·헌법·커널은
  바꾸지 않았다. 앱과 JSON 발행을 철회해도 기존 관측과 거래 경로는 그대로 남는다.
- **상세 문서**: `specs/178-flutter-operator-mobile/`와 비공개 앱 저장소의
  `README.md`, `docs/architecture.md`, `docs/design-system.md`, `docs/verification.md`.

## 최근 완료 — 2026-09-02 KST (#726 비용 현실형 장중 페이퍼 챌린저)

자동매매의 속도 장점이 곧 단타 수익이라는 가정을 버리고, 현재 저빈도 라이브 전략을 바꾸지
않은 채 장중 엣지를 별도로 검증하는 엔진을 출시했다. 엔진 완료와 시장 엣지 확인을 분리했으며,
실제 장중 자료가 없다는 사실을 무엣지나 합격으로 바꾸지 않았다.

- **고정 후보**: SPY·QQQ·IWM·TLT·GLD, 15·30·60분, 추세 지속·개장 범위 돌파·VWAP
  평균회귀의 빠른/느린 변형 18개를 결과 전에 고정했다.
- **현실 비용**: 다음 봉에서만 정수주 체결을 허용하고 KIS 기준·스트레스 수수료, 호가 차이,
  미끄러짐, 거래량 참여에 따른 부분·완전 미체결과 당일 청산을 장부에 남긴다.
- **시간·통계 경계**: 개발 504세션에서만 후보를 고르고 차단 126·최종 확인 126세션을 나중에
  평가한다. 최소 200거래와 PSR·DSR·PBO·낙폭·이익계수·분기·종목·상위 거래 집중도,
  스트레스 비용 관문을 모두 요구한다.
- **정직한 상태**: 현재 저장소에는 756세션의 실제 5분봉 자료가 없다. 따라서 지금 확인된 것은
  엔진의 정상 작동이며 시장 판정은 `INSUFFICIENT_EVIDENCE`다. 합성 시험 자료는 합격에 쓸 수 없다.
- **안전 분리**: 모든 결과는 `capital_fraction=0`, `live_eligible=false`,
  `promotion_allowed=false`, `orders_submitted=0`이다. 브로커·실거래 DB·라이브 설정·센티넬·
  sidecar를 읽거나 쓰지 않는다.
- **검증과 배포**: 목표 시험 27개와 전체 3278 passed/7 skipped, 린트, 엄격 하네스 14/14,
  HANDOFF 사실 검사와 PR 품질 관문을 통과했다. 기능 merge `73d07485`, off-hours 배포
  `33571077660`, exact-main KIS 읽기 전용 스모크 `33571146732` 6/6이 성공했다.
- **다음 경계**: 실제 자료 수집은 공급자 비용·자격증명·시점보존 계약이 필요한 별도 위험 작업이다.
  운영자 승인과 별도 스펙 없이 구매·수집하지 않는다. 자료를 확보해도 역사 관문 통과 뒤 최소
  60세션 자본 0 전진 페이퍼를 먼저 거치며 자동 라이브 승격은 없다.
- **상세 인계**: `HANDOFF-177-INTRADAY-PAPER-CHALLENGER.md`와
  `specs/177-intraday-paper-challenger/`.

## 최근 진행 — 2026-09-02 KST (#724 독립 예약·정수주 자금성 보정 생산 활성, 실제 체결 대기)

GitHub 예약 공급자가 늦어져도 같은 공급자를 더 자주 호출하는 방식에만 의존하지 않도록 서버에
독립 예약을 추가했고, 소액 계좌에서 1주도 살 수 없는 목표 하나가 안전하게 살 수 있는 다른 목표까지
전부 막던 자금성 분모 오류를 함께 고쳤다. 안전 기준을 낮추거나 실제 주문을 수동으로 만들지는 않았다.

- **코드와 배포**: #724 merge `7c28b127`. 배포 run `33552760635`의 2차 실행에서
  `auto-invest-live-canary.service`·`.timer`와 root 소유 예약 helper를 설치했고
  `systemd-analyze verify`를 통과했다. timer는 활성 상태이며 다음 발화는
  2026-09-02 14:35 UTC다.
- **중복 방지**: GitHub와 서버 예약은 같은 root 소유 거래일 선점을 사용한다. 먼저 성공적으로
  선점한 출처만 중개사 쓰기 경로에 들어가며, 늦은 다른 출처는 최초 run ID와 출처를 읽기 전용으로
  반환한다. 수동 서비스 시작·원격 systemd 주문·수동 workflow 실주문은 계속 거부한다.
- **정수주 계약**: 기존 66% 기준은 유지하되 현재 목표 금액으로 1주 이상 표현할 수 있는 목표에만
  적용한다. 최소 하나의 표현 가능 목표가 필요하며, 모든 양수 목표는 여전히 100% 시세·전체 25%·
  종목별 15% 오차와 현금·최소 주문·허용 종목·노출 한도를 통과해야 한다. 증거 schema는 1.1이다.
- **생산 사전 확인**: KIS 스모크 `33552911287`은 6/6, 현금 `$934.27`, NAV `$1434.07`,
  미체결 0건을 확인했다. 주문 없는 exact-main 사전점검 `33553034421`은
  `ENTRY_READY`, 정수주 표현 가능·자금 확보 비율 1.0, 사후 측정 `CLEAR`, 계좌 정합 `OK`,
  증거 `VALID`, 실제 주문 0건을 재현했다.
- **남은 완료 관문**: 첫 자동 실행의 실제 주문 접수와 실제 체결, 전략 추가 전용 감사, 같은 실행의
  계좌 정합, 다른 예약 출처의 중복 중개사 쓰기 0건을 생산에서 확인해야 한다. 지정가가 체결되지
  않아도 가격을 추격하지 않는다.

## 최근 진행 — 2026-09-01 KST (#709~#722 운영 검증 캐너리 무장·예약·보고·자본·정규장·배포 감사·시세 조회 보정, 실제 체결 대기)

“전략이 하나도 없다”는 거짓 결론과 실제 운영 오류를 함께 바로잡았다. 장기 시간 분리 성과가
강한 기존 정확 배포 전략은 현재 알파 확정 기준은 못 넘지만, 작은 실제 주문·체결 배관을 검증할
운영 캐너리에는 적격이다. 진단 연구와 자본 증거가 서로 덮어쓰던 오류를 분리하고 NAV 10%만
무장했다.

- **전략과 한계**: 홀드아웃 235개월에서 연복리 `8.682649%`, 샤프 `1.831434`, 최대낙폭
  `5.572914%`다. 기준 샤프 `1.264685`, 낙폭 `17.268823%`보다 낫지만 초과수익 PSR은
  `0.611914`이고 100bp 비용의 초과 샤프는 `-0.060163`이라 알파로 확정하지 않았다.
- **생산 상태**: #709 merge `0b955ac`, 배포 `33351793153`, KIS 스모크 `33351793304`
  성공. #710 merge `96d9c31`이 `armed:true`, 단 1, 자본 `$142`,
  `entry_route: operational_canary`를 기록했다. #711 merge `aa7b333`, 배포 `33353769899`,
  KIS 스모크 `33353769849`도 성공했다. 현재 전략 주문과 체결은 0건이다.
- **발견한 후속 오류**: `GITHUB_TOKEN`이 만든 PR 머지는 일반 PR·push 검사를 재귀 발화하지
  않아 #710이 검사 0개로 머지됐고 exact-main 증거도 자동 갱신되지 않았다. 수동 증거 갱신 뒤
  사전점검에서는 첫 매수 재검증을 `python3`로 직접 실행해 `pydantic` import에서 실패했다.
  두 실패 모두 주문 전 단계에서 차단돼 실제 주문은 없었다.
- **후속 오류 폐쇄**: #711은 자동 PR 머지 전 전체 테스트·린트·엄격 하네스·본문 관문을 직접
  실행하고, merge 뒤 exact-main 수익 증거와 무주문 사전점검을 연쇄 실행하게 했다. 고정 `uv`
  런타임으로 `pydantic` 의존성 오류도 제거했다. 생산 사전점검 `33353865976`에서 모든 단계가
  성공했고 `orders_submitted: 0`, `reconciliation_state: OK`를 기록했다.
- **예약 신뢰성 보정 완료**: 최근 schedule 실행은 15:00 UTC 정각 예약인데도 42~80분,
  두 차례는 약 9시간 늦었다. #712 merge `9ccb64b`는 라이브 캐너리만
  `America/New_York` 10:17 단일 예약으로 옮기고 money-path 표시도 같은 시각으로 맞췄다.
  배포 `33355972625`, exact-main 수익 증거 `33355972593`, money-path `33355972617`이 성공했다.
- **운영 보고 오류 폐쇄**: 생산 사전점검은 실제로 `ENTRY_READY`였지만 money-path가 JSON 앞
  메타데이터와 같은 이름의 표 행 때문에 이를 `preflight evidence absent`로 표시했다. #713
  merge `376f5d9`는 제목 범위의 기계 판독 JSON만 읽고, 손상 시에는 계속 실패 폐쇄한다.
  배포 `33358651283`과 money-path `33358651281`이 성공했고 생산 sidecar가
  `ok=True`, `reason=ENTRY_READY`를 재현했다. 주문 권한·자본·전략·안전 기준은 바꾸지 않았다.
- **정확한 main 인계와 자본 계약 폐쇄**: #714 merge `5c875de`는 최신 사전점검·배포 사실을
  다음 세션 인계에 반영했다. 이어 #715 merge `c413742`는 설정의 캐너리 자본 5%와 스펙·주문
  계산의 10%가 달랐던 결함을 제거하고, 선언값과 기대값이 다르면 주문 전에 실패하도록 했다.
  배포 `33361511135`, KIS 스모크 `33361511151` 6/6, 정확한 main 무주문 사전점검
  `33361615036`이 성공했다. 사전점검은 자본 `$142`, IAUM 1주·SCHX 2주, 총 `$105.32`,
  `orders_submitted: 0`, `reconciliation_state: OK`를 재현했다.
- **정규장 방어 출시**: GitHub 예약은 과거 최대 약 9시간 지연됐지만
  `rebalance-once` 명령 자체에는 XNYS 정규장 확인이 없었다. #716은
  실제 live·비미리보기 주문만 XNYS 정규장 안에서 허용한다. 시작부터 닫힌 장은 데이터베이스와
  중개사 접근 전에, 장중 시작 뒤 도중 마감은 실행 권한 잠금 뒤 각 실제 중개사 쓰기 직전에
  `market_hours_gate`로 감사 거부한다. CLI와 실주문 워크플로는 종료 코드 75를 전파하되, 앞선
  주문이 이미 접수된 부분 실행이면 후속 측정·보고를 계속하고 주문 0건으로 오인하지 않는다.
- **단일 예약 호출 누락과 후속 보정**: #716 merge `3d2592b`, 배포 `33379808130`, KIS 스모크
  `33379808023` 6/6, 정확한 main 무주문 사전점검 `33380005999`이 성공했다. 그러나
  2026-08-31 뉴욕 10:17 예약은 뉴욕 11:07까지 실행 기록 자체가 만들어지지 않았다. 단순 대기만으로
  이 실패를 없앨 수 없으므로 `Codex/176-schedule-redundancy`에서 같은 정규장에 10:17·11:17·12:17·13:17
  네 번 실행 기회를 주고, 서버의 추가 전용 거래일 장부와 파일 잠금으로 실제 주문 명령은 첫 실행만
  허용하도록 구현했다. 닫힌 장은 거래일 권한을 소비하지 않고, 중복 실행은 첫 실행 증거를 덮어쓰지
  않는다. #717 merge `e5aa0ef`로 main에 들어갔지만 배포 run `33408124750`은 장중 배포 금지에
  따라 2026-08-31 20:00 UTC 이후로 연기됐다. 서버가 exact main으로 올라오기 전에는 생산 활성
  상태가 아니며, 다음 거래일 최초 실행과 후속 중복 차단을 실제로 확인해야 한다.
- **거래일 전체 예약 미생성과 촘촘한 복구**: 원격 실행 이력을 다시 확인한 결과 2026-08-31에는
  `rebalance-live-canary`의 schedule run이 한 건도 없었고 마지막 예약 실행은 8월 28일이었다.
  시간당 한 번의 후속 기회도 거의 한 시간을 더 기다리게 하므로 #719 merge `203386e`는 최초
  10:17을 유지한 채 13:53까지 최대 12분 간격의 비정각 기회 19개로 늘렸다. 수동 실행은 계속
  주문 0건이며, production 서명·XNYS 정규장·NAV 10%·단 1·서버 거래일 1회 선점은 바꾸지 않았다.
  배포 run `33421400493`은 장중 배포 금지로 안전하게 연기됐고 로그상 서버 타이머의 다음 실행은
  2026-08-31 21:00 UTC다. 이 exact-main 배포가 확인되기 전에는 새 예약 복구가 생산 활성 상태가 아니다.
- **배포 감사 경계와 exact-main 배포 확인**: 배포 감사 workflow `33439750122`는 임의 원격 셸을
  금지하는 forced-command 경계에 `bash -s`를 보내 종료 코드 126으로 거부됐다. #721 merge
  `bbbfa70`은 GitHub와 서버가 모두 검증하는 고정 `deploy-audit` 명령과 root 소유 읽기 전용 helper로
  바꿨다. exact-main 배포 `33442103343`과 감사 `33442221977`이 성공했고, 서버의 추가 전용 감사
  사슬은 마지막 이벤트 `DEPLOY_COMPLETED`를 반환했다. 주문·배포 쓰기 권한은 넓히지 않았다.
- **거래소별 시세 오류 발견과 생산 보정 완료**: exact-main KIS 스모크 `33442299948`은 AAPL·현금
  `$934.27`·ORANY 28주·NAV `$1427.63`·미체결 0건은 정상이나 IAUM을 첫 후보 `NAS`로 조회할 때
  KIS가 500을 반환해 5/6 실패했다. #722 merge `4a305ba`는 5xx 뒤 `NYS/AMS`의
  제한된 후보를 계속 읽고 유효 시세의 실제 거래소를 주문 코드에 연결한다. 전 후보가 실패하면 마지막
  5xx를 다시 던지고 4xx는 즉시 실패해 중개사 전체 장애나 인증 오류를 숨기지 않는다. 관련 37개와
  전체 3236개 시험·린트·PR 품질 관문을 통과했고 exact-main 배포 `33445034323` 뒤 KIS 스모크
  `33445033903`이 IAUM `AMS` 시세를 찾아 6/6으로 회복했다. 무주문 사전점검 `33445149085`도
  `ENTRY_READY`, 주문 0건, 사후 측정 `CLEAR`, 계좌 정합 `OK`, 증거 `VALID`를 재현했다.
- **완료 조건**: 뉴욕 정규장 예약의 최초 유효 실행에서 실제 KIS
  지정가 접수·체결, 전략 장부·추가 전용 감사·계좌 정합을 같은 실행 ID로 확인해야 한다.

## 최근 완료 — 2026-08-31 KST (#706 PEAD 역사 엣지와 21가족 진단 교정)

“검토한 전략이 모두 쓸모없는가”와 “현재 실자본을 맡길 전략이 있는가”를 다시 분리했다.
기존 프로그램에는 21번째 가족을 성과와 무관하게 막는 용량 오류가 있었고, 이를 기준 완화가
아닌 보수 합계 유지 방식으로 교정했다. 교정 뒤 공개 PEAD는 역사 관문에 합격했지만, 현재
계좌의 시점보존 자료·공매도·정수 주문·체결 동등성이 없어 실거래 문은 계속 닫혀 있다.

- **프로그램 오류와 교정**: 기존 가족당 0.010 × 최대 20가족은 보수 상한 0.200을 모두
  소진해 21번째 가족을 자동 차단했다. 새 진단 `3.2`는 16후보 가족 11개에 0.010,
  64후보 가족 10개에 0.009를 배정해 독립성 가정 없이 상한 0.200을 유지한다. 샤프 0.6
  신호 검출률은 각각 84.0%, 80.4%다.
- **공개 역사 결과**: 816개 후보·21가족 중 사전등록된 PEAD 후보
  `pead-ann06of07-scale100-fd242e072b8b`가 `PUBLISHED_EDGE`다. PBO `0`, PSR
  `0.999985`, 연 1.5% 비용 후 초과수익 `6.892267%`, 최대낙폭 `13.8916%`이며
  3%·5% 비용 스트레스도 양수다.
- **과장 방지**: 최근 36개월 세 구간 수익은 모두 양수지만 `3.273% → 2.130% → 0.424%`로
  약해졌다. 공개 자료 미리보기는 온전한 미사용 홀드아웃이 아니며 당시 종목·상장폐지·공매도
  대차·현재 KIS 주문 동등성이 없다. 따라서 역사 엣지와 실자본 적격을 같은 뜻으로 쓰지 않는다.
- **독립 감사**: 생산자 요약을 믿지 않고 별도 소비자가 후보 ID 816개, 전략 지문 816개,
  가족 21개와 가족 크기 16×11·64×10, 보수 상한 0.200을 원시 행에서 다시 계산했다.
  기존 실자본 소비자는 새 진단 `3.2`를 의도대로 거부한다.
- **안전과 검증**: 연구 캐너리 거짓, 승격 거짓, 배포 설정 없음, 자본 0%, 주문 0건이다.
  전체 3192 passed/6 skipped, 린트, 엄격 하네스 14/14, HANDOFF 사실 검사와 PR 품질
  관문을 통과했다. 기능 merge `9d22a2b`, 배포 `33337092305` 성공이다.
- **생산 게시 보정**: 첫 전략 공장 `33337092281`은 두 시도 모두 PEAD 계산 뒤 셸 문자열
  비교에서 거짓 실패해 게시하지 못했다. #707 merge `77d0cf8`은 같은 기준을 JSON 자료형으로
  검사하고 불일치 필드를 출력하도록 고쳤다. 배포 `33338912939`, 전략 공장
  `33338912951`이 성공했고 sidecar `fa0a253`이 816개·21가족과 자본 0을 재현했다.
- **다음 관찰 지점**: 2026-09-01부터 SEC 공시시각을 보존한 신규 실적발표 200건과 최소
  12개월을 누적하고, 상장폐지 포함 수익률·공매도 대차·정수 주문·현재 계좌 체결 동등성을
  별도 계약으로 통과해야 자본 논의를 시작할 수 있다.
- **상세 인계**: `HANDOFF-175-PEAD-PUBLISHED-EDGE.md`와
  `specs/175-pead-program-gate/`.

## 최근 완료 — 2026-08-31 KST (#701 회계 기반 횡단면 팩터와 20가족 장부)

가격 추세 변형과 다른 경제 원천을 보기 위해 가치(HML), 영업수익성(RMW), 보수투자(CMA)의
8개 고정 조합과 0.5배·1.0배를 교차한 16개 후보를 결과 전에 사전등록했다. 역사적 프리미엄,
연구 관문 합격, 현재 계좌 실행 가능성을 서로 다른 상태로 기록한다.

- **시간 분리**: 2015년 7월 보관본의 1963-07~2013-12를 개발에만 쓰고 2014년을
  차단했다. 최신 공식 자료의 2015-01~2026-06 138개월은 후보 선택 뒤 한 번 열었다.
- **생산 판정**: 개발 승자 `accounting-factor-defensive-three-scale100-7cd34091ae29`은
  HML 20%+RMW 40%+CMA 40%다. PBO `0.277778`, 홀드아웃 PSR `0.271536`, 연 1.5%
  비용 후 현금 초과 `-1.439565%`, 양의 시대 1/4, 최근 구간 1/3으로 `NO_FACTORY_EDGE`다.
- **장부**: 기존 784개·19가족 뒤에 16개·1가족만 더해 800개·20가족으로 소비자가
  독립 재계산했다. 프로그램 오합격 상한은 허용 예산과 같은 0.20이므로 21번째 가족은
  사전 재교정 없이는 자동 차단한다.
- **실행 경계**: 공식 팩터 자료에는 당시 종목·수량·공매도 대차·정수 주문·KIS 체결
  동등성이 없다. 역사 결과와 무관하게 배포 설정 없음, 연구 캐너리 거짓, 승격 거짓,
...
- **공개 ID 결함 발견**: JSONL 문법은 모두 유효하지만 광범위 계좌번호 정규식이 공개
  `candidate_id`의 숫자 접미사 4개를 가렸다. 800개 지문은 고유하나 후보 ID는 799개만
  고유해 독립 소비자가 실패 폐쇄했다. 공개 후보 ID 보존, 과거 ID의 지문 기반 대체,
  게시 전 800 ID·800 지문·20가족 검증을 설계했다.
- **#704 최종 복구**: merge `c85e0a8`, 배포 `33327235868`, 전략 공장 `33327235891`이
  성공했다. 공개 ledger 959줄은 모두 JSON 객체이며, 게시 검증과 독립 소비자가 후보 ID
  800개·전략 지문 800개·20가족을 재계산했다. 소비자는 장부 오류 없이 성과·실행 관문으로만
  부적격 종료코드 3을 냈다.
- **다음 우선순위**: 21번째 가족 PEAD를 시작하기 전에 최대 가족 수·프로그램 오합격 예산·
  검출력을 다시 교정한다. 현재 자본은 열지 않는다.
- **상세 인계**: `HANDOFF-174-ACCOUNTING-CROSS-SECTIONAL-FACTORS.md`와
  `specs/174-accounting-cross-sectional-factors/`.

## 최근 완료 — 2026-08-30 KST (#699 판정 경로 교정과 독립 월말·월초 전략)

운영자의 의심대로 합격 판정과 감사 장부에 실제 오류가 있었다. 정상 표준 확정을 낡은 증거로
오인할 수 있던 빈 앵커드 결합, 교정을 읽지 않는 자본 소비자, 별도 전체 교정 없는 앵커드·탐색
자본 경로, 레짐 16개 누락, 첫 자본 10%/20% 설명 불일치를 고쳤다. 기준은 낮추지 않았다.

- **독립 전략 결과**: 월말 1~4일 × 월초 1~4일 16개를 공식 일별 자료로 고정 재생했다.
  개발 승자는 월말 2일+월초 4일이지만 홀드아웃 PSR `0.619345`, 연 현금 초과
  `+0.213632%`, 양의 시대 2/4, 편도 25bp 스트레스 `-3.335994%`라
  `NO_FACTORY_EDGE`로 탈락했다. PBO `0.126984`와 낙폭 방어는 통과했다.
- **정확한 감사 크기**: 기존 752개·17가족에 출시 레짐 16개와 달력 16개를 더해
  784개·19가족을 소비자가 독립 재계산한다.
- **자본 경로**: 표준 전진은 같은 커밋·같은 방법·오합격 통제·검출력 통과를 모두 요구한다.
  앵커드·탐색은 별도 전체 경로 교정 전 진단 전용이며 강등·정지·하향 재산정은 유지한다.
- **현재 상태**: `PREVIEW_ONLY`, `ACCUMULATING_EDGE`, 전진 4/20, 단0, 자본·주문 0건.
- **생산 증거**: 기능 merge `fc5af95`, 배포 `33318101840`, KIS 스모크
  `33318101829`, 공식 전략 공장 `33318101856` 성공. 공장 sidecar는 같은 main 커밋,
  784개 고유 후보·19개 전략 가족과 위의 동일한 5개 실패 관문을 재현했다. KIS 6/6,
  최근 주문·미체결 0건이며 공장도 주문·자본·라이브 전략을 바꾸지 않았다.
- **검증**: 3155 passed/6 skipped, ruff, 엄격 하네스 14/14, HANDOFF 사실 검사,
  결과 스키마, PR 품질 관문을 통과했다.
- **다음 우선순위**: 가격 시계열 변형 대신 회계 기반 횡단면 다요인 전략을 사전등록한다.
- **상세 인계**: `HANDOFF-173-TURN-OF-MONTH-AND-ROUTE-CALIBRATION.md`와
  `specs/173-independent-turn-of-month-edge/`.

## 최근 완료 — 2026-08-30 KST (#697 전략 합격 경로 검출력 감사와 연구 교착 해소)

“검토한 전략이 하나도 쓸모없다”는 결론과 “현재 실자본 합격 전략이 없다”는 결론을
분리했다. 전자는 틀리고 후자는 현재 증거상 맞다. 동시에 합격 판정 코드에 실제 설계 결함이
있었음을 확인하고, 기준을 낮추지 않은 채 오류를 고쳤다.

- **유망 전략은 존재**: 역사적으로 강한 `globalfixed-ensemble-3-6-9-12`가 있고,
  `regime-corr24-thr0p2-weak6-cash`는 8개 역사 관문 중 7개를 통과했다. 후자는 최근
  세 구간 승리 관문만 실패했고, 개선이 234개월 중 3개월에 집중돼 아직 실자본 후보가 아니다.
- **합격 경로 오류 확인**: 기존 “검출력 84%”는 가족 PBO와 홀드아웃 PSR 두 핵심 통계
  관문 교정이지 실제 8관문 전체의 검출력 증명이 아니었다. 새 감사기는 이를
  `PARTIAL_COVERAGE`로 표시하고 미교정 핵심 관문이 있으면 합격을 거부한다.
- **전진 기준 과신 차단**: 48개 관측에서 심은 샤프 1.5 신호의 검출률은 paper `42.40%`,
  live `15.45%`로 절대 요구치 `80%`에 못 미쳤다. 이제 이런 경우 `CALIBRATED`가 아니라
  `UNDERPOWERED`를 발행한다. 계산은 계속해도 자본 근거로 쓸 수 없다.
- **연구 교착 오류 수정**: 다음 후보 식별자에 `n_obs // 5`를 섞어 관측 수가 바뀔 때마다
  같은 연구를 새 후보처럼 만들던 구조를 제거했다. 관측 4→5에서도 같은 의미 기반 ID를
  유지하며, 출시된 후보를 중복 실행하지 않는다.
- **고정 후보 전진 관찰**: 레짐 후보와 지문, 2026-07 동결일, 최소 20관측을 계약으로
  고정했다. 공개 자료만 쓰는 월별 자동화가 현재 0/20 `OBSERVATION_WAIT`를 발행했고,
  주문·자본·라이브 설정은 건드리지 않는다.
- **다음 우선순위**: 기존 두 후보의 전진 관찰은 자동으로 계속한다. 동시에 자동 실행 루프가
  `candidate-parallel-edge-challenger-d59c4161bcba`를 다음 독립 신호군 연구로 안정적으로
  발행해, 20관측을 기다리느라 신규 탐색이 멈추지 않는다.
- **생산 증거**: 기능 merge `cb14c9e`, 배포 실행 `33284833563`, 새 레짐 관찰 실행
  `33284833549`가 성공했다. 최신 자본 경로는 `ACCUMULATING_EDGE`, `PREVIEW_ONLY`,
  `globalfixed` 4/20이며 자본·실주문은 0건이다.
- **검증**: #697에서 전체 3138 passed/6 skipped, ruff, 엄격 하네스 14/14,
  HANDOFF 사실 검사, PR 품질 관문을 통과했다. 최신 main 인계 브랜치 재검증은
  3139 passed/6 skipped, ruff, 엄격 하네스 14/14, HANDOFF 사실 검사 통과다.
- **상세 인계**: `HANDOFF-172-STRATEGY-ACCEPTANCE-PATH-AUDIT.md`와
  `specs/172-strategy-acceptance-path-audit/`.

## 최근 완료 — 2026-08-28 KST (#692 독립 저회전 레짐 후보 즉시 검증)

기존 최우수 전략의 최근 주식·채권 동반 약세 실패를 직접 겨냥해 16개 후보와 판정 기준을
결과 확인 전에 고정하고 즉시 검증했다. 통계 관문은 정상 작동했지만 선택 후보의 개선이
세 번의 월간 차이에 집중돼 독립적인 현재 엣지로 인정하지 않았다.

- **사전등록 후보**: 주식·채권 상관 창 12/24개월, 상관 기준 0/0.2, 동반 약세
  3/6개월, 피난 자산 현금/금의 조합 16개다. 2006-12까지 개발, 2007-01 격리,
  2007-02 이후 234개월을 홀드아웃으로 고정했다.
- **선택 결과**: `regime-corr24-thr0p2-weak6-cash`는 50bp 비용 후 연복리
  `8.695121%`, 샤프 `1.841788`, 최대낙폭 `5.659028%`다. 기존 전략은 각각
  `8.548482%`, `1.802237`, `5.659028%`였다.
- **판정 결과**: 가족 PBO `0.171429`, 홀드아웃 PSR `0.998215`, 최신 60개월 샤프,
  연회전율 `1.683761`은 통과했다. 그러나 최근 세 구간 중 한 구간만 기존 전략을 이겨
  사전등록 판정은 `NO_RESEARCH_EDGE`다.
- **집중도 감사**: 두 전략의 월간 수익 차이는 234개월 중 3개월에만 존재했고 모두
  2022-09~2023-03에 모였다. 최대 한 달이 전체 절대 개선의 `64.1856%`를 차지해,
  여러 레짐에서 반복된 독립 엣지라고 부를 수 없다.
- **기준 교정**: 16개 후보 가족은 무효 합격률 `1.0%`, 연 샤프 0.6 신호 검출률
  `84.0%`; 64개 후보는 각각 `0.4%`, `80.4%`였다. 이번 탈락은 통과 불가능한
  기준 때문이 아니라 시간 분산 실패 때문이다.
- **운영 결함 발견**: #692 배포는 미국 장중이라 서버 패키지 교체를 안전하게 미뤘지만
  SSH 관찰 스크립트만 최신화됐다. 최신 스크립트가 이전 명령줄에 없는
  `--token-cache`를 호출해 생산 동등성 JSON이 비었다. #693은 임시 DB를 기존 토큰과
  같은 `data/` 아래 만들도록 해 두 버전 모두에서 동작하게 했고, 재실행에서 세 쌍 모두
  통과했다.
- **돈 경로**: 최종 자동 판정 `33093842870`은 계좌 NAV `$1447.51`, IAUM 1주와 SCHX
  2주의 주문 구현 가능성, 실행 준비 참을 확인했다. 전략 증거는 미확정이라 `WAIT_EDGE`,
  단 0, 자본·주문 0건이다.
- **후속 오류와 보정**: 단 0의 불필요한 성과 조회는 #694에서 제거했다. 재실행은 동등성
  3/3을 통과한 뒤 건너뛴 성과 stderr 파일 부재와 해제되지 않은 임시 DB `RETURN` 트랩
  때문에 자본 결정 전에 실패했다. #695는 빈 증거 파일을 항상 만들고 성공 경로에서 임시
  파일을 지운 뒤 트랩을 해제했다. 최종 실행은 세 오류 문구 없이 자본 결정과 사이드카
  발행까지 성공했다.
- **상세 인계**: `HANDOFF-171-PARALLEL-REGIME-EDGE-CHALLENGER.md`와
  `specs/171-parallel-regime-edge-challenger/production-verification.md`.

## 최근 완료 — 2026-08-27 KST (#689·#690 소액 계좌 실행 동등성과 증거 판정 교정)

고가 정수 주 때문에 약 145달러 연구 자본이 영원히 주문 불가능하던 문제와, 종목별 최신
거래일 합집합으로 강화 캐너리가 거짓 무결성 실패를 내던 문제를 분리해 고쳤다.

- **실행 매핑**: 신호 `SPY/IEF/GLD`는 그대로 두고 거래만 `SCHX/SPTI/IAUM`으로 바꿨다.
  세 상품은 252개 공통 거래일의 수익률 상관·추적오차·수익률 차이·거래대금 기준을 매번
  통과해야 하며, 실패 증거 또는 36시간이 지난 증거는 첫 자본을 열 수 없다.
- **생산 확인**: 기능 merge `ecc84b9`, 교정 merge `0e8db90`, 최종 배포
  `33025942987`, KIS smoke `33025962368`이 성공했다. KIS 6/6 통과, 최근 주문·
  미체결은 0건이다.
- **주문 구현성**: 현재 추세 활성 두 종목 기준 145달러로 IAUM 1주와 SCHX 2주를
  한도 안에서 계획할 수 있어 `fundable=true`; 실제 실행은 드라이런이며 주문 0건이다.
- **현재 전략 판정**: 강화 캐너리는 무결성 0건으로 통과했지만 최근 5년 앵커드 평가는
  벤치마크 대비 샤프 우위 0/3이라 `NO_EDGE`; 자동 판정은 `WAIT_EDGE`, 자본 0이다.
- **판정 결함 교정**: 장기 first-write-wins DB의 옛 가격으로 만든 실패 동등성 문서를
  소비자가 구조상 진짜라는 이유만으로 통과 처리하던 오류를 고쳤다. 이제 문서 자체의
  `passed=true`까지 필수이고, 생산 동등성 점검은 매번 깨끗한 임시 KIS DB를 사용한다.
- **최종 자동 판정**: `33026038166`에서 세 동등성 쌍, 강화 캐너리, 145달러 주문
  구현성은 모두 통과했다. 현재 전략은 최근 앵커드 평가 0/3 우위라 `WAIT_EDGE`이며,
  자본·주문은 0건이다.
- **상세 인계**: `HANDOFF-170-SMALL-ACCOUNT-EXECUTION-PARITY.md`와
  `specs/170-small-account-execution-parity/production-result.md`.

## 최근 완료 — 2026-08-26 KST (#687 연구 가족 기준과 검정력 교정)

운영자의 의심대로 이전 v3 관문은 같은 탐색 위험을 DSR, PBO, 원시 752개 Bonferroni로
중복 처벌해 진짜 엣지 발견률을 지나치게 낮췄다. 이를 실제 17개 연구 가족과 측정된 검정력
예산으로 교정했지만, 현재 옵션 가족은 새 기준에서도 독립적으로 탈락했다.

- **기준 오류 수정**: 연구 자본 진입은 홀드아웃 PSR 0.95와 현재 가족 PBO 0.25를 요구한다.
  DSR 0.95와 원시 Bonferroni는 계산·대조하되 같은 실패를 다시 막는 문턱으로 쓰지 않는다.
- **전체 탐색 책임**: 752개 원시 행을 17개 가족으로 소비자가 재분류한다. 가족당 거짓 진입
  상한 1%, 프로그램 예산 20%, 최대 20개 가족이며 21번째는 자동 차단한다.
- **검정력**: 고정 모의실험에서 16개 가족은 무효 진입 1.0%·샤프 0.6 검출 84.0%,
  64개 가족은 무효 진입 0.4%·검출 80.4%다. 이전 중복 관문은 같은 신호를 약 42%와
  36%만 검출했다.
- **현재 판정**: 생산 공장 `32979548961`은 752/752 고유 행, 17/17 가족,
  PBO `0.371429`, 선택 후보 없음, `eligible=false`를 발행했다.
- **돈 경로**: 자동 자본 판정 `32980657704`은 `WAIT_EDGE`, 단 0→0, 자본 이동·주문
  0건이다. 145달러 단1 미리보기는 SPYM 한 주도 주문별 50% 캡을 넘어 차단됐다.
- **계좌 확인**: KIS 읽기 전용 `32980668128`은 5/5 통과했고 현금 `$934.27`,
  NAV `$1456.75`, 최근 주문·미체결 0건을 확인했다.
- **배포 구분**: 배포 실행 `32979548873`은 유닛 동기화에 성공했지만 미국 장중이라 코드
  교체를 거부했다. 다음 허용 시각은 `2026-08-26T20:00:00Z`, 타이머는 21:00 UTC다.
- **검증**: 3106 passed/5 skipped, ruff, YAML, 엄격 하네스 14/14, HANDOFF 사실 검사,
  PR 품질 관문을 통과했다.
- **상세 인계**: `HANDOFF-169-CALIBRATED-RESEARCH-ENTRY.md`.

## 이전 완료 — 2026-08-26 KST (#686 소비자 독립 증거와 소액 자금 구현성)

생산자 자기신고만으로 10% 연구 자본을 열던 경로를 소비자 독립 재계산으로 교체하고,
현재 계좌가 목표 포트폴리오를 정수 주로 실제 표현할 수 있는지 첫 주문 직전까지 확인한다.

- **실제 오류 수정**: 옵션 생산자가 개발 구간을 7개로 나눠 PBO가 항상 `None`이던 오류를
  8개 짝수 구간으로 고쳤다. 소비자는 752개 감사 행과 16개 현재 가족 행을 다시 센다.
- **정확한 구현성**: 현재 시세, 종목 매핑, 정수 주, 최소 주문액, 현금, 주문별·종목별·전체
  캡을 실제 주문 계획과 같은 코드로 적용한다. 기존 보유까지 포함하고 매도 후 매수 순서를 쓴다.
- **생산 결과**: v3는 752/752 고유 행을 확인했지만 현재 옵션은
  `NO_CROSS_INDEX_PREMIUM`, 선택 후보 없음, PBO `0.371429`로 자본 진입 불가다.
- **돈 경로**: 자동 승인 실행 `32973803207`은 `WAIT_EDGE`, 단 0→0, 자본 이동·주문 0건이다.
  현재 약 145달러 연구 자본은 SPYM/GLDM 정수 주와 캡 아래에서 목표 비중 구현에 실패한다.
- **생산 확인**: PR #686 merge `eed92c6`, 배포 `32972227005`, KIS 스모크
  `32972227160`, 전략 공장 `32972227100`이 성공했다. KIS 최근 주문·미체결은 모두 0건이다.
- **검증**: 3100 passed/5 skipped, ruff, YAML, 엄격 하네스 14/14, HANDOFF 사실 검사,
  PR 품질 관문을 통과했다.

## 이전 완료 — 2026-08-26 KST (#684 옵션 선택·목적 교정과 전체 시스템 리뷰)

옵션 프리미엄 존재와 포트폴리오 채택을 한 기준으로 묶었던 실제 설계 오류를 고쳤다.
PUT 안에서만 후보를 선택하고 결과를 보지 않은 WPUT에 그대로 재생해, 기준 오류와 후보 실패를
분리했다. 동시에 자동매매 시스템 전체를 데이터에서 체결 조정까지 감사했다.

- **중첩 선택**: 84개월 외부 학습, 1개월 금지 구간, 12개월 외부 평가 안에
  48개월 내부 학습, 1개월 금지 구간, 12개월 내부 검증을 뒀다. 12개 외부 구간마다
  내부 구간이 최소 2개 있고 시간 순서 위반은 0건이다.
- **독립 재생**: WPUT는 후보 선택, 기준 조정, 동률 처리에 전혀 쓰지 않았다. PUT가 고른
  후보 식별자와 월별 가중치를 그대로 WPUT에 적용했고, WPUT 전체 경로를 바꿔도 PUT 선택이
  변하지 않는 회귀 테스트를 추가했다.
- **생산 결과**: PUT 단독 프리미엄은 현금 대비 연 `+4.812579%`, PSR `0.954087`로
  통과했다. WPUT는 `+0.536821%`, PSR `0.638860`이라 독립 확인에 실패했다. 포트폴리오
  채택과 자동 타이밍도 두 지수 모두 실패해 최종 판정은 `NO_CROSS_INDEX_PREMIUM`이다.
- **전체 리뷰**: 주문 자동화 8.0/10, 자본·위험 안전 8.5/10이지만 수익 엣지 준비도는
  2.0/10이다. 현재 투자 가능 엣지 부재, 팩토리 소비자의 생산자 자기신고 신뢰,
  누적 752개 전체 다중검정 미해결, 옵션 기준지수와 실제 주문의 실행 동등성 부재가
  가장 높은 우선순위다.
- **생산 확인**: PR #684 merge `534b9cd`, 배포 `32910288188`, 전략 공장
  `32910288219`가 성공했다. 생산 공장은 16/16, 752/752 고유 감사, 12개 중첩 구간,
  같은 `NO_CROSS_INDEX_PREMIUM`을 발행했고 모든 승격 플래그는 거짓이다.
- **돈 경로**: `PREVIEW_ONLY`, 단 0, 자본 0, 실주문 불가, 전진 0/20, 전략 체결
  0건이다. 최신 KIS 읽기 전용 점검은 변경 전 main에서 5/5 통과했고, 이번 변경은
  브로커·주문·자본·마진·허용 종목·한도 파일을 건드리지 않았다.
- **검증**: 3087 passed/5 skipped, ruff, YAML, 결정론적 2회 재생, 엄격 하네스 14/14,
  HANDOFF 사실 검사, PR 품질 관문을 통과했다.
- **상세 인계**: `HANDOFF-168-OPTIONS-SELECTION-OBJECTIVE-REPAIR.md`.

## 이전 완료 — 2026-08-26 KST (#682 옵션 변동성 위험 프리미엄·752회 판정)

알려진 풋옵션 보험 프리미엄을 직접 검증해 “전략이 전혀 없는가”와 “채택 기준이
잘못 연결됐는가”를 분리했다. 경제적 프리미엄은 확인됐지만 주식 대체 채택 기준과
프리미엄 존재 기준을 섞은 목적 설계 오류, 그리고 개발 승자 선택 실패가 드러났다.

- **공식 자료·고정 분할**: 현재 Cboe PUT 연속 자료가 시작하는 2007-01-03 이후만
  사용했다. 결과 확인 전에 개발 84개월, 격리 1개월, 홀드아웃 147개월과 16개 후보,
  비용, 꼬리손실 기준을 고정했다.
- **사전 선택 후보**: `options-vrp-tail_guarded-7a1729980754`는 홀드아웃 PSR
  `0.803758`, 연 현금 초과 `+0.662141%`, 주식 대비 샤프 `+0.039261`, 최대낙폭
  `6.427959%`, 최악 5% 월평균 `-2.166609%`로 관찰 기준만 통과했다. 같은 비중의
  단순 PUT보다 연 `-1.958738%`, 샤프 `-0.043496`라 자동 타이밍 추가가치는 없다.
- **기준 오류**: 100% PUT 기준은 연 현금 초과 `+5.177143%`, PSR `0.964744`, 낙폭과
  최악손실도 주식보다 작았다. 최근 주식 대비 샤프가 `-0.124261`이라는 한 항목 때문에
  탈락했다. 평균 0 대조군은 탈락했고 모의실험은 거짓 합격 `2.4%`, 심은 엣지 검출
  `81.6%`라 통계 장치보다 목적 혼합이 원인이다.
- **사후 후보**: 50% 단순 PUT과 12개월 릿지 50%는 홀드아웃에서 실거래급 수치 관문을
  통과했지만 결과를 본 뒤 확인했으므로 승격 불가다. 현재 결과는 후보가 존재할 가능성을
  보여 주지만 독립 확증은 아니다.
- **생산 확인**: #682 merge `2141a7c`, 전략 공장 `32897412078`, 배포
  `32897412147`, KIS 읽기 전용 `32898262246`이 성공했다. 전략 공장은 16/16,
  752/752 고유 감사와 같은 `GATE_OR_REFERENCE_SUSPECT`를 발행했다.
- **돈 경로**: main 재평가는 `PREVIEW_ONLY`, `ACCUMULATING_EDGE`, 단 0, 자본 0,
  전진 관측 0/20, 실주문 불가다. KIS는 현금 934.27달러, NAV 1454.23달러, 외부 보유
  ORANY 28주, 최근 주문·미체결 0건이다.
- **검증**: 머지 직전 3084 passed/5 skipped, ruff, YAML, 엄격 하네스 14/14,
  HANDOFF 사실 검사와 PR 품질 관문이 통과했다.
- **상세 인계**: `HANDOFF-167-INDEPENDENT-OPTIONS-VARIANCE-RISK-PREMIUM.md`.

## 이전 완료 — 2026-08-26 KST (#680 에너지 교차시장 전략·736회 판정)

기존의 분산투자 보조 전략 기준을 모든 전략에 공통 적용하던 목적 분류를 바로잡고,
에너지 자체의 매매 시점을 맞히는 독립 후보 16개를 사전등록해 즉시 검증했다. 기준은
통과 가능했지만 사전 선택 후보는 에너지 단순보유보다 위험조정 성과가 나아지지 않았다.

- **목적별 판정**: 기존 분산 보조 기준은 그대로 두고, 단독 수익 기준과 자본 없는 연구
  관찰 기준을 별도로 추가했다. 기준 변경이나 과거 후보 재분류는 하지 않았다.
- **새 전략군**: WTI 추세, 3:2:1 정제마진, 에너지 시장 폭, 확장형 릿지 예측의 네
  문법에 6/12개월과 최대 에너지 50/100%를 결합한 16개다. EIA 월 `t` 자료는
  목표 월 `t+2`부터만 사용한다.
- **분리 검증**: 개발 120개월에서 `energy-cross-ridge_forecast-d869beb7c67e`
  하나만 선택하고 한 달 격리 뒤 219개월 홀드아웃에서 다시 고르지 않았다. 기존
  720개와 합친 736개 전략 지문이 모두 고유하다.
- **생산 판정**: 전략 공장 run `32861194112`, main `08970e7`. 홀드아웃 PSR
  `0.822115`, 50bp 후 연 현금 초과 `+1.912757%`, 에너지 단순보유 대비 샤프 변화
  `-0.002315`, 최대낙폭 `35.632935%` 대 `64.750875%`, 활성 비율 `95.8904%`로
  `NO_FACTORY_EDGE`다.
- **기준 결론**: 16후보 전체 선택 모의실험에서 무효 합격률 `4.4%`, 강한 신호 검출률
  `84.0%`다. 기준은 통과 가능하지만, 이번 후보는 거의 상시 에너지 노출이라 독립적인
  매매 시점 엣지가 부족했다.
- **사후 후보**: 시장 폭 후보는 PSR `0.861771`, 연 현금 초과 `+1.657890%`, 샤프
  개선 `+0.073612`로 연구 관찰 기준만 통과했다. 홀드아웃을 본 뒤 찾았으므로
  `promotion_allowed=false`이며 승격할 수 없다.
- **생산·계좌 확인**: 전략 공장은 공식 자료 새로고침부터 736/736 출판까지 성공했다.
  KIS 읽기 전용 run `32861858561`은 merge 커밋에서 5/5 통과했고 현금 934.27달러,
  총평가 1454.23달러, 기존 외부 보유 ORANY 28주, 최근 주문·미체결 0건이다.
- **배포 구분**: deploy run `32861194460`은 안전한 장중 연기로 끝났다. 서버는 미국
  정규장 중 변경을 거부했고 `auto-invest-deploy.timer`가 2026-08-25 21:00 UTC 이후
  재시도한다. 따라서 생산 연구 결과는 최신이지만 서버 코드 반영 완료로는 아직 부르지 않는다.
- **검증**: 3072 passed/5 skipped, ruff, YAML, 엄격 하네스 14/14, HANDOFF 사실
  검사, PR 품질 관문을 통과했다.
- **상세 인계**: `HANDOFF-166-INDEPENDENT-ENERGY-CROSS-MARKET.md`.

## 이전 완료 — 2026-08-25 KST (#678 USDA 작물 수급 전략·720회 판정)

USDA가 매달 당시 공개한 작물 수급 전망으로 독립 전략군을 사전등록하고, 기준을 바꾸지
않은 채 즉시 검증했다. 기준은 실제 강한 양성 전략을 통과시키고 무효 전략을 막았지만,
USDA 후보는 비용 후 수익과 포트폴리오 개선이 부족해 탈락했다.

- **공식 시점별 자료**: ESMIS의 2010-07-09~2026-08-12 발표 192개를 사용했다.
  동일 날짜 보관 별칭 5개는 옥수수·밀·대두의 시장연도·기말재고·총사용량이 모두 같을
  때만 허용하고, 다르면 안전 중단한다.
- **고정 후보**: 옥수수·밀·대두·동시 긴축, 1/3회 변화, GLD 최대 50/100% 조합 16개다.
  개발 60개월에서 하나만 선택하고 한 달 격리 뒤 131개월 홀드아웃을 다시 고르지 않고
  평가했다. 기존 704개와 합친 720개 전략 지문이 모두 고유하다.
- **사전 선택 결과**: `usda-crop-synchronized_tightening-02b25b9d8d4c`의 홀드아웃
  PSR은 `0.204521`, 50bp 비용 후 연 초과수익은 `-1.710304%`, 혼합 샤프 변화는
  `-0.117035`, 혼합 낙폭은 `15.316546%` 대 기존 `14.842601%`다.
- **사후 선택 감사**: 대두 3회 변화 후보는 사후 PSR `0.965194`, 50bp 후 연
  `+3.078124%`였지만 개발 샤프가 `0.110106`뿐이고 혼합 샤프가 `-0.049875`
  악화됐다. 홀드아웃을 본 뒤 선택할 수 없으며, 모든 관문을 통과한 실거래·종이 후보는
  각각 0개다.
- **기준 결론**: 16후보 보정에서 무효 실거래 합격률 `5.0%`, 연 샤프 0.6 신호 검출률
  `84.8%`다. 현실 양성 대조군 PSR `0.962691`은 통과하고 무효 대조군 `0.434559`는
  탈락했다. 이번 거부는 기준 불능이 아니라 후보 실패다.
- **생산 확인**: #678 merge `4f59ef7`, deploy `32846066131`, 전략 공장
  `32846066082`, KIS 읽기 전용 스모크 `32846263963`이 성공했다. 생산 전략 공장은
  16/16·720/720·자료 192개·별칭 5개와 동일한 `NO_FACTORY_EDGE`를 발행했다.
- **돈 경로**: 2026-08-25T12:12Z 재평가는 `PREVIEW_ONLY`, `ACCUMULATING_EDGE`,
  단 0, 자본 0, 전진 관측 0/20, 실주문 가능 `false`다. KIS는 현금 934.27달러,
  총평가 1454.23달러, 기존 외부 보유 ORANY 28주, 최근 주문·체결·미체결 0건이다.
- **검증**: 머지 직전 전체 3060 passed/5 skipped, ruff, YAML, 엄격 하네스 14/14,
  HANDOFF 사실 검사와 PR 품질 관문이 통과했다.
- **상세 인계**: `HANDOFF-165-USDA-CROP-SUPPLY-DEMAND.md`.

## 이전 완료 — 2026-08-25 KST (#673~#675 판정·장부·연구 캐너리 계약 교정)

운영자가 반복 지적한 “합격 전략이 없는 것이 기준 오류인가”를 후보 성과와 판정 배관으로
나눠 감사했다. 기준은 보수적이고 작은 표본에서 검출력이 낮았으며, 동시에 실제 구현 오류
세 가지가 있었다. 쌍체결 비교, 무레버리지 전진 장부, 16/16 생산자-소비자 계약을 차례로
고쳐 현재 전략 성과와 시스템 오류를 섞지 않게 했다.

- **통계 교정**: 전략과 기준자산의 같은 날짜 활성수익을 비교하는 PSR로 바꿨다. 완전 관문의
  무효자료 거짓 합격률은 4.82%지만, 48개 관측에서 연율 샤프 1.5 신호 검출률도 15.68%라
  현재 문턱은 거짓 양성을 잘 막는 대신 짧은 표본의 진짜 효과도 자주 놓친다.
- **장부 교정**: 레버리지와 서로 다른 자본 베이시스가 섞인 옛 전진 장부를 승격 증거에서
  제외하고 `forward_v2_trend.db`·`forward_v2_notrend.db`의 무레버리지 7개 트랙으로 새로
  시작했다. 현재 정확한 `globalfixed` 전진 관측은 0개다.
- **계약 교정**: 전략 공장 v2는 16개 가족을 의도적으로 발행하지만 세 소비자가 exact
  64/64를 요구해 미래 합격 전략도 영구 거부했다. 공용 버전 판정기로 16/16 완주,
  704/704 누적 고유 감사, 모든 차단 관문, 선택 후보·설정·지문을 함께 확인하게 했다.
- **마땅한 전략**: 정확 배포 후보 `globalfixed-ensemble-3-6-9-12`는 2007~2026년
  235개월 홀드아웃에서 비용 후 연복리 8.68%, 샤프 1.83, 최대낙폭 5.57%로 기준자산의
  8.29%, 1.26, 17.27%보다 좋다. 역사적으로 유망하지만 깨끗한 현재 전진 관측 0개라
  지금 실거래 합격이라고 부를 수는 없다.
- **현재 신규 전략**: EIA 공급·수요 후보는 비용 후 연 초과수익 +0.447504%, 기존 전략과
  상관 -0.135530, 혼합 샤프 +0.118590, 낙폭 개선을 보였지만 홀드아웃 PSR은
  0.705637이다. 경제적으로 유망하지만 통계적으로 미확정이라 `NO_FACTORY_EDGE`다.
- **생산 확인**: deploy `32813701562`, KIS `32813701577` 5/5, 전략 공장
  `32813806129`, 자본 사다리 `32814438836`, 수익 증거 `32814663999`, 실계좌 손익
  `32814666412`, 정합성 `32814668670`, 돈 경로 `32814909747`, 자본 준비도
  `32814940041`이 main `8b05fb9`에서 성공했다. 정합성은 `CLEAR/OK`, 주문 0건이다.
- **돈 경로**: 현재 `PREVIEW_ONLY`, 단 0, 자본 0, 전략 체결 0건이다. 자본 사다리 계산은
  단1=10%인데 돈 경로 안내문 일부가 옛 25%를 표시한 후속 보고 오류도 #676에서 교정했다.
  생산 money-path `32816555495`는 첫 자본 10%·약 145달러를 표시하고, capital-path
  `32816589404`는 `ACCUMULATING_EDGE`·`PREVIEW_ONLY`를 확인했다.
- **후속 생산**: #676 merge `194b7ed`, deploy `32816471355`, 라이브 미리보기
  `32816471308`, live-profit `32816515451`가 성공했다. `armed:false`라 승인·실주문 단계는
  건너뛰었고 체결 0건, 손익 0달러, 정합성 `CLEAR/OK`다.
- **검증**: #676 전 전체 3050 passed/5 skipped, ruff, YAML, 엄격 하네스 14/14,
  PR 품질 관문을 통과했다. 현재 후보가 미확정이라 계약·보고 교정 뒤에도 자본·주문은 변하지 않았다.
- **상세 인계**: `HANDOFF-164-RESEARCH-CANARY-EVIDENCE-PARITY.md`.

## 이전 완료 — 2026-08-25 KST (#671 전체 관문 감사·독립 원자재 공급/수요·704회 판정)

운영자의 의심을 통계선 하나가 아니라 실제 승격에 쓰는 다섯 관문 전체로 다시 감사했다.
이전 감사는 불완전했지만, 완전한 관문은 현실 양성 대조군을 통과시켰으므로 영구 탈락
구조는 아니다. 새 공급·수요 전략은 경제성 네 항목을 통과했지만 통계 확신이 부족했다.

- **전체 관문 감사**: AQR 분산 시계열 추세에서 연 50bp를 먼저 차감한 결과 PSR
  `0.962691`, 연 초과수익 `+4.580010%`, 기존 전략 상관 `-0.124257`, 80/20 혼합
  샤프 개선 `+0.170016`, 혼합 최대낙폭 `11.261123%` 대 기존 `17.268823%`로 다섯
  관문을 모두 통과했다. 같은 자료의 평균 제거 무효 대조군은 PSR `0.434559`, 연
  초과수익 `-1.373408%`, 혼합 샤프 `-0.042803`으로 실패했다.
- **기준 결론**: 스펙 157은 PSR 0.95만 현실 자료로 확인했으므로 전체 합격 기준 감사로는
  불완전했다. 스펙 158은 후보와 같은 기존 전략·비용·상관·혼합 샤프·낙폭 계산을 사용해
  완전한 기준이 실제 강한 분산 전략을 알아볼 수 있음을 확인했다. 대조군은 후보 선택이나
  다중검정 수에 들어가지 않고 승격 대상도 아니다.
- **새 전략군**: EIA 주간 상업 원유 재고·원유 생산·정유 투입·석유제품 공급량을 5일
  게시 지연 뒤 사용한다. 재고 감소, 수요 증가, 정유 당김, 동시 수급의 네 문법과
  52/104주, GSG 최대 50/100%를 결합한 16개를 결과 전에 고정했다.
- **분리 검증**: 개발 96개월에서 `commodity-supply-demand-synchronized_balance-fc61d460f7f2`
  하나만 선택하고 한 달 격리 뒤 142개월 홀드아웃에서 다시 고르지 않았다. 과거 688개와
  현재 16개를 합친 704개 전략 지문이 모두 고유하다.
- **생산 판정**: 전략 공장 run `32800129499`, main `7cca336`. 홀드아웃 PSR
  `0.705637`, 50bp 비용 후 연 초과수익 `+0.447504%`, 기존 전략 상관 `-0.135530`,
  혼합 샤프 개선 `+0.118590`, 혼합 낙폭 `11.073856%` 대 기존 `14.842601%`다.
  경제성 네 항목은 통과했지만 PSR만 라이브 0.95와 종이 0.80에 못 미쳐
  `ECONOMICALLY_PROMISING_STATISTICALLY_UNCONFIRMED`, `NO_FACTORY_EDGE`다.
- **배포·계좌**: deploy run `32800129484`, released-work run `32800129575`, 전략
  공장 run이 성공했다. KIS smoke run `32800376155`은 5/5, 키 유효, 현금
  934.27달러, NAV 1457.59달러, 기존 외부 보유 ORANY 28주, 최근 주문 0건,
  열린 미체결 0건이다.
- **돈 경로**: money-path run `32800725744`은 `PREVIEW_ONLY`, `NO_EDGE_YET`,
  단 0/5, 자본 0, 실주문 불가, 전략 체결·손익 0을 확인했다. capital-path run
  `32800727676`도 `ACCUMULATING_EDGE`이며 우선 승격 후보가 없다고 판정했다.
- **검증**: 전체 3020 passed/5 skipped, ruff, YAML, 엄격 하네스 14/14,
  HANDOFF 사실 검사, PR #671 품질 관문과 충돌 없음 상태가 통과했다.
- **다음 판단**: PSR `0.705637`을 본 뒤 종이 기준을 낮추면 결과 맞춤이다. 같은 16개를
  반복하지 않고 USDA 작물 수급 또는 에너지 교차시장 복제를 새 SDD로 사전등록한다.
- **상세 인계**: `HANDOFF-163-FULL-GATE-SUPPLY-DEMAND.md`.

## 이전 완료 — 2026-08-25 KST (#669 현실 관문 감사·독립 원자재 재고/포지셔닝·688회 판정)
...
* **2026-08-27 최신 베이스라인** — `main` `0e8db90`에 스펙 170과 후속 증거 판정
  교정이 출시됐다. 소액 연구 자본의 정수 주 주문 구현성과 실행 ETF 동등성은 통과한다.
* **현재 돈 경로** — `PREVIEW_ONLY`, 단 0, 자본 0, 주문 0건이다. 자동 판정
  `33026038166`은 실행 준비를 참으로 확인했지만 전략이 `NO_EDGE`라 `WAIT_EDGE`다.
* **현재 운영 검증** — KIS run `33025962368`은 현재 merge 커밋에서 읽기 전용
  6/6를 통과했고 최근 주문·열린 미체결은 0건이다. 세 실행 ETF 동등성 쌍도 모두 통과했다.
* **서버 배포** — deploy run `33025942987`이 `main` `0e8db90` 배포를 성공했다.
  후속 자동 판정도 같은 코드에서 완료됐으며 자본 이동과 주문은 없었다.
...
- `HANDOFF-178-BOUNDED-SAME-SESSION-RETRY.md` — 첫 자동 실행이 완전 무접수로 끝난 고정
  사고에만 원래 거래일 선점을 보존하면서 root 서버 예약이 같은 장에서 한 번 자동 복구하고,
  접수 가능성이 조금이라도 있으면 실패 폐쇄하는 production 계약이다.
- `HANDOFF-173-TURN-OF-MONTH-AND-ROUTE-CALIBRATION.md` — 판정 거짓 음성·교정
  미소비·레짐 장부 누락·첫 자본 설명 오류를 고치고, 독립 달력 전략 16개를 784개·19가족
  장부에서 검증했다. 달력 전략은 비용 후 신뢰도와 시간 분산 부족으로 탈락했다.
- `HANDOFF-172-STRATEGY-ACCEPTANCE-PATH-AUDIT.md` — 유망 전략 부재와 실자본
  합격 부재를 분리하고, 실제 8관문 교정 범위·48관측 절대 검출력·후보 ID 교착을 고쳤다.
  두 고정 후보는 주문 없이 전진 관찰하고 다음 독립 신호군 연구는 병렬로 계속한다.
- `HANDOFF-171-PARALLEL-REGIME-EDGE-CHALLENGER.md` — 주식·채권 동반 약세의 독립
  레짐 후보 16개를 사전등록해 검증했다. 선택 후보는 7/8 관문을 통과했지만 개선이 3개월에
  집중돼 자본·주문 0건을 유지했다.
- `HANDOFF-170-SMALL-ACCOUNT-EXECUTION-PARITY.md` — 소액 계좌의 정수 주 주문
  불가능, 거래일 정렬, 실행 ETF 동등성, 실패 증거 거짓 승인 오류를 교정했다. 실행 계층은
  준비됐지만 현재 전략이 최근 3개 구간 모두 기준자산을 이기지 못해 자본·주문 0건이다.
- `HANDOFF-169-CALIBRATED-RESEARCH-ENTRY.md` — 752개 원시 후보를 17개 실제 연구
  가족으로 재구성하고 중복 통계 문턱을 검정력 보정 기준으로 교체했다. 당시 옵션 가족은
  새 기준에서도 PBO 0.371429와 소액 구현 불가로 단 0·자본 0·주문 0건을 유지했다.
- `HANDOFF-168-OPTIONS-SELECTION-OBJECTIVE-REPAIR.md` — 옵션 프리미엄 존재,
  포트폴리오 채택, 자동 타이밍을 분리하고 PUT에서 선택한 후보를 WPUT에 독립 재생했다.
  현재 수익 엣지 준비도와 시스템 전체 위험을 함께 감사했다.
- `HANDOFF-167-INDEPENDENT-OPTIONS-VARIANCE-RISK-PREMIUM.md` — 알려진 PUT 프리미엄,
  목적 관문 혼합, 개발 승자 선택 실패, 752개 생산 감사와 다음 독립 확증 경로.
- `HANDOFF-166-INDEPENDENT-ENERGY-CROSS-MARKET.md` — 목적별 판정 경로를 분리하고
  에너지 교차시장 후보 16개와 736개 고유 감사를 출시했다. 기준은 통과 가능했지만
  사전 선택 후보가 에너지 단순보유 대비 샤프를 개선하지 못해 단 0·자본 0·주문 0건을 유지한다.
- `HANDOFF-165-USDA-CROP-SUPPLY-DEMAND.md` — USDA 시점별 발표 192개로 작물 수급
  후보 16개와 720개 고유 감사를 출시했다. 생산은 `NO_FACTORY_EDGE`라 단 0·자본 0·
  주문 0건을 유지한다.
- `HANDOFF-164-RESEARCH-CANARY-EVIDENCE-PARITY.md` — 쌍체결 활성수익, 무레버리지
  전진 장부, 16/16 연구 캐너리 소비 계약을 교정했다. 역사적 `globalfixed`는 유망하지만
  깨끗한 전진 관측 0/20이라 실거래 후보가 아니다.
- `HANDOFF-163-FULL-GATE-SUPPLY-DEMAND.md` — 후보와 같은 전체 다섯 관문이 현실
  양성 대조군을 통과하고 무효 대조군을 막음을 입증했다. EIA 공급·수요 후보 16개는
  경제성 네 항목을 통과했지만 PSR가 부족해 단 0·자본 0·주문 0건을 유지한다.
