...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `6c849a2` — Merge pull request #663 from jinooaction/Codex/154-credit-spread-carry |
| main 테스트 | #663 기능 머지 코드와 인계 갱신 브랜치 모두 `uv run pytest -q` → 2981 passed, 5 skipped. 인계 갱신 전에는 낡은 커밋 행을 정확히 잡아 하네스 2건만 실패했고, 최신 행 반영 뒤 전체 통과했다. |
| main 린트 | #663 기능 머지 코드와 인계 갱신 브랜치 모두 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 기능 PR 없음. 이 인계 갱신은 `Codex/handoff-after-credit-spread-carry`에서 진행한다. |
| 출시 완료 스펙 | 최신 기능: #663(스펙 154, 독립 회사채 스프레드 전략 64개와 640개 고유 감사 장부), #662(스펙 153, 가족별 다중검정과 개발/홀드아웃 분리 관문), #659/#660(스펙 152, 미국 국채 5개 만기 캐리 전략 64개와 공개자료 보고 정합 보정), #657(스펙 151, 독립 거시 전략 64개), #653(스펙 150, 자동 전략 공장). |
| 골격 스펙 | 없음. 다음 신규 기능은 독립 외환 금리차 전략을 새 SDD로 사전등록한다. |
| 최근 출시 작업 | 스펙 154가 투자등급 회사채와 국채의 금리차를 이용한 전략 64개를 생산 자료로 검증했다. 최고 후보도 홀드아웃 혼합 PSR 0.550035와 샤프 개선 0.033178로 기준 0.95/+0.05에 못 미쳐 탈락했다. |
| 활성 작업 | 기능 구현은 없음. 스펙 154 생산 결과와 계좌 사실을 인계에 닫는 중이다. 다음 우선순위는 같은 후보를 반복하지 않고 독립 외환 금리차 전략군을 사전등록하는 것이다. |
...
## 최근 완료 — 2026-08-24 KST (#663 독립 회사채 스프레드 전략 공장·640회 판정)
