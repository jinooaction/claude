...
## 최근 마일스톤 — 2026-05-29 (스펙 019: 레짐 인식 + 완전 공분산 ERC)
...
* **main의 테스트**: 1161 통과, 4 스킵 (라이브 KIS 스모크 4건은 `KIS_LIVE_TEST=1` 환경변수로 게이트).
...
| 마지막 main 커밋 | `6c1d849 Merge pull request #93 from jinooaction/claude/spec-019-regime-erc` |
| 활성 작업 | **없음 — 스펙 019(레짐 인식 + 완전 공분산 ERC) 완료·머지(2026-05-29, PR #93 `6c1d849`).** 라이브 worker dry-run 가동 중. 레짐 감지(추세/횡보/하락 3상태, SMA50/200 기반)로 신호 배율 자동 조절 + 공분산 행렬 기반 ERC(등기여 위험 반복 최적화) 사이징 추가. walk-forward 표본 외 검증 통과. 다음 후보: 레짐 필터를 order_router·replay에 실제 배선, ERC 모드 YAML 룰 예제, 교차 단면 모멘텀 랭킹. 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015, 016(슬라이스 1·2·3 전부), 017(슬라이스 1·2·2b·3 전부), 018(슬라이스 1 다요인 신호 + 슬라이스 2 사이징 감사 기록), 019(레짐 인식 + 공분산 ERC) |
| 진행 중 스펙 | 없음. 스펙 019 출시 완료. 후속(미착수): 레짐 필터 실제 배선(order_router·replay), 교차 단면 모멘텀 랭킹, 양방향 그룹 budget-split |
| 골격 스펙 (즉시 착수 가능) | **스펙 020 — 레짐 필터 배선** — `strategy/regime.py`·`strategy/sizing.py`의 ERC/레짐 함수가 구현됐으나 아직 `execution/order_router.py`·`backtest/replay.py`에 실제 배선이 안 됨. 다음 단계는 이 두 경로에 레짐 스케일과 ERC 그룹 가중치를 연결해 walk-forward 검증 받는 것 |
...
| main 테스트 | 1161 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
