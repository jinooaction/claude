...
| 열린 PR | #793 초안: 사용자 실행 명령·실제 시세 입력. 실주문 연결 미완료로 병합하지 않았다. #790~#792 병합 완료. |
...
| 활성 작업 | 스펙184 실행 명령·입력기와 기존 계좌 보유 분리는 관련196개·전체3792/10·린트·하네스 검증 완료, PR793 초안이다. 기존 계좌 사용은 확정됐다. 새 입력기의 인증 후 KIS 검증, 전체 계좌 관측·단타 권한·실주문 운용 연결이 남았다. 기존 main의 실제 조회9개도 재검증했지만 전체 현금/NAV는 미검증이다. 실제 자료·전략 합격·정식 전진60세션·생산 체결 증거를 대체하지 않는다. |
