...
## 최근 마일스톤 — 2026-05-27 (스펙 015: 라이브 체결 동기화)
...
- `HANDOFF-020-SPEC-015-FILL-INGESTION.md` — **최신**. 스펙 015 라이브 체결 동기화 출시 (2026-05-27, PR #73 `e746f52`). 접수 주문의 실제 체결을 브로커 조회로 멱등하게 FILL 기록·보유 갱신·상태 전이. Kernel 터치 0건. 스펙 014 브레이커·스펙 011 성과·정합성을 라이브에서 작동하게 하는 키스톤. 다음 세션 참고.
...
| 마지막 main 커밋 | `e746f52 Merge PR #73 — feat(015): 라이브 체결 동기화` |
| 활성 작업 | **없음 — 스펙 015(라이브 체결 동기화) 완료·머지(2026-05-27, PR #73 `e746f52`).** 라이브 worker dry-run 가동 중. 다음 후보: 체결 가격 정밀도(누적 한계가), 미체결 명시 조회(`inquire-nccs`), 헬스 연동(T014 보류), 모델 교체 노브. 실거래 전환은 운영자 명시 지시 필요(이제 라이브 측정·안전 토대 작동) |
| 출시 완료 스펙 | 001, 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015 |
| 진행 중 스펙 | 없음 |
...
| main 테스트 | 1024 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
