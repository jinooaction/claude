...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `77d0cf8` — Merge pull request #707 from jinooaction/Codex/175-final-handoff |
| main 테스트 | 스펙 175 기능 브랜치에서 `uv run pytest` → 3192 passed, 6 skipped, 실패 0. 건너뛴 6개는 `KIS_LIVE_TEST=1` 전용이다. |
| main 린트 | 스펙 175 기능 브랜치에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 2026-08-31 #707까지 병합됐고 열린 기능 PR은 없다. 이 최종 생산 인계 갱신만 별도 브랜치에서 진행 중이다. |
| 출시 완료 스펙 | 최신 기능: #706(스펙 175, 21가족 교정·실적발표 후 주가 지연반응 PEAD 16개·816행 감사), #701(스펙 174, 회계 기반 횡단면 팩터 16개·800행·20가족 검증), #699(스펙 173, 판정 경로 교정·독립 월말월초 16개), #697(스펙 172, 8관문 합격 경로 감사). |
| 골격 스펙 | 없음. 스펙 175까지 완료됐다. 다음 기능은 PEAD를 실거래로 바로 승격하는 것이 아니라 2026-09-01 이후 시점보존 전진 관찰과 현재 계좌 실행 동등성 계약을 채우는 일이다. |
| 최근 출시 작업 | 21번째 가족이 성과와 무관하게 막히던 고정 `1% × 20가족` 한도를 보수 합계 0.200을 유지하는 크기별 상한으로 교정했다. 공개 PEAD는 PBO 0, PSR 0.999985, 연 1.5% 비용 후 초과수익 6.892%로 `PUBLISHED_EDGE`에 합격했다. |
| 활성 작업 | 없음. 보정 배포 run `33338912939`와 전략 공장 run `33338912951`이 성공했다. 공개 sidecar commit `fa0a253`에서 독립 소비자가 816개 고유 후보·지문, 21가족, 보수 상한 0.200, `PUBLISHED_EDGE`를 재계산했다. 기존 돈 경로는 진단용 `3.2`를 거부하며 자본·실주문은 0건이다. 다음은 2026-09-01 이후 시점보존 전진 관찰이다. |
| 안전 경계 | main은 헌법 X.4 v10.1.0이며 변경하지 않았다. 새 `3.2`는 21가족 연구 진단일 뿐 실자본 계약이 아니다. 손실 예산 20%, 정규장, 킬스위치, 주문·노출 한도는 유지했고 자본은 0이다. |
