...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `534b9cd` — Merge pull request #684 from jinooaction/Codex/165-options-selection-objective-repair |
| main 테스트 | #684 머지 직전 `uv run pytest` → 3087 passed, 5 skipped. 건너뛴 5개는 KIS 실연결 전용이며 최신 변경 전 KIS 읽기 전용 점검은 5/5 통과했다. 이번 변경은 브로커·주문 경로를 건드리지 않았다. |
| main 린트 | #684 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | #684 기능·생산·배포는 완료됐다. 이 인계 갱신 외 진행 중 기능은 없다. |
| 출시 완료 스펙 | 최신 기능: #684(스펙 165, 옵션 선택·목적 기준 교정과 WPUT 독립 재생), #682(스펙 164, 옵션 변동성 위험 프리미엄 16개·752개 고유 감사), #680(스펙 163, 에너지 교차시장 16개·736개 고유 감사), #678(스펙 162, USDA 작물 수급 16개·720개 고유 감사), #675(스펙 161, 연구 캐너리 증거 계약 정합), #674(스펙 160, 무레버리지 전진 장부). |
| 골격 스펙 | 없음. 다음 우선순위는 생산자 자기신고를 독립 검증하는 `family-complete-v3`와 실제 소액 주문으로 목표 비중을 만들 수 있는지 확인하는 자금 구현 가능성 관문을 하나의 등급 4 스펙으로 사전등록하는 것이다. 그다음 누적 752개 전체 탐색의 다중검정 예산을 추가한다. |
| 최근 출시 작업 | PUT에서만 안쪽 중첩 선택하고, 선택에 쓰지 않은 주간 WPUT에 같은 후보와 가중치를 재생했다. PUT 단독 프리미엄은 연 +4.812579%, PSR 0.954087이지만 WPUT는 +0.536821%, PSR 0.638860이라 독립 확인에 실패했다. 판정은 `NO_CROSS_INDEX_PREMIUM`이다. |
| 활성 작업 | 기능 구현 없음. 최신 돈 경로는 `PREVIEW_ONLY`, `ACCUMULATING_EDGE`, 단 0, 자본 0, 실주문 불가이며 `globalfixed` 전진 관측은 0/20이다. #684 배포 실행 `32910288188`과 생산 전략 공장 `32910288219`가 성공했고, 전략 체결은 0건이다. |
| 안전 경계 | 헌법 X.4 v9.0.0. 완결 전략군·누적 고유 감사·정확한 설정 지문·강화 검사를 통과한 후보에만 NAV 10% 연구 단 1을 허용한다. 기존 20% 이상 승격 기준과 K1/K2, 손실 예산 20%, 정규장, 현금 1% 여유, production 기계 승인, 서명·nonce, 추가-전용 감사 로그를 유지한다. ORANY 28주는 시작 전 외부 보유로 전략 손익에서 제외한다. |

## 최근 완료 — 2026-08-26 KST (#684 옵션 선택·목적 교정과 전체 시스템 리뷰)
