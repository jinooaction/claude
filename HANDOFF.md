...
## 최근 마일스톤 — 2026-05-29 (스펙 023: 가격 기반 퀄리티 팩터 필터)

PR #100 머지 커밋 `674c8dc`. **가격 시계열만으로 퀄리티를 측정하는 옵트인 필터를 추가**했습니다. KIS API 재무 데이터 없이 롤링 샤프 비율과 최대 드로다운을 합성해 "수익을 내면서 덜 다치는" 종목을 고르는 횡단면 퀄리티 팩터입니다.

- **`strategy/quality.py`(신규, 비커널)**: `price_quality_score(bars, *, lookback_bars)` — 롤링 샤프 / (1 + |최대 드로다운|) 점수. 데이터 부족(< 30봉) → `Decimal("-Inf")`. `quality_ranked()` — 유니버스 전체 내림차순 순위.
- **`config/rules.py`(비커널)**: `QualityFilter` 모델(`universe`, `lookback_bars`, `top_n`|`top_pct`). `TradingRule.quality_filter` 옵트인 필드.
- **`execution/order_router.py`(비커널)**: 랭킹 필터(스펙 021) 이후 퀄리티 필터 분기. 미통과 → `SKIPPED_BY_QUALITY(not_in_top_quality)`.
- **`backtest/replay.py`(비커널)**: 동일 패턴 (session_date 이하 바만 — 미래 참조 없음).
- **안전 경계**: Kernel 터치 0건. 하향 전용(스킵만, 수량 증가 없음). 옵트인(기존 룰 byte 동일). 결정론적 Decimal(헌법 X.2).
- **검증**: 신규 테스트 8건(SC-01~SC-08), 전체 1195 통과, 4 스킵, 린트 깨끗.

## 이전 마일스톤 — 2026-05-29 (스펙 022: 최소 분산 포트폴리오 최적화)

PR #99 머지 커밋 `204dfc9`. ERC를 넘어 포트폴리오 분산을 직접 최소화하는 `mode="min_variance"` 사이징 모드 추가. 분석적 해법 `w* = Σ^{-1}·1 / (1'·Σ^{-1}·1)` (numpy linalg.solve). 수치 실패 → ERC → 역변동성 순 fallback 체인. Kernel 터치 0건. 옵트인. 신규 테스트 8건, 전체 1187 통과.
...
* **스펙 022 (최소 분산 포트폴리오 최적화)** — **출시 완료** (2026-05-29, PR #99 `204dfc9`). `SizingConfig.mode="min_variance"` — numpy `linalg.solve`로 분석적 최소 분산 해(`w* = Σ^{-1}·1 / (1'·Σ^{-1}·1)`). ridge 정규화(ε = max_diag × 1e-6). 음수 클램핑 + 재정규화(롱-온리). 수치 실패 → ERC → 역변동성 fallback 체인. **Kernel 터치 0건. 옵트인. max 1 클램핑(하향 전용).** 신규 테스트 8건, 전체 1187 통과.
* **스펙 023 (가격 기반 퀄리티 팩터 필터)** — **출시 완료** (2026-05-29, PR #100 `674c8dc`). 롤링 샤프 / (1 + |최대 드로다운|) 합성 점수로 유니버스 순위를 매겨 하위 종목을 `SKIPPED_BY_QUALITY`로 차단. 새 모듈 `strategy/quality.py`(`price_quality_score`, `quality_ranked`). `TradingRule.quality_filter`(`QualityFilter`: `universe`, `lookback_bars`, `top_n`|`top_pct`). `order_router`·`replay` 적용. **Kernel 터치 0건. 하향 전용. 옵트인.** 신규 테스트 8건, 전체 1195 통과.
...
* **main의 테스트**: 1179 통과, 4 스킵 (라이브 KIS 스모크 4건은 `KIS_LIVE_TEST=1` 환경변수로 게이트).
...
| 마지막 main 커밋 | `674c8dc Merge pull request #100 from jinooaction/claude/spec-023-quality-factor` |
| 활성 작업 | **없음 — 스펙 023(가격 기반 퀄리티 팩터 필터) 완료·머지(2026-05-29, PR #100 `674c8dc`).** 라이브 worker dry-run 가동 중. 다음 후보: 기대 수익률 모델 + 최대 샤프 최적화(스펙 024), 실거래 캐너리 전환(운영자 명시 지시 필요) |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015, 016(슬라이스 1·2·3 전부), 017(슬라이스 1·2·2b·3 전부), 018(슬라이스 1 다요인 신호 + 슬라이스 2 사이징 감사 기록), 019(레짐 인식 + 공분산 ERC), 020(레짐·ERC 거래 루프 실배선), 021(횡단면 모멘텀 순위 필터), 022(최소 분산 포트폴리오 최적화), **023(가격 기반 퀄리티 팩터 필터)** |
| 진행 중 스펙 | 없음. 스펙 023 출시 완료. 후속(미착수): 기대 수익률 + 최대 샤프(스펙 024), 실거래 캐너리 전환 |
| 골격 스펙 (즉시 착수 가능) | **스펙 024 후보** — 모멘텀 신호를 기대 수익률로 활용해 평균-분산 전선에서 최대 샤프 포인트 직접 최적화 |
...
| main 테스트 | 1195 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
