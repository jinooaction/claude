...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `58162b1` — Merge pull request #548 from jinooaction/codex/handoff-after-kis-setup-pending (`HANDOFF.md`만 바꾼 인수인계 머지; 최신 코드 변경 머지는 `d3d1117` #547) |
| main 테스트 | #548 handoff refresh 기준 `uv run pytest -q` → 2691 passed, 5 skipped. 5개 skip은 `KIS_LIVE_TEST=1` opt-in live smoke다. |
| main 린트 | #548 handoff refresh 기준 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음(이 문서 편집 시점). |
...
| 최근 출시 작업 | #548은 #547 이후의 최신 KIS setup-pending 상태를 `HANDOFF.md`에 반영한 인수인계 갱신이다. #547은 KIS smoke가 SSH exit 255에서 멈춰 상태를 못 남기던 문제를 고쳐 `setup_pending`을 sidecar에 남기게 했다. money-gate-alignment도 이 상태를 `BLOCKED` KIS 이슈로 읽고 "서버에 새 deploy 공개키 설치 후 KIS smoke 재실행"을 다음 행동으로 낸다. #545는 operator-status Telegram 알림 재시도를 보강했고, #543/#542/#540/#538/#536은 돈 경로 차단·KIS secret 누락·전략 의도 게이트 실패를 사람이 읽을 수 있게 드러냈다. |
| 활성 작업 | 열린 PR 없음. GitHub repo secret 목록에는 `VULTR_SSH_USER`(2026-07-29T14:18:16Z)와 `VULTR_SSH_PRIVATE_KEY`(2026-07-29T14:18:17Z)가 등록돼 있다(값은 출력·커밋 금지). 2026-07-30 KST 재검증에서 로컬 SSH는 `gh-deploy`/`auto-invest`/`root` 모두 `Permission denied`였고, `Verify operator setup` run `30498370211`은 `ssh_failed`로 실패했다. KIS smoke run `30498371908`는 main `58162b1` 기준 workflow success이며 sidecar는 `secrets_present=true`, `key_valid=true`, `smoke_state=setup_pending`, `smoke_exit=255`다. money-gate-alignment run `30498408711`은 `BLOCKED`, operator-status run `30498543627`은 `ACTION_REQUIRED`/Telegram `SENT`다. 최신 다음 행동은 서버에서 `deploy/repair-ssh-boundary.sh`에 새 deploy 공개키를 설치한 뒤 KIS smoke를 다시 실행하는 것이다. #548은 문서-only라 deploy-on-merge 트리거가 없고, 직전 코드 머지 #547 deploy-on-merge run `30461091918`은 같은 서버 SSH 거부로 실패했다. money-path run은 `30404941885` 기준 `PREVIEW_ONLY`/자본 사다리 `BLOCKED`, micro GTAA 최신 run은 `armed=false`, `LIVE 스텝=skipped`, strategy-intent gate `ok=false`, `reason=latest_intent_loss`, `cumulative_pnl_usd=-1.14`다. |
| 안전 경계 | #547은 등급 2 운영 진단 보정이고 #548 및 이 갱신은 등급 2 인수인계 갱신이다. SSH 실패를 성공으로 꾸미지 않고 `setup_pending`으로 정확히 남긴다. GitHub repo secret 두 개는 등록돼 있지만 secret 값은 출력·커밋하지 않았다. 이번 재검증은 로컬 SSH와 GitHub read-only 진단, 상태판 갱신만 수행했고, 브로커 호출·주문·자본 배분·live 재무장·전략 교체·whitelist/caps 확대·손실 예산·KIS secret·감사 로그·헌법·kernel manifest는 바꾸지 않았다. 현재 돈 경로는 `PREVIEW_ONLY`라 실주문 불가다. |
...
6. 현재 기준(2026-07-29T23:08Z operator-status, 23:06Z money-gate-alignment, 23:05Z KIS smoke, 10:35Z money-path): micro GTAA는 `armed:false`, money-path `live_money_state.status=PREVIEW_ONLY`, 최신 micro sidecar는 `event=schedule`, `LIVE 스텝=skipped`, strategy-intent gate `ok=false`, `reason=latest_intent_loss`, `cumulative_pnl_usd=-1.14`다. money-gate-alignment 최신 run `30498408711`은 `BLOCKED`이고 KIS smoke를 `setup_pending`으로 읽는다. GitHub repo secrets는 등록됐지만 서버가 아직 새 deploy 공개키를 받아들이지 않아 `Permission denied (publickey,password)`로 막힌다. 다음 행동은 서버에서 `deploy/repair-ssh-boundary.sh`로 forced-command gateway를 설치하고 KIS smoke를 다시 실행한 뒤 자본 사다리와 전략 의도 게이트를 다시 읽는 것이다. 스펙 062의 2026-06-22 `armed:true` 기록은 역사이며 현재 상태 근거로 쓰지 않는다.
