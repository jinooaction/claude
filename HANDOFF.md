...
## 한눈 요약표 — 2026-09-07 KST 단타 자금 검토 기능 병합

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `a9d7e41` — Merge pull request #773 from jinooaction/codex/182-kis-account-connector |
| main 테스트 | #773 코드0f3dbda 전체3541 passed/8 skipped,실패0(841.59초). 관련41개 통과. 실제 읽기 전용 KIS34064890546은7/7(16.50초). |
| main 린트 | #773 코드0f3dbda에서 `uv run ruff check src tests scripts/intraday_capital_review.py` 통과. |
| 열린 PR | #773 병합 완료. `codex/182-capital-review-proof`는 검증 인계만 기록하며 열린 PR은 GitHub에서 다시 확인한다. |
...
| 활성 작업 | 운영자는600/12 준비 기준을 확정했다. 후속 브랜치에서 확정 설정·600달러 모의 사전점검·KIS 계좌GET 수집기를 구현했고 관련62개 통과, 전체 회귀/병합/실제 계좌 계약 검증은 진행 중이다. 실제 자금 배정·주문0이며 검증된NAV·전략/전진 증거·생산 주문 연결은 남았다. |
...
`codex/182-confirmed-budget-preflight`에서 confirmed-budget.json과
...
관련62개 통과, 전체 회귀/병합/서버 실제 읽기 계약은 진행 중이다.
