...
## 최근 마일스톤 — 2026-05-29 (스펙 024: 최대 샤프 포트폴리오 최적화)
...
| 마지막 main 커밋 | `86b2c32 Merge pull request #101 from jinooaction/claude/spec-024-max-sharpe` |
| 활성 작업 | **없음 — 스펙 024(최대 샤프 포트폴리오 최적화) 완료·머지(2026-05-29, PR #101 `86b2c32`).** 라이브 worker dry-run 가동 중. 다음 후보: 실거래 캐너리 전환(운영자 명시 지시 필요), 추가 알파 팩터 |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015, 016(슬라이스 1·2·3 전부), 017(슬라이스 1·2·2b·3 전부), 018(슬라이스 1 다요인 신호 + 슬라이스 2 사이징 감사 기록), 019(레짐 인식 + 공분산 ERC), 020(레짐·ERC 거래 루프 실배선), 021(횡단면 모멘텀 순위 필터), 022(최소 분산 포트폴리오 최적화), 023(가격 기반 퀄리티 팩터 필터), **024(최대 샤프 포트폴리오 최적화)** |
| 진행 중 스펙 | 없음. 스펙 024 출시 완료. 후속(미착수): 실거래 캐너리 전환(운영자 명시 지시 필요) |
| 골격 스펙 (즉시 착수 가능) | **실거래 캐너리** — Vultr 서버에서 KIS 시크릿으로 `AUTO_INVEST_MODE=live` 전환. 아키텍처 준비 완료, 운영자 명시 지시만 필요. |
...
| main 테스트 | 1203 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
