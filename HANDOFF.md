...
## 한눈 요약표 — 2026-09-03 KST 최신 코드 main 기준

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `020f8aa0` — Merge pull request #746 from jinooaction/codex/176-kis-order-rejection-fix |
| main 테스트 | #746 exact 기능 트리에서 `uv run pytest` → 3349 passed, 7 skipped, 실패 0. |
| main 린트 | #746 exact 기능 트리에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. `codex/176-kis-order-protocol-fix`에서 공식 KIS 공통 REST 헤더 정합 보정을 검증 중이다. |
| 출시 완료 스펙 | 최신 기능: #746(스펙 176, 브로커 거부의 닫힌 KIS 코드·주문 분류 진단 1.1), #745(스펙 176, 자동 실행 주문 0건의 고정 읽기 전용 결과 진단), #744(스펙 176, 서버 timer 조기 실패의 고정 읽기 전용 실행 진단), #728(스펙 178, 읽기 전용 Flutter 운영자 상태 계약과 앱 설계), #726(스펙 177, 자본 0의 비용 현실형 장중 페이퍼 연구 엔진), #724(스펙 176, 독립 서버 예약과 정수주 표현 가능 목표 기준의 자금성 계약), #722(스펙 176, 거래소별 5xx 뒤 제한된 시세 탐색과 전 후보 장애 보존), #721(스펙 176, 고정 읽기 전용 배포 감사 명령), #719(스펙 176, 10:17~13:53 최대 12분 간격 19개 예약 기회), #717(스펙 176, 거래일 1회 주문 선점·최초 실행 증거 보존), #716(스펙 176, 시작 시점과 각 중개사 쓰기 직전 XNYS 정규장 실패 폐쇄·부분 실행 증거 보존), #715(스펙 176, 선언한 NAV 10%와 실제 주문 자본 일치), #714(스펙 176, 정확한 main 사전점검 인계), #712(스펙 176, 뉴욕 현지 비정각 실거래 예약), #709(스펙 176, 역할 분리 운영 증거와 NAV 10% 실거래 검증 캐너리). |
...
| 활성 작업 | #746 production 진단에서 IAUM·SCHX가 공통으로 `APBK1672`, `TTTT1002U`, `AMEX`, `00`에서 거부된 사실을 확인했다. 공식 KIS 현재 예제와의 남은 공통 차이인 `custtype=P`, `tr_cont=""`를 공유 REST 헤더에 고정하고 다음 자동 정규장 실행으로 실제 해결 여부를 검증 중이다. |
...
않는다. 현재 `codex/176-kis-order-protocol-fix`에서 두 헤더를 고정하고 요청 캡처 회귀를 추가했다.
관련 61개와 전체 `3350 passed, 7 skipped`, ruff, 엄격 하네스 14/14, HANDOFF 사실 검사를
통과했다. PR·merge·exact-main 배포 뒤 다음 자동 정규장 실행의 실제 접수·체결·감사·계좌 대사로만
해결을 확정한다. 수동 주문·서비스 수동 시작·기존 거래일 선점 삭제는 하지 않는다.
