...
## 최근 마일스톤 — 2026-05-26 (스펙 013: 운영 관측·신뢰성 — `auto-invest health`)
...
* **스펙 013 (운영 관측·신뢰성 — 통합 헬스 롤업)** — **출시 완료** (2026-05-26, PR #69 머지 커밋 `8b29d42`). `auto-invest health`: 워커 생존·halt·정합성·최근 오류·활동 신선도 5개 점검을 합쳐 종합 판정(`OK`/`DEGRADED`/`CRITICAL`)과 모니터링용 종료 코드(0/1/2)를 냄. **100% 읽기 전용**(감사 로그 append 0건, `db.migrate` 미호출, 거래 워커 루프 무수정). Kernel 터치 0건(`reports/health.py`·`cli.py`만). 테스트 22건.
...
- `HANDOFF-018-SPEC-012-TUNER-CANARY.md` — **최신**. 스펙 012 튜너 L2/L3 → 하드닝 캐너리 자동 투입 출시 (2026-05-26, PR #67 `943c08b`). 위험 변경을 캐너리로 자동 검증(합격해도 자동 승격 0건). 다음 세션 참고.
...
| 마지막 main 커밋 | `8b29d42 Merge PR #69 — feat(013): 운영 관측·신뢰성 auto-invest health 롤업` |
| 활성 작업 | **없음 — 스펙 013(운영 관측·신뢰성: `auto-invest health` 통합 헬스 롤업) 완료·머지(2026-05-26, PR #69 `8b29d42`).** 라이브 worker dry-run 가동 중. 다음 후보: L1 적용 표면 확장, L2/L3 합격→운영자 승격 큐, 모델 교체 노브, 헬스 점검 확장(토큰 KPI 등급·예산·하트비트). 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001, 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013 |
| 진행 중 스펙 | 없음 |
| 골격 스펙 (즉시 착수 가능) | 없음 — 빌드 가능한 스펙 전부 출시. 후속은 L1 적용 표면 확장·헬스 점검 확장 등(스펙 012/013 이후 후보) |
...
| main 테스트 | 964 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
