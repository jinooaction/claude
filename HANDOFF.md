...
## 한눈 요약표 — 2026-09-08 KST 다중 USD 조회 보정 검증 중

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `add7a0c` — Merge pull request #782 from jinooaction/codex/182-preparation-completion-proof |
| main 테스트 | #781 코드02191df 전체3619 passed/9 skipped,실패0(767.73초). 관련103개 통과. 실제 KIS34077759009는 기존7개 통과/새 계좌1개 실패(USD_MARGIN_ROW_COUNT、USD10행、21.96초). 실계좌 전체 계약은 미합격이다. |
| main 린트 | #781 코드02191df에서 `uv run ruff check src tests` 통과. |
| 열린 PR | #782까지 병합 완료. `codex/182-multirow-cash-observation`에서 USD 단일 행 가정 제거를 검증 중이다. 열린 PR은 GitHub에서 다시 확인한다. |
| 출시 완료 스펙 | 최근 출시 코드: 스펙182 확정600/12·모의 주문·계좌 읽기/미검증 자산/현금 진단(#775~781). 전체 계좌 계약(T020/T027)과 실거래(T010~T012)는 미완료. 기존182 실행 라이브러리(#771), 자본 검토(#773), 178 읽기 전용 앱, 177/181 서버 진단 모의 수집, 176 저빈도 운용은 아래 경과를 참조한다. |
...
| 활성 작업 | FR-022 다중 USD 행 보존 PR783. 관련113개/전체3629 passed·9 skipped(805.92초), 린트·하네스14/14 통과. 고객센터 답변은 조회 완료(T020)의 선행 조건이 아니다. 병합·실제 GET 재검증 대기. 집계/NAV(T027), T010~T012 미완료. 자금 배정·주문0. |
...
하네스14/14·인계 검사 통과. PR783 정상 병합·배포/실제 GET 재검증을 진행한다.
