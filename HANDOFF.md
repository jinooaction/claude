...
## 한눈 요약표 — 2026-08-27 KST 최신 코드 main 기준

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `0e8db90` — Merge pull request #690 from jinooaction/codex/170-proxy-evidence-truth |
| main 테스트 | #690 머지 직전 `uv run pytest` → 3118 passed, 6 skipped. 건너뛴 6개는 KIS 실연결 전용이며 머지 후 KIS 읽기 전용 점검은 6/6 통과했다. |
| main 린트 | #690 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 2026-08-27 사실 확인 시 열린 PR은 없다. 스펙 170 기능·교정·생산 확인은 모두 완료됐다. |
| 출시 완료 스펙 | 최신 기능: #689·#690(스펙 170, 소액 계좌 실행 ETF 동등성·주문 구현성·실패 증거 거짓 승인 차단), #687(스펙 167, 실제 17개 전략군과 검정력 보정 연구 진입), #686(스펙 166, 소비자 독립 v3 증거와 정확한 소액 자금 구현성), #684(스펙 165, 옵션 선택·목적 기준 교정과 WPUT 독립 재생). |
| 골격 스펙 | 없음. 다음 전략 우선순위는 현재 `globalfixed`의 최근 구간 실패를 해결할 독립 저회전·레짐 적응 후보를 사전등록해 같은 교정 관문으로 검증하는 것이다. |
| 최근 출시 작업 | 신호 `SPY/IEF/GLD`는 유지하고 실행을 `SCHX/SPTI/IAUM`으로 교체했다. 깨끗한 KIS 데이터에서 세 쌍 모두 동등성 기준을 통과하고, 실패 동등성 문서는 첫 자본을 열지 못하게 고쳤다. |
| 활성 작업 | 자동 판정 `33026038166`은 `WAIT_EDGE`, 단 0→0, 자본·주문 0건이다. 145달러 주문 구현성과 실행 동등성은 통과했지만 최근 5년 앵커드 판정은 `NO_EDGE`다. 실행 불가능 문제가 아니라 현재 엣지 증거가 남은 차단점이다. |
