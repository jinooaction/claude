...
## 최근 마일스톤 — 2026-05-28 (스펙 017 슬라이스 1: 변동성 기반 포지션 사이징)
...
* **스펙 017 (변동성 기반 포지션 사이징)** — **슬라이스 1 출시 완료** (2026-05-28, PR #83 머지 커밋 `c291d75`). 측정 토대 위에 리스크 사이징 시작. 그동안 사이징은 룰마다 고정 정수 수량(`Action.qty`)뿐이었는데, 슬라이스 1은 실현 변동성이 타깃을 초과하면 기준 수량을 줄이는 결정론적 **변동성 throttle**(하향 전용)을 추가. 새 비커널 모듈 `strategy/sizing.py`(`realized_volatility`·`volatility_scale`·`sized_quantity`) + 룰의 선택적 `SizingConfig`(기본 `fixed`=v1 byte 동일). 백테스트 `replay`와 라이브 `OrderRouter` 양쪽이 K1 게이트 **전에** 같은 함수 호출 → 워크포워드 표본 외 검증 가능 + 단일 잣대(헌법 X.2). **안전 경계: K1 캡 무변경(사이저는 제안만, K1이 상한). 스케일 ≤ 1이라 v1 대비 노출 증가 불가. Kernel 터치 0건(전부 `strategy/sizing.py` 신규·`config/rules.py`·`backtest/replay.py`·`execution/order_router.py` 비커널). 감사 K4 무변경. 결정론적·LLM 미사용. dry-run 그대로.** 테스트 신규 18건. 자세히는 `HANDOFF-025-SPEC-017-VOL-SIZING.md`.
...
- `HANDOFF-025-SPEC-017-VOL-SIZING.md` — **최신**. 스펙 017 슬라이스 1 변동성 기반 포지션 사이징 (2026-05-28, PR #83 `c291d75`). 측정 토대 위에 리스크 사이징 시작. 실현 변동성이 타깃 초과 시 기준 수량을 줄이는 결정론적 변동성 throttle(하향 전용). 새 비커널 모듈 `strategy/sizing.py` + 룰의 선택적 `SizingConfig`(기본 fixed=v1). 백테스트·라이브 양쪽이 K1 게이트 전에 같은 함수 호출. K1 캡 무변경·Kernel 터치 0건·테스트 신규 18건. 다음: 슬라이스 2(양방향 타깃 변동성·리스크 패리티)/3(상관). **다음 세션 참고.**
...
| 마지막 main 커밋 | `c291d75 Merge PR #83 — feat(017): 변동성 기반 포지션 사이징 (슬라이스 1)` |
| 활성 작업 | **없음 — 스펙 017 슬라이스 1(변동성 throttle 포지션 사이징) 완료·머지(2026-05-28, PR #83 `c291d75`).** 라이브 worker dry-run 가동 중. 측정 토대(스펙 016) 위에 리스크 사이징 1단계를 깔았다. 다음 후보: 스펙 017 슬라이스 2(양방향 타깃 변동성 + 멀티 포지션 역변동성/리스크 패리티), 슬라이스 3(상관 인식 배분), 또는 신호/알파 과학(다요인·레짐 인식) — 전부 워크포워드로 표본 외 검증 받으며 진행. 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015, 016(슬라이스 1·2·3 전부), 017(슬라이스 1: 변동성 throttle) |
| 진행 중 스펙 | 스펙 017 — 슬라이스 1 출시, 슬라이스 2·3 미착수 |
| 골격 스펙 (즉시 착수 가능) | **스펙 017 슬라이스 2/3 또는 신호/알파 과학** — 리스크 사이징 1단계(하향 전용 throttle)가 깔렸으니, 다음은 양방향 타깃 변동성·리스크 패리티(슬라이스 2)나 신호 자체 개선. 전부 워크포워드 검증 필수 |
...
| main 테스트 | 1086 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
