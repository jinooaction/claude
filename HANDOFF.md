...
성공으로 바뀌지 않는다.186/PR811 전체 검사/실제 서버 확인은 별도 진행 중이며
...
186 FR009 작업 중: 결제 기준 전체 자산표만으로 현재 국내 보유까지 없다고 단정하지
않도록 TTTC8434R 국내 현재잔고 GET을 같은 비공개 수집 경로에 추가했다. 전일 매매와
펀드 결제분 포함, 종목별 전 페이지·반복 요약·수량/대출·시각·한도를 검사한다. 원본은
기존 계좌 묶음 장부에 추가하고 공개 출력은 건수/숫자 상태만 반환한다. 관련72개
(0.44초), 린트·하네스14/14·인계 통과. 전체 회귀 진행 중, 아직 출시/실제 서버 확인은
하지 않았다.184 전체 계좌 공급자/T009/T010 완료를 뜻하지 않는다. 원래 해외 계좌
조회·금액 비노출·기존 장부는 유지하며 실제 주문/취소/자본 배분은 수행하지 않았다.
...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `1b901ab` — Merge pull request #809 from jinooaction/codex/account-source-collection |
| main 테스트 | 코드1c9472b/main1b901ab 전체3873 passed/12 skipped(772.80초). 제외는 실제 KIS11개와 가동 전 상태 전용1개. 실제34686417861은11 passed/23.59초, 원본9응답·장부순번7·자산 구성 분석 성공. |
| main 린트 | 코드1c9472b/main1b901ab의 `uv run ruff check src tests` 통과. |
