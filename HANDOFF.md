...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `ebfb7cf` — Merge pull request #655 from jinooaction/Codex/151-independent-macro-regime |
| main 테스트 | #655 머지 코드 기준 `uv run pytest -q` → 2937 passed, 5 skipped. |
| main 린트 | #655 머지 코드 기준 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. |
| 출시 완료 스펙 | 최신 기능: #653(스펙 150, 256후보 자동 전략 공장·다중검정 보정·합격 시 NAV 10% 연구 캐너리), #646~#651(스펙 149, 첫 주문 직전 최신 엣지 재검증·자동 강등), #644(스펙 148, 정합성 halt 조건부 자동 복구), #642(스펙 147, 전략/계정 성과 장부 분리와 저회전 AI). |
| 골격 스펙 | 스펙 151의 T001~T028 중 문서 포인터 T003만 완료됐다. 수익률곡선·물가 방향·고용 충격·VIX 충격 회복 네 전략군 64개를 구현·재생·검증하는 T001~T002와 T004~T028이 남았다. |
| 최근 출시 작업 | 스펙 150의 자동 전략 공장은 출시 완료 상태다. 스펙 151은 서로 다른 경제 근거를 가진 거시 국면 전략군과 512회 누적 다중검정 계약을 사전등록했으며 아직 실행 코드는 출시하지 않았다. |
| 활성 작업 | 코드 PR 없음. 다음 작업은 스펙 151 구현이다. 사전 탐색 192개는 모두 탈락했고 기존 생산 256개와 공식 후보 64개를 합쳐 512회로 보정해야 한다. 현재 `armed:false`, 단 0, 자본 0, 최근 주문 0건, 미체결 0건이며 전략 수익도 0달러다. |
...
## 최근 계획 — 2026-08-23 KST (#655 독립 거시 국면 전략군 사전등록)

현재 `main` 최신 머지는 `ebfb7cf`(#655)이고 문서 커밋은 `a2b2ac9`다. 기존 가격 문법을 반복하지 않고 경제적 이유와 자료원이 다른 네 전략군을 구현 전에 고정했으며, 수익 후보를 찾았다는 뜻은 아니다.

- **공식 후보 64개**: 수익률곡선 경기 순환, 물가 방향, 실시간 고용 충격, VIX 충격 회복을 각각 16개로 고정했다. 각 전략군은 동일 3자산과 기존 잠정 최고 가격 후보라는 두 기준 포트폴리오 위에서 세 개의 이진 매개변수만 바꾼다.
- **사전 탐색을 숨기지 않음**: 설계 중 실행한 세 묶음 192개는 최고 샤프 우위가 `0.089` 이하였고 PSR `0.640` 이하, PBO `0.698` 이상으로 모두 탈락했다. 이 시도도 다중검정에서 빼지 않아 기존 생산 256개와 공식 64개를 합친 총 512회를 보정한다.
- **시점 오염 차단**: DGS2·DGS10 금리, CPIAUCNS 물가, SAHMREALTIME 실시간 고용, Cboe VIX 자료를 해당 시점에 공개된 값만 사용하도록 계약했다. 자료가 없거나 오래되면 브로커 호출 전 재조정을 차단하되 자동 청산 신호로 해석하지 않는다.
- **합격 기준 유지**: DSR·PSR `0.95`, PBO `0.10` 이하, 구간 승률 `60%`, 샤프 우위 `0.20`, 칼마·낙폭 방어·50bp 비용 후 양의 수익을 모두 요구한다. 통과 전에는 현재 전략, 자본, 주문 권한을 바꾸지 않는다.
- **검증**: 전체 2937 passed/5 skipped, ruff, 엄격 하네스 14/14, HANDOFF 사실 검사, 명세 전제조건 검사, PR 품질 관문이 통과했다.
- **다음 작업**: `specs/151-independent-macro-regime/tasks.md`의 T001부터 순서대로 자료 적재, 순수 목표비중 함수, 64개 생성기, 192개 재생, 512회 보정, 생산 주문 없는 실행을 구현한다.
