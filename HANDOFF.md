...
| 마지막 main 커밋 | `2bd01b1 feat(021): 횡단면 모멘텀 순위 필터 — 유니버스 상위 N/P% 종목만 주문 통과` |
| 활성 작업 | **없음 — 스펙 021(횡단면 모멘텀 순위 필터) 완료·머지(2026-05-29, PR #97 `2bd01b1`).** 라이브 worker dry-run 가동 중. 다음 후보: 퀄리티 팩터(ROE·수익성), 평균분산 포트폴리오 최적화, 실거래 캐너리 전환(운영자 명시 지시 필요) |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015, 016(슬라이스 1·2·3 전부), 017(슬라이스 1·2·2b·3 전부), 018(슬라이스 1 다요인 신호 + 슬라이스 2 사이징 감사 기록), 019(레짐 인식 + 공분산 ERC), 020(레짐·ERC 거래 루프 실배선), **021(횡단면 모멘텀 순위 필터)** |
| 진행 중 스펙 | 없음. 스펙 021 출시 완료. 후속(미착수): 퀄리티 팩터, 포트폴리오 최적화, 실거래 캐너리 |
| 골격 스펙 (즉시 착수 가능) | **스펙 022 후보** — 퀄리티 팩터(ROE·수익성·재무 건전성 기반 필터)가 다음 알파 격차. 또는 평균분산(Markowitz) 포트폴리오 최적화로 샤프 비율 직접 최대화 |
...
| main 테스트 | 1179 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
