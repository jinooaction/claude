...
| 활성 작업 | 진단 1.2의 production 관측은 IAUM·SCHX 모두 `account`, `service_registration`을 지목했다. 운영자가 KIS 앱에서 해외증권 거래신청과 해외 ETF용 해외ETP 거래신청을 확인한 뒤 다음 자동 정규장 실행으로 실제 접수·체결·감사·대사를 검증한다. |
...
PR #751 merge `8efc1486` 및 exact-main 배포 run `33888110029` 뒤, 2026-09-04 15:23 UTC
root 서버 예약이 기존 최초 선점을 보존하며 같은 장 복구 슬롯을 정확히 한 번
소비했다. 읽기 전용 observer run `33889497101`에서 이전 IAUM `EGW00201`
초당 호출 한도 오류는 사라졌지만, IAUM·SCHX 모두 HTTP 200, `rt_cd=7`,
`msg_cd=APBK1672`로 명시적 거부됐다. 주문 접수·미체결·신규 체결은 0건이고,
체결 동기화·사후 측정·계좌 대사는 정상 완료됐다. 같은 세션의 추가 주문 시도는
헌법과 복구 슬롯이 막는다.

KIS 공식 공개 오류 목록은 `APBK1672`의 의미를 제공하지 않으므로 코드만 보고
원인을 추측하지 않는다. `codex/176-kis-rejection-message-topics` 브랜치는 root 전용
주문 로그에 이미 남은 `msg1`을 읽되 원문·부분 문자열·길이·해시를 외부에
내보내지 않고, 계좌·서비스 등록·거래 권한·거래소·종목·시장 시간·가격·수량·
매수 가능 금액·통화·주문 유형·기타·원문 없음의 닫힌 주제만 진단 1.2로
변환한다. 서버 helper와 GitHub observer가 같은 키·열거값·거부 건수를 독립적으로
검증하며, 이 경로는 주문·재시도·서비스·timer를 시작할 수 없다.
설계 커밋은 `290cc31`, 안전 경계 구현 커밋은 `81dc3cb`이며 PR #752 merge
`88858902`로 출시됐다. 관련 회귀 41개와 전체 `3373 passed, 7 skipped`, ruff,
shell 문법, 엄격 하네스 14/14, HANDOFF 사실 검사가 통과했다. exact-main 읽기 전용
observer run `33892995912`는 IAUM·SCHX 모두 `message_topics=[account,
service_registration]`으로 관측했고 다른 주문 원인 주제는 없었다.

exact-main KIS smoke run `33893241198`은 6/6이었고, 일반 push 배포가 장중 연기돼
첫 주문 없는 사전점검 run `33893335555`는 server 코드 불일치를 주문 전에 안전 차단했다.
헌법 15.4.0의 오너 승인 단회 장중 긴급 배포 run `33893714229`로 exact main을 배포한 뒤
production preflight run `33893825580`은 `workflow_dispatch` 주문 없는 경로에서
`ENTRY_READY`, 사후 측정 `CLEAR`, 계좌 대사 `OK`, 증거 `VALID`, 주문 제출 0건으로 성공했다.

따라서 다음 차단은 코드·시세·자금성·주문 형식이 아니라 KIS 계좌의 거래서비스 신청 영역이다.
운영자가 한국투자 앱의 `메뉴 > 고객지원 > 거래서비스 신청 > 해외증권 거래신청`을 완료하고,
IAUM·SCHX가 ETF이므로 `해외ETP 거래신청`도 확인해야 한다. 본인 인증과 금융 약관 동의는
시스템이 대신할 수 없다. 완료 뒤 다음 자동 정규장 scheduler만 사용해 실제 접수·체결·전략
감사·계좌 대사와 중복 주문 0건을 확인한다.
