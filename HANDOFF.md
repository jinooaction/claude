...
## 최근 마일스톤 — 2026-05-27 (스펙 016 슬라이스 3: 워크포워드 표본 외 검증)
...
| 마지막 main 커밋 | `9242faa Merge PR #81 — feat(016): 워크포워드(표본 외) 검증 하니스 (슬라이스 3)` |
| 활성 작업 | **없음 — 스펙 016 슬라이스 3(워크포워드 표본 외 검증) 완료·머지(2026-05-27, PR #81 `9242faa`).** 라이브 worker dry-run 가동 중. 측정 토대 1·2·3단계(정직·통일·표본 외 검증) 모두 완료. 다음 후보(세계 최고 수준 로드맵): **신호/알파 과학**(다요인·레짐 인식)과 **포지션 사이징**(변동성·상관) — 이제 워크포워드로 표본 외 검증을 받으며 안전하게 개선 가능. 실거래 전환은 운영자 명시 지시 필요 |
| 출시 완료 스펙 | 001(P2 정합성 배선 포함), 002, 003, 004, 005, 006, 007, 008, 009, 010, 011, 012, 013, 014, 015, 016(슬라이스 1: 거래비용 + 슬라이스 2: 단일 잣대 + 슬라이스 3: 워크포워드 검증) |
| 진행 중 스펙 | 없음 (스펙 016은 슬라이스 1·2·3 전부 출시) |
| 골격 스펙 (즉시 착수 가능) | **신호/알파 과학 또는 포지션 사이징** — 측정 토대(정직·통일·표본 외 검증)가 완성됐으니, 다음은 알파 자체를 개선하는 작업. 새 스펙으로 시작 권장 |
...
| main 테스트 | 1068 통과, 4 스킵 (라이브 KIS smoke 4건, `KIS_LIVE_TEST=1` 가드) |
