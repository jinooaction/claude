...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `6d60770` — Merge pull request #662 from jinooaction/Codex/153-edge-gate-calibration |
| main 테스트 | #662 머지 코드 기준 `uv run pytest -q` → 2968 passed, 5 skipped. 현재 스펙 154 구현 브랜치는 `uv run pytest -q` → 2981 passed, 5 skipped. |
| main 린트 | #662 머지 코드와 현재 스펙 154 구현 브랜치 모두 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. |
| 출시 완료 스펙 | 최신 기능: #662(스펙 153, 가족별 다중검정과 개발/홀드아웃 분리 관문), #659/#660(스펙 152, 미국 국채 5개 만기 캐리 전략 64개와 공개자료 보고 정합 보정), #657(스펙 151, 독립 거시 전략 64개), #653(스펙 150, 자동 전략 공장). |
| 골격 스펙 | `specs/154-credit-spread-carry/` T001~T027 완료, T028~T031 전체 검증·PR·생산 재생·인계 진행 중. |
| 최근 출시 작업 | 스펙 153이 이질적 후보를 한 통계 가족으로 묶고 홀드아웃에서 다시 승자를 고르던 결함을 고쳤다. 고정 시드 500회에서 무엣지 거짓 합격 3.6%, 연율 샤프 0.60 엣지 발견 83.4%다. 교정된 국채 후보는 실제 홀드아웃 경제 관문에서 탈락했다. |
| 활성 작업 | `Codex/154-credit-spread-carry`. 회사채 스프레드 후보 64개를 구현했고 로컬 최신 공개자료 사전실측은 640개 고유 감사 기록을 재구성했다. 잠정 최고 후보도 홀드아웃 혼합 PSR 0.550035와 샤프 개선 0.033178로 기준 0.95/+0.05에 못 미쳐 `NO_FACTORY_EDGE`다. production 재생·PR·배포 전이며 `LQD` 라이브 허용목록, 자본, 주문은 바꾸지 않았다. |
