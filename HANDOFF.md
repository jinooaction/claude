...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `f91cc8c` — Merge pull request #653 from jinooaction/Codex/150-autonomous-strategy-factory |
| main 테스트 | #653 머지 코드 기준 `uv run pytest -q` → 2937 passed, 5 skipped. |
| main 린트 | #653 머지 코드 기준 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. |
...
| 골격 스펙 | 없음. 스펙 150의 T001~T026이 모두 완료됐다. 다음 기능은 소진된 SPY·IEF·GLD 매개변수 문법과 독립적인 새 경제 가설·자료원을 SDD로 정의해야 한다. |
| 최근 출시 작업 | 네 전략군을 64개씩 네 묶음으로 자동 생성·검증하고 모든 시도를 누적 장부에 남긴다. 합격 후보만 정확한 설정 지문·강화 검사 뒤 NAV 10% 연구 캐너리로 연결하며, 불합격이면 자본 0을 유지한다. |
| 활성 작업 | 코드 PR 없음. 생산에서 256개 고유 후보를 모두 검증했으나 최종 판정은 `NO_FACTORY_EDGE`다. 마지막 잠정 최고 후보도 PBO `0.738095 > 0.10`, PSR `0.629816 < 0.95` 등으로 탈락했다. `armed:false`, 단 0, 자본 0, 최근 주문 0건, 미체결 0건이다. |
