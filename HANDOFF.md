...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `d2528ad` — Merge pull request #796 from jinooaction/codex/182-account-assets |
...
| 출시 완료 스펙 | 스펙182 US8 문의 없는 직접 잔고 대조(T032~T036, #790/#791) 실제9/9 검증 완료. 스펙183 모바일 운영판과 기존182 확정600/12·모의 주문·실제 GET 조회는 유지한다. 집계/NAV(T027)와 단타 실거래(T010~T012)는 미완료다. |
...
| 최근 출시 작업 | 기존 서버 계정으로 체결→결제→체결 잔고를 직접 대조한다. 실제 세 조회 모두 USD1행·각 output1/2/3은1행·공란 통화0·공란 종료를 확인했다. 체결 통화행 안정성과 보고 숫자 일치는 확인했지만 실시간 현금/NAV로 인정하지 않는다. |
