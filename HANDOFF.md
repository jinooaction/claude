...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `c85e0a8` — Merge pull request #704 from jinooaction/Codex/174-sidecar-identity-integrity |
| main 테스트 | #703 후속 진실 표면 보정에서 3172 passed, 6 skipped였고 공개 감사 ID 무결성 보정에서 `uv run pytest -q` → 3176 passed, 6 skipped, 실패 0. 건너뛴 6개는 `KIS_LIVE_TEST=1` 전용이다. |
| main 린트 | 공개 감사 ID 무결성 보정에서 `uv run ruff check src tests`와 가리기·복구·게시 검증 스크립트 린트 → All checks passed. |
| 열린 PR | 2026-08-31 #704까지 병합됐고 열린 PR은 없다. 이 인계 갱신만 별도 브랜치에서 진행 중이다. |
...
| 활성 작업 | 없음. money-path run `33325907470`은 `NO_EDGE_YET`, 앵커드 OOS 748개 실패, forward 4/20, `PREVIEW_ONLY`, 단0, 자본·실주문 0건이다. #704 뒤 전략 공장 `33327235891`은 성공했고 게시 전 검증과 독립 소비자가 후보 ID 800개·지문 800개·20가족을 재계산했다. 다음 새 기능은 21번째 가족 PEAD의 오합격 예산·검출력 재교정부터 시작한다. |
