...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `203386e` — Merge pull request #719 from jinooaction/Codex/176-dense-schedule-recovery |
| main 테스트 | `Codex/176-dense-schedule-recovery`에서 #719의 정확한 코드 트리로 `uv run pytest` → 3228 passed, 7 skipped, 실패 0. 6개는 `KIS_LIVE_TEST=1` 전용이며 마지막 생산 KIS 스모크 `33379808023`에서 6/6 통과했다. 나머지 1개는 사다리 가동 전 상태 전용 시험이라 현재 단 1에서 건너뛴다. |
| main 린트 | `Codex/176-dense-schedule-recovery`에서 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 없음. |
| 출시 완료 스펙 | 최신 기능: #719(스펙 176, 10:17~13:53 최대 12분 간격 19개 예약 기회), #717(스펙 176, 거래일 1회 주문 선점·최초 실행 증거 보존), #716(스펙 176, 시작 시점과 각 중개사 쓰기 직전 XNYS 정규장 실패 폐쇄·부분 실행 증거 보존), #715(스펙 176, 선언한 NAV 10%와 실제 주문 자본 일치), #714(스펙 176, 정확한 main 사전점검 인계), #712(스펙 176, 뉴욕 현지 비정각 실거래 예약), #709(스펙 176, 역할 분리 운영 증거와 NAV 10% 실거래 검증 캐너리), #706(스펙 175, 21가족 교정·PEAD 진단), #701(스펙 174, 회계 기반 횡단면 팩터), #699(스펙 173, 판정 경로 교정·월말월초 전략). |
| 골격 스펙 | 없음. 스펙 176 코드·단 1 센티넬·정규장 주문 방어·거래일 1회 선점·12분 예약 복구는 main에 출시됐다. 남은 완료 관문은 장 마감 뒤 정확한 main 배포와 다음 거래일 실제 KIS 주문·체결·감사·정합 및 후속 중복 차단 확인이다. |
...
| 활성 작업 | 2026-08-31 라이브 캐너리 예약 run은 0건이었고 마지막 schedule run은 8월 28일이다. #719 merge `203386e`는 다음 거래일부터 뉴욕 10:17~13:53에 최대 12분 간격 19개 기회를 제공하되 서버 거래일 1회 주문권을 유지한다. 배포 run `33421400493`은 장중 배포 금지에 따라 안전하게 연기됐고 서버는 아직 기존 exact main `3d2592b`다. 서버 배포 타이머의 다음 실행은 2026-08-31 21:00 UTC다. exact-main 배포·KIS 스모크·무주문 사전점검 뒤 다음 거래일 최초 주문·체결·감사·정합과 후속 중복 차단을 계속 추적한다. 실제 전략 주문·체결은 아직 0건이다. |
...
## 최근 진행 — 2026-09-01 KST (#709~#719 운영 검증 캐너리 무장·예약·보고·자본·정규장 계약 보정, 실제 체결 대기)
