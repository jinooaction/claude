...
## 최근 마일스톤 — 2026-06-03 (스펙 034: 체계적 유니버스 구성 + 넓은 횡단면 정직한 검증 ✅)

main 머지 `7425151`(PR #159). Kernel 터치 0건. 운영자 지시 "세계 최고 수준이 되기 위한
작업 분석·우선순위 판단 뒤 자율 수행". 상세 `HANDOFF-039-SPEC-034-UNIVERSE-CONSTRUCTION.md`.

- **분석·우선순위**: 가장 큰 수익 병목은 **유니버스**로 판단. 알파 스택 전체(횡단면 모멘텀·
  다요인 합성·최적화기)가 넓은 횡단면을 전제로 만들어졌는데 검증은 늘 상관 높은 15개
  메가캡으로만 돌았다. 모든 마일스톤이 "넓은 유니버스가 다음 레버"라 적었지만 아무도 안
  당겼고, 유니버스는 손으로 고른 하드코딩 리스트였다(세계 최고 수준 시스템은 유동성으로
  *구성*한다).
- **새 역량(비커널, 재사용)**: ① `strategy/universe.py` — `median_dollar_volume`/
  `liquidity_rank`/`select_universe`(중앙값 달러 거래대금 순위 → 최소 이력·유동성 필터 →
  상위 N, 결정론 Decimal, 선택 전용, 테스트 13건). ② CLI `auto-invest build-universe`
  (적재 데이터셋에서 유동성 상위 유니버스 즉석 구성, text/JSON/`--emit-toml`). ③
  `fetch_sp500_subset.py --all`(전체 횡단면 약 505종목 추출).
- **정직한 검증**: 유동성 상위 119종목(15→119, 약 8배)으로 표본 외 워크포워드 + 디플레이티드
  샤프. 설정 A(equal top-20): 0/4 구간 승, 평균 샤프 0.49 vs 단순 보유 1.41. 설정 B
  (min_variance top-40): 1/4, DSR 0.057(3시도 보정). **폭을 8배로 넓혀도 옛 강세장
  데이터에선 단순 보유(균등가중 전체 보유)를 표본 외로 못 이긴다.** "좁은 유니버스 탓"
  가설을 닫음 — 병목은 도구가 아니라 데이터/regime. 디플레이티드 샤프가 우연 승리(B의
  1/4)를 0.057로 깎아냄 = 검증 장치가 스스로를 속이기를 거부(헌법 X).
- **함의(다음 레버)**: 진짜 레버는 forward 페이퍼(현재 데이터, 스펙 032/033) + 컨테이너가
  못 닿는 데이터(펀더멘털·인트라데이·대체 데이터) + 다국면 regime. 이번 결과가 그 방향을
  재확인.
- **안전**: Kernel 터치 0건. 돈 안 움직임(백테스트·CLI·스크립트 전용). 결정론·LLM 미사용.
  데이터 아티팩트는 깃 무시. 전체 1427 통과·4 스킵, 린트 깨끗.
...
| 진행 중 스펙 | 없음. 스펙 034 출시 완료. 다음 레버: **현재 데이터 forward 페이퍼 트랙**(스펙 032/033, 옛 데이터 백테스트가 아닌 "지금 통하는가"의 유일한 정직한 판정). 스펙 034 발견: 폭(유니버스)을 8배로 넓혀도 옛 강세장 데이터에선 롱-온리 엣지가 단순 보유를 못 이김 — 병목은 도구가 아니라 데이터/regime. |
