...
## 한눈 요약표 — 2026-08-25 KST 최신 코드 main 기준

| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `ebbefee` — Merge pull request #669 from jinooaction/Codex/157-commodity-inventory-positioning-positive-controls |
| main 테스트 | #669 기능 머지 직전 `uv run pytest -q` → 3013 passed, 5 skipped. 건너뛴 5개는 KIS 실연결 환경 전용이며 머지 뒤 KIS 생산 검사는 별도로 5/5 통과했다. |
| main 린트 | #669 기능 머지 직전 `uv run ruff check src tests` → All checks passed. |
| 열린 PR | 기능 PR 없음. 이 인계 갱신은 `Codex/handoff-after-commodity-positioning-gate-controls`에서 진행한다. |
| 출시 완료 스펙 | 최신 기능: #669(스펙 157, 현실 관문 대조군·독립 원자재 재고/포지셔닝 16개·688개 고유 감사 장부), #667(스펙 156, 독립 원자재 기간구조), #665(스펙 155, 독립 외환 금리차·관문 검출력), #663(스펙 154, 회사채 스프레드), #662(스펙 153, 가족별 다중검정과 개발/홀드아웃 분리), #659/#660(스펙 152, 미국 국채 캐리), #657(스펙 151, 독립 거시 전략), #653(스펙 150, 자동 전략 공장). |
| 골격 스펙 | 없음. 다음 신규 기능은 이번 16개와 홀드아웃을 재사용하지 않고 EIA 제품별 생산·재고와 USDA 작물 재고/사용처럼 독립 공급·수요 자료를 새 SDD로 사전등록한다. |
| 최근 출시 작업 | 스펙 157이 Fama-French 시장 초과수익 PSR 0.996536과 AQR 분산 추세 PSR 0.974381을 같은 통계선에서 통과시키고 평균 제거 대조군 PSR 0.500000을 막아 통계선이 영구 탈락 구조가 아님을 입증했다. 새 원자재 후보는 홀드아웃 PSR 0.525291, 50bp 후 연 초과수익 -1.769314%, 혼합 샤프 변화 -0.194567이라 실거래·종이 후보 모두 탈락했다. |
| 활성 작업 | 기능 구현은 없음. 스펙 157 생산 결과와 계좌 사실을 인계에 닫는 중이다. 최신 돈 경로는 `PREVIEW_ONLY`, `NO_EDGE_YET`, 단 0/5, 자본 0, 실주문 불가다. |
| 안전 경계 | 헌법 X.4 v8.0.0. 공장 전 관문·정확한 설정 지문·강화 검사를 통과한 후보에만 NAV 10% 단 1을 허용한다. 기존 20% 이상 승격 기준과 K1/K2, 손실 예산 20%, 정규장, 현금 1% 여유, production 기계 승인, 서명·nonce, 추가-전용 감사 로그를 유지한다. ORANY 28주는 시작 전 외부 보유로 전략 손익에서 제외한다. |

## 최근 완료 — 2026-08-25 KST (#669 현실 관문 감사·독립 원자재 재고/포지셔닝·688회 판정)

합격 기준 의심을 실제 역사 대조군으로 감사하고, CFTC 포지션과 EIA 원유 재고를 쓰는
독립 전략 16개를 생산에서 검증했다. 통계선은 강한 실제 효과를 통과시켰지만 새 전략은
미사용 구간 수익이 약해 탈락했으므로 돈 경로는 `PREVIEW_ONLY`, 단 0/5, 자본 0이다.

- **현실 관문 감사**: 2007년 이후 Fama-French 미국 시장 초과수익은 PSR
  `0.996536`, AQR 분산 시계열 추세는 `0.974381`로 라이브 통계선 0.95를 통과했다.
  두 자료의 평균 제거 대조군은 각각 `0.500000`으로 탈락했다. 대조군은 후보 선택과
  다중검정에 0건을 기여하며 승격 대상도 아니다.
- **새 전략군**: CFTC 12개 고정 선물 계약의 관리 자금·생산자 순포지션을 미결제약정으로
  정규화하고, EIA `WCESTUS1` 상업 원유 재고와 결합했다. CFTC 3일, EIA 5일 게시
  지연 뒤 알려진 값만 쓰며 네 문법·26/52주·GSG 최대 50/100% 조합 16개를 고정했다.
- **분리 검증**: 개발 96개월에서 `commodity-positioning-managed_money_trend-9480c3c1956f`
  하나만 선택하고 한 달 격리 뒤 131개월 홀드아웃에서 다시 고르지 않았다. 과거 672개와
  현재 16개를 합친 688개 전략 지문이 모두 고유하다.
- **생산 판정**: 전략 공장 run `32789350616`, main `ebbefee`. 홀드아웃 PSR
  `0.525291`, 50bp 비용 후 연 초과수익 `-1.769314%`, 기존 전략과 상관
  `0.088256`, 혼합 샤프 변화 `-0.194567`이라 `NO_FACTORY_EDGE`다. 라이브 0.95뿐
  아니라 종이 0.80과 비용 후 양의 수익도 실패했다.
- **기준 결론**: 이번 탈락은 0.95 한 줄 때문에 생긴 경계 사례가 아니다. 수익이 음수이고
  기존 포트폴리오에 섞어도 샤프가 하락했다. 다만 현실 대조군은 PSR 통계선의 검출력을
  증명할 뿐 모든 경제성 관문이 영구 최적이라는 뜻이나 미래 수익 보장은 아니다.
- **배포·계좌**: deploy run `32789350661`, 전략 공장 run이 성공했다. KIS smoke
  run `32789992141`은 main `ebbefee`에서 5/5, 키 유효, 현금 934.27달러,
  NAV 1457.59달러, 기존 외부 보유 ORANY 28주, 최근 주문 0건, 열린 미체결 0건이다.
- **돈 경로**: money-path run `32790126398`은 `PREVIEW_ONLY`, `NO_EDGE_YET`,
  단 0/5, 자본 0, 실주문 불가, 전략 체결·손익 0을 확인했다. capital-path run
  `32790129322`도 `ACCUMULATING_EDGE`이며 우선 승격 후보가 없다고 판정했다.
- **검증**: 머지 직전 전체 3013 passed/5 skipped, ruff, YAML, 엄격 하네스 14/14,
  HANDOFF 사실 검사, PR #669 품질 관문과 충돌 없음 상태가 통과했다.
- **다음 판단**: 같은 포지셔닝 16개를 반복하거나 기준을 낮추지 않는다. 다음은 EIA
  제품별 생산·재고와 USDA 작물 재고/사용처럼 독립 공급·수요 자료의 게시 지연·개정
  한계를 먼저 고정한 새 전략군이다.
- **상세 인계**: `HANDOFF-162-COMMODITY-POSITIONING-GATE-CONTROLS.md`.

## 이전 완료 — 2026-08-25 KST (#667 독립 원자재 기간구조 전략 공장·672회 판정)

원자재 선물 곡선의 캐리와 롤수익을 나타내는 독립 신호를 구현·배포하고 생산
공개자료로 검증했다. 코드와 자동화는 정상이나 합격 전략은 없으므로 현재 돈 경로는
`PREVIEW_ONLY`, 단 0/5, 자본 0, 실주문 불가다.

- **새 전략군**: BlackRock의 GSG와 S&P GSCI 총수익, 세계은행 원자재 현물 지수,
  이전 달 미국 3개월 국채금리를 결합했다. 총수익에서 현물수익과 현금수익을 뺀 값을
  기간구조 대용 신호로 정의하고 캐리 양수, 캐리+모멘텀, 캐리 순위, 방어 캐리의
  3/12개월·최대 50/100% 조합 총 16개를 만들었다.
- **사전등록과 분리**: 거래비용 10/25/50bp, 개발 96개월, 한 달 격리, 손대지 않은
  홀드아웃 142개월, 라이브 PSR 0.95와 종이 PSR 0.80을 결과 전에 고정했다. 개발에서
  `commodity-carry_rank-9d2f0baf1d27` 하나만 선택하고 홀드아웃에서 다시 고르지 않았다.
- **계산 교정**: 첫 로컬 배관 재생은 현금수익률 차감을 빠뜨려 PSR 0.757994를 냈다.
  이 값은 무효 처리해 672개 감사 장부와 어떤 승격 판정에도 넣지 않았다. 후보·비용·
  구간·기준을 유지한 채 현금수익률을 포함한 v2 정의를 결과 전에 다시 사전등록했다.
- **생산 판정**: 전략 공장 run `32759149869`, 잠정 최고
  `commodity-carry_rank-9d2f0baf1d27`. 홀드아웃 PSR `0.611866`, 50bp 비용 후
  연 초과수익 `-0.641336%`, 기존 전략과 상관 `0.134074`, 혼합 샤프 변화
  `-0.183303`이라 `NO_FACTORY_EDGE`다. 혼합 낙폭은 14.84%에서 11.71%로 좋아졌지만
  수익과 샤프가 부족해 라이브와 종이 관문 모두 탈락했다.
- **관문 진단**: 16개 가족의 무신호 거짓 합격률은 5.0%, 참 연율 샤프 0.60 신호
  검출률은 84.8%다. 이번 실패는 기준이 영구적으로 통과 불가능해서가 아니라 개발과
  홀드아웃에서 신호 방향이 뒤집히고 비용 후 초과수익이 음수였기 때문이다.
- **감사·배포**: 과거 656개와 현재 16개를 합친 672개 고유 감사 기록을 생산에서
  확인했다. deploy run `32759149802`, released-work run `32759149819`,
  autonomous-work run `32759149845`, 생산 전략 공장 run이 모두 성공했다.
- **계좌와 주문**: KIS smoke run `32759829708`은 main `7fe7334`에서 5/5 통과했다.
  현금 934.27달러, NAV 1457.59달러, 기존 외부 보유 ORANY 28주, 최근 주문 0건,
  열린 미체결 0건이다. 원자재 후보는 배포 설정·자본·허용목록을 바꾸지 않았다.
- **검증**: 머지 직전 전체 3002 passed/5 skipped, ruff, YAML, 엄격 하네스 14/14,
  HANDOFF 사실 검사, PR #667 품질 관문과 충돌 없음 상태가 통과했다.
- **다음 판단**: 같은 기간구조 16개를 반복하거나 홀드아웃 성과로 후보를 바꾸지 않는다.
  다음은 CFTC 포지션과 EIA·USDA 재고처럼 경제적으로 다른 공개자료를 먼저 고정한
  독립 원자재 재고·포지셔닝 전략군을 사전등록한다.
- **상세 인계**: `HANDOFF-161-INDEPENDENT-COMMODITY-TERM-STRUCTURE.md`.

## 이전 완료 — 2026-08-24 KST (#665 독립 외환 금리차 전략 공장·관문 검출력·656회 판정)

외환 금리차라는 새 수익 원천과 합격 관문의 신호 검출력을 함께 구현·배포하고 생산
공개자료로 검증했다. 코드와 자동화는 정상이나 합격 전략은 없으므로 현재 돈 경로는
`PREVIEW_ONLY`, 단 0/5, 자본 0, 실주문 불가다.

- **새 전략군**: AUD·CAD·JPY·GBP·USD의 연준 H.10 현물환과 OECD 단기금리를
  같은 달러 가치 방향으로 맞췄다. 순수 금리차, 금리차+모멘텀, 금리차+가치,
  방어 금리차의 3/12개월·외화 최대 50/100% 조합 총 16개를 개발 1990~2006년,
  한 달 격리, 2007년 이후 234개월 홀드아웃으로 검증했다.
- **관문 진단**: 고정 시드 500회에서 16개 가족의 무신호 거짓 합격은 5.0%다.
  참 연율 샤프 0.60 신호는 84.8% 검출하지만 샤프 0.40 신호는 52.0%만 라이브
  합격한다. 기존 모든 실패를 한 가족으로 묶던 관문은 샤프 0.60도 8.2%만 잡았다.
  따라서 이전 관문은 실제로 둔감했고 스펙 153에서 고쳤다. 이번에는 라이브 0.95를
  낮추지 않고 샤프 0.40 신호의 79.0%를 자본 없는 종이 후보로 보존하는 단계를 추가했다.
- **구현 결함 교정**: H.10 휴일 자리표시자를 자료 결측으로 잘못 세던 문제와 순수
  금리차·가치 전략의 3/12개월 설정이 점수에 반영되지 않던 문제를 최종 실행 전에
  고쳤다. 통화·구간·비용·합격선·후보 수는 결과를 본 뒤 바꾸지 않았다.
- **생산 판정**: 전략 공장 run `32739773654`, 묶음
  `fx-carry-factory-fe21b34de70e`, 잠정 최고 `fx-pure_carry-a695cb75e58a`.
  홀드아웃 PSR `0.352590`, 50bp 비용 후 USD 현금 대비 총수익 `-0.11126707`,
  혼합 샤프 변화 `-0.033920`이라 `NO_FACTORY_EDGE`다. 16개 모두 홀드아웃 초과
  샤프가 음수라 기준 0.95 하나 때문에 탈락한 것이 아니다.
- **감사·배포**: 과거 640개와 현재 16개를 합친 656개 고유 감사 기록을 생산에서
  확인했다. deploy run `32739773672`, 생산 전략 공장, 공개자료 수집이 모두 성공했다.
- **계좌와 주문**: KIS smoke run `32739773694`은 main `69c53ce`에서 5/5 통과했다.
  현금 934.27달러, NAV 1457.59달러, 기존 외부 보유 ORANY 28주, 최근 주문 0건,
  열린 미체결 0건이다. money-path run `32739871151`은 `PREVIEW_ONLY`,
  `armed:false`, 단 0, 자본 0, 전략 체결·손익 0을 재확인했다.
- **검증**: 머지 직전 전체 2992 passed/5 skipped, ruff, YAML, 엄격 하네스 14/14,
  HANDOFF 사실 검사, PR #665 품질 관문과 충돌 없음 상태가 통과했다.
- **안전 경계**: FXA·FXC·FXY·FXB·UUP를 라이브 허용목록에 넣지 않았고 주문·자본·
  자동 무장·한도·헌법·커널을 바꾸지 않았다. 종이 후보도 브로커 접근 권한이 없다.
- **다음 판단**: 자동매매 프로그램이 많다는 사실은 검증된 표본 외 수익을 뜻하지
  않는다. 하지만 검사기가 약한 신호를 못 잡는 문제는 숫자로 계속 감시한다. 다음은
  외환 16개나 기준을 재조정하지 않고 독립 원자재 기간구조 전략군을 사전등록한다.
- **상세 인계**: `HANDOFF-160-INDEPENDENT-FX-CARRY-GATE-POWER.md`.

## 이전 완료 — 2026-08-24 KST (#663 독립 회사채 스프레드 전략 공장·640회 판정)

회사채 스프레드라는 새 수익 원천을 구현·배포하고 생산 공개자료로 검증했다. 코드와
자동화는 정상이나 합격 전략은 없으므로 현재 돈 경로는 `PREVIEW_ONLY`, 단 0/5,
자본 0, 실주문 불가다.

- **새 전략군**: 재무부 HQM 투자등급 회사채 10년·20년 금리와 같은 만기 국채를
  결합해 캐리·평균회귀·추세·완충 전략을 각각 16개씩 총 64개 만들었다. 개발 구간은
  1990~2006년, 한 달 격리 뒤 2007년 이후를 손대지 않은 최종 검증 구간으로 쓴다.
- **감사 장부 교정**: 과거 생산 장부 576줄에는 반복 실행 192줄이 섞여 있었다.
  가격 256, 사전 탐색 192, 거시 64, 국채 64를 원본 지문에서 재구성하고 현재 회사채
  64개를 더해 `audit_catalog.jsonl` 640줄·고유 지문 640개를 생산에서 확인했다.
- **생산 판정**: 전략 공장 run `32711600218`, 묶음
  `credit-spread-factory-v2-0c525304d5c9`, 잠정 최고
  `credit-carry_buffer-d174c247f539`. 개발 DSR 0.999900과 PBO 0.000000은 통과했지만
  최종 혼합 PSR `0.550035 < 0.95`, 혼합 샤프 개선 `0.033178 < 0.05`라
  `NO_FACTORY_EDGE`다. 50bp 비용 후 양의 수익·상관·낙폭 관문은 통과했다.
- **자료와 배포**: HQMCB10YR·HQMCB20YR은 1984-01~2026-07 각 511개월이고,
  DGS10·DGS30은 2026-08-20까지다. 게시 지연·신선도·연구/주문 비중 일치 관문이
  모두 통과했다. deploy run `32647098681`은 merge `6c849a2` 배포에 성공했다.
- **계좌와 주문**: KIS smoke run `32688063778`은 5/5 통과했다. 현금 934.27달러,
  NAV 1457.59달러, 기존 외부 보유 ORANY 28주, 최근 주문 0건, 열린 미체결 0건이다.
  money-path는 `PREVIEW_ONLY`, `armed:false`, 단 0, 자본 0, 전략 체결·손익 0이다.
- **검증**: 기능 머지 전과 인계 갱신 뒤 전체 2981 passed/5 skipped, ruff,
  엄격 하네스 14/14, HANDOFF 사실 검사, PR 품질 관문이 통과했다. 인계 갱신 전
  전체 시험의 실패 2건은 낡은 main 커밋 행을 정확히 차단한 하네스였고 갱신 뒤 해소됐다.
- **안전 경계**: `LQD`를 라이브 허용목록에 넣지 않았고 주문·자본·자동 무장·한도·
  헌법·커널을 바꾸지 않았다. 회사채 후보가 나중에 합격해도 별도 허용목록 승인과
  `Backtest -> Canary -> Full` 단계를 건너뛸 수 없다.
- **다음 판단**: 기준을 낮추거나 같은 회사채 64개를 재탐색하지 않는다. 다음은
  독립 외환 금리차 전략군을 새 SDD로 사전등록해 자료·통화 위험·거래 가능 상품을
  먼저 고정한 뒤 같은 개발/격리/홀드아웃 계약으로 검증한다.
- **상세 인계**: `HANDOFF-159-INDEPENDENT-CREDIT-SPREAD-FACTORY.md`.

## 이전 관찰 — 2026-08-23 KST (스펙 153 수익 엣지 판정 교정)

운영자의 문제 제기가 맞았다. 기존 판정은 실패한 모든 연구를 보존한다는 올바른 목적을
통계 가족의 범위와 혼동했고, 이름과 달리 홀드아웃에서 64개 중 승자를 다시 골랐다.

- **확인한 원인**: DSR 원 논문은 특정 전략 종류의 독립 시도를 요구하고 PBO 원 논문은
  같은 선택 과정의 후보를 같은 시간 블록에서 비교한다. 기존 코드는 가격·거시·국채 576개를
  한 분포와 한 PBO 행렬에 넣었다. 국채 후보 선택도 2007년 이후 홀드아웃 성과를 사용했다.
- **교정 설계**: 과거 576회는 추가 전용 감사 장부에 그대로 둔다. 통계는 현재 가족 64개만
  사용하고 후보 수익 상관으로 독립 시도 수를 환산한다. 1990~2006 개발 구간에서 한 후보를
  고정하고 한 달 격리 뒤 홀드아웃에서는 그 후보를 다시 선택하지 않는다.
- **증거 역할 분리**: 개발 DSR 0.95와 PBO 0.10은 탐색 불안정 진단이다. 최종 통계 차단은
  독립 홀드아웃 PSR 0.95가 담당한다. 국채는 `diversifier`로 사전등록해 단독 대체 조건이
  아니라 80/20 혼합 샤프 +0.05, 낙폭 비악화, 상관 0.80 미만, 50bp 비용 후 양수를 요구한다.
- **관문 교정 결과**: 고정 시드 500회에서 무엣지 거짓 합격 3.6%, 연율 샤프 0.60 심은 엣지
  발견 83.4%다. 구버전의 심은 엣지 발견은 8.2%였다. 합성 통합 자료에서는 실제
  `FACTORY_EDGE` 경로도 재현되어 영구 탈락 구조를 제거했다.
- **안전 경계**: 판정 증거를 `gate_version=2.0`으로 구분한다. 구버전은 새 증거로 재사용할 수
  없고, 합격해도 연구 캐너리 적격일 뿐 `Backtest -> Canary -> Full`, 정확한 전략 지문,
  강화 캐너리, 자본·주문 관문을 건너뛰지 않는다.
- **현재 검증**: 전체 `2968 passed, 5 skipped`, ruff, 집중 회귀가 통과했다. PR, 생산 공개자료
  재생, 배포, KIS 주문 없는 검사는 아직 수행 전이므로 실제 국채 전략 판정은 확정하지 않는다.

## 최근 관찰 — 2026-08-23 KST (#659/#660 독립 국채 캐리 전략 공장 출시·576회 판정)

현재 `main` 최신 머지는 `5872ff4`(#660)이고 기능 머지는 `4820cc1`(#659)다. 미국
국채 만기 캐리라는 새 경제 원천을 생산에서 검증했지만, 실거래에 올릴 합격 전략은
없었다.

- **생산 자료**: public-data run `32617443986`이 16/16을 발행했고 교차 검증 5/5가
  통과했다. DGS3MO·DGS2·DGS5·DGS10·DGS30은 모두 최신 2026-08-20까지 있으며,
  보고 문구도 특정 거시 스펙 전용이 아니라 검증된 공개자료 기반 전략 전체를
  포괄하도록 고쳤다.
- **576회 정확 계산**: factory run `32617443941`, 묶음
  `treasury-carry-factory-5496f4ae998a`가 기존 512회와 국채 공식 64회를 합쳐
  576개 고유 전략 지문을 확인했다. 개발 204개월, 격리 1개월, 홀드아웃 234개월이다.
- **최종 판정**: 잠정 최고 `treasury-curve_barbell-dfacba3b4e91`은 비용 50bp 후
  양의 수익, 낮은 기존 전략 상관 0.322, 혼합 낙폭 개선은 통과했다. 그러나 DSR
  `0.018830`, PBO `0.880952`, PSR `0.677424`, 구간 승률 `0.50`, 샤프 우위
  `0.099378`, 혼합 샤프 개선 `0.021310`이 기준에 못 미쳐 `NO_FACTORY_EDGE`다.
- **연구·주문 일치**: 연구와 실행은 같은 국채 목표 비중 함수를 사용한다. 자료·코드·
  후보·전략 지문·목표 비중 다이제스트가 다르면 브로커 조회 전에 실패 폐쇄한다.
  선택 후보와 배포 설정은 모두 비어 있고 연구 캐너리 자격도 없다.
- **배포·계좌**: deploy run `32617443947` 성공. 구현 커밋 KIS smoke run
  `32616655186`은 5/5, 키 유효, 현금 934.27달러, NAV 1457.59달러, 기존 외부 보유
  ORANY 28주, 최근 주문 0건, 열린 미체결 0건을 확인했다.
- **돈 상태**: money-path run `32616997853`과 capital-path run `32617019459`은
  `PREVIEW_ONLY`, `NO_EDGE_YET`, 단 0/5, 배치 자본 0, 접수·체결 0건, 전략 손익
  0달러를 확인했다. 합격 후보가 없어 강화 캐너리와 자동 무장은 실행하지 않았다.
- **검증**: 전체 2960 passed/5 skipped, ruff, 집중 61개, YAML, diff, 엄격 하네스
  14/14, HANDOFF 사실 검사, PR 품질 관문이 통과했다.
- **다음 판단**: 국채 캐리의 같은 조합을 반복하거나 기준을 낮추지 않는다. 다음은
  회사채 스프레드 또는 외환 금리차처럼 다른 위험 프리미엄을 쓰는
  `independent_credit_or_fx_carry`를 새 SDD로 사전등록한다.
- **상세 인계**: `HANDOFF-158-INDEPENDENT-TREASURY-CARRY-FACTORY.md`.

## 최근 관찰 — 2026-08-23 KST (#657 독립 거시 전략 공장 출시·512회 판정)

현재 `main` 최신 머지는 `001254b`(#657)이고 기능 커밋은 `3709cab`다. 서로 다른 경제 근거를 가진 네 거시 전략군을 시점 기준 자료로 구현하고 생산 512회 판정을 끝냈지만, 실거래에 올릴 합격 전략은 없었다.

- **생산 자료**: public-data run `32608652165`가 13/13을 발행했고 FRED CPI와 BLS 미러, 재무부와 FRED/연준 금리 대조 등 교차 검증 5/5가 통과했다. 자료 커밋은 `001254b`와 일치한다.
- **512회 정확 계산**: factory run `32608652168`, 묶음 `macro-strategy-factory-0e24eeb78685`가 생산 256회+사전 탐색 192회+공식 64회, 고유 전략 지문 512개를 모두 계산했다.
- **최종 판정**: 잠정 최고 `macro-curve_cycle-a9f40878ef88`는 DSR `0.999970`, 구간 승률 60%, 샤프·칼마·낙폭·50bp 비용 관문을 통과했다. 그러나 PBO `0.833333 > 0.10`, PSR `0.812002 < 0.95`라 `NO_FACTORY_EDGE`이며 선택 후보와 연구 캐너리 자격은 없다.
- **연구·주문 일치**: 연구와 실행은 같은 목표 비중 함수를 사용한다. 거시 전략이 나중에 채택돼도 최신 자료, 교차 검증, 후보 ID·전략 지문, 공장 코드 커밋 계보가 맞지 않으면 브로커 호출 전에 차단한다.
- **배포·계좌**: deploy run `32608652200`과 KIS smoke run `32608652207`이 성공했다. KIS 5/5, 현금 934.27달러, NAV 1457.59달러, ORANY 28주, 최근 주문 0건, 열린 미체결 0건이다.
- **돈 상태**: money-path는 `PREVIEW_ONLY`, `armed:false`, 단 0/5, 승인 자본 0, 접수·체결 0건, 전략 손익 0달러를 새 커밋 기준으로 발행했다. 승자가 없으므로 강화 검사나 자동 무장을 실행하지 않았다.
- **검증**: 전체 2949 passed/5 skipped, ruff, YAML·셸·diff, 엄격 하네스 14/14, HANDOFF 사실 검사, PR 품질 관문이 통과했다.
- **다음 판단**: 같은 SPY·IEF·GLD 거시 문턱을 반복하거나 기준을 낮추지 않는다. 다음은 자산군 확대 또는 캐리처럼 수익 원천이 다른 `independent_asset_universe_or_carry`를 새 SDD로 사전등록한다.
- **상세 인계**: `HANDOFF-157-INDEPENDENT-MACRO-STRATEGY-FACTORY.md`.

## 최근 관찰 — 2026-08-23 KST (#653 자동 전략 공장 출시·전체 탐색 완료)

현재 `main` 최신 머지는 `f91cc8c`(#653)이고 기능 커밋은 `8698225`다. 자동 전략 공장과 합격 시 NAV 10% 연구 캐너리 연결은 출시됐지만, 생산 후보 256개가 모두 검증 탈락해 실거래는 열리지 않았다.

- **자동 탐색 완료**: 네 전략군을 64개씩 네 묶음으로 실행했다. 생산 장부는 256줄이며 후보 ID도 256개로 모두 고유하다. run `32583681179`, `32583856609`, `32583876789`, `32583914149`가 모두 성공했다.
- **최종 판정**: 마지막 묶음 `strategy-factory-e676a1d58b7e`는 64/64 완료, 누적 다중검정 256회, `NO_FACTORY_EDGE`다. 잠정 최고 `factory-trend_inverse_vol-001ad6d2e302`는 DSR `0.999192`는 통과했지만 PBO `0.738095`, PSR `0.629816`, 구간 승률 `0.50`, 샤프 우위 `0.087475`로 탈락했다.
- **실거래 연결 조건**: 공장 모든 관문, 강화된 주문 없는 검사, 신선한 계좌 정합성, 정확히 같은 전략 설정 지문이 모두 맞아야 단 0에서 NAV 10% 단 1로 올라간다. 20% 이상은 기존 전진 관측·성과 계약을 그대로 요구한다.
- **현재 돈 상태**: 승자가 없으므로 `armed:false`, 단 0, 승인 자본 0이다. KIS smoke run `32583681175`는 5/5 통과했고 현금 934.27달러, NAV 1457.59달러, ORANY 28주, 최근 주문 0건, 열린 미체결 0건을 확인했다. 주문·체결·전략 수익은 새로 발생하지 않았다.
- **배포**: deploy run `32583681149`의 첫 시도는 워커 시작 확인 시간 초과로 이전 버전으로 되돌렸고, 같은 run의 재시도 2는 모든 단계를 통과했다.
- **검증**: 전체 2937 passed/5 skipped, ruff, 엄격 하네스 14/14, HANDOFF 사실 검사, PR 품질 관문이 통과했다.
- **다음 판단**: 현재 SPY·IEF·GLD 추세·상대 모멘텀·역변동성·저회전 방어 매개변수 공간은 소진됐다. 기준을 낮추거나 같은 조합을 반복하지 말고, 독립적인 거시 국면 신호나 새 자료원처럼 경제적 이유가 다른 후보군을 새 SDD로 추가해야 한다.
- **상세 인계**: `HANDOFF-156-AUTONOMOUS-STRATEGY-FACTORY.md`.
...
- **폐기한 잘못된 수익 판정**: 2026-06-23의 BHP·MRK·RELX 매도 3건과 ORANY 28주는 시스템 가동 전 보유 자산이다. 과거 문서의 `FIRST_PROFIT_OBSERVED`와 약 +230달러 수익은 전략 수익 증거가 아니므로 폐기한다.
- **현재 전략 성과**: 생산 측정 run `32548824195`에서 전략 체결 0건, 실현·미실현·총손익 0달러, 전략 NAV 293달러로 정정됐다. 과거 보유 관련 체결 3건, 실현 +15.93달러, 미실현 +209.86달러는 제외 증거로만 남는다.
- **측정 계약**: 계약 `sha256:2542c0ddd4499481582d820ebee48fadbbfbab9b6208c749d843c025b74288d8`은 BHP·MRK·ORANY·RELX를 제외한다. live 성장·forward 판정·최초 수익 증거는 같은 계약의 최신 연속 구간만 사용한다.
- **현재 주문 차단 이유**: 최신 `resume-readiness`는 `STALE`, 정합성은 `MISMATCH`, halt는 존재한다. 주문 제출 0건, 정지 해제 0건이며 자동매매는 아직 실행 불가다. 정합성을 새로 `OK`로 확인하기 전에는 정지를 해제하지 않는다.
- **저회전 AI 실제 판정**: 연구 run `32548715047`은 회전율을 34.34에서 8.374194로 75.61388% 줄였지만 Sharpe 우위, PSR, DSR, 구간 승률, 최대 낙폭 관문을 실패해 `NO_EDGE`다. 후보는 자동 거절됐고 실거래 승격은 없다.
- **배포·외부 확인**: deploy run `32548715032` 성공. KIS smoke run `32548715072`은 5/5 통과, 매수가능현금 934.27달러, ORANY 28주, 열린 미체결 0건을 확인했다. 수동 production run은 `manual-no-order-preflight`로 주문 없이 서명·서버 권위·사후 측정을 검증했다.
- **검증**: 전체 2888 passed/6 skipped, ruff, 셸 구문, diff, 엄격 하네스 14/14, HANDOFF 사실 검사, PR 품질 관문이 통과했다.
- **다음 판단**: 먼저 최신 계좌 정합성을 다시 실행해 `OK` 증거를 만들어야 한다. 그 전에는 halt 해제와 실주문을 금지한다. 저회전 후보는 비용 문제를 줄였지만 수익 엣지가 없으므로 승격하지 않는다.
- **상세 인계**: `HANDOFF-153-ACCOUNT-LEDGER-LOW-TURNOVER.md`.

## 최근 관찰 — 2026-08-16 KST (#639/#640 일봉 교차자산 AI 후보 출시·실제 판정 완료)

현재 `main` 최신 머지는 `a6b60d5`(#640)이고 기능 커밋은 `4c3012a`, 사이드카 보정 커밋은 `7bb20d3`다.
...
- **실제 판정**: run `31933437181`은 2021-06-11~2026-08-14 공통 271주, 종목별 1,300일봉, 15개 표본 외 구간에서 `NO_EDGE`를 확정했다. 25bp 비용 후 CAGR 1.89%, Sharpe 0.249, 최대낙폭 11.46%였고 동일비중은 14.14%, 1.753, 5.33%였다.
- **막힌 이유**: Sharpe 차이 -1.504, PSR 0.013, DSR 0.557, 구간 승률 20%, 50bp 총수익 -4.41%로 기준을 크게 못 넘었다. 총 회전율 34.34가 비용 민감도를 키웠고 최신 연구 비중은 전액 현금이다.
- **자동화·배포**: #640은 sidecar push에 fetch한 원격 예상 해시를 명시해 lease 보호를 유지하면서 발행 실패를 고쳤다. deploy run `31933437182`와 연구 run `31933437181`이 모두 성공했다.
- **안전 경계**: 주문·취소·자본·live 전략·허용 종목·한도 변경은 모두 0건이다. KIS quotations 읽기와 연구 DB 적재만 수행하며 기존 `Backtest -> Canary -> Full`을 유지한다.
- **다음 판단**: 현재 후보는 승격하지 않는다. 다음 연구는 주간 회전 확대가 아니라 최소 보유기간·거래 임계값·비용 후 잔차수익 목표로 회전율을 줄이고, 동일비중을 이기지 못하면 즉시 중단해야 한다.
- **상세 인계**: `HANDOFF-152-DAILY-CROSS-ASSET-ML.md`.
...
- **보정·배포**: 첫 사전점검 run `31926148633`에서 발견한 기계 승인 job의 checkout 누락을 #634로 고쳤고 deploy run `31926354677`이 성공했다.
- **주문 없는 실서버 검증**: 수동 run `31926380224`가 사람 대기 없이 preview, 기계 승인, production 서명·nonce·배포 정합, 서버 권위 검증을 모두 통과했다. 결정은 `manual-no-order-preflight`, 열린 주문은 0건이며 새 주문·체결도 0건이다.
- **현재 자동매매 상태**: money-path run `31926494074`는 `REAL_ORDER_PATH_ARMED`, `can_submit_real_orders: true`, 자본 293달러를 발행했다. 이어진 capital readiness run `31926510629`도 `CAPITAL_ARMABLE`과 `REAL_ORDER_PATH_ARMED`를 확인했다.
- **다음 자동 확인점**: 다음 예약은 `2026-08-17T15:00:00Z`(2026-08-18 00:00 KST)다. 예약 이벤트, 미국 정규장, KIS 매수가능현금 1% 여유, 손실 차단기, K1 포지션 한도, K2 허용 종목, 서명·nonce·배포 정합을 모두 통과할 때만 실주문을 제출한다.
...
현재 `main` 최신 머지는 `5d20ea8`(#626)이고 안전 경계 기능 커밋은 `86f56b2`다.

- **고친 뿌리 원인**: production 실제 주문 작업이 서버의 forced-command SSH 관문에 없는 임의 직접 셸 명령을 사용해 다음 정규장에 거부될 구조였다. 이제 production 환경 전용 Ed25519 개인키로 run·commit·자본·만료·nonce를 서명하고, 서버 root 소유 helper가 공개키·센티넬·rung·NAV·배포 정합·nonce를 다시 검증한 뒤에만 기존 live CLI를 호출한다.
- **첫 수익 증거**: 주문 불가능한 `live-canary-fills`와 `live-canary-profit` 고정 명령을 추가했다. 체결 1건 이상, 시세 결측 0, 데이터 경고 0, 총손익 양수일 때만 `FIRST_PROFIT_OBSERVED`로 기록하고 최초 시각·손익은 이후에도 보존한다.
- **배포·실서버 확인**: deploy run `31922602120`, live-profit run `31922633808`, KIS smoke run `31922634930`이 성공했다. KIS는 5/5, 매수가능현금 934.27달러, NAV 1466.83달러, 최근 주문·열린 미체결 0건이다. live-profit은 체결 0건·총손익 0달러·결측/경고 0으로 `NO_FILLS_YET`를 발행했다.
- **자동 재평가 확인**: live-profit 완료 뒤 money-path run `31922655716`, 이어 capital readiness run `31922666593`이 자동 성공했다. 현재 `REAL_ORDER_PATH_ARMED`, `CAPITAL_ARMABLE`, 단 1, 293달러이며 다음 예약은 `2026-08-17T15:00:00Z`다.
- **검증**: 전체 2853 passed/6 skipped, ruff, 셸·YAML·diff, HANDOFF 사실 검사, 엄격 하네스 14/14, PR 품질 관문을 통과했다.
- **남은 단일 외부 조건**: 다음 예약에서 production reviewer 승인과 모든 기존 preflight를 통과한 뒤 실제 주문·체결·완전한 양의 총손익이 관측돼야 한다. 수익은 보장하거나 기준을 바꾸지 않으며 그 전에는 목표를 완료로 선언하지 않는다.
- **상세 인계**: `HANDOFF-150-LIVE-CANARY-GATEWAY-PROFIT-EVIDENCE.md`.
...
현재 `main` 최신 머지는 `0505e84`(#611, broad no-edge vol-target drawdown contract)다.
기능 커밋은 `bae22b3`이다.

- **문제 정의**: 광역 no-edge 2차 후보에서 상대가치와 꼬리위험 방어까지 닫혔지만, `NO_EDGE_YET`는 계속된다. 안전한 해결은 주문 게이트를 여는 것이 아니라, 변동성 목표와 낙폭 제어가 확률 신뢰도를 올릴 수 있는지 no-live 계약으로 분리하고, 자본 사다리로 올릴 수 없는 제외 조건을 고정하는 것이다.
- **구현 상태**: 스펙 137을 추가했고, `broad_no_edge_vol_target_drawdown.py`와 `broad_no_edge_vol_target_drawdown_probe.py`가 `rebalance-paper-forward`, `regime-stratify`, `execution-quality`, `money-path`, `edge-autoarm`, `released-work`, `pipeline-liveness`를 함께 읽어 변동성 목표·낙폭 제어 계약을 발행한다. 계약은 `volatility_target_scaling`, `drawdown_deleveraging_overlay`, `psr_sensitivity_hurdle`, `live_drawdown_exclusion`, `broad_no_edge_vol_target_context` 5개 lane을 만든다. `completed_candidate_id: candidate-broad-no-edge-vol-target-drawdown-experiment`와 완료된 tasks.md 덕분에 released-work가 이번 후보를 소비한다.
- **post-merge 자동화 확인**: #611 merge 뒤 `Released work ledger` run `31912195901`, `Deploy on merge to main` run `31912195904`, `Autonomous work execution loop` run `31912195895`가 모두 success다. push 직후 autonomous-work는 released-work와 병렬 실행되어 이전 후보를 한 번 더 봤기 때문에 read-only 수동 재실행 run `31912233910`을 실행했다. 최신 autonomous-work sidecar는 commit `0505e84`, selected_work `wait-for-fresh-evidence`, status `OBSERVATION_WAIT`다.
- **변동성·낙폭 증거**: 최신 sidecar로 `broad_no_edge_vol_target_drawdown_probe.py`를 재생하면 `CONTRACT_READY`, lane 5개 모두 `PROPOSED`, completed candidate `candidate-broad-no-edge-vol-target-drawdown-experiment`, next candidate `wait-for-fresh-evidence`다. money state는 `PREVIEW_ONLY`, rung은 0, demote threshold는 10, halt threshold는 20, live drawdown은 0이다.
- **돈 경로 증거**: latest money-path는 `PREVIEW_ONLY`/`NO_EDGE_YET`이고, PSR은 기준 미달이다. edge-autoarm은 `WAIT_EDGE`/`NO_EDGE`라 rung은 0에 머물렀고, latest intent gate는 `latest_intent_loss`다. 즉 돈을 바로 움직일 수 있는 상태가 아니라, no-live 후보 발굴 체인의 2차 후보를 끝까지 닫은 상태다.
- **검증 상태**: #611 브랜치에서 focused 검증 60 passed, `uv run pytest` 2799 passed/5 skipped, `uv run ruff check src tests` 통과, `git diff --check` 통과, `uv run python scripts/check_handoff_facts.py` OK, `uv run python scripts/agent_harness_probe.py --strict` OK(14/14), PR quality gate 통과, current sidecar replay와 released-work/autonomous-work post-merge sidecar를 확인하고 merge했다.
- **안전 경계**: 이 변경은 no-live 변동성 목표·낙폭 제어 계약이다. 브로커 API 호출, 실제 주문, live 재무장, 자본 배분, 라이브 전략 교체, whitelist/caps, 손실 예산, KIS secret 값, 감사 로그, 헌법, kernel manifest는 바꾸지 않았다.
- **다음 관찰 지점**: 지금은 실행 가능한 신규 후보가 없다. 다음 scheduled sidecar가 쌓인 뒤 `money-path`, `edge-autoarm`, `capital-path-readiness`, `autonomous-work`, `released-work`를 다시 읽어 `NO_EDGE_YET` 또는 `OBSERVATION_WAIT`가 바뀌었는지 확인해야 한다. 주문 제출, live 재무장, 자본 배분은 하지 않는다.
- **상세 인계**: `HANDOFF-143-BROAD-NO-EDGE-VOL-TARGET-DRAWDOWN.md`.
...
현재 `main` 최신 머지는 `2b95151`(#607, broad no-edge cross-asset relative value contract)다.
기능 커밋은 `bda57ae`이다.

- **문제 정의**: 광역 no-edge 첫 파동과 2차 frontier 라우팅은 닫혔지만, `NO_EDGE_YET`가 계속되는 상태에서 같은 절대 모멘텀 변형만 반복하면 후보 공간이 좁다. 안전한 해결은 주문 게이트를 여는 것이 아니라, 자산 간 상대가치와 현금성 대체 수익률을 no-live 후보 축으로 고정하는 것이다.
- **구현 상태**: 스펙 135를 추가했고, `broad_no_edge_cross_asset_relative_value.py`와 `broad_no_edge_cross_asset_relative_value_probe.py`가 `rebalance-paper-forward`, `public-data`, `regime-stratify`, `money-path`, `edge-autoarm`, `released-work`, `pipeline-liveness`를 함께 읽어 상대가치 계약을 발행한다. 계약은 `equity_duration_spread`, `duration_gold_spread`, `cash_proxy_hurdle`, `broad_no_edge_exclusion` 4개 lane을 만든다. `completed_candidate_id: candidate-broad-no-edge-cross-asset-relative-value-experiment`와 완료된 tasks.md 덕분에 released-work가 이번 후보를 소비한다.
- **post-merge 자동화 확인**: #607 merge 뒤 `Released work ledger` run `31891597802`, `Deploy on merge to main` run `31891597781`, `Autonomous work execution loop` run `31891597778`이 모두 success다. 다만 push 직후 autonomous-work는 released-work와 병렬 실행되어 이전 후보를 한 번 더 보았다. 그래서 read-only 수동 재실행 run `31891650370`을 실행했고, 최신 sidecar는 commit `2b95151`, selected_work `candidate-broad-no-edge-tail-risk-convexity-experiment`, status `EXECUTION_READY`, risk_grade 2다.
- **상대가치 증거**: 최신 sidecar로 `broad_no_edge_cross_asset_relative_value_probe.py`를 재생하면 `CONTRACT_READY`, lane 4개 모두 `PROPOSED`, completed candidate `candidate-broad-no-edge-cross-asset-relative-value-experiment`, next candidate `candidate-broad-no-edge-tail-risk-convexity-experiment`다.
- **돈 경로 증거**: latest money-path는 `PREVIEW_ONLY`/`NO_EDGE_YET`이고, PSR은 `0.579446 < 0.95`다. edge-autoarm은 `WAIT_EDGE`/`NO_EDGE`라 rung은 0에 머물렀고, latest intent gate는 `latest_intent_loss`다. 즉 돈을 바로 움직일 수 있는 상태가 아니라, no-live 후보 발굴 환경만 다음 단계로 전진했다.
- **검증 상태**: #607 브랜치에서 focused 검증 8 passed/52 deselected, `uv run pytest` 2785 passed/5 skipped, `uv run ruff check src tests scripts/broad_no_edge_cross_asset_relative_value_probe.py` 통과, `git diff --check` 통과, `uv run python scripts/check_handoff_facts.py` OK, `uv run python scripts/agent_harness_probe.py --strict` OK(14/14), PR quality gate 통과, current sidecar replay와 released-work/autonomous-work post-merge sidecar를 확인하고 merge했다.
- **안전 경계**: 이 변경은 no-live 상대가치 계약이다. 브로커 API 호출, 실제 주문, live 재무장, 자본 배분, 라이브 전략 교체, whitelist/caps, 손실 예산, KIS secret 값, 감사 로그, 헌법, kernel manifest는 바꾸지 않았다.
- **다음 작업 후보**: `candidate-broad-no-edge-tail-risk-convexity-experiment`다. `regime-stratify`, `execution-quality`, `rebalance-paper-forward` 증거를 함께 읽어 tail-risk 방어 후보, 비용 부담, 레짐별 대기 조건을 no-live 계약으로 만들어야 한다. 주문 제출, live 재무장, 자본 배분은 하지 않는다.
- **상세 인계**: `HANDOFF-141-BROAD-NO-EDGE-CROSS-ASSET-RELATIVE-VALUE.md`.

## 최근 관찰 — 2026-08-15 KST (#605 광역 no-edge 2차 frontier 자동 전개)

현재 `main` 최신 머지는 `8ca6569`(#605, broad frontier expansion no-edge refresh)다.
기능 커밋은 `9299f0b`이다.

- **문제 정의**: `candidate-broad-frontier-expansion-no-edge-122eb31c06bd` parent가 released-work에 소비된 뒤에도 광역 no-edge 탐색이 다시 `wait-for-fresh-evidence`로 멈추면, 돈을 벌 가능성이 있는 후보를 더 넓게 찾는 자동 루프가 끊긴다. 안전한 해결은 주문 게이트를 여는 것이 아니라, parent 완료 뒤의 2차 no-live 실험 후보를 결정론적으로 전개하는 것이다.
- **구현 상태**: 스펙 134를 추가했고, `autonomous_work_execution.py`가 parent 후보 완료 뒤 `cross_asset_relative_value`, `tail_risk_convexity`, `vol_target_drawdown` 순서의 2차 broad no-edge 후보를 고른다. parent 후보가 이미 released 된 경우에는 같은 parent 지문을 다시 발행하지 않는다. `completed_candidate_id: candidate-broad-frontier-expansion-no-edge-122eb31c06bd`와 완료된 tasks.md 덕분에 released-work가 이번 후보를 소비한다.
- **post-merge 자동화 확인**: #605 merge 뒤 `Released work ledger` run `31885623630`, `Autonomous work execution loop` run `31885623635`, `Deploy on merge to main` run `31885623640`이 모두 success다. released-work는 commit `8ca6569`, released_count 53이고 스펙 134 후보를 released로 포함한다. 최신 autonomous-work sidecar는 commit `8ca6569`, selected_work `candidate-broad-no-edge-cross-asset-relative-value-experiment`, status `EXECUTION_READY`, risk_grade 2다.
- **돈 경로 증거**: latest money-path는 `PREVIEW_ONLY`/`NO_EDGE_YET`이고, PSR은 `0.579446 < 0.95`다. edge-autoarm은 `WAIT_EDGE`/`NO_EDGE`라 rung은 0에 머물렀고, rebalance-micro-gtaa도 `armed=false`, latest intent gate `latest_intent_loss`다. 즉 돈을 바로 움직일 수 있는 상태가 아니라, no-live 후보 발굴 환경만 다음 단계로 전진했다.
- **검증 상태**: #605 브랜치에서 focused autonomous-work 검증 6 passed/47 deselected, `uv run pytest` 2778 passed/5 skipped, `uv run ruff check src tests` 통과, `git diff --check` 통과, `uv run python scripts/check_handoff_facts.py` OK, `uv run python scripts/agent_harness_probe.py --strict` OK(14/14), PR quality gate 통과, current sidecar replay와 released-work/autonomous-work post-merge sidecar를 확인하고 merge했다.
- **안전 경계**: 이 변경은 no-live 후보 라우팅이다. 브로커 API 호출, 실제 주문, live 재무장, 자본 배분, 라이브 전략 교체, whitelist/caps, 손실 예산, KIS secret 값, 감사 로그, 헌법, kernel manifest는 바꾸지 않았다.
- **다음 작업 후보**: `candidate-broad-no-edge-cross-asset-relative-value-experiment`다. `rebalance-paper-forward`, `public-data`, `regime-stratify`, `money-path`를 읽어 자산 간 상대가치 no-live 후보와 제외 기준을 정의해야 한다. 주문 제출, live 재무장, 자본 배분은 하지 않는다.
...
현재 `main` 최신 머지는 `15719d8`(#601, broad no-edge regime cost robustness contract)다.
기능 커밋은 `f0c5cee`이다.

- **문제 정의**: broad no-edge 축에서 다중 보유 기간·신호군 후보는 닫혔지만, 레짐 전환이나 비용·슬리피지에 약하면 paper 성과가 실제 돈 경로로 올라갈 수 없다. 다음 병목은 `regime-stratify`와 `execution-quality`를 함께 읽어 레짐별 취약 구간과 비용 민감도를 no-live 계약으로 고정하는 것이었다.
- **구현 상태**: 스펙 132를 추가했고, `broad_no_edge_regime_cost_robustness.py`와 `broad_no_edge_regime_cost_robustness_probe.py`가 regime-stratify, execution-quality, money-path, edge-autoarm, rebalance-paper-forward, released-work, evolution-ledger, pipeline-liveness를 함께 읽어 no-live 레짐·비용 견고성 계약을 발행한다. 계약은 레짐 라벨을 `PASS`/`WAIT`/`STRESS`로 나누고, 10/25/50bp 비용 민감도 행을 만든다. `completed_candidate_id: candidate-broad-no-edge-regime-cost-robustness-experiment`와 완료된 tasks.md 덕분에 released-work가 이번 후보를 소비한다.
- **post-merge 자동화 확인**: #601 merge 뒤 `Deploy on merge to main` run `31596002457`, `Released work ledger` run `31596002387`, `Autonomous work execution loop` run `31596002386`이 모두 success다. released-work는 commit `15719d8`, released_count 51이고 스펙 132 후보를 released로 포함한다. 최신 autonomous-work sidecar는 commit `15719d8`, selected_work `candidate-broad-no-edge-data-gap-audit`, status `EXECUTION_READY`, risk_grade 2다.
- **레짐·비용 증거**: 최신 sidecar로 `broad_no_edge_regime_cost_robustness_probe.py`를 재생하면 `CONTRACT_READY`, regime window 2개, label 6개, stress label 2개, wait label 2개, cost stress rows 10/25/50bp, release_gate PASS다. 즉 이번 후보는 완료 처리됐고, broad no-edge frontier 지도에서는 자산군 확장, 다중 horizon, 레짐·비용 세 축이 released, 데이터 결측 감사가 open 상태다.
- **검증 상태**: #601 브랜치에서 focused 검증 14 passed/46 deselected, `uv run pytest` 2769 passed/5 skipped, `uv run ruff check src tests` 통과, `git diff --check` 통과, `uv run python scripts/check_handoff_facts.py` OK, `uv run python scripts/agent_harness_probe.py --strict` OK(14/14), PR quality gate 통과, current sidecar replay와 released-work/autonomous-work local replay를 확인하고 merge했다. 이 HANDOFF 갱신 전 main 기준 `uv run pytest -q`는 HANDOFF가 아직 #599를 가리켜 하네스 관련 2개만 실패했고, 이 갱신이 그 원인을 바로잡는다.
...
- **다음 작업 후보**: `candidate-broad-no-edge-data-gap-audit`다. `public-data` summary, `regime.json`, `regime_timeline.csv`를 읽어 데이터 결측 원인이 no-edge 판정에 끼친 영향을 분리하는 읽기 전용 감사를 정의해야 한다. 주문 제출, live 재무장, 자본 배분은 하지 않는다.
- **상세 인계**: `HANDOFF-139-BROAD-NO-EDGE-REGIME-COST-ROBUSTNESS.md`.

## 최근 관찰 — 2026-08-12 KST (#597 검증 실패 승격 재검토 계약 완료)

현재 `main` 최신 머지는 `cc7f050`(#597, validation failure promotion recheck contract)다.
기능 커밋은 `8de3136`이다.

- **문제 정의**: 검증 실패 child의 마지막 병목은 learning ledger가 폐기한 후보를 언제 다시 열 수 있는지 명확하지 않다는 점이었다. 억제 기억이 없으면 같은 실패 후보를 반복하고, 재검토 조건이 없으면 실패 지문이 바뀌어도 후보가 닫힌 채 남을 수 있다.
- **구현 상태**: 스펙 130을 추가했고, `validation_failure_promotion_recheck.py`와 `validation_failure_promotion_recheck_probe.py`가 learning ledger, autonomous-promotion, candidate-results를 읽어 후보별 `SUPPRESSION_ACTIVE`/`RECHECK_ALLOWED`/`WAITING_FOR_EVIDENCE` 계약을 JSON/Markdown으로 발행한다. 현재 `candidate-1ed634d8bf6d`와 `candidate-cc96b35062da`는 최신 ledger가 `rejected`, promotion stage가 `DISCARD`, candidate-result가 `fail`이라 억제를 유지한다. `completed_candidate_id: candidate-broad-validation-failure-promotion-recheck-contract`와 완료된 tasks.md 덕분에 released-work가 이번 child를 소비한다.
- **post-merge 자동화 확인**: #597 merge 뒤 `Deploy on merge to main` run `31574288074`, `Released work ledger` run `31574288078`, `Autonomous work execution loop` run `31574288096`이 모두 success다. released-work는 commit `cc7f050`, released_count 49이고 스펙 130 후보를 released로 포함한다. 최신 autonomous-work sidecar는 commit `cc7f050`, selected_work `candidate-broad-no-edge-multi-horizon-signal-experiment`, status `EXECUTION_READY`, risk_grade 2다. 최신 KIS smoke sidecar는 schedule run `31564350691`에서 `c025cd4` 기준 `smoke_state=success`, `key_valid=true`지만, 이번 merge commit의 직접 smoke 증거는 아니다.
- **승격 재검토 증거**: 최신 sidecar로 `validation_failure_promotion_recheck_probe.py`를 재생하면 `CONTRACT_READY`, candidate_count 2, suppressed_count 2, allowed_recheck_count 0, waiting_count 0이다. 두 후보 모두 result/pass, validation layer pass, promotion not discard, latest ledger recheck condition, failure fingerprint change 중 하나가 충족될 때만 no-live 재검토 후보로 되살릴 수 있다.
- **검증 상태**: #597 브랜치에서 `uv run pytest` 2750 passed/5 skipped, `uv run ruff check src tests` 통과, `git diff --check` 통과, `uv run python scripts/check_handoff_facts.py` OK, `uv run python scripts/agent_harness_probe.py --strict` OK(14/14), PR quality gate 통과, current sidecar replay와 released-work/autonomous-work local replay를 확인하고 merge했다. #597 merge 직후 main 기준 `uv run pytest`는 HANDOFF가 아직 #595를 가리켜 하네스 관련 2개만 실패했고, 이 HANDOFF 갱신이 그 원인을 바로잡는다.
- **안전 경계**: 이 변경은 억제된 실패 후보의 재검토 조건을 고정하고 검증 실패 child 묶음을 닫는 운영 보정이다. 브로커 API 호출, 실제 주문, live 재무장, 자본 배분, 라이브 전략 교체, whitelist/caps, 손실 예산, KIS secret 값, 감사 로그, 헌법, kernel manifest는 바꾸지 않았다. money-path는 `PREVIEW_ONLY`/`NO_EDGE_YET`, edge-autoarm은 `WAIT_EDGE`/`NO_EDGE`다.
- **다음 작업 후보**: `candidate-broad-no-edge-multi-horizon-signal-experiment`다. 검증 실패 child 4개는 모두 released로 닫혔으므로, 다음은 broad no-edge 축에서 보유 기간과 신호군을 넓히는 no-live 실험 설계다. 주문 제출, live 재무장, 자본 배분은 하지 않는다.
- **상세 인계**: `HANDOFF-137-VALIDATION-FAILURE-PROMOTION-RECHECK.md`.

## 최근 관찰 — 2026-08-12 KST (#595 검증 실패 패키지 종류별 확장 계약 완료)

현재 `main` 최신 머지는 `027b34f`(#595, validation failure package-kind expansion contract)다.
기능 커밋은 `bc473ec`이다.

- **문제 정의**: 스펙 128에서 검증 실패 패키지 2개는 데이터 결측이 아니라는 점이 닫혔다. 그 다음 병목은 `strategy_backtest` 실패와 `portfolio_backtest` 실패를 같은 실패로 뭉개지 않고, 전략군·포트폴리오 구성·보유 기간·산출 증거별 no-live 후보 축으로 나누는 것이었다.
- **구현 상태**: 스펙 129를 추가했고, `validation_failure_package_kind_expansion.py`와 `validation_failure_package_kind_expansion_probe.py`가 candidate-packages와 candidate-results를 읽어 패키지 종류별 실패 계약을 JSON/Markdown으로 발행한다. 보고서는 `strategy_backtest`와 `portfolio_backtest`를 별도 bucket으로 나누고, 각 bucket에 패키지 참조, 실패 진단, 실행 지표, 제한된 출력 힌트, 다음 실험 축을 보존한다. `completed_candidate_id: candidate-broad-validation-failure-package-kind-expansion-contract`와 완료된 tasks.md 덕분에 released-work가 이번 child를 소비한다.
- **post-merge 자동화 확인**: #595 merge 뒤 `Released work ledger` run `31567052065`, `Autonomous work execution loop` run `31567052426`, `Deploy on merge to main` run `31567052052`가 모두 success다. released-work는 commit `027b34f`, released_count 48이고 스펙 129 후보를 released로 포함한다. 최신 autonomous-work sidecar는 commit `027b34f`, selected_work `candidate-broad-validation-failure-promotion-recheck-contract`, status `EXECUTION_READY`, risk_grade 2다. 최신 KIS smoke sidecar는 `c025cd4` 기준 schedule run에서 `smoke_state=success`, `key_valid=true`지만, 이번 merge commit의 직접 smoke 증거는 아니다.
- **패키지 종류별 증거**: 최신 sidecar로 `validation_failure_package_kind_expansion_probe.py`를 재생하면 `CONTRACT_READY`, package_count 2, bucket_count 2, retryable_count 2, command_count 4, execution_evidence_count 4다. portfolio bucket은 segment wins `0/3`과 `1/3`, strategy bucket은 segment wins `1/3`이며, 두 bucket 모두 벤치마크 Sharpe가 후보보다 높다는 실패 구조를 보존한다.
- **검증 상태**: #595 브랜치에서 `uv run pytest` 2744 passed/5 skipped, `uv run ruff check src tests` 통과, `git diff --check` 통과, `uv run python scripts/check_handoff_facts.py` OK, `uv run python scripts/agent_harness_probe.py --strict` OK(14/14), PR quality gate 통과, current sidecar replay와 released-work/autonomous-work local replay를 확인하고 merge했다. #595 merge 직후 main 기준 `uv run pytest -q`는 HANDOFF가 아직 #593을 가리켜 하네스 관련 2개만 실패했고, 이 HANDOFF 갱신이 그 원인을 바로잡는다.
- **안전 경계**: 이 변경은 검증 실패 구조를 넓게 나누고 다음 no-live child 후보로 전진시키는 운영 보정이다. 브로커 API 호출, 실제 주문, live 재무장, 자본 배분, 라이브 전략 교체, whitelist/caps, 손실 예산, KIS secret 값, 감사 로그, 헌법, kernel manifest는 바꾸지 않았다. money-path는 `PREVIEW_ONLY`/`NO_EDGE_YET`, edge-autoarm은 `WAIT_EDGE`/`NO_EDGE`다.
- **다음 작업 후보**: `candidate-broad-validation-failure-promotion-recheck-contract`다. learning ledger, autonomous-promotion, candidate-result evidence를 함께 읽어 어떤 실패 지문 변화가 억제된 후보 재검토를 허용하는지 결정론적 계약으로 만들어야 한다. 같은 패키지를 무작정 재시도하거나 실거래를 여는 작업이 아니다.
- **상세 인계**: `HANDOFF-136-VALIDATION-FAILURE-PACKAGE-KIND.md`.

## 최근 관찰 — 2026-08-12 KST (#593 검증 실패 데이터 준비도 계약 완료)

현재 `main` 최신 머지는 `d7e473d`(#593, validation failure data readiness contract)다.
기능 커밋은 `e27d58b`이다.

- **문제 정의**: 스펙 127이 검증 실패 명령과 실행 증거를 고정했지만, 다음 병목은 그 실패가 데이터 결측 때문인지, 데이터는 준비됐지만 전략 엣지가 부족한지 분리하는 것이었다. 같은 검증 명령을 반복하는 대신 history root, portfolio TOML, 관측 기간, public-data, regime-stratify 증거를 한 계약으로 묶어야 했다.
- **구현 상태**: 스펙 128을 추가했고, `validation_failure_data_readiness.py`와 `validation_failure_data_readiness_probe.py`가 candidate-packages, candidate-results, candidate history support, portfolio TOML, public-data, regime-stratify를 읽어 패키지별 데이터 준비도 계약을 발행한다. 현재 패키지 2개와 포트폴리오 데이터 표면 3개는 모두 `PASS_DATA_READY`다. candidate-results가 `fail`이고 retryable 정보가 candidate-packages의 nested `promotion_patch`에만 있어도 broad validation failure child 순서가 끊기지 않도록 autonomous-work도 보강했다.
- **post-merge 자동화 확인**: #593 merge 뒤 `Released work ledger` run `31554206845`, `Autonomous work execution loop` run `31554206817`, `Deploy on merge to main` run `31554206841`이 모두 success다. released-work는 commit `d7e473d`, released_count 47이고 스펙 128 후보를 released로 포함한다. 최신 autonomous-work sidecar는 commit `d7e473d`, selected_work `candidate-broad-validation-failure-package-kind-expansion-contract`, status `EXECUTION_READY`, risk_grade 2, package_kind package_count 2, retryable_count 2다.
- **데이터 준비도 증거**: 최신 sidecar로 `validation_failure_data_readiness_probe.py`를 재생하면 `CONTRACT_READY`, package_count 2, surface_count 3, data_ready_count 2, waiting_count 0, blocked_count 0, execution_evidence_count 3이다. public-data는 연구용 CPI freshness 문제로 `PARTIAL_RESEARCH_INPUT`이지만, 현재 검증 패키지 직접 입력은 history support와 portfolio TOML 기준으로 준비 완료다.
- **검증 상태**: #593 브랜치에서 `uv run pytest` 2739 passed/5 skipped, `uv run ruff check src tests` 통과, `git diff --check` 통과, `uv run python scripts/check_handoff_facts.py` OK, `uv run python scripts/agent_harness_probe.py --strict` OK(14/14), PR quality gate 통과, current sidecar replay와 released-work/autonomous-work local replay를 확인하고 merge했다. deploy run `31554206841`도 success지만, 서버 audit_log는 이 컨테이너에서 확인할 수 없다.
- **안전 경계**: 이 변경은 검증 실패 데이터 준비도를 읽기 쉽게 고정하고 다음 no-live child 후보로 전진시키는 운영 보정이다. 브로커 API 호출, 실제 주문, live 재무장, 자본 배분, 라이브 전략 교체, whitelist/caps, 손실 예산, KIS secret 값, 감사 로그, 헌법, kernel manifest는 바꾸지 않았다. money-path는 `PREVIEW_ONLY`/`NO_EDGE_YET`, edge-autoarm은 `WAIT_EDGE`/`NO_EDGE`다.
- **다음 작업 후보**: `candidate-broad-validation-failure-package-kind-expansion-contract`다. strategy_backtest와 portfolio_backtest 실패를 나눠 전략군, 포트폴리오 구성, 보유 기간, 산출 증거별 no-live 후보 축을 재정렬해야 한다. 같은 패키지를 무작정 재시도하거나 실거래를 여는 작업이 아니다.
- **상세 인계**: `HANDOFF-135-VALIDATION-FAILURE-DATA-READINESS.md`.

## 최근 관찰 — 2026-08-12 KST (#591 검증 실패 명령 재현 계약 완료)

현재 `main` 최신 머지는 `dcf2fc8`(#591, validation failure command replay contract)다.
기능 커밋은 `5bbf0e8`이다.

- **문제 정의**: 스펙 126이 검증 실패 parent 뒤 첫 child 후보를 골랐지만, 그 child를 닫지 않으면 같은 실패 명령과 증거 상태를 다음 세션이 다시 해석해야 했다. 안전한 해결은 명령을 무작정 다시 실행하는 것이 아니라, 실패 명령·안전 재현 범위·실행 증거·다음 진단 action을 기계 판독 계약으로 고정하는 것이다.
- **구현 상태**: 스펙 127을 추가했고, `validation_failure_command_replay.py`와 `validation_failure_command_replay_probe.py`가 candidate-packages와 candidate-results를 읽어 no-live 명령 재현 계약을 발행한다. 계약은 명령을 실행하지 않고, 기존 candidate-result-executor 안전 판정을 재사용한다. `completed_candidate_id: candidate-broad-validation-failure-command-replay-contract` 마커와 완료된 tasks.md 덕분에 released-work가 이번 child를 소비한다.
- **post-merge 자동화 확인**: #591 merge 뒤 `Released work ledger` run `31504787462`, `Autonomous work execution loop` run `31504787388`, `Candidate implementation factory` run `31504787427`, `Candidate result executor` run `31504787411`, `Deploy on merge to main` run `31504787391`이 모두 success다. released-work는 commit `dcf2fc8`, released_count 46이고 스펙 127 후보를 released로 포함한다. 최신 autonomous-work sidecar는 commit `dcf2fc8`, timestamp `2026-08-11T15:01:47Z`, selected_work `candidate-broad-validation-failure-data-readiness-contract`, status `EXECUTION_READY`, risk_grade 2다.
- **최신 검증 실패 증거**: post-merge candidate-result-executor는 패키지 2개를 `fail`로 기록했고, 각 패키지에 실행 증거 2개씩을 남겼다. 새 command replay probe를 최신 sidecar에 다시 돌리면 `CONTRACT_READY`, package_count 2, command_count 4, replay_safe_count 4, missing_execution_count 0, unsafe_command_count 0이다. 즉 이제 실패 명령은 “증거 없음”이 아니라 “안전하게 읽을 수 있는 실패 실행 증거 있음” 상태다.
- **검증 상태**: #591 브랜치에서 `uv run pytest` 2733 passed/5 skipped, `uv run ruff check src tests` 통과, `git diff --check` 통과, `uv run python scripts/check_handoff_facts.py` OK, `uv run python scripts/agent_harness_probe.py --strict` OK(14/14), PR quality gate 2회 통과, current sidecar replay와 released-work/autonomous-work local replay를 확인하고 merge했다.
- **안전 경계**: 이 변경은 검증 실패 증거를 읽기 쉽게 고정하는 no-live 운영 보정이다. 브로커 API 호출, 실제 주문, live 재무장, 자본 배분, 라이브 전략 교체, whitelist/caps, 손실 예산, KIS secret 값, 감사 로그, 헌법, kernel manifest는 바꾸지 않았다. money-path는 `PREVIEW_ONLY`/`NO_EDGE_YET`, edge-autoarm은 `WAIT_EDGE`/`NO_EDGE`다.
- **다음 작업 후보**: `candidate-broad-validation-failure-data-readiness-contract`다. candidate history support, portfolio TOML, public-data, regime-stratify 증거를 함께 읽어 history root, 관측 기간, 데이터 결측 원인을 검증 패키지별 PASS/WAIT/FAIL로 분리해야 한다. 같은 패키지를 무작정 재시도하거나 실거래를 여는 작업이 아니다.
- **상세 인계**: `HANDOFF-134-VALIDATION-FAILURE-COMMAND-REPLAY.md`.

## 최근 관찰 — 2026-08-11 KST (#589 검증 실패 frontier 후속 후보 전개)

현재 `main` 최신 머지는 `25b1962`(#589, broad validation failure frontier progression)다.
기능 커밋은 `cfc28a9`이다.

- **문제 정의**: 운영자가 지적한 "검토 범위가 갇혀 있다"는 문제는 실제로 있었다. #582가 검증 실패 parent 후보 `candidate-broad-frontier-expansion-validation-failures-22f38b8629eb`를 만들었지만, 그 parent를 완료한 뒤에는 구체적인 후속 축 없이 다시 대기로 밀릴 수 있었다. 안전한 해결은 주문 게이트를 우회하는 것이 아니라, 막힌 검증 실패를 명령 재현·데이터 준비도·패키지 종류·승격 재검토 조건으로 나눠 다음 no-live 후보를 자동 발행하는 것이다.
- **구현 상태**: 스펙 126을 추가했고, `autonomous_work_execution.py`가 `broad_validation_failure_frontier_map`을 JSON/Markdown으로 발행한다. parent 후보가 released-work에 들어 있고 retryable `execution_failed` 패키지 2개가 남아 있으면 첫 후속 후보 `candidate-broad-validation-failure-command-replay-contract`를 선택한다. 이 후보가 released되면 다음은 `candidate-broad-validation-failure-data-readiness-contract`로 전진한다.
- **post-merge 자동화 확인**: #589 merge 뒤 `Autonomous work execution loop` run `31501059038`, `Released work ledger` run `31501059269`, `Deploy on merge to main` run `31501059251`이 모두 success다. released-work는 commit `25b1962`, timestamp `2026-08-11T14:21:29Z`, released_count 45이고 스펙 126 parent 후보를 released로 포함한다. 최신 autonomous-work sidecar는 commit `25b1962`, timestamp `2026-08-11T14:21:26Z`, selected_work `candidate-broad-validation-failure-command-replay-contract`, status `EXECUTION_READY`, risk_grade 2, blocked package count 2다.
- **검증 상태**: #589 브랜치에서 focused autonomous-work tests 45 passed, `uv run pytest` 2728 passed/5 skipped, `uv run ruff check src tests` 통과, `git diff --check` 통과, `uv run python scripts/check_handoff_facts.py` OK, `uv run python scripts/agent_harness_probe.py --strict` OK(14/14), PR quality gate 통과, parent-release local replay와 child advance replay를 확인하고 merge했다.
- **안전 경계**: 이 변경은 검증 실패를 더 넓게 보는 no-live 작업 후보 확장이다. 브로커 API 호출, 실제 주문, live 재무장, 자본 배분, 라이브 전략 교체, whitelist/caps, 손실 예산, KIS secret 값, 감사 로그, 헌법, kernel manifest는 바꾸지 않았다. money-path는 `PREVIEW_ONLY`/`NO_EDGE_YET`, edge-autoarm은 `WAIT_EDGE`/`NO_EDGE`다.
- **다음 작업 후보**: `candidate-broad-validation-failure-command-replay-contract`다. candidate-result-executor와 candidate-packages를 읽어 실패 명령, 종료 코드, 제한된 stdout/stderr 요약, 안전 재현 가능 범위를 계약으로 닫아야 한다. 같은 패키지를 막연히 재시도하거나 실거래를 여는 작업이 아니다.
- **상세 인계**: `HANDOFF-133-VALIDATION-FAILURE-FRONTIER.md`.
...
현재 `main` 최신 머지는 `f26b86b`(#585, broad no-edge frontier closeout)다.
#584 기능 커밋은 `b863e0b`, #584 merge commit은 `bd36342`, #585 closeout merge commit은 `f26b86b`이다.
...
- **구현 상태**: `autonomous_work_execution.py`가 broad no-edge parent와 후속 broad no-edge 후보 release를 parent 지문 입력에서 제외한다. 새 `broad_no_edge_frontier_map`은 자산군·전략군, 신호군·보유 기간, 레짐·비용 견고성, 데이터 결측 원인 감사를 deterministic 순서로 노출한다. `specs/124-broad-no-edge-frontier/contracts/broad-no-edge-frontier.md`에는 `completed_candidate_id: candidate-broad-frontier-expansion-no-edge-58298dfc172c`가 들어가 parent 후보가 released-work에서 닫힌다.
- **post-merge 자동화 확인**: #584 merge 뒤 deploy run `31476005233`, released-work run `31476005722`, autonomous-work run `31476005231`은 모두 success다. #585 merge 뒤 released-work run `31476801539`는 commit `f26b86b`, released_count 43으로 parent 후보를 released 처리했다. autonomous-work run `31476801548`도 commit `f26b86b`에서 success이고 selected_work는 `candidate-broad-no-edge-asset-universe-rotation-experiment`, overall_status는 `EXECUTION_READY`다.
- **검증 상태**: #584 브랜치에서 `uv run pytest` 2720 passed/5 skipped, `uv run ruff check src tests` 통과, `git diff --check` 통과, `uv run python scripts/check_handoff_facts.py` OK, `uv run python scripts/agent_harness_probe.py --strict` OK(14/14), PR quality gate 통과, 실제 sidecar replay 통과를 확인했다. #585 closeout 브랜치에서도 `uv run pytest` 2720 passed/5 skipped, `uv run ruff check src tests` 통과, `git diff --check` 통과, released-work/autonomous-work local replay, `check_handoff_facts.py`, strict agent harness를 확인했다.
...
- **다음 작업 후보**: `candidate-broad-no-edge-asset-universe-rotation-experiment`다. `public-data`, `rebalance-paper-forward`, `money-path`, `edge-autoarm`, `released-work`, `autonomous-work`를 함께 읽고, 주식·현금성·채권성·방어 자산군 대체 후보의 no-live 후보군과 제외 기준을 SDD로 정의해야 한다. 주문 제출, live 재무장, 자본 배분은 하지 않는다.
...
## 최근 마일스톤 — 2026-08-12 KST (#593 검증 실패 데이터 준비도 계약)

#593으로 스펙 128이 main에 들어갔다. `validation_failure_data_readiness_probe.py`는 candidate-packages,
candidate-results, candidate history support, portfolio TOML, public-data, regime-stratify를 읽어 검증 실패
패키지 2개와 포트폴리오 데이터 표면 3개를 모두 `PASS_DATA_READY`로 분리한다. released-work run
`31554206845`는 commit `d7e473d`에서 released_count 47과
`candidate-broad-validation-failure-data-readiness-contract` 완료를 확인했다. 최신 autonomous-work run
`31554206817`은 다음 후보로 `candidate-broad-validation-failure-package-kind-expansion-contract`를 골랐다.
돈 경로는 계속 `PREVIEW_ONLY`/`NO_EDGE_YET`라 실주문은 불가다. 상세는
`HANDOFF-135-VALIDATION-FAILURE-DATA-READINESS.md`.
...
- `HANDOFF-162-COMMODITY-POSITIONING-GATE-CONTROLS.md` — 실제 역사 양성·널
  대조군으로 PSR 통계선을 감사하고 CFTC·EIA 원자재 후보 16개와 688개 고유 장부를
  출시했다. 생산은 `NO_FACTORY_EDGE`라 단 0·자본 0·주문 0건을 유지한다.
- `HANDOFF-161-INDEPENDENT-COMMODITY-TERM-STRUCTURE.md` — 원자재 기간구조 전략
  16개와 672개 고유 감사 장부를 출시했다. 생산은 비용 후 초과수익과 혼합 샤프가
  음수인 `NO_FACTORY_EDGE`라 단 0·자본 0·주문 0건을 유지한다.
- `HANDOFF-160-INDEPENDENT-FX-CARRY-GATE-POWER.md` — 외환 금리차 전략 16개,
  관문 검출력 곡선, 656개 고유 감사 장부를 출시했다. 생산은 모든 후보의 홀드아웃
  초과 샤프가 음수인 `NO_FACTORY_EDGE`라 단 0·자본 0·주문 0건을 유지한다.
- `HANDOFF-159-INDEPENDENT-CREDIT-SPREAD-FACTORY.md` — 투자등급 회사채 스프레드
  전략 64개와 640개 고유 감사 장부를 출시했다. 생산은 `NO_FACTORY_EDGE`라
  단 0·자본 0·주문 0건을 유지하며 다음 전략군은 독립 외환 금리차다.
- `HANDOFF-158-INDEPENDENT-TREASURY-CARRY-FACTORY.md` — 미국 국채 5개 만기의
  캐리 전략 64개와 576회 검정을 출시했다. 생산은 `NO_FACTORY_EDGE`라
  단 0·자본 0·주문 0건을 유지한다.
- `HANDOFF-157-INDEPENDENT-MACRO-STRATEGY-FACTORY.md` — 시점 기준 독립 거시
  전략 64개와 사전 탐색·기존 생산을 합친 512회 검정을 출시했다. 생산은
  `NO_FACTORY_EDGE`라 단 0·자본 0·주문 0건을 유지한다.
- `HANDOFF-156-AUTONOMOUS-STRATEGY-FACTORY.md` — 네 전략군 256개 자동 생성·누적
  다중검정·NAV 10% 연구 캐너리 연결을 출시했다. 생산 전체 탐색은
  `NO_FACTORY_EDGE`로 끝나 단 0·자본 0·주문 0건을 유지한다.
...
- `HANDOFF-134-VALIDATION-FAILURE-COMMAND-REPLAY.md` — #591이 검증 실패 명령 재현 계약을 출시하고, released-work가 `candidate-broad-validation-failure-command-replay-contract`를 소비해 다음 후보가 `candidate-broad-validation-failure-data-readiness-contract`로 전진한 상태
- `HANDOFF-133-VALIDATION-FAILURE-FRONTIER.md` — #589가 검증 실패 parent 완료 뒤 첫 no-live child 후보 `candidate-broad-validation-failure-command-replay-contract`를 자동 선택하게 한 상태
- `HANDOFF-132-BROAD-ASSET-UNIVERSE.md` — #587이 broad no-edge 자산군 방어 회전 계약을 출시하고, released-work가 `candidate-broad-no-edge-asset-universe-rotation-experiment`를 소비한 상태
