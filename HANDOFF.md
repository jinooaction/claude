...
186 FR007 구현: 동일 수집 묶음의 공통 USD 예수금과 현재/결제 잔고 통화 행,
단일 USD 환산액과 외화 총액, HTS 총자산식 후보를 정확한 Decimal 산술로 대조한다.
관련46개(0.45초)·린트·하네스14/14·인계 통과. 자료 부족은0으로 메우지 않으며 숫자
일치를 계좌 인증으로 승격하지 않는다. 전체 회귀·실제 서버 대조 결과는 후속이다.
...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `6b92ae1` — Merge pull request #805 from jinooaction/codex/account-source-collection |
| main 테스트 | 코드bc35254/main6b92ae1 전체3839 passed/12 skipped(774.73초). 제외는 실제 KIS11개와 가동 전 상태 전용1개. 실제34684108577은11 passed/25.09초, 원본9응답·장부순번5·구조 분석 성공. |
| main 린트 | 코드bc35254/main6b92ae1의 `uv run ruff check src tests` 통과. |
