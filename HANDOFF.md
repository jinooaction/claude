...
2026-09-14 후속: 운영자는 유료 아닌 효율적인 자료 경로부터 찾도록 지시했다.
Massive 구독 승인 대기를 주 작업의 막힘으로 삼지 않는다. 무료 경로 조사 결과는
specs/187-massive-history-research/free-source-review.md에 있다. KIS30일은 로컬 제한이고
실제 최대 보관 기간이 아니다. Alpaca 무료 과거 SIP의 공식 근거와 기존 수집 코드가 있으나
무료 Paper 계정의 권한 표현 차이로 작은 실제 GET 확인이 필요하다. 실제 장기 자료 미확보.

187 장기 자료 연구 후속: Massive 전용 읽기 수집·원본 캐시 재개·기존18후보 연구기와
독립 증거 검사를 연결했다. 관련16개, 전체4771 passed/12 skipped(850.21초),
린트·하네스14/14·인계 사실 검사 통과. /tmp/claude-187-full.log.
실행 코드9b23717, 초안PR827. 실제 자료 단계가 남아 머지하지 않았다.
2023-09-07~2026-09-11은 정확히756거래일이다. 현재 Massive 키가 없으며
개인 Stocks Starter 첫1개월29달러 비용 질문은 후속 무료 우선 지시로 보류한다. 결제·실제 다운로드·
실제 재검증은 미수행이며 T004는 미완료다. 키를 채팅이나 저장소에 남기지 않는다.
기존 KIS 계좌·수집·위험 관문을 유지하고 Massive 자료를 KIS 자료로 합치지 않는다.
연구 통과도 실거래 적격으로 승격하지 않는다. 실제 주문·자금 배정 없음.
...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `d72325b` — Merge pull request #826 from jinooaction/codex/185-archive-inventory-handoff |
| main 테스트 | 실행 코드f253182/main5fa2e20 전체4755 passed/12 skipped(859.46초). 실제34790306811 KIS11개 통과(28.40초), 코드별2묶음·중복 제거5일 확인. 전략은 자료 부족. |
| main 린트 | 실행 코드f253182/main5fa2e20의 `uv run ruff check src tests` 및 하네스14/14 검사 통과. |
| 열린 PR | #827 초안:187 장기 자료 수집·연구 연결. 코드 검사 완료. 후속 무료 경로 조사 반영, 실제 자료 미확보. #793은 머지 완료. |
...
| 활성 작업 | 187 무료 경로 우선: KIS 실제 과거 범위와 Alpaca 무료 SIP 권한 확인이 다음 단계다. 유료 경로 보류, 실제756일 미확보. 184 구현·배포 완료와 실제 가동 적격은 구분한다. |
