...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `0b8992b` — Merge pull request #779 from jinooaction/codex/182-unverified-account-assets |
| main 테스트 | #779 코드e7162ee 전체3605 passed/9 skipped,실패0(799.88초). 관련89개 통과. 실제 KIS34075774149는 기존7개 통과/새 계좌 조회1개 실패(USD_MARGIN_ROW_COUNT); 미제공/중복 구분 보정 중이다. |
| main 린트 | #779 코드e7162ee에서 `uv run ruff check src tests` 통과. |
| 열린 PR | #779 병합 완료. `codex/182-unreported-usd-margin`에서 USD 구성 내역 미제공과 중복·형식 오류를 구분한다. 열린 PR은 GitHub에서 다시 확인한다. |
...
| 활성 작업 | 확정600/12·모의 검증·페이지/OTCB 보정은 병합·배포됐다. 실제 조회의 USD 구성 내역 단일행 확인 실패를 미제공/중복으로 구분하는 보정에서 관련99개 통과, 전체 회귀/병합/서버 재검증 중이다. 자금 배정·주문0, NAV·전략/전진 증거·생산 주문 연결은 미완료다. |
