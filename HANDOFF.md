...
| 항목 | 상태 |
|------|------|
| 마지막 main 커밋 | `9b8c9e1` — Merge pull request #837 from jinooaction/codex/190-cost-aware-intraday |
| main 테스트 | 실행8ca0c9c 전체4797 passed/13 skipped(828.14초). 마지막 실제KIS는 이전main6580c05의35541770144 12개 통과(111.83초); 이번190은 브로커 코드 미변경. |
| main 린트 | 실행8ca0c9c의 `uv run ruff check src tests`와 신규 연구CLI·후속 문서 검증, 하네스14/14 검사 통과. |
| 열린 PR | #827 유료 공급자 초안 보류. #837 병합·배포 완료,190 출시 인계 문서 후속. |
...
| 활성 작업 | HF Data Library 실제1645세션 기존18후보와190 신규6후보 미통과.190 연구 경로 출시 완료. 후속 가설/실제 비용 근거 검토, 최종확인495일 성과 미개봉. |
