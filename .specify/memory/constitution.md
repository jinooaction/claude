...
4. **Deploy ≠ live money; capital sizing is governed by the evidence-gated capital ladder under a standing operator delegation (amended v8.0.0).** Auto-deploy lands code and restarts the worker; it does NOT by itself move the system from dry-run to real orders. Live exposure — both the arming decision and the **amount of capital deployed** — is governed by the **spec 050/150 capital ladder**, authorised by the operator's standing delegation (mason, 2026-06-11: "1·2·3 모두 세계 최고 수준이 목표. 3번도 나는 자동과 자율에 맡길거야. 기준은 계좌 잔고와 포트폴리오"):
   ...
   - **Entry (rung 0→1, research)**: ALL of — exactly 64 unique live-expressible trials are complete and preserved; the selected candidate passes the preregistered full-trial DSR ≥ 0.95, PBO ≤ 0.10, PSR ≥ 0.95, at least ten time segments with ≥60% wins, Sharpe superiority ≥0.20, Calmar superiority, drawdown ≤80% of benchmark, and positive return under 50bp cost; evidence is fresh; hardened-canary PASS; and the selected and live strategy fingerprints are identical. Any partial, stale, approximate, contradictory, or mismatched evidence remains at 0%. Backtest workflow itself cannot order or move capital.
...
5. **Strategy reassignment is governed by the evidence-gated 5-gate tournament under the same standing operator delegation (amended v6.0.0).** Item 4 governs HOW MUCH capital the live system deploys; this item governs WHICH strategy it deploys. The operator delegated strategy selection (mason, 2026-06-16: "자율 전략 진화 폐회로 — 더 나은 전략을 시스템이 스스로 라이브로 교체. 완전 자율 + 5중 안전장치"). The autonomous forward tournament (spec 037/049) continuously races the live strategy (the *incumbent*) against challengers; when a challenger clears ALL FIVE gates the system reassigns the live strategy to that challenger WITHOUT per-change operator approval:
   ...
   **Spec 150 factory route**: a complete factory winner under item 4's stricter 64-trial DSR/PBO contract MAY replace the live candidate configuration only while the ladder is rung 0 and disarmed. That zero-capital assignment is not a funded reassignment: it opens no order and inherits no trust. After deployment, the exact configured strategy MUST pass the hardened canary again; only then may item 4 grant the bounded 10% research rung. It still cannot reach 20% without the existing exploration contract or reach 25% without EDGE_CONFIRMED.
...
**Version**: 8.0.0 | **Ratified**: 2026-05-01 | **Last Amended**: 2026-08-23
