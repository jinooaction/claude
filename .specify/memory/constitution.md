...
4. **Deploy ≠ live money; capital sizing is governed by the evidence-gated capital ladder under a standing operator delegation (amended v9.0.0).** Auto-deploy lands code and restarts the worker; it does NOT by itself move the system from dry-run to real orders. Live exposure — both the arming decision and the **amount of capital deployed** — is governed by the **spec 050/150 capital ladder**, authorised by the operator's standing delegation (mason, 2026-06-11: "1·2·3 모두 세계 최고 수준이 목표. 3번도 나는 자동과 자율에 맡길거야. 기준은 계좌 잔고와 포트폴리오"):
   ...
   - **Entry (rung 0→1, research)**: ALL of — the versioned factory evidence contract is complete. Legacy evidence without `gate_version=2.0` still requires exactly 64/64 unique live-expressible trials. Current `gate_version=2.0` evidence requires at least 16 preregistered trials, 100% family completion, and all four cumulative-audit gates (`complete_family_trials`, `prior_audit_complete`, `global_audit_trials`, `unique_audit_fingerprints`) with matching counts and globally unique strategy fingerprints. Every producer-declared gate is blocking unless explicitly marked `blocking=false` as diagnostic, and every blocking gate MUST pass; variable family size does NOT lower any preregistered statistical, multiplicity, holdout, cost, benchmark, drawdown, or robustness threshold. The producer MUST issue `FACTORY_EDGE` and `research_canary_eligible=true` with a selected candidate, deploy config, and strategy fingerprint; evidence is fresh; hardened-canary is PASS; and the selected-config, evidence, and live strategy fingerprints are identical. Any partial, stale, approximate, contradictory, duplicated, or mismatched evidence remains at 0%. Backtest workflow itself cannot order or move capital.
...
**Version**: 9.0.0 | **Ratified**: 2026-05-01 | **Last Amended**: 2026-08-25
