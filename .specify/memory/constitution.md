...
4. **Deploy ≠ live money; capital sizing is governed by the evidence-gated capital ladder under a standing operator delegation (amended v10.0.0).** Auto-deploy lands code and restarts the worker; it does NOT by itself move the system from dry-run to real orders. Live exposure — both the arming decision and the **amount of capital deployed** — is governed by the **spec 050/150/166 capital ladder**, authorised by the operator's standing delegation (mason, 2026-06-11: "1·2·3 모두 세계 최고 수준이 목표. 3번도 나는 자동과 자율에 맡길거야. 기준은 계좌 잔고와 포트폴리오"):
   - **Rungs**: 0 = 0% (disarmed) → 1 = 10% research canary → 2 = 20% exploration canary → 3 = 25% → 4 = 50% → 5 = 100% of the **measured real account NAV** (KIS balance, multi-exchange sweep). The account balance is the delegation's ceiling; deposits are physically operator-only.
   - **Entry (rung 0→1, research)**: ALL of — `gate_version=3.0` (`family-complete-v3`) evidence contains at least 16 preregistered current-family trials and raw records for the whole research program. The consumer, not the producer, MUST independently recount complete audit rows, current-family rows, prior rows, candidate identifiers, strategy fingerprints, the exact current-family audit tail, selected-row identity, and all four producer audit counts (`complete_family_trials`, `prior_audit_complete`, `global_audit_trials`, `unique_audit_fingerprints`). Public history MUST be point-in-time, historical samples MUST NOT be reused for promotion, benchmark and live execution MUST be equivalent, thresholds MUST be frozen before results, every producer-declared blocking gate MUST pass, and selected-config/evidence/live fingerprints MUST be identical. The selected standardized PSR MUST equal the selected raw trial row and survive every globally unique program trial using `min(1, (1 - PSR) × trials) <= 0.05`; the consumer MUST recompute family DSR and PBO from the published current-family return and even-segment matrices, match the producer values, and require DSR at least 0.95 and PBO at most 0.20. A fresh read-only order preview at exactly 10% of current measured NAV MUST then reproduce the configured target after symbol mapping, whole-share rounding, minimum notional, available-cash filtering, and all exposure caps: 100% positive-target quote coverage, at least 66% funded positive target legs, L1 capital-weight error at most 25%, and maximum per-leg error at most 15%. The same preview is revalidated before the first strategy fill. Legacy and `gate_version=2.0` evidence are diagnostic-only and can NEVER open capital. Any partial, stale, approximate, contradictory, duplicated, reused, non-vintage, execution-mismatched, statistically uncorrected, or unfundable evidence remains at 0%. The backtest and preview workflows themselves cannot order or move capital.
   ...
   - **Channel**: ladder decisions are computed by tested pure code (`portfolio/capital_ladder.py`), executed as sentinel PRs through the ladder gate (`.github/workflows/forward-edge-autoarm.yml`), and real orders flow ONLY through the market-hours schedule of `rebalance-live-canary.yml`, which enforces the ladder-authority capital guard. The guarded go-live channel obligations of v4.0.0 (market-hours guard, deploy audit events, post-restart health check with auto-revert, and untouched principles I, II, IV, V) continue to apply to worker-mode transitions.
...
5. **Strategy reassignment is governed by the evidence-gated 5-gate tournament under the same standing operator delegation (amended v6.0.0).** Item 4 governs HOW MUCH capital the live system deploys; this item governs WHICH strategy it deploys. The operator delegated strategy selection (mason, 2026-06-16: "자율 전략 진화 폐회로 — 더 나은 전략을 시스템이 스스로 라이브로 교체. 완전 자율 + 5중 안전장치"). The autonomous forward tournament (spec 037/049) continuously races the live strategy (the *incumbent*) against challengers; when a challenger clears ALL FIVE gates the system reassigns the live strategy to that challenger WITHOUT per-change operator approval:
   ...
   **Spec 150 factory route**: a complete factory winner under item 4's versioned complete-family and cumulative-audit contract MAY replace the live candidate configuration only while the ladder is rung 0 and disarmed. That zero-capital assignment is not a funded reassignment: it opens no order and inherits no trust. After deployment, the exact configured strategy MUST pass the hardened canary again; only then may item 4 grant the bounded 10% research rung. It still cannot reach 20% without the existing exploration contract or reach 25% without EDGE_CONFIRMED.
...
**Version**: 10.0.0 | **Ratified**: 2026-05-01 | **Last Amended**: 2026-08-26
