<!--
Sync Impact Report (v15.4.0 -> 15.5.0)
==================
Version change: 15.4.0 -> 15.5.0 (MINOR: X.4 adds one fail-closed same-session
automatic recovery attempt after a server-timer order run was atomically claimed but
every planned broker write was explicitly rejected before acceptance. The original
market-session claim remains immutable. A separate root-owned append-only retry claim
may be consumed once only by a later automatic server-timer invocation after a reviewed
and deployed incident manifest binds the exact first run, market session, rejection
signature, first code commit, and remediation commit. The first run must prove zero
submitted, unknown, open, partial, or filled orders; successful fill synchronization,
measurement, and reconciliation; valid evidence; and no halt. The retry must repeat
operational revision, deploy audit, XNYS, rung-1, first-entry, broker-open-order,
limit-order, whitelist, exposure, loss, kill-switch, audit, fill-sync, measurement, and
reconciliation gates. Missing or ambiguous evidence, a GitHub/manual source, an
unreviewed repair, or a consumed retry remains fail-closed. this changes the safety
perimeter.)
Migration impact:
  - The original one-per-session claim ledger remains append-only and is never removed,
    rewritten, or treated as unused.
  - Only a root-owned server timer may consume the separate one-per-session recovery
    ledger, and only for an exact reviewed incident manifest already present in deployed
    code.
  - A retry is forbidden after any submitted, unknown, open, partial, or filled order, or
    after failed fill synchronization, measurement, reconciliation, evidence, or halt
    checks.
  - A consumed recovery slot is terminal for that session even if the retry itself fails.
Templates requiring updates:
  ✅ .specify/templates/{plan,spec,tasks}-template.md inspected and unchanged; their
    generic Constitution Check already covers this bounded recovery contract.
  ⚠ spec 176 requirements, data model, implementation plan, tasks, gateway, scheduler,
    evidence schema, tests, deployment evidence, and HANDOFF require synchronized work.
  ⚠ .specify/memory/kernel.toml remains unchanged; the constitution, live-order gateway,
    scheduler, and audit boundary are already K-meta/K1/K2/K4/K6.

Sync Impact Report (v15.3.1 -> 15.4.0)
==================
Version change: 15.3.1 -> 15.4.0 (MINOR: VIII.A extends verified terminal-rollback orphan recovery to a forward emergency hand-off when production is already on a later healthy ordinary deploy but has not yet reached the newly authorized current-main target. The installed production HEAD must descend from the verified rollback baseline, be an ancestor of the exact authorized target, and have its own later append-only live deploy chain with one start and completion, zero failure or rollback, no unexpected event, and an in-window WORKER_STARTED. Under exclusive maintenance and broker-write locks, the helper rechecks the unchanged production HEAD, active worker and timer, and a fresh read-only KIS smoke with open_unfilled=0; it then appends the new exact owner authorization and a non-terminal DEPLOY_EMERGENCY_ORPHAN_RECOVERED link before removing only the stale request. The same interlock remains held, the new request is installed, and the unchanged exact-target emergency deploy with >=90-second health and rollback proceeds. A pre-start interruption after this link may be inherited only with exactly one matching authorization and one matching orphan-recovered row. Missing ancestry, audit, health, lock, broker, or identity proof remains halted. No arbitrary cleanup, manual order, service start, executable, checkout, force flag, capital, strategy, whitelist, risk, audit, or reconciliation bypass is added. this changes the safety perimeter.)
Migration impact:
  - A healthy intermediate production SHA MAY hand a verified old rollback orphan forward only when Git proves rollback-baseline -> production -> exact authorized current main and the production SHA has a complete later live deploy chain.
  - The helper MUST append DEPLOY_EMERGENCY_ORPHAN_RECOVERED after the new owner authorization and fresh open-order proof, remove only the prior request, retain the same exclusive maintenance boundary, and continue through the unchanged emergency deploy state machine.
  - A pre-DEPLOY_STARTED HALTED hand-off with that non-terminal event remains recoverable only when both audit rows are unique and their current/prior identities and completed-deploy proof match exactly.
  - Exact-target cleanup-only recovery and all ordinary fail-closed paths remain unchanged.
Templates requiring updates:
  ⚠ spec 179 requirements, helper, typed audit payload, hand-off tests, deploy docs, and production evidence require synchronized correction.
  ✅ .specify/templates/{plan,spec,tasks}-template.md inspected and unchanged; they contain no rollback hand-off detail.
  ⚠ .specify/memory/kernel.toml remains unchanged; the constitution, deployment helper, and audit contract are already K-meta/K6/K4.

Sync Impact Report (v15.3.0 -> 15.3.1)
==================
Version change: 15.3.0 -> 15.3.1 (PATCH: VIII.A clarifies that workflow status and emergency routing MUST be derived only from the current deploy attempt's journal segment. A historical market-hours refusal in a bounded journal tail cannot turn a present non-market failure green or authorize an emergency. The exact current failure "emergency request target does not match current main" may route an already validated registered-owner request to the fixed root helper solely so that helper can prove or reject the closed stale-orphan recovery contract; the message itself is never recovery proof or mutation authority. Missing current-attempt boundaries or an unclassified current failure remains fail-closed. this changes the safety perimeter.)
Migration impact:
  - The deploy workflow MUST isolate the journal segment beginning at the latest current service-start marker before classifying the attempt.
  - Ordinary market-hours deferral MUST require a market-hours refusal inside that current segment; older matching lines MUST NOT affect status.
  - A current exact stale-target refusal MAY reach the fixed root helper only under a validated one-shot registered-owner request; all file, ledger, Git, lock, health, timer, and broker proofs remain inside that helper and unchanged.
Templates requiring updates:
  ✅ spec 179 workflow evidence contract and regression tests require synchronized correction.
  ✅ .specify/templates/{plan,spec,tasks}-template.md inspected and unchanged; they contain no deploy-journal parsing detail.
...
- The GitHub repository owner or the exact constitution-registered system-owner actor `masonoh-kidsnote` MAY authorize one emergency live deploy during an open XNYS session only when ALL of the following hold. Registration is fixed in the protected default-branch workflow source; writer/collaborator status alone grants no authority. Adding or changing a registered actor requires a dedicated constitutional safety-perimeter amendment and full grade-4 validation.
  ...
  6. **Workflow status is current-attempt bound.** A workflow MUST classify success, market-hours deferral, failure, and emergency eligibility only from the journal segment created by its current deploy attempt. A matching sentence from an earlier attempt MUST NOT make a current failure green or authorize the exception. If the current segment cannot be isolated, the attempt is an unclassified failure. Under an otherwise valid one-shot registered-owner request, the current exact refusal `emergency request target does not match current main` MAY route control to the fixed root helper only so that the helper can validate the closed recovery contracts above. That refusal text alone is neither recovery evidence nor permission to mutate production; every file, ledger, Git, lock, health, timer, and broker invariant still MUST pass inside the helper.
  7. **A verified healthy intermediate deploy may hand recovery forward.** When the stale terminal-rollback orphan is valid but production HEAD is neither the rollback baseline nor the exact authorized target, the fixed helper MAY recover it only if Git proves the production HEAD descends from the rollback baseline and is an ancestor of the exact authorized current-main target. A later ordinary live-deploy correlation after the rollback MUST bind that exact production HEAD and prove one `DEPLOY_STARTED`, one terminal `DEPLOY_COMPLETED`, zero `DEPLOY_FAILED` or `DEPLOY_ROLLED_BACK`, zero unexpected deploy events, and a `WORKER_STARTED` row strictly between start and completion. Under the already exclusive maintenance and broker-write locks, the helper MUST recheck the unchanged production HEAD, active worker, active live timer, and a fresh read-only broker smoke with zero open orders. It MUST append the new exact `DEPLOY_EMERGENCY_AUTHORIZED` row followed by a non-terminal `DEPLOY_EMERGENCY_ORPHAN_RECOVERED` row linking the prior request, prior rollback correlation, completed intermediate deploy correlation, recovered production SHA, and new target before removing only the prior request. It MUST retain the same maintenance interlock, install the new one-shot request, quiesce broker-writing processes, repeat the broker smoke, and continue through the unchanged exact-target emergency deploy and rollback state machine. The orphan-recovered row is not a successful terminal event and cannot release the interlock. If interruption occurs before `DEPLOY_STARTED`, a later exact registered-owner request MAY inherit the resulting `HALTED` interlock only when exactly one matching authorization and one matching orphan-recovered row exist and all linked identities are exact; otherwise it remains halted.
...
4. **Deploy ≠ live money; capital sizing is governed by the evidence-gated capital ladder under a standing operator delegation (amended v11.0.0).** Auto-deploy lands code and restarts the worker; it does NOT by itself move the system from dry-run to real orders. Live exposure — both the arming decision and the **amount of capital deployed** — is governed by the **spec 050/150/166/167/176 capital ladder**, authorised by the operator's standing delegation (mason, 2026-06-11: "1·2·3 모두 세계 최고 수준이 목표. 3번도 나는 자동과 자율에 맡길거야. 기준은 계좌 잔고와 포트폴리오"):
   ...
   - **Channel**: ladder decisions are computed by tested pure code (`portfolio/capital_ladder.py`) and executed as sentinel PRs through the ladder gate (`.github/workflows/forward-edge-autoarm.yml`). Real-order wake-up authority is restricted to exactly two automated sources: (a) the signed, nonce-protected market-hours `schedule` of `rebalance-live-canary.yml` and (b) the root-owned `auto-invest-live-canary.timer` fallback on the production host. The fallback MUST start after the primary GitHub opportunity, MUST NOT be remotely or manually exposed as an order command, and MUST reproduce operational-main revision, ladder-authority, first-entry revalidation, XNYS, limit-order, whitelist, position-cap, kill-switch, circuit-breaker, fill-sync, reconciliation, and append-only evidence obligations. Operational-main revision means either exact deployed current main, or an audited deployed commit that is an ancestor of current `origin/main` and differs from it only by the frozen deployment-excluded paths `*.md`, `specs/**`, `.verify/**`, and `.trigger/**`; unreadable history or any other changed path MUST fail closed. First-entry evidence MUST match current main, while deploy audit MUST match the actual deployed operational commit, and both identities MUST be recorded. Both scheduler and internal `systemd-order` MUST independently repeat this test before the shared claim. Both sources MUST converge on one root-owned market-session claim so the normal path reaches at most one broker-writing execution per XNYS session. A claimed run that fails or partially executes MUST NOT be retried. The only exception is one separate automatic recovery attempt under the closed contract below; it does not erase or reuse the original claim.

     **Same-session automatic recovery (one bounded exception).** A later root-owned `auto-invest-live-canary.timer` invocation MAY consume one separate append-only retry claim for the same XNYS session only when every condition below is independently revalidated immediately before the broker boundary:
     1. The immutable original claim names `server_timer` as its source, and root-owned non-symlink evidence binds the same session and run ID to `ENTRY_READY`, a completed first run, `orders_submitted=0`, successful fill synchronization, measurement and reconciliation, evidence quality `VALID`, reconciliation `OK`, and no halt.
     2. Every planned first-run order has exactly one explicit `REJECTED_BY_BROKER` result. No result or broker read may indicate `SUBMITTED`, `SUBMISSION_UNKNOWN`, `PARTIALLY_FILLED`, `FILLED`, cancellation uncertainty, or an open order. A fresh read-only broker check MUST again prove zero open orders.
     3. A reviewed incident manifest in the exact deployed code binds the market session, first run ID and source, first code commit, closed broker-rejection signature, and a remediation commit. Git MUST prove that the first and remediation commits are ancestors of the deployed code, that the deployed code differs from the first code, and that the manifest itself is tracked by that deployed code. A generic error code, mutable environment switch, runtime input, or prose diagnosis alone is not retry authority.
     4. The separate root-owned retry ledger is exclusively locked and has no prior row for the session. The helper appends the session, first run identity, retry run identity, deployed code, manifest digest, and timestamp before the retry command. That row is never deleted or rewritten; once appended, no further recovery attempt is allowed that session even if this retry fails.
     5. The retry source remains the automatic server timer. Manual service start, GitHub `workflow_dispatch`, `repository_dispatch`, signed GitHub schedule, arbitrary shell, operator command, or remote status command cannot request or consume it. The retry repeats current-main/deploy-audit checks, XNYS regular-session proof, rung-1 and 10% capital limits, fresh first-entry revalidation, limit-order and whitelist rules, position and loss caps, kill switch, append-only order audit, fill synchronization, measurement, and account reconciliation. Other automated wake-up sources only observe the first and retry identities and MUST NOT write again.

     Missing, stale, malformed, symlinked, non-root-owned, identity-mismatched, ambiguous, unreviewed, already-consumed, or broker-uncertain evidence MUST return the ordinary duplicate result without a broker write. This exception changes only recovery liveness after a proven zero-acceptance protocol defect; it does not authorize price chasing, a market order, a capital increase, strategy promotion, whitelist expansion, manual trading, or a third attempt. `workflow_dispatch`, `repository_dispatch`, arbitrary shell, and any other event remain real-order-ineligible. The guarded go-live channel obligations of v4.0.0 (market-hours guard, deploy audit events, post-restart health check with auto-revert, and untouched principles I, II, IV, V) continue to apply to worker-mode transitions.
...
**Version**: 15.5.0 | **Ratified**: 2026-05-01 | **Last Amended**: 2026-09-04
